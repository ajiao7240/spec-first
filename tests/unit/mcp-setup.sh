...
assert_contains "verify prints friendly next steps" "Next steps:" "$verify_text"
...
assert_contains "verify prompts host restart" "Restart Claude Code" "$verify_text"
last_verify_line="$(printf '%s\n' "$verify_text" | sed '/^[[:space:]]*$/d' | tail -n 1)"
assert_contains "verify output ends with restart guidance" "downstream workflows" "$last_verify_line"
