...
assert_contains "write-provider-config.sh hashes canonical GitNexus command JSON" 'gitnexus_command_hash="$(printf '\''%s\n'\'' "$gitnexus_commands_json" | hash_text)"' "$(cat "$SCRIPTS_DIR/write-provider-config.sh")"
...
assert_contains "write-provider-config.sh hashes canonical code-review-graph command JSON" 'code_review_graph_command_hash="$(printf '\''%s\n'\'' "$code_review_graph_commands_json" | hash_text)"' "$(cat "$SCRIPTS_DIR/write-provider-config.sh")"
