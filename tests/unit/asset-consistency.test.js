...
const HIGH_RISK_SKILL_ANCHORS = [
  ...
  },
  {
    ...
    anchors: [
      ...
      'empty strings, empty arrays, and missing fields as absent input',
      'freshness_stale',
  ...
  {
    skillName: 'spec-repo-profile-refresh',
    sourcePath: 'skills/spec-repo-profile-refresh/SKILL.md',
    mirrorPath: 'docs/10-prompt/skills/spec-repo-profile-refresh/SKILL.md',
    anchors: [
      '.spec-first/specs/repo-profile.yaml',
      'standalone skill, not a `/spec:*` workflow command',
      '`spec-graph-bootstrap` is recommended upstream context, not a hard prerequisite.',
      'Default mode is `preview`',
      '`apply` is allowed only after preview or explicit user confirmation',
