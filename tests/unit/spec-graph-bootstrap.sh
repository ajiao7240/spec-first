...
assert_contains "all-repos graph bootstrap creates workspace-aware parent AGENTS GitNexus block" "本工作区包含多个 child Git repo" "$(cat "$ALL_REPOS_WORKSPACE/AGENTS.md")"
assert_contains "all-repos graph bootstrap points parent AGENTS to workspace graph targets" '`.spec-first/workspace/graph-targets.json`' "$(cat "$ALL_REPOS_WORKSPACE/AGENTS.md")"
assert_not_contains "all-repos graph bootstrap does not point parent AGENTS to repo-local graph facts" '`.spec-first/graph/graph-facts.json`' "$(cat "$ALL_REPOS_WORKSPACE/AGENTS.md")"
...
assert_not_contains "all-repos graph bootstrap does not point parent CLAUDE to repo-local graph facts" '`.spec-first/graph/graph-facts.json`' "$(cat "$ALL_REPOS_WORKSPACE/CLAUDE.md")"
...
assert_contains "graph-bootstrap normalizes Claude GitNexus host block" "**必须先**读取" "$(cat "$PRIMARY_REPO/CLAUDE.md")"
assert_contains "graph-bootstrap host block is self-contained" "capabilities.query_global_graph" "$(cat "$PRIMARY_REPO/AGENTS.md")"
assert_contains "graph-bootstrap host block points to graph readiness facts" ".spec-first/graph/graph-facts.json" "$(cat "$PRIMARY_REPO/AGENTS.md")"
assert_contains "graph-bootstrap host block uses GitNexus as preferred tool" "使用 GitNexus 作为首选工具" "$(cat "$PRIMARY_REPO/AGENTS.md")"
...
assert_contains "graph-bootstrap creates CLAUDE GitNexus self-contained host block" "**必须先**读取" "$(cat "$MISSING_HOST_REPO/CLAUDE.md")"
