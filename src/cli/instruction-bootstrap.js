...
- 父级多仓 workspace 的只读代码问题可使用 \`workspace-graph-targets.v1\` advisory facts 做候选 repo 发现并优先 GitNexus；父 workspace 维护入口中的 \`init\`、setup 和 graph bootstrap 可默认按 discovered child repos 执行并只写父目录 advisory summary；其他写入、修复、测试、review autofix 或 commit 前仍必须有明确 \`target_repo\` / per-child scope
${hostLine}
${surfaceLine}
...
- Read-only code questions from a parent multi-repo workspace may use \`workspace-graph-targets.v1\` advisory facts for candidate repo discovery and prefer GitNexus-first evidence; parent-workspace maintenance \`init\`, setup, and graph bootstrap may default to discovered child repos and write only parent advisory summaries; other writes, fixes, tests, review autofix, or commits still require explicit \`target_repo\` / per-child scope
${hostLine}
${surfaceLine}
