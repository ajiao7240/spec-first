...
- Read-only code questions from a parent multi-repo workspace may use \`workspace-graph-targets.v1\` advisory facts for candidate repo discovery and prefer GitNexus-first evidence; parent-workspace maintenance \`init\` may refresh parent host runtime assets, while setup and graph bootstrap may default to discovered child repos and write only parent advisory summaries; no-argument \`spec-standards\` may write only parent \`.spec-first/standards/\` advisory baselines, while child-local standards baselines still require explicit \`--repo <child>\`; other writes, fixes, tests, review autofix, or commits still require explicit \`target_repo\` / per-child scope
${hostLine}
${surfaceLine}
