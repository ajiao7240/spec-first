...
function matchStrictManagedCodingGuidelinesSection(lines, startIndex, heading) {
  ...
  const sectionHeadings = isZh
    ? ['### 1. 编码前思考', '### 2. 简洁优先', '### 3. 精准修改', '### 4. 目标驱动执行', '### 5. 工具参数卫生']
    : ['### 1. Think Before Coding', '### 2. Simplicity First', '### 3. Surgical Changes', '### 4. Goal-Driven Execution', '### 5. Tool Parameter Hygiene'];
  const bulletCounts = [4, 5, 4, 3, 3];
