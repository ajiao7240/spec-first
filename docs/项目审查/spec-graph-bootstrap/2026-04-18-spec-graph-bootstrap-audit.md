...
`spec-graph-bootstrap` 是当前仓库里最关键的上游 skill 之一，因为它直接生成 `spec-plan`、`spec-work`、`spec-review` 会优先消费的 Stage-0 决策输入；它现在的优势是“控制面已经可稳定装配并被下游消费”，它现在的核心问题是“装配完成度强于事实真实性”。  
...
4. 最后核对 `context-routing` 与 `spec-plan` / `spec-work` / `spec-review` 如何消费这些产物。
...
- `skills/spec-review/SKILL.md:27-45`
...
`spec-plan`、`spec-work`、`spec-review` 已经把 minimal context 和 verification summary 当成高优先级 machine context 使用。  

证据：

- `skills/spec-plan/SKILL.md:80-90`
- `skills/spec-work/SKILL.md:39-53`
- `skills/spec-review/SKILL.md:29-44`
...
- `spec-plan` / `spec-work` / `spec-review` 可以直接读取这些元字段
...
- `spec-review` 遇到低置信 `risk_signals` 时，应把它当 review hint，而不是事实断言
