...
Effort 口径：XS≈单文件几行；S≈一个聚焦改动+窄测试；M≈跨文件/需设计。所有项均 light-contract（无硬 gate、无重型框架）。

**A 组｜快赢（XS，确定性收敛，可一批做掉）**
...
| 把已存在的 `plan-status-taxonomy.test.js` 接入 CI/pre-commit（不新增 schema/archive） | W2 | XS |
| 整组删除 4 个 legacy 文件（review-judge/stage-gate/task-manager.sh + e2e.sh）+ 删 run-test-suite:100 那行 | L1 | XS |
...
| eval 评分器改读 `coverage_tags`/schema 而非文件名 regex（`write-audit-artifacts.js:261-264`），让 readiness 分数诚实 | W1-P2 | S |
| `runtime-production-guardrails.md` 末尾加 "维度→workflow" 索引表（11 行，仿 adapter-map）+ contract test 断言每维度有行 | W6 | S |
| 可选：`clean` 主路径应用前 echo dry-run 摘要 + help 补 `--dry-run` 提示（**不**加硬 prompt/`--force`） | W3 | S |

**C 组｜中型 / 按需（M，价值高但需设计或需先有消费者）**

| 行动 | 来源 | Effort |
| --- | --- | --- |
| **统一 fixture schema 契约**（5 套发散→1 套最小 envelope + adapter）——eval 维度最高 ROI | W1-P1 | M |
| 给 spec-plan/code-review/debug/compound 补 `trigger`+`boundary` fixture（仅这 4 个，不求四件套） | W1-P3 | M |
| fresh-source-eval 升为有节奏的 L2（PR 模板占位 + judge↔人工 agreement 抽样，**不做硬 gate**） | W1-P4 | S–M |
| 可选/按需：validator 加向后兼容 `details[].{keyword,instanceLocation}`（**仅当有消费者**） | W4 | S |
| 可选/低成本：`doctor --json` 把 manifest staleness 提升为 `runtime_catalog_freshness` 字段；CLAUDE.md closeout 加 `runtime impact` 一行 | L4(b)(c) | S |

**一句话总结：** spec-first 在 agent-native 架构层面已是范本级实现，eval 方法论方向也对（已对标业界）。经两轮收敛，**唯一系统性维度是 eval 覆盖**（C 组 W1：统一 schema > 修评分器 > 补 4 个高频 > judge 对齐）；其余全是 XS/S 级确定性收敛——**A 组 5 项快赢可一次做完**（含两处本报告前提已 stale 的 1 行修正）。架构无需改动，不引入重型 eval 框架，不加硬 gate。
