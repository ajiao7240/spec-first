...
note: 本文是对 spec-first 当前仓库的 agent-native architecture audit 报告。2026-06-13 用更严格的 10-lens + 对抗验证重跑并与 2026-06-12 首轮发现汇总收敛；同日经用户质询后回源复核，撤回 W2（plan 归档）并下修为 status 口径小 lint；并对 W1（eval readiness）做项目深挖 + 业界调研，细化为 schema 统一 / 评分器修正 / 高频覆盖 / judge 对齐四步。发现与建议是 review evidence，不是已拍板实施计划；落地前仍需回源核对当前 source。
---
...
- **修正后建议（XS）：** 唯一剩余动作是把已存在的 `plan-status-taxonomy.test.js` **接入 CI/pre-commit 门**（当前只在 `npm test` 本地跑），让未来漂移自动拦截。**不新增** schema/archive/迁移；不动 129 个合规条目。
...
- **事实（已核对）：** `runtime-production-guardrails.md` 实际有 **11 个 `##` 维度**（原报告全文写 "8" 系笔误：external-facts / workspace-authority / least-privilege / secret-redaction / network-posture / approval / audit-tracing / checkpoints / completion / HITL / eval-gates）。其中 10 个被 `agent-native-architecture-contracts.test.js:195-206` 锁住。
...
- **修正后建议（XS）：** **整组删除 4 个文件**（3 脚本 + `tests/integration/e2e.sh`，e2e.sh 唯一用途就是驱动这三个、不测任何当前产品行为）+ 删 `run-test-suite.cjs:100` 那行 + 跑 `npm test` 确认绿 + CHANGELOG 记录（非 user-visible）。CLAUDE.md "不删预存死代码除非被要求" 的豁免条件已满足（审查 P3 + 2026-06-10 P1-10 均已点名退役）。
...
| **W2 "Plan 缺 DELETE/归档"（Warning，2026-06-13 用户质询后自查）** | **partially-refuted→降为小 lint** | 状态流转 closeout 已自动写回（`spec-work:223`）；物理删除对决策证据 artifact 有害；仅剩 status 口径漂移需一个 enum 校验。原 Warning 没降够，参见 W2 修正 |
...
| ---: | --- | --- | --- | --- |
| P1 | **统一 fixture schema 契约**（5 套发散→1 套最小 envelope + adapter）；解决字段名漂移（缺陷 A） | W1-P1 | eval-readiness | 中 |
| P1 | **eval 评分器改读 `coverage_tags`/schema 而非文件名 regex**（`write-audit-artifacts.js:261-264`）；让 readiness 分数诚实（缺陷 B） | W1-P2 | eval-readiness | 小 |
| P2 | 给 spec-plan/code-review/debug/compound 补 `trigger`+`boundary` 两类 fixture（**仅这 4 个高频高风险，不求四件套**） | W1-P3 | eval-readiness | 中 |
| P3 | fresh-source-eval 升为有节奏的 L2：PR 模板放输出占位 + 抽样做 judge↔人工 agreement（**不做硬 gate**） | W1-P4 | eval-readiness | 小 |
...
| P3 | 加 plan frontmatter `status` enum 校验测试，规范 completed/complete/closed/derived 口径漂移（**不做** archive 功能——复核已撤回，物理删除对决策证据有害） | W2（已下修） | crud | 小 |
...
**一句话总结：** spec-first 在 agent-native 7 大原则上已是范本级实现，eval 方法论方向也对（已对标业界）；真正的提升空间集中在一个维度——**统一发散的 fixture schema、修正文件名匹配的评分器、给 4 个高频 workflow 补 trigger/boundary、把 fresh-source-eval 升为有节奏的 judge 对齐**；架构层面无需改动，且不应引入重型 eval 框架或硬 gate。
