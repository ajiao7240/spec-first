...
| 原报告问题摘要 | 它应聚焦 spec-first/plugin 生态 bug report，不应泛化成任意仓库 issue creator。 |
| 审查日期 | 2026-06-20 |
| 审查结论等级 | 优化 |
| 优先级 | P1 |
...
- 事实 1：原报告位置为 docs/项目审查/2026-06-20-全量-skill-agent-优化建议.md:43；深审 docs/项目审查/2026-06-20-全量-skill-agent-优化建议.md:161。
- 事实 2：存在：`skills/report-bug/SKILL.md`，160 行。
- 事实 3：资源目录计数：references=0，scripts=0，evals=0，assets=0。
- 事实 4：原报告建议为：补输入/输出/失败合同，明确 bug report 目标是 spec-first plugin，不泛化到任意 repo；GitHub issue 创建必须 preview-first 并保护环境信息。增加 “用户只想本地草稿，不要提交 issue” near-neighbor。

### 2.2 推断

- 推断 1：该对象应进入“优化”处理路径。
  - 推断依据：原报告定位/风险/优化动作与当前 source 结构信号一致；主要风险为“直接创建 GitHub issue 会泄露本地路径、环境信息、private repo 细节；也可能把本地草稿需求误发布。”。
  - 可信度：高
...
| 待验证项 | 验证方式 | 影响范围 | 优先级 |
|---|---|---|---|
| 当前对象真实消费者与调用路径 | `rg 'report-bug' skills agents src docs tests`，并区分 source 引用、历史文档和 generated mirror | 决定保留、合并或退役 | P1 |
...
从 $yao-meta-skill 的元能力视角看，report-bug 的核心问题不是“是否还能写得更多”，而是边界、复用、治理证据是否与其复用等级匹配。当前建议：优化。

### 3.2 主要问题

- 直接创建 GitHub issue 会泄露本地路径、环境信息、private repo 细节；也可能把本地草稿需求误发布。
...
- 补 preview-first issue draft、target repo boundary、redaction checklist、用户确认后才提交的 mutating gate。
...
从 skill-creator 视角看，report-bug 需要稳定“触发描述、执行步骤、输出契约、失败回退、验证样例”这五件事。当前最大缺口是：补输入/输出/失败合同，明确 bug report 目标是 spec-first plugin，不泛化到任意 repo；GitHub issue 创建必须 preview-first 并保护环境信息。增加 “用户只想本地草稿，不要提交 issue” near-neighbor。
...
report-bug 应被放在 支撑/实验链路，服务主链路或支撑链路的明确节点；不应作为新的泛入口扩散。

### 5.2 架构定位判断

它应聚焦 spec-first/plugin 生态 bug report，不应泛化成任意仓库 issue creator。
...
三方一致认为：report-bug 的优化应先落在边界、输出契约、失败处理和验证证据上；不应在缺消费者证据时新增平行对象。
...
- 原报告的定位/风险/优化建议已经指向具体对象问题：直接创建 GitHub issue 会泄露本地路径、环境信息、private repo 细节；也可能把本地草稿需求误发布。
...
|---|---|---|---|---|---|---|
| 补输入/输出/失败合同，明确 bug report 目标是 spec-first plugin，不泛化到任意 repo；GitHub issue 创建必须 preview-first 并保护环境信息 | Skill owner / workflow maintainer | 保留当前对象且进入 source 修改阶段 | 低到中 | 降低误触发和 handoff 歧义 | 可能增加少量维护成本 | P1 |
...
作为 支撑/实验链路 中的可复用方法节点，只承担原报告确认的职责：把本地问题整理成可审阅的 bug report 或 GitHub issue 草稿。

### 9.2 推荐职责边界

- Own：把本地问题整理成可审阅的 bug report 或 GitHub issue 草稿。
...
|---|---|---|---|---|
| Source 引用回源 | `rg 'report-bug' skills agents src docs tests`，排除 generated mirrors | 明确消费者、调用路径和历史引用 | Skill owner | P1 |
| 结构契约检查 | 检查 required headings / output schema / failure modes | 缺口补齐且入口不过度膨胀 | Maintainer | P1 |
| Eval / fixture | 新增或规范 normalized eval，覆盖触发、near-neighbor、失败模式 | 至少覆盖原报告验证建议：增加 eval 覆盖“只要本地草稿”“要提交 GitHub issue”“非 spec-first 仓库 bug”“包含敏感日志”。 | Test owner | P1 |
...
- report-bug 当前建议为 优化，优先级 P1。
- 原报告核心风险：直接创建 GitHub issue 会泄露本地路径、环境信息、private repo 细节；也可能把本地草稿需求误发布。
- 当前 source 事实：存在：`skills/report-bug/SKILL.md`，160 行；资源目录计数：references=0，scripts=0，evals=0，assets=0。
