...
|---|---|
| 对象类型 | Skill |
| 对象名称 | spec-skill-audit |
| 来源报告 | 2026-06-20-全量-skill-agent-优化建议.md |
| 原报告问题摘要 | 这是本轮事实来源，整体最成熟；下一步主要是降低 false-positive taxonomy。 |
| 审查日期 | 2026-06-20 |
| 审查结论等级 | 优化 |
| 优先级 | P1 |
...
- 事实 1：原报告位置为 docs/项目审查/2026-06-20-全量-skill-agent-优化建议.md:61；深审 docs/项目审查/2026-06-20-全量-skill-agent-优化建议.md:305。
- 事实 2：存在：`skills/spec-skill-audit/SKILL.md`，166 行。
- 事实 3：资源目录计数：references=10，scripts=20，evals=4，assets=0。
- 事实 4：原报告建议为：优先改进 false-positive taxonomy：runtime mention vs runtime patch、secret detection regex vs secret exfiltration、`sudo` command suggestion vs execution。增加 “输出到 docs/项目审查 的 report mode” 作为可选 future，不默认写 source。

### 2.2 推断

- 推断 1：该对象应进入“优化”处理路径。
  - 推断依据：原报告定位/风险/优化动作与当前 source 结构信号一致；主要风险为“runtime mention vs runtime patch、secret-detection regex vs secret exfiltration、`sudo` suggestion vs execution 的误分会制造虚假 P0/P1。”。
  - 可信度：高
...
| 待验证项 | 验证方式 | 影响范围 | 优先级 |
|---|---|---|---|
| 当前对象真实消费者与调用路径 | `rg 'spec-skill-audit' skills agents src docs tests`，并区分 source 引用、历史文档和 generated mirror | 决定保留、合并或退役 | P1 |
...
从 $yao-meta-skill 的元能力视角看，spec-skill-audit 的核心问题不是“是否还能写得更多”，而是边界、复用、治理证据是否与其复用等级匹配。当前建议：优化。

### 3.2 主要问题

- runtime mention vs runtime patch、secret-detection regex vs secret exfiltration、`sudo` suggestion vs execution 的误分会制造虚假 P0/P1。
- Source 结构信号：contract headings 基本完整，eval 文件数 4，references 10，scripts 20。
- 若该对象被团队复用，应补 owner、review cadence、trigger/output eval 或至少明确 deferred reason；否则不宜伪装成 governed asset。

### 3.3 优化建议

- 修 scanner taxonomy；增加 report-to-docs mode 作为显式 opt-in future，不默认写 source 文档；继续把 score 标注为 review signal。
...
从 skill-creator 视角看，spec-skill-audit 需要稳定“触发描述、执行步骤、输出契约、失败回退、验证样例”这五件事。当前最大缺口是：优先改进 false-positive taxonomy：runtime mention vs runtime patch、secret detection regex vs secret exfiltration、`sudo` command suggestion vs execution。增加 “输出到 docs/项目审查 的 report mode” 作为可选 future，不默认写 source。
...
spec-skill-audit 应被放在 扩展链路，服务主链路或支撑链路的明确节点；不应作为新的泛入口扩散。

### 5.2 架构定位判断

这是本轮事实来源，整体最成熟；下一步主要是降低 false-positive taxonomy。
...
三方一致认为：spec-skill-audit 的优化应先落在边界、输出契约、失败处理和验证证据上；不应在缺消费者证据时新增平行对象。
...
- 原报告的定位/风险/优化建议已经指向具体对象问题：runtime mention vs runtime patch、secret-detection regex vs secret exfiltration、`sudo` suggestion vs execution 的误分会制造虚假 P0/P1。
- 当前 source 事实显示：资源目录计数：references=10，scripts=20，evals=4，assets=0。
...
|---|---|---|---|---|---|---|
| 优先改进 false-positive taxonomy：runtime mention vs runtime patch、secret detection regex vs secret exfiltration、`sudo` command suggestion vs execution | Skill owner / workflow maintainer | 保留当前对象且进入 source 修改阶段 | 低到中 | 降低误触发和 handoff 歧义 | 可能增加少量维护成本 | P1 |
...
作为 扩展链路 中的可复用方法节点，只承担原报告确认的职责：审计 skill source 的触发、边界、契约、eval、runtime governance 和安全风险。

### 9.2 推荐职责边界

- Own：审计 skill source 的触发、边界、契约、eval、runtime governance 和安全风险。
...
|---|---|---|---|---|
| Source 引用回源 | `rg 'spec-skill-audit' skills agents src docs tests`，排除 generated mirrors | 明确消费者、调用路径和历史引用 | Skill owner | P1 |
| 结构契约检查 | 检查 required headings / output schema / failure modes | 缺口补齐且入口不过度膨胀 | Maintainer | P1 |
| Eval / fixture | 新增或规范 normalized eval，覆盖触发、near-neighbor、失败模式 | 至少覆盖原报告验证建议：新增 false-positive regression fixtures，并用本次 `spec-compound` / `spec-mcp-setup` 片段做回归样本。 | Test owner | P1 |
...
- spec-skill-audit 当前建议为 优化，优先级 P1。
- 原报告核心风险：runtime mention vs runtime patch、secret-detection regex vs secret exfiltration、`sudo` suggestion vs execution 的误分会制造虚假 P0/P1。
- 当前 source 事实：存在：`skills/spec-skill-audit/SKILL.md`，166 行；资源目录计数：references=10，scripts=20，evals=4，assets=0。
