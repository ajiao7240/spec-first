...
| 原报告问题摘要 | Proof 是 HITL / external evidence helper，高价值在身份归属和可分享证据，不在把 endpoint 细节塞进入口。 |
| 审查日期 | 2026-06-20 |
| 审查结论等级 | 优化 |
| 优先级 | P1 |
...
- 事实 1：原报告位置为 docs/项目审查/2026-06-20-全量-skill-agent-优化建议.md:42；深审 docs/项目审查/2026-06-20-全量-skill-agent-优化建议.md:153。
- 事实 2：存在：`skills/proof/SKILL.md`，405 行。
- 事实 3：资源目录计数：references=1，scripts=0，evals=0，assets=0。
- 事实 4：原报告建议为：将 endpoint、curl 示例、local bridge 细节迁到 references/scripts；入口保留 Proof/HITL 触发、identity attribution、baseToken mutation safety、输出 URL/本地同步合同。补 token/privacy/redaction failure modes。

### 2.2 推断

- 推断 1：该对象应进入“优化”处理路径。
  - 推断依据：原报告定位/风险/优化动作与当前 source 结构信号一致；主要风险为“token、baseToken mutation、本地 bridge、外部 URL 都涉及隐私和权限；入口过重也会扩大误触发面。”。
  - 可信度：高
...
| 待验证项 | 验证方式 | 影响范围 | 优先级 |
|---|---|---|---|
| 当前对象真实消费者与调用路径 | `rg 'proof' skills agents src docs tests`，并区分 source 引用、历史文档和 generated mirror | 决定保留、合并或退役 | P1 |
...
从 $yao-meta-skill 的元能力视角看，proof 的核心问题不是“是否还能写得更多”，而是边界、复用、治理证据是否与其复用等级匹配。当前建议：优化。

### 3.2 主要问题

- token、baseToken mutation、本地 bridge、外部 URL 都涉及隐私和权限；入口过重也会扩大误触发面。
- Source 结构信号：contract headings 不完整，eval 文件数 0，references 1，scripts 0。
- 若该对象被团队复用，应补 owner、review cadence、trigger/output eval 或至少明确 deferred reason；否则不宜伪装成 governed asset。

### 3.3 优化建议

- 入口保留触发、身份归属、privacy/redaction、输出 URL/同步合同；endpoint、curl、bridge 操作迁到 references/scripts。
...
从 skill-creator 视角看，proof 需要稳定“触发描述、执行步骤、输出契约、失败回退、验证样例”这五件事。当前最大缺口是：将 endpoint、curl 示例、local bridge 细节迁到 references/scripts；入口保留 Proof/HITL 触发、identity attribution、baseToken mutation safety、输出 URL/本地同步合同。补 token/privacy/redaction failure modes。
...
proof 应被放在 支撑/实验链路，服务主链路或支撑链路的明确节点；不应作为新的泛入口扩散。

### 5.2 架构定位判断

Proof 是 HITL / external evidence helper，高价值在身份归属和可分享证据，不在把 endpoint 细节塞进入口。
...
三方一致认为：proof 的优化应先落在边界、输出契约、失败处理和验证证据上；不应在缺消费者证据时新增平行对象。
...
- 原报告的定位/风险/优化建议已经指向具体对象问题：token、baseToken mutation、本地 bridge、外部 URL 都涉及隐私和权限；入口过重也会扩大误触发面。
- 当前 source 事实显示：资源目录计数：references=1，scripts=0，evals=0，assets=0。
...
|---|---|---|---|---|---|---|
| 将 endpoint、curl 示例、local bridge 细节迁到 references/scripts；入口保留 Proof/HITL 触发、identity attribution、baseToken mutation safety、输出 URL/本地同步合同 | Skill owner / workflow maintainer | 保留当前对象且进入 source 修改阶段 | 低到中 | 降低误触发和 handoff 歧义 | 可能增加少量维护成本 | P1 |
...
作为 支撑/实验链路 中的可复用方法节点，只承担原报告确认的职责：生成或同步 Proof/HITL 证据链接，用于人工确认、身份归属和外部证明。

### 9.2 推荐职责边界

- Own：生成或同步 Proof/HITL 证据链接，用于人工确认、身份归属和外部证明。
...
|---|---|---|---|---|
| Source 引用回源 | `rg 'proof' skills agents src docs tests`，排除 generated mirrors | 明确消费者、调用路径和历史引用 | Skill owner | P1 |
| 结构契约检查 | 检查 required headings / output schema / failure modes | 缺口补齐且入口不过度膨胀 | Maintainer | P1 |
| Eval / fixture | 新增或规范 normalized eval，覆盖触发、near-neighbor、失败模式 | 至少覆盖原报告验证建议：补 no-token-leak、missing bridge、user requests draft-only、external publish denied、output URL contract 五类 eval。 | Test owner | P1 |
...
- proof 当前建议为 优化，优先级 P1。
- 原报告核心风险：token、baseToken mutation、本地 bridge、外部 URL 都涉及隐私和权限；入口过重也会扩大误触发面。
- 当前 source 事实：存在：`skills/proof/SKILL.md`，405 行；资源目录计数：references=1，scripts=0，evals=0，assets=0。
