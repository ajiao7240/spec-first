...
| 原报告问题摘要 | 保持 internal helper，只由公开 workflow 委托；不应出现在用户 public entrypoint 菜单。 |
| 审查日期 | 2026-06-20 |
| 审查结论等级 | 优化 |
| 优先级 | P1 |
...
- 事实 1：原报告位置为 docs/项目审查/2026-06-20-全量-skill-agent-优化建议.md:41；深审 docs/项目审查/2026-06-20-全量-skill-agent-优化建议.md:145。
- 事实 2：存在：`skills/git-worktree/SKILL.md`，118 行。
- 事实 3：资源目录计数：references=0，scripts=1，evals=0，assets=0。
- 事实 4：原报告建议为：保持 internal-only，不暴露为用户入口。待当前 dirty work 收口后，补标准 contract summary；把 `--copy-env` 安全边界升级为 reviewer-visible trust note，并把 `detect --json` facts contract 写成稳定 schema 小节。

### 2.2 推断

- 推断 1：该对象应进入“优化”处理路径。
  - 推断依据：原报告定位/风险/优化动作与当前 source 结构信号一致；主要风险为“`--copy-env`、linked worktree、submodule、separate git dir 等边界复杂；若用户直接调用，容易绕过目标 repo 和 secret 保护。”。
  - 可信度：高
...
| 待验证项 | 验证方式 | 影响范围 | 优先级 |
|---|---|---|---|
| 当前对象真实消费者与调用路径 | `rg 'git-worktree' skills agents src docs tests`，并区分 source 引用、历史文档和 generated mirror | 决定保留、合并或退役 | P1 |
...
从 $yao-meta-skill 的元能力视角看，git-worktree 的核心问题不是“是否还能写得更多”，而是边界、复用、治理证据是否与其复用等级匹配。当前建议：优化。

### 3.2 主要问题

- `--copy-env`、linked worktree、submodule、separate git dir 等边界复杂；若用户直接调用，容易绕过目标 repo 和 secret 保护。
- Source 结构信号：contract headings 不完整，eval 文件数 0，references 0，scripts 1。
- 若该对象被团队复用，应补 owner、review cadence、trigger/output eval 或至少明确 deferred reason；否则不宜伪装成 governed asset。

### 3.3 优化建议

- 补 contract summary、internal-only 边界、`detect --json` schema、env copy trust note；把 create 前 fail-closed 条件写成稳定事实合同。
...
从 skill-creator 视角看，git-worktree 需要稳定“触发描述、执行步骤、输出契约、失败回退、验证样例”这五件事。当前最大缺口是：保持 internal-only，不暴露为用户入口。待当前 dirty work 收口后，补标准 contract summary；把 `--copy-env` 安全边界升级为 reviewer-visible trust note，并把 `detect --json` facts contract 写成稳定 schema 小节。
...
git-worktree 应被放在 核心链路，服务主链路或支撑链路的明确节点；不应作为新的泛入口扩散。

### 5.2 架构定位判断

保持 internal helper，只由公开 workflow 委托；不应出现在用户 public entrypoint 菜单。
...
三方一致认为：git-worktree 的优化应先落在边界、输出契约、失败处理和验证证据上；不应在缺消费者证据时新增平行对象。
...
- 原报告的定位/风险/优化建议已经指向具体对象问题：`--copy-env`、linked worktree、submodule、separate git dir 等边界复杂；若用户直接调用，容易绕过目标 repo 和 secret 保护。
- 当前 source 事实显示：资源目录计数：references=0，scripts=1，evals=0，assets=0。
...
|---|---|---|---|---|---|---|
| 保持 internal-only，不暴露为用户入口 | Skill owner / workflow maintainer | 保留当前对象且进入 source 修改阶段 | 低到中 | 降低误触发和 handoff 歧义 | 可能增加少量维护成本 | P1 |
...
作为 核心链路 中的可复用方法节点，只承担原报告确认的职责：为公开 workflow 提供隔离 worktree 创建、检测和清理的 internal helper。

### 9.2 推荐职责边界

- Own：为公开 workflow 提供隔离 worktree 创建、检测和清理的 internal helper。
...
|---|---|---|---|---|
| Source 引用回源 | `rg 'git-worktree' skills agents src docs tests`，排除 generated mirrors | 明确消费者、调用路径和历史引用 | Skill owner | P1 |
| 结构契约检查 | 检查 required headings / output schema / failure modes | 缺口补齐且入口不过度膨胀 | Maintainer | P1 |
| Eval / fixture | 新增或规范 normalized eval，覆盖触发、near-neighbor、失败模式 | 至少覆盖原报告验证建议：扩展既有 `git-worktree-contracts`，覆盖 env copy opt-in、linked worktree 拒绝、not-git-repo、separate git dir。 | Test owner | P1 |
...
- git-worktree 当前建议为 优化，优先级 P1。
- 原报告核心风险：`--copy-env`、linked worktree、submodule、separate git dir 等边界复杂；若用户直接调用，容易绕过目标 repo 和 secret 保护。
- 当前 source 事实：存在：`skills/git-worktree/SKILL.md`，118 行；资源目录计数：references=0，scripts=1，evals=0，assets=0。
