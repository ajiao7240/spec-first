...
|---|---|
| 对象类型 | Skill |
| 对象名称 | gemini-imagegen |
| 来源报告 | 2026-06-20-全量-skill-agent-优化建议.md |
| 原报告问题摘要 | 这是 provider-specific image generation helper，必须把 credentials、network、输出文件和 privacy 边界显式化。 |
| 审查日期 | 2026-06-20 |
| 审查结论等级 | 优化 |
| 优先级 | P1 |
...
- 事实 1：原报告位置为 docs/项目审查/2026-06-20-全量-skill-agent-优化建议.md:37；深审 docs/项目审查/2026-06-20-全量-skill-agent-优化建议.md:113。
- 事实 2：存在：`skills/gemini-imagegen/SKILL.md`，239 行。
- 事实 3：资源目录计数：references=0，scripts=10，evals=0，assets=0。
- 事实 4：原报告建议为：增加 provider credential、network、输出路径、覆盖写入、reference image privacy 的 failure modes。将 API 参数长表移到 references；用 mock runner 对 script 参数解析和输出文件合同做 smoke eval。

### 2.2 推断

- 推断 1：该对象应进入“优化”处理路径。
  - 推断依据：原报告定位/风险/优化动作与当前 source 结构信号一致；主要风险为“API key、reference image、输出覆盖、外部网络失败都属于高频失败面；当前入口不够支撑安全 handoff。”。
  - 可信度：高
...
| 待验证项 | 验证方式 | 影响范围 | 优先级 |
|---|---|---|---|
| 当前对象真实消费者与调用路径 | `rg 'gemini-imagegen' skills agents src docs tests`，并区分 source 引用、历史文档和 generated mirror | 决定保留、合并或退役 | P1 |
...
从 $yao-meta-skill 的元能力视角看，gemini-imagegen 的核心问题不是“是否还能写得更多”，而是边界、复用、治理证据是否与其复用等级匹配。当前建议：优化。

### 3.2 主要问题

- API key、reference image、输出覆盖、外部网络失败都属于高频失败面；当前入口不够支撑安全 handoff。
- Source 结构信号：contract headings 不完整，eval 文件数 0，references 0，scripts 10。
- 若该对象被团队复用，应补 owner、review cadence、trigger/output eval 或至少明确 deferred reason；否则不宜伪装成 governed asset。

### 3.3 优化建议

- 补 provider credential contract、reference image privacy、输出路径/覆盖策略、失败 reason_code；把参数长表和 provider 细节迁入 references 或 scripts help。
...
从 skill-creator 视角看，gemini-imagegen 需要稳定“触发描述、执行步骤、输出契约、失败回退、验证样例”这五件事。当前最大缺口是：增加 provider credential、network、输出路径、覆盖写入、reference image privacy 的 failure modes。将 API 参数长表移到 references；用 mock runner 对 script 参数解析和输出文件合同做 smoke eval。
...
gemini-imagegen 应被放在 支撑/实验链路，服务主链路或支撑链路的明确节点；不应作为新的泛入口扩散。

### 5.2 架构定位判断

这是 provider-specific image generation helper，必须把 credentials、network、输出文件和 privacy 边界显式化。
...
三方一致认为：gemini-imagegen 的优化应先落在边界、输出契约、失败处理和验证证据上；不应在缺消费者证据时新增平行对象。
...
- 原报告的定位/风险/优化建议已经指向具体对象问题：API key、reference image、输出覆盖、外部网络失败都属于高频失败面；当前入口不够支撑安全 handoff。
- 当前 source 事实显示：资源目录计数：references=0，scripts=10，evals=0，assets=0。
...
|---|---|---|---|---|---|---|
| 增加 provider credential、network、输出路径、覆盖写入、reference image privacy 的 failure modes | Skill owner / workflow maintainer | 保留当前对象且进入 source 修改阶段 | 低到中 | 降低误触发和 handoff 歧义 | 可能增加少量维护成本 | P1 |
...
作为 支撑/实验链路 中的可复用方法节点，只承担原报告确认的职责：通过 Gemini 相关 provider 脚本生成图片，并管理提示词、参考图和输出文件。

### 9.2 推荐职责边界

- Own：通过 Gemini 相关 provider 脚本生成图片，并管理提示词、参考图和输出文件。
...
|---|---|---|---|---|
| Source 引用回源 | `rg 'gemini-imagegen' skills agents src docs tests`，排除 generated mirrors | 明确消费者、调用路径和历史引用 | Skill owner | P1 |
| 结构契约检查 | 检查 required headings / output schema / failure modes | 缺口补齐且入口不过度膨胀 | Maintainer | P1 |
| Eval / fixture | 新增或规范 normalized eval，覆盖触发、near-neighbor、失败模式 | 至少覆盖原报告验证建议：用 mock runner 验证参数解析、缺 credential、网络失败、目标文件已存在、reference image 输入五类。 | Test owner | P1 |
...
- gemini-imagegen 当前建议为 优化，优先级 P1。
- 原报告核心风险：API key、reference image、输出覆盖、外部网络失败都属于高频失败面；当前入口不够支撑安全 handoff。
- 当前 source 事实：存在：`skills/gemini-imagegen/SKILL.md`，239 行；资源目录计数：references=0，scripts=10，evals=0，assets=0。
