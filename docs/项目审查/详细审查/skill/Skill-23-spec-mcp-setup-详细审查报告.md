...
| 原报告问题摘要 | 这是 setup/runtime readiness workflow，确实会写 provider/runtime artifacts，但不是把 generated mirrors 当 source 修。 |
| 审查日期 | 2026-06-20 |
| 审查结论等级 | 优化 |
| 优先级 | P1 |
...
- 事实 1：原报告位置为 docs/项目审查/2026-06-20-全量-skill-agent-优化建议.md:54；深审 docs/项目审查/2026-06-20-全量-skill-agent-优化建议.md:249。
- 事实 2：存在：`skills/spec-mcp-setup/SKILL.md`，172 行。
- 事实 3：资源目录计数：references=2，scripts=44，evals=0，assets=0。
- 事实 4：原报告建议为：不是 P0 阻塞；source 文本已说明 generated runtime 不是 source。建议补显式 `When To Use/When Not To Use/Outputs` headings 以满足本仓 audit 结构；将 provider/project writes 与 generated mirror source fixes 的区别写成固定术语，并修 audit 规则误报。对 `sudo` 建议输出补 “只打印建议、不自动执行” 证据。

### 2.2 推断

- 推断 1：该对象应进入“优化”处理路径。
  - 推断依据：原报告定位/风险/优化动作与当前 source 结构信号一致；主要风险为“provider/project writes、generated runtime regeneration、source patch 三者如果术语不清，会让 scanner 和用户都误判风险；`sudo` 建议也必须是“只打印建议”。”。
  - 可信度：高
...
| 待验证项 | 验证方式 | 影响范围 | 优先级 |
|---|---|---|---|
| 当前对象真实消费者与调用路径 | `rg 'spec-mcp-setup' skills agents src docs tests`，并区分 source 引用、历史文档和 generated mirror | 决定保留、合并或退役 | P1 |
...
从 $yao-meta-skill 的元能力视角看，spec-mcp-setup 的核心问题不是“是否还能写得更多”，而是边界、复用、治理证据是否与其复用等级匹配。当前建议：优化。

### 3.2 主要问题

- provider/project writes、generated runtime regeneration、source patch 三者如果术语不清，会让 scanner 和用户都误判风险；`sudo` 建议也必须是“只打印建议”。
- Source 结构信号：contract headings 不完整，eval 文件数 0，references 2，scripts 44。
- 若该对象被团队复用，应补 owner、review cadence、trigger/output eval 或至少明确 deferred reason；否则不宜伪装成 governed asset。

### 3.3 优化建议

- 补显式 Purpose/When To Use/When Not To Use/Inputs/Outputs/Failure Modes；定义 provider runtime write、generated mirror regeneration、source fix 三个术语；修 scanner false positive。
...
从 skill-creator 视角看，spec-mcp-setup 需要稳定“触发描述、执行步骤、输出契约、失败回退、验证样例”这五件事。当前最大缺口是：不是 P0 阻塞；source 文本已说明 generated runtime 不是 source。建议补显式 `When To Use/When Not To Use/Outputs` headings 以满足本仓 audit 结构；将 provider/project writes 与 generated mirror source fixes 的区别写成固定术语，并修 audit 规则误报。对 `sudo` 建议输出补 “只打印建议、不自动执行” 证据。
...
spec-mcp-setup 应被放在 扩展链路，服务主链路或支撑链路的明确节点；不应作为新的泛入口扩散。

### 5.2 架构定位判断

这是 setup/runtime readiness workflow，确实会写 provider/runtime artifacts，但不是把 generated mirrors 当 source 修。
...
三方一致认为：spec-mcp-setup 的优化应先落在边界、输出契约、失败处理和验证证据上；不应在缺消费者证据时新增平行对象。
...
- 原报告的定位/风险/优化建议已经指向具体对象问题：provider/project writes、generated runtime regeneration、source patch 三者如果术语不清，会让 scanner 和用户都误判风险；`sudo` 建议也必须是“只打印建议”。
- 当前 source 事实显示：资源目录计数：references=2，scripts=44，evals=0，assets=0。
...
|---|---|---|---|---|---|---|
| 不是 P0 阻塞；source 文本已说明 generated runtime 不是 source | Skill owner / workflow maintainer | 保留当前对象且进入 source 修改阶段 | 低到中 | 降低误触发和 handoff 歧义 | 可能增加少量维护成本 | P1 |

### 8.2 中期重构

| 建议 | 适用角色 | 前提 | 代价 | 收益 | 风险 | 优先级 |
|---|---|---|---|---|---|---|
| 建立 normalized eval / smoke fixture 覆盖主要分流和失败模式 | Test owner / reviewer | 已确定对象仍被消费，且有可构造 fixture | 中 | 让建议从 prose 变成可回归证据 | fixture 过窄会制造虚假信心 | P1 |
...
作为 扩展链路 中的可复用方法节点，只承担原报告确认的职责：安装、检查和修复 spec-first harness、MCP、provider 与 runtime readiness。

### 9.2 推荐职责边界

- Own：安装、检查和修复 spec-first harness、MCP、provider 与 runtime readiness。
...
|---|---|---|---|---|
| Source 引用回源 | `rg 'spec-mcp-setup' skills agents src docs tests`，排除 generated mirrors | 明确消费者、调用路径和历史引用 | Skill owner | P1 |
| 结构契约检查 | 检查 required headings / output schema / failure modes | 缺口补齐且入口不过度膨胀 | Maintainer | P1 |
| Eval / fixture | 新增或规范 normalized eval，覆盖触发、near-neighbor、失败模式 | 至少覆盖原报告验证建议：运行 `npm run test:mcp-setup` 和 scanner fixture，覆盖 runtime mention、init regeneration、sudo suggestion、secret regex protection。 | Test owner | P1 |
...
- spec-mcp-setup 当前建议为 优化，优先级 P1。
- 原报告核心风险：provider/project writes、generated runtime regeneration、source patch 三者如果术语不清，会让 scanner 和用户都误判风险；`sudo` 建议也必须是“只打印建议”。
- 当前 source 事实：存在：`skills/spec-mcp-setup/SKILL.md`，172 行；资源目录计数：references=2，scripts=44，evals=0，assets=0。
