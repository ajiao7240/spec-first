# spec-app-consistency-audit 详细审查报告
...
|---|---|
| 对象类型 | Skill |
| 对象名称 | spec-app-consistency-audit |
| 来源报告 | 2026-06-20-全量-skill-agent-优化建议.md |
| 原报告问题摘要 | 这是 mobile app 一致性审查 workflow，脚本面完整，下一步重点是 failure envelope 和 eval。 |
| 审查日期 | 2026-06-20 |
| 审查结论等级 | 优化 |
| 优先级 | P1 |
...
- 推断 1：该对象应进入“优化”处理路径。
  - 推断依据：原报告定位/风险/优化动作与当前 source 结构信号一致；主要风险为“PRD/Figma/source 任一缺失时容易降级不清；secret regex 防护被 scanner 当风险会污染优先级。”。
  - 可信度：高
...
| 待验证项 | 验证方式 | 影响范围 | 优先级 |
|---|---|---|---|
| 当前对象真实消费者与调用路径 | `rg 'spec-app-consistency-audit' skills agents src docs tests`，并区分 source 引用、历史文档和 generated mirror | 决定保留、合并或退役 | P1 |
...
从 $yao-meta-skill 的元能力视角看，spec-app-consistency-audit 的核心问题不是“是否还能写得更多”，而是边界、复用、治理证据是否与其复用等级匹配。当前建议：优化。

### 3.2 主要问题

- PRD/Figma/source 任一缺失时容易降级不清；secret regex 防护被 scanner 当风险会污染优先级。
- Source 结构信号：contract headings 基本完整，eval 文件数 0，references 3，scripts 22。
- 若该对象被团队复用，应补 owner、review cadence、trigger/output eval 或至少明确 deferred reason；否则不宜伪装成 governed asset。

### 3.3 优化建议

- 在入口显式挂载关键 references；补 missing PRD/Figma/source、headless failure、secret-path redaction 的 eval；同步修 scanner 对保护性 regex 的分类。
...
spec-app-consistency-audit 应被放在 扩展链路，服务主链路或支撑链路的明确节点；不应作为新的泛入口扩散。

### 5.2 架构定位判断

这是 mobile app 一致性审查 workflow，脚本面完整，下一步重点是 failure envelope 和 eval。
...
三方一致认为：spec-app-consistency-audit 的优化应先落在边界、输出契约、失败处理和验证证据上；不应在缺消费者证据时新增平行对象。
...
- 原报告的定位/风险/优化建议已经指向具体对象问题：PRD/Figma/source 任一缺失时容易降级不清；secret regex 防护被 scanner 当风险会污染优先级。
- 当前 source 事实显示：资源目录计数：references=3，scripts=22，evals=0，assets=0。
...
|---|---|---|---|---|---|---|
| 将 3 个 references 在入口显式挂载，避免 unused-reference signal | Skill owner / workflow maintainer | 保留当前对象且进入 source 修改阶段 | 低到中 | 降低误触发和 handoff 歧义 | 可能增加少量维护成本 | P1 |
...
作为 扩展链路 中的可复用方法节点，只承担原报告确认的职责：跨 PRD、Figma、源码、路由、组件、i18n、analytics 审查移动 App 一致性。

### 9.2 推荐职责边界

- Own：跨 PRD、Figma、源码、路由、组件、i18n、analytics 审查移动 App 一致性。
...
|---|---|---|---|---|
| Source 引用回源 | `rg 'spec-app-consistency-audit' skills agents src docs tests`，排除 generated mirrors | 明确消费者、调用路径和历史引用 | Skill owner | P1 |
| 结构契约检查 | 检查 required headings / output schema / failure modes | 缺口补齐且入口不过度膨胀 | Maintainer | P1 |
| Eval / fixture | 新增或规范 normalized eval，覆盖触发、near-neighbor、失败模式 | 至少覆盖原报告验证建议：运行 focused script smoke，验证每类 missing input 都输出 reason_code 而非 freeform 失败。 | Test owner | P1 |
...
- spec-app-consistency-audit 当前建议为 优化，优先级 P1。
- 原报告核心风险：PRD/Figma/source 任一缺失时容易降级不清；secret regex 防护被 scanner 当风险会污染优先级。
- 当前 source 事实：存在：`skills/spec-app-consistency-audit/SKILL.md`，383 行；资源目录计数：references=3，scripts=22，evals=0，assets=0。
