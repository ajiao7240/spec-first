...
| 原报告问题摘要 | 这是 Knowledge Harness 的 promotion workflow；核心价值是防止未验证经验进入 durable knowledge。 |
| 审查日期 | 2026-06-20 |
| 审查结论等级 | 重构 |
| 优先级 | P1 |
...
- 事实 1：原报告位置为 docs/项目审查/2026-06-20-全量-skill-agent-优化建议.md:48；深审 docs/项目审查/2026-06-20-全量-skill-agent-优化建议.md:201。
- 事实 2：存在：`skills/spec-compound/SKILL.md`，630 行。
- 事实 3：资源目录计数：references=3，scripts=1，evals=1，assets=1。
- 事实 4：原报告建议为：不改 source/runtime 边界文本。建议修 audit scanner 的误报；本 skill 侧可把 promotion gate、YAML schema、session recall、knowledge taxonomy 下沉，入口保留 capture criteria、invalidations、frontmatter output。补 “不把未验证经验写入 docs/solutions” eval。

### 2.2 推断

- 推断 1：该对象应进入“重构”处理路径。
  - 推断依据：原报告定位/风险/优化动作与当前 source 结构信号一致；主要风险为“entry 约 10.4k tokens，promotion gate、YAML schema、session recall、taxonomy 混在入口，容易弱化“ verified + reusable + invalidation condition”的硬边界。”。
  - 可信度：高
...
| 待验证项 | 验证方式 | 影响范围 | 优先级 |
|---|---|---|---|
| 当前对象真实消费者与调用路径 | `rg 'spec-compound' skills agents src docs tests`，并区分 source 引用、历史文档和 generated mirror | 决定保留、合并或退役 | P1 |
...
从 $yao-meta-skill 的元能力视角看，spec-compound 的核心问题不是“是否还能写得更多”，而是边界、复用、治理证据是否与其复用等级匹配。当前建议：重构。

### 3.2 主要问题

- entry 约 10.4k tokens，promotion gate、YAML schema、session recall、taxonomy 混在入口，容易弱化“ verified + reusable + invalidation condition”的硬边界。
- Source 结构信号：contract headings 基本完整，eval 文件数 1，references 3，scripts 1。
- 若该对象被团队复用，应补 owner、review cadence、trigger/output eval 或至少明确 deferred reason；否则不宜伪装成 governed asset。

### 3.3 优化建议

- 不改 source/runtime 边界文本；优先修 audit scanner 的 runtime mention false positive；入口瘦身到 capture criteria、promotion gate、frontmatter output。
...
从 skill-creator 视角看，spec-compound 需要稳定“触发描述、执行步骤、输出契约、失败回退、验证样例”这五件事。当前最大缺口是：不改 source/runtime 边界文本。建议修 audit scanner 的误报；本 skill 侧可把 promotion gate、YAML schema、session recall、knowledge taxonomy 下沉，入口保留 capture criteria、invalidations、frontmatter output。补 “不把未验证经验写入 docs/solutions” eval。
...
spec-compound 应被放在 核心链路，服务主链路或支撑链路的明确节点；不应作为新的泛入口扩散。

### 5.2 架构定位判断

这是 Knowledge Harness 的 promotion workflow；核心价值是防止未验证经验进入 durable knowledge。
...
三方一致认为：spec-compound 的优化应先落在边界、输出契约、失败处理和验证证据上；不应在缺消费者证据时新增平行对象。
...
- 原报告的定位/风险/优化建议已经指向具体对象问题：entry 约 10.4k tokens，promotion gate、YAML schema、session recall、taxonomy 混在入口，容易弱化“ verified + reusable + invalidation condition”的硬边界。
- 当前 source 事实显示：资源目录计数：references=3，scripts=1，evals=1，assets=1。
...
|---|---|---|---|---|---|---|
| 不改 source/runtime 边界文本 | Skill owner / workflow maintainer | 保留当前对象且进入 source 修改阶段 | 低到中 | 降低误触发和 handoff 歧义 | 可能增加少量维护成本 | P1 |
...
作为 核心链路 中的可复用方法节点，只承担原报告确认的职责：把已验证的问题解决经验沉淀为 `docs/solutions/` durable knowledge。

### 9.2 推荐职责边界

- Own：把已验证的问题解决经验沉淀为 `docs/solutions/` durable knowledge。
...
|---|---|---|---|---|
| Source 引用回源 | `rg 'spec-compound' skills agents src docs tests`，排除 generated mirrors | 明确消费者、调用路径和历史引用 | Skill owner | P1 |
| 结构契约检查 | 检查 required headings / output schema / failure modes | 缺口补齐且入口不过度膨胀 | Maintainer | P1 |
| Eval / fixture | 新增或规范 normalized eval，覆盖触发、near-neighbor、失败模式 | 至少覆盖原报告验证建议：增加 “未验证经验不得写入 `docs/solutions/`”“generated runtime 只作 evidence 不 patch” 两类 eval。 | Test owner | P1 |
...
- spec-compound 当前建议为 重构，优先级 P1。
- 原报告核心风险：entry 约 10.4k tokens，promotion gate、YAML schema、session recall、taxonomy 混在入口，容易弱化“ verified + reusable + invalidation condition”的硬边界。
- 当前 source 事实：存在：`skills/spec-compound/SKILL.md`，630 行；资源目录计数：references=3，scripts=1，evals=1，assets=1。
