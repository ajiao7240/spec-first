...
|---|---|
| 对象类型 | Skill |
| 对象名称 | spec-write-tasks |
| 来源报告 | 2026-06-20-全量-skill-agent-优化建议.md |
| 原报告问题摘要 | 这是成熟的 standalone plan-to-task-pack compiler；应继续防止 task pack 变成第二 source of truth。 |
| 审查日期 | 2026-06-20 |
| 审查结论等级 | 优化 |
| 优先级 | P2 |
...
- 推断 1：该对象应进入“优化”处理路径。
  - 推断依据：原报告定位/风险/优化动作与当前 source 结构信号一致；主要风险为“derived artifact 如果缺 stale/invalidation 规则，会在 plan/PRD 更新后继续被执行。”。
  - 可信度：中
...
| 待验证项 | 验证方式 | 影响范围 | 优先级 |
|---|---|---|---|
| 当前对象真实消费者与调用路径 | `rg 'spec-write-tasks' skills agents src docs tests`，并区分 source 引用、历史文档和 generated mirror | 决定保留、合并或退役 | P2 |
...
从 $yao-meta-skill 的元能力视角看，spec-write-tasks 的核心问题不是“是否还能写得更多”，而是边界、复用、治理证据是否与其复用等级匹配。当前建议：优化。

### 3.2 主要问题

- derived artifact 如果缺 stale/invalidation 规则，会在 plan/PRD 更新后继续被执行。
- Source 结构信号：contract headings 基本完整，eval 文件数 5，references 2，scripts 0。
...
spec-write-tasks 应被放在 核心链路，服务主链路或支撑链路的明确节点；不应作为新的泛入口扩散。

### 5.2 架构定位判断

这是成熟的 standalone plan-to-task-pack compiler；应继续防止 task pack 变成第二 source of truth。
...
三方一致认为：spec-write-tasks 的优化应先落在边界、输出契约、失败处理和验证证据上；不应在缺消费者证据时新增平行对象。
...
- 原报告的定位/风险/优化建议已经指向具体对象问题：derived artifact 如果缺 stale/invalidation 规则，会在 plan/PRD 更新后继续被执行。
- 当前 source 事实显示：资源目录计数：references=2，scripts=0，evals=5，assets=0。
...
|---|---|---|---|---|---|---|
| 结构较成熟 | Skill owner / workflow maintainer | 保留当前对象且进入 source 修改阶段 | 低到中 | 降低误触发和 handoff 歧义 | 可能增加少量维护成本 | P2 |
...
作为 核心链路 中的可复用方法节点，只承担原报告确认的职责：把已定计划编译成可执行 task pack，并保留 derived artifact 边界。

### 9.2 推荐职责边界

- Own：把已定计划编译成可执行 task pack，并保留 derived artifact 边界。
...
|---|---|---|---|---|
| Source 引用回源 | `rg 'spec-write-tasks' skills agents src docs tests`，排除 generated mirrors | 明确消费者、调用路径和历史引用 | Skill owner | P2 |
| 结构契约检查 | 检查 required headings / output schema / failure modes | 缺口补齐且入口不过度膨胀 | Maintainer | P2 |
| Eval / fixture | 新增或规范 normalized eval，覆盖触发、near-neighbor、失败模式 | 至少覆盖原报告验证建议：新增 plan changed after task-pack、missing acceptance source、over-specified task、handoff to doc-review 四类 eval。 | Test owner | P2 |
...
- spec-write-tasks 当前建议为 优化，优先级 P2。
- 原报告核心风险：derived artifact 如果缺 stale/invalidation 规则，会在 plan/PRD 更新后继续被执行。
- 当前 source 事实：存在：`skills/spec-write-tasks/SKILL.md`，431 行；资源目录计数：references=2，scripts=0，evals=5，assets=0。
