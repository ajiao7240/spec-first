...
| 原报告问题摘要 | 这是 HOW planning workflow，不应让 plan artifact 变成第二 source of truth 或执行状态机。 |
| 审查日期 | 2026-06-20 |
| 审查结论等级 | 重构 |
| 优先级 | P1 |
...
- 事实 1：原报告位置为 docs/项目审查/2026-06-20-全量-skill-agent-优化建议.md:56；深审 docs/项目审查/2026-06-20-全量-skill-agent-优化建议.md:265。
- 事实 2：存在：`skills/spec-plan/SKILL.md`，757 行。
- 事实 3：资源目录计数：references=10，scripts=0，evals=1，assets=0。
- 事实 4：原报告建议为：待当前 dirty work 收口后再动。建议继续做 progressive disclosure：plan-only safety、canonical section、deepening workflow、visual/html rendering 各归 references；入口只保留 admission、output contract、handoff、artifact-summary。把 examples 转 normalized eval。

### 2.2 推断

- 推断 1：该对象应进入“重构”处理路径。
  - 推断依据：原报告定位/风险/优化动作与当前 source 结构信号一致；主要风险为“canonical sections、deepening workflow、visual rendering、handoff 细节堆在入口，容易增加 plan 噪音和漂移面。”。
  - 可信度：高
...
| 待验证项 | 验证方式 | 影响范围 | 优先级 |
|---|---|---|---|
| 当前对象真实消费者与调用路径 | `rg 'spec-plan' skills agents src docs tests`，并区分 source 引用、历史文档和 generated mirror | 决定保留、合并或退役 | P1 |
...
从 $yao-meta-skill 的元能力视角看，spec-plan 的核心问题不是“是否还能写得更多”，而是边界、复用、治理证据是否与其复用等级匹配。当前建议：重构。

### 3.2 主要问题

- canonical sections、deepening workflow、visual rendering、handoff 细节堆在入口，容易增加 plan 噪音和漂移面。
- Source 结构信号：contract headings 基本完整，eval 文件数 1，references 10，scripts 0。
- 若该对象被团队复用，应补 owner、review cadence、trigger/output eval 或至少明确 deferred reason；否则不宜伪装成 governed asset。

### 3.3 优化建议

- 继续 progressive disclosure：入口只保留 admission、output contract、source inheritance、handoff、artifact summary；细节归 references。
...
从 skill-creator 视角看，spec-plan 需要稳定“触发描述、执行步骤、输出契约、失败回退、验证样例”这五件事。当前最大缺口是：待当前 dirty work 收口后再动。建议继续做 progressive disclosure：plan-only safety、canonical section、deepening workflow、visual/html rendering 各归 references；入口只保留 admission、output contract、handoff、artifact-summary。把 examples 转 normalized eval。
...
spec-plan 应被放在 核心链路，服务主链路或支撑链路的明确节点；不应作为新的泛入口扩散。

### 5.2 架构定位判断

这是 HOW planning workflow，不应让 plan artifact 变成第二 source of truth 或执行状态机。
...
三方一致认为：spec-plan 的优化应先落在边界、输出契约、失败处理和验证证据上；不应在缺消费者证据时新增平行对象。
...
- 原报告的定位/风险/优化建议已经指向具体对象问题：canonical sections、deepening workflow、visual rendering、handoff 细节堆在入口，容易增加 plan 噪音和漂移面。
- 当前 source 事实显示：资源目录计数：references=10，scripts=0，evals=1，assets=0。
...
|---|---|---|---|---|---|---|
| 待当前 dirty work 收口后再动 | Skill owner / workflow maintainer | 保留当前对象且进入 source 修改阶段 | 低到中 | 降低误触发和 handoff 歧义 | 可能增加少量维护成本 | P1 |
...
作为 核心链路 中的可复用方法节点，只承担原报告确认的职责：把清晰目标转成可执行技术方案，明确 scope、artifact、验证和 handoff。

### 9.2 推荐职责边界

- Own：把清晰目标转成可执行技术方案，明确 scope、artifact、验证和 handoff。
...
|---|---|---|---|---|
| Source 引用回源 | `rg 'spec-plan' skills agents src docs tests`，排除 generated mirrors | 明确消费者、调用路径和历史引用 | Skill owner | P1 |
| 结构契约检查 | 检查 required headings / output schema / failure modes | 缺口补齐且入口不过度膨胀 | Maintainer | P1 |
| Eval / fixture | 新增或规范 normalized eval，覆盖触发、near-neighbor、失败模式 | 至少覆盖原报告验证建议：把 examples 转 normalized eval，覆盖 plan-only、安全边界、dirty worktree、existing review origin、task handoff。 | Test owner | P1 |
...
- spec-plan 当前建议为 重构，优先级 P1。
- 原报告核心风险：canonical sections、deepening workflow、visual rendering、handoff 细节堆在入口，容易增加 plan 噪音和漂移面。
- 当前 source 事实：存在：`skills/spec-plan/SKILL.md`，757 行；资源目录计数：references=10，scripts=0，evals=1，assets=0。
