# agent-native-architecture 详细审查报告
...
|---|---|
| 对象类型 | Skill |
| 对象名称 | agent-native-architecture |
| 来源报告 | 2026-06-20-全量-skill-agent-优化建议.md |
| 原报告问题摘要 | 保留为 internal architecture taxonomy 和 agent-native 设计参照，不应成为 public workflow 或普通实现入口。 |
| 审查日期 | 2026-06-20 |
| 审查结论等级 | 优化 |
| 优先级 | P2 |
...
- 事实 1：原报告位置为 docs/项目审查/2026-06-20-全量-skill-agent-优化建议.md:32；深审 docs/项目审查/2026-06-20-全量-skill-agent-优化建议.md:73。
- 事实 2：存在：`skills/agent-native-architecture/SKILL.md`，328 行。
- 事实 3：资源目录计数：references=16，scripts=0，evals=1，assets=0。
- 事实 4：原报告建议为：保留为 canonical taxonomy，但把 `Why Now` 和长篇原则案例继续下沉到 references；入口只保留 taxonomy、路由表、输出边界和何时 handoff 到 public workflow。补 3-5 个 trigger/near-neighbor eval，防止它被当作普通 implementation workflow。

### 2.2 推断

- 推断 1：该对象应进入“优化”处理路径。
  - 推断依据：原报告定位/风险/优化动作与当前 source 结构信号一致；主要风险为“长篇理念材料留在入口会增加初载成本，也容易让 reviewer 把它当成“所有 agent 设计问题都先走这里”的强路由。”。
  - 可信度：中
...
| 待验证项 | 验证方式 | 影响范围 | 优先级 |
|---|---|---|---|
| 当前对象真实消费者与调用路径 | `rg 'agent-native-architecture' skills agents src docs tests`，并区分 source 引用、历史文档和 generated mirror | 决定保留、合并或退役 | P2 |
...
从 $yao-meta-skill 的元能力视角看，agent-native-architecture 的核心问题不是“是否还能写得更多”，而是边界、复用、治理证据是否与其复用等级匹配。当前建议：优化。

### 3.2 主要问题

- 长篇理念材料留在入口会增加初载成本，也容易让 reviewer 把它当成“所有 agent 设计问题都先走这里”的强路由。
- Source 结构信号：contract headings 基本完整，eval 文件数 1，references 16，scripts 0。
- 若该对象被团队复用，应补 owner、review cadence、trigger/output eval 或至少明确 deferred reason；否则不宜伪装成 governed asset。

### 3.3 优化建议

- 入口保留 taxonomy、routing table、source/runtime 边界和 handoff；把 `Why Now`、Core Principles 长例、Quick Start 继续下沉到 references。
...
从 skill-creator 视角看，agent-native-architecture 需要稳定“触发描述、执行步骤、输出契约、失败回退、验证样例”这五件事。当前最大缺口是：保留为 canonical taxonomy，但把 `Why Now` 和长篇原则案例继续下沉到 references；入口只保留 taxonomy、路由表、输出边界和何时 handoff 到 public workflow。补 3-5 个 trigger/near-neighbor eval，防止它被当作普通 implementation workflow。
...
agent-native-architecture 应被放在 扩展链路，服务主链路或支撑链路的明确节点；不应作为新的泛入口扩散。

### 5.2 架构定位判断

保留为 internal architecture taxonomy 和 agent-native 设计参照，不应成为 public workflow 或普通实现入口。
...
三方一致认为：agent-native-architecture 的优化应先落在边界、输出契约、失败处理和验证证据上；不应在缺消费者证据时新增平行对象。
...
- 原报告的定位/风险/优化建议已经指向具体对象问题：长篇理念材料留在入口会增加初载成本，也容易让 reviewer 把它当成“所有 agent 设计问题都先走这里”的强路由。
- 当前 source 事实显示：资源目录计数：references=16，scripts=0，evals=1，assets=0。
...
|---|---|---|---|---|---|---|
| 保留为 canonical taxonomy，但把 `Why Now` 和长篇原则案例继续下沉到 references；入口只保留 taxonomy、路由表、输出边界和何时 handoff 到 public workflow | Skill owner / workflow maintainer | 保留当前对象且进入 source 修改阶段 | 低到中 | 降低误触发和 handoff 歧义 | 可能增加少量维护成本 | P2 |
...
作为 扩展链路 中的可复用方法节点，只承担原报告确认的职责：提供 agent-native 架构 taxonomy、设计原则和生产护栏参考，供上游 workflow 作为内部架构 lens 使用。

### 9.2 推荐职责边界

- Own：提供 agent-native 架构 taxonomy、设计原则和生产护栏参考，供上游 workflow 作为内部架构 lens 使用。
...
- agent-native-architecture 当前建议为 优化，优先级 P2。
- 原报告核心风险：长篇理念材料留在入口会增加初载成本，也容易让 reviewer 把它当成“所有 agent 设计问题都先走这里”的强路由。
- 当前 source 事实：存在：`skills/agent-native-architecture/SKILL.md`，328 行；资源目录计数：references=16，scripts=0，evals=1，assets=0。
