...
|---|---|
| 对象类型 | Skill |
| 对象名称 | spec-compound-refresh |
| 来源报告 | 2026-06-20-全量-skill-agent-优化建议.md |
| 原报告问题摘要 | 这是 durable knowledge maintenance workflow，风险比普通 docs refresh 高，因为它可能合并、替换或删除历史经验。 |
| 审查日期 | 2026-06-20 |
| 审查结论等级 | 重构 |
| 优先级 | P1 |
...
- 事实 1：原报告位置为 docs/项目审查/2026-06-20-全量-skill-agent-优化建议.md:49；深审 docs/项目审查/2026-06-20-全量-skill-agent-优化建议.md:209。
- 事实 2：存在：`skills/spec-compound-refresh/SKILL.md`，711 行。
- 事实 3：资源目录计数：references=4，scripts=1，evals=0，assets=1。
- 事实 4：原报告建议为：拆 action flows、schema、maintenance model 到 references；补 stale/overlap/delete/merge/update 四类 eval fixture。明确删除 docs/solutions 是 preview-first mutating action，并要求 invalidation condition 被保留或更新。

### 2.2 推断

- 推断 1：该对象应进入“重构”处理路径。
  - 推断依据：原报告定位/风险/优化动作与当前 source 结构信号一致；主要风险为“删除/合并 `docs/solutions/` 如果缺 preview-first 和 invalidation tracking，会破坏 Knowledge Harness 的可追溯性。”。
  - 可信度：高
...
| 待验证项 | 验证方式 | 影响范围 | 优先级 |
|---|---|---|---|
| 当前对象真实消费者与调用路径 | `rg 'spec-compound-refresh' skills agents src docs tests`，并区分 source 引用、历史文档和 generated mirror | 决定保留、合并或退役 | P1 |
...
从 $yao-meta-skill 的元能力视角看，spec-compound-refresh 的核心问题不是“是否还能写得更多”，而是边界、复用、治理证据是否与其复用等级匹配。当前建议：重构。

### 3.2 主要问题

- 删除/合并 `docs/solutions/` 如果缺 preview-first 和 invalidation tracking，会破坏 Knowledge Harness 的可追溯性。
- Source 结构信号：contract headings 基本完整，eval 文件数 0，references 4，scripts 1。
- 若该对象被团队复用，应补 owner、review cadence、trigger/output eval 或至少明确 deferred reason；否则不宜伪装成 governed asset。

### 3.3 优化建议

- 把 action flows、schema、maintenance model 下沉；入口保留 read stale evidence、decide action、preview mutation、preserve invalidation 的 contract。
...
从 skill-creator 视角看，spec-compound-refresh 需要稳定“触发描述、执行步骤、输出契约、失败回退、验证样例”这五件事。当前最大缺口是：拆 action flows、schema、maintenance model 到 references；补 stale/overlap/delete/merge/update 四类 eval fixture。明确删除 docs/solutions 是 preview-first mutating action，并要求 invalidation condition 被保留或更新。
...
spec-compound-refresh 应被放在 核心链路，服务主链路或支撑链路的明确节点；不应作为新的泛入口扩散。

### 5.2 架构定位判断

这是 durable knowledge maintenance workflow，风险比普通 docs refresh 高，因为它可能合并、替换或删除历史经验。

### 5.3 与 spec-first 主链路的关系

它与 `需求输入 -> Graph -> Spec -> Plan -> Work -> Review -> Evidence -> Knowledge` 的关系是：提供 核心链路 能力，但必须通过 source refs、artifact summary、verification 和 handoff 与上下游衔接。若缺少这些字段，就会停留在 prompt 建议而不是工程闭环。
...
三方一致认为：spec-compound-refresh 的优化应先落在边界、输出契约、失败处理和验证证据上；不应在缺消费者证据时新增平行对象。
...
- 原报告的定位/风险/优化建议已经指向具体对象问题：删除/合并 `docs/solutions/` 如果缺 preview-first 和 invalidation tracking，会破坏 Knowledge Harness 的可追溯性。
- 当前 source 事实显示：资源目录计数：references=4，scripts=1，evals=0，assets=1。
...
|---|---|---|---|---|---|---|
| 拆 action flows、schema、maintenance model 到 references；补 stale/overlap/delete/merge/update 四类 eval fixture | Skill owner / workflow maintainer | 保留当前对象且进入 source 修改阶段 | 低到中 | 降低误触发和 handoff 歧义 | 可能增加少量维护成本 | P1 |
...
作为 核心链路 中的可复用方法节点，只承担原报告确认的职责：刷新、合并、替换或退役过期的 `docs/solutions/` 知识文档。

### 9.2 推荐职责边界

- Own：刷新、合并、替换或退役过期的 `docs/solutions/` 知识文档。
...
|---|---|---|---|---|
| Source 引用回源 | `rg 'spec-compound-refresh' skills agents src docs tests`，排除 generated mirrors | 明确消费者、调用路径和历史引用 | Skill owner | P1 |
| 结构契约检查 | 检查 required headings / output schema / failure modes | 缺口补齐且入口不过度膨胀 | Maintainer | P1 |
| Eval / fixture | 新增或规范 normalized eval，覆盖触发、near-neighbor、失败模式 | 至少覆盖原报告验证建议：新增 stale/update/merge/delete/replace 五类 eval，特别覆盖“删除必须先 preview 且保留替代理由”。 | Test owner | P1 |
...
- spec-compound-refresh 当前建议为 重构，优先级 P1。
- 原报告核心风险：删除/合并 `docs/solutions/` 如果缺 preview-first 和 invalidation tracking，会破坏 Knowledge Harness 的可追溯性。
- 当前 source 事实：存在：`skills/spec-compound-refresh/SKILL.md`，711 行；资源目录计数：references=4，scripts=1，evals=0，assets=1。
- 视角 A 强调复用边界、治理证据和资源分层。
- 视角 B 强调触发描述、输出模板、失败处理和 eval。
- 视角 C 强调它必须服务 核心链路，不能制造第二入口或第二真相源。
