...
- 事实 1：原报告位置为 docs/项目审查/2026-06-20-全量-skill-agent-优化建议.md:33；深审 docs/项目审查/2026-06-20-全量-skill-agent-优化建议.md:81。
- 事实 2：存在：`skills/agent-native-audit/SKILL.md`，291 行。
- 事实 3：资源目录计数：references=0，scripts=0，evals=0，assets=0。
- 事实 4：原报告建议为：明确它是内部 review 方法还是 public audit 候选。如果继续保留，补轻量 contract summary、输入/输出表、failure modes，并复用 `agent-native-architecture` references，避免在入口重复展开 8 项原则。
...
| 待验证项 | 验证方式 | 影响范围 | 优先级 |
|---|---|---|---|
| 当前对象真实消费者与调用路径 | `rg 'agent-native-audit' skills agents src docs tests`，并区分 source 引用、历史文档和 generated mirror | 决定保留、合并或退役 | P1 |
...
从 $yao-meta-skill 的元能力视角看，agent-native-audit 的核心问题不是“是否还能写得更多”，而是边界、复用、治理证据是否与其复用等级匹配。当前建议：重构。
...
- 先裁决保留、合并还是退役；若保留，补轻量 contract summary，并明确只审 agent-native architecture choices，不审所有 skill/source 质量。
...
从 skill-creator 视角看，agent-native-audit 需要稳定“触发描述、执行步骤、输出契约、失败回退、验证样例”这五件事。当前最大缺口是：明确它是内部 review 方法还是 public audit 候选。如果继续保留，补轻量 contract summary、输入/输出表、failure modes，并复用 `agent-native-architecture` references，避免在入口重复展开 8 项原则。
...
agent-native-audit 应被放在 扩展链路，服务主链路或支撑链路的明确节点；不应作为新的泛入口扩散。
...
三方一致认为：agent-native-audit 的优化应先落在边界、输出契约、失败处理和验证证据上；不应在缺消费者证据时新增平行对象。
...
|---|---|---|---|---|---|---|
| 明确它是内部 review 方法还是 public audit 候选 | Skill owner / workflow maintainer | 保留当前对象且进入 source 修改阶段 | 低到中 | 降低误触发和 handoff 歧义 | 可能增加少量维护成本 | P1 |
...
作为 扩展链路 中的可复用方法节点，只承担原报告确认的职责：审查 agent-native 设计是否具备能力对等、上下文注入、工具粒度、生产护栏等关键条件。

### 9.2 推荐职责边界

- Own：审查 agent-native 设计是否具备能力对等、上下文注入、工具粒度、生产护栏等关键条件。
