...
|---|---|
| 对象类型 | Skill |
| 对象名称 | spec-dhh-rails-style |
| 来源报告 | 2026-06-20-全量-skill-agent-优化建议.md |
| 原报告问题摘要 | 这是 Rails/Ruby style lens，不应覆盖项目本地 standards，也不应替代 code-review。 |
| 审查日期 | 2026-06-20 |
| 审查结论等级 | 优化 |
| 优先级 | P2 |
...
- 事实 1：原报告位置为 docs/项目审查/2026-06-20-全量-skill-agent-优化建议.md:51；深审 docs/项目审查/2026-06-20-全量-skill-agent-优化建议.md:225。
- 事实 2：存在：`skills/spec-dhh-rails-style/SKILL.md`，186 行。
- 事实 3：资源目录计数：references=6，scripts=0，evals=0，assets=0。
- 事实 4：原报告建议为：补轻量 contract summary 与 `When Not To Use`，明确它是 Rails/Ruby style lens，不覆盖项目自有 standards、不替代 `spec-code-review`。增加 route negatives：非 Ruby、通用 frontend、用户只问 DHH 观点。

### 2.2 推断

- 推断 1：该对象应进入“优化”处理路径。
  - 推断依据：原报告定位/风险/优化动作与当前 source 结构信号一致；主要风险为“personality/style lens 若缺 negative boundary，容易对非 Rails 或已有强约定项目提出偏好型建议。”。
  - 可信度：中
...
| 待验证项 | 验证方式 | 影响范围 | 优先级 |
|---|---|---|---|
| 当前对象真实消费者与调用路径 | `rg 'spec-dhh-rails-style' skills agents src docs tests`，并区分 source 引用、历史文档和 generated mirror | 决定保留、合并或退役 | P2 |
...
从 $yao-meta-skill 的元能力视角看，spec-dhh-rails-style 的核心问题不是“是否还能写得更多”，而是边界、复用、治理证据是否与其复用等级匹配。当前建议：优化。

### 3.2 主要问题

- personality/style lens 若缺 negative boundary，容易对非 Rails 或已有强约定项目提出偏好型建议。
- Source 结构信号：contract headings 不完整，eval 文件数 0，references 6，scripts 0。
- 若该对象被团队复用，应补 owner、review cadence、trigger/output eval 或至少明确 deferred reason；否则不宜伪装成 governed asset。

### 3.3 优化建议

- 补 `When Not To Use`、Inputs/Outputs、project standards precedence；明确只在 Rails/Ruby 且用户需要 DHH/Rails 风格判断时启用。
...
从 skill-creator 视角看，spec-dhh-rails-style 需要稳定“触发描述、执行步骤、输出契约、失败回退、验证样例”这五件事。当前最大缺口是：补轻量 contract summary 与 `When Not To Use`，明确它是 Rails/Ruby style lens，不覆盖项目自有 standards、不替代 `spec-code-review`。增加 route negatives：非 Ruby、通用 frontend、用户只问 DHH 观点。
...
spec-dhh-rails-style 应被放在 扩展链路，服务主链路或支撑链路的明确节点；不应作为新的泛入口扩散。

### 5.2 架构定位判断

这是 Rails/Ruby style lens，不应覆盖项目本地 standards，也不应替代 code-review。
...
三方一致认为：spec-dhh-rails-style 的优化应先落在边界、输出契约、失败处理和验证证据上；不应在缺消费者证据时新增平行对象。
...
- 原报告的定位/风险/优化建议已经指向具体对象问题：personality/style lens 若缺 negative boundary，容易对非 Rails 或已有强约定项目提出偏好型建议。
- 当前 source 事实显示：资源目录计数：references=6，scripts=0，evals=0，assets=0。
...
|---|---|---|---|---|---|---|
| 补轻量 contract summary 与 `When Not To Use`，明确它是 Rails/Ruby style lens，不覆盖项目自有 standards、不替代 `spec-code-review` | Skill owner / workflow maintainer | 保留当前对象且进入 source 修改阶段 | 低到中 | 降低误触发和 handoff 歧义 | 可能增加少量维护成本 | P2 |
...
作为 扩展链路 中的可复用方法节点，只承担原报告确认的职责：以 DHH/Rails 风格作为条件 lens 审视 Rails/Ruby 设计与实现取舍。

### 9.2 推荐职责边界

- Own：以 DHH/Rails 风格作为条件 lens 审视 Rails/Ruby 设计与实现取舍。
...
|---|---|---|---|---|
| Source 引用回源 | `rg 'spec-dhh-rails-style' skills agents src docs tests`，排除 generated mirrors | 明确消费者、调用路径和历史引用 | Skill owner | P2 |
| 结构契约检查 | 检查 required headings / output schema / failure modes | 缺口补齐且入口不过度膨胀 | Maintainer | P2 |
| Eval / fixture | 新增或规范 normalized eval，覆盖触发、near-neighbor、失败模式 | 至少覆盖原报告验证建议：增加 non-Rails、Rails with local standard conflict、general frontend、用户只问观点四类 negative cases。 | Test owner | P2 |
...
- spec-dhh-rails-style 当前建议为 优化，优先级 P2。
- 原报告核心风险：personality/style lens 若缺 negative boundary，容易对非 Rails 或已有强约定项目提出偏好型建议。
- 当前 source 事实：存在：`skills/spec-dhh-rails-style/SKILL.md`，186 行；资源目录计数：references=6，scripts=0，evals=0，assets=0。
