...
| 原报告问题摘要 | 当前是较健康的 brownfield PRD workflow，应保持轻量，不新增第二套 PRD topology。 |
| 审查日期 | 2026-06-20 |
| 审查结论等级 | 优化 |
| 优先级 | P2 |
...
- 事实 1：原报告位置为 docs/项目审查/2026-06-20-全量-skill-agent-优化建议.md:58；深审 docs/项目审查/2026-06-20-全量-skill-agent-优化建议.md:281。
- 事实 2：存在：`skills/spec-prd/SKILL.md`，166 行。
- 事实 3：资源目录计数：references=4，scripts=2，evals=1，assets=0。
- 事实 4：原报告建议为：保持当前轻量。补 brownfield PRD readiness eval：existing-system evidence、domain language drift、single-file PRD vs split topology、plan-ready / not-plan-ready 判定。

### 2.2 推断

- 推断 1：该对象应进入“优化”处理路径。
  - 推断依据：原报告定位/风险/优化动作与当前 source 结构信号一致；主要风险为“继续扩张入口会抵消它现在的优势；readiness checker 的事实输出也不能替代 LLM 对 plan-ready 的语义裁决。”。
  - 可信度：中
...
| 待验证项 | 验证方式 | 影响范围 | 优先级 |
|---|---|---|---|
| 当前对象真实消费者与调用路径 | `rg 'spec-prd' skills agents src docs tests`，并区分 source 引用、历史文档和 generated mirror | 决定保留、合并或退役 | P2 |
...
从 $yao-meta-skill 的元能力视角看，spec-prd 的核心问题不是“是否还能写得更多”，而是边界、复用、治理证据是否与其复用等级匹配。当前建议：优化。

### 3.2 主要问题

- 继续扩张入口会抵消它现在的优势；readiness checker 的事实输出也不能替代 LLM 对 plan-ready 的语义裁决。
- Source 结构信号：contract headings 基本完整，eval 文件数 1，references 4，scripts 2。
- 若该对象被团队复用，应补 owner、review cadence、trigger/output eval 或至少明确 deferred reason；否则不宜伪装成 governed asset。

### 3.3 优化建议

- 保持入口轻量；补 brownfield readiness eval 和 output eval，不把 `docs/prds/` 作为新 source 目录。
...
从 skill-creator 视角看，spec-prd 需要稳定“触发描述、执行步骤、输出契约、失败回退、验证样例”这五件事。当前最大缺口是：保持当前轻量。补 brownfield PRD readiness eval：existing-system evidence、domain language drift、single-file PRD vs split topology、plan-ready / not-plan-ready 判定。
...
spec-prd 应被放在 核心链路，服务主链路或支撑链路的明确节点；不应作为新的泛入口扩散。

### 5.2 架构定位判断

当前是较健康的 brownfield PRD workflow，应保持轻量，不新增第二套 PRD topology。
...
三方一致认为：spec-prd 的优化应先落在边界、输出契约、失败处理和验证证据上；不应在缺消费者证据时新增平行对象。
...
- 原报告的定位/风险/优化建议已经指向具体对象问题：继续扩张入口会抵消它现在的优势；readiness checker 的事实输出也不能替代 LLM 对 plan-ready 的语义裁决。
- 当前 source 事实显示：资源目录计数：references=4，scripts=2，evals=1，assets=0。
...
|---|---|---|---|---|---|---|
| 保持当前轻量 | Skill owner / workflow maintainer | 保留当前对象且进入 source 修改阶段 | 低到中 | 降低误触发和 handoff 歧义 | 可能增加少量维护成本 | P2 |
...
作为 核心链路 中的可复用方法节点，只承担原报告确认的职责：面向既有系统编写、校准或验证 brownfield PRD 级需求。

### 9.2 推荐职责边界

- Own：面向既有系统编写、校准或验证 brownfield PRD 级需求。
...
|---|---|---|---|---|
| Source 引用回源 | `rg 'spec-prd' skills agents src docs tests`，排除 generated mirrors | 明确消费者、调用路径和历史引用 | Skill owner | P2 |
| 结构契约检查 | 检查 required headings / output schema / failure modes | 缺口补齐且入口不过度膨胀 | Maintainer | P2 |
| Eval / fixture | 新增或规范 normalized eval，覆盖触发、near-neighbor、失败模式 | 至少覆盖原报告验证建议：复用 `tests/unit/spec-prd-contracts.test.js`，新增 existing-system evidence、domain-language drift、plan-ready/not-ready 判定 fixture。 | Test owner | P2 |
...
- spec-prd 当前建议为 优化，优先级 P2。
- 原报告核心风险：继续扩张入口会抵消它现在的优势；readiness checker 的事实输出也不能替代 LLM 对 plan-ready 的语义裁决。
- 当前 source 事实：存在：`skills/spec-prd/SKILL.md`，166 行；资源目录计数：references=4，scripts=2，evals=1，assets=0。
