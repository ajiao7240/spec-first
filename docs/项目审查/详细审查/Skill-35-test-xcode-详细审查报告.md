...
| 原报告问题摘要 | 这是 Xcode/iOS verification helper，核心风险是外部 MCP 可用性和 simulator/device 副作用。 |
| 审查日期 | 2026-06-20 |
| 审查结论等级 | 优化 |
| 优先级 | P1 |
...
- 事实 1：原报告位置为 docs/项目审查/2026-06-20-全量-skill-agent-优化建议.md:66；深审 docs/项目审查/2026-06-20-全量-skill-agent-优化建议.md:345。
- 事实 2：存在：`skills/test-xcode/SKILL.md`，209 行。
- 事实 3：资源目录计数：references=0，scripts=0，evals=0，assets=0。
- 事实 4：原报告建议为：补 XcodeBuildMCP unavailable fallback、simulator cleanup、scheme discovery failure、输出 summary schema。增加 static smoke tests，至少覆盖 frontmatter、required headings、no direct unsafe device mutation。

### 2.2 推断

- 推断 1：该对象应进入“优化”处理路径。
  - 推断依据：原报告定位/风险/优化动作与当前 source 结构信号一致；主要风险为“XcodeBuildMCP 不可用、scheme discovery 失败、simulator cleanup、真实设备 mutation 都需要明确降级。”。
  - 可信度：高
...
| 待验证项 | 验证方式 | 影响范围 | 优先级 |
|---|---|---|---|
| 当前对象真实消费者与调用路径 | `rg 'test-xcode' skills agents src docs tests`，并区分 source 引用、历史文档和 generated mirror | 决定保留、合并或退役 | P1 |
...
从 $yao-meta-skill 的元能力视角看，test-xcode 的核心问题不是“是否还能写得更多”，而是边界、复用、治理证据是否与其复用等级匹配。当前建议：优化。

### 3.2 主要问题

- XcodeBuildMCP 不可用、scheme discovery 失败、simulator cleanup、真实设备 mutation 都需要明确降级。
...
- 补 MCP unavailable fallback、scheme discovery contract、simulator cleanup、no unsafe device mutation、summary schema。
...
从 skill-creator 视角看，test-xcode 需要稳定“触发描述、执行步骤、输出契约、失败回退、验证样例”这五件事。当前最大缺口是：补 XcodeBuildMCP unavailable fallback、simulator cleanup、scheme discovery failure、输出 summary schema。增加 static smoke tests，至少覆盖 frontmatter、required headings、no direct unsafe device mutation。
...
test-xcode 应被放在 支撑/实验链路，服务主链路或支撑链路的明确节点；不应作为新的泛入口扩散。

### 5.2 架构定位判断

这是 Xcode/iOS verification helper，核心风险是外部 MCP 可用性和 simulator/device 副作用。
...
三方一致认为：test-xcode 的优化应先落在边界、输出契约、失败处理和验证证据上；不应在缺消费者证据时新增平行对象。
...
- 原报告的定位/风险/优化建议已经指向具体对象问题：XcodeBuildMCP 不可用、scheme discovery 失败、simulator cleanup、真实设备 mutation 都需要明确降级。
...
|---|---|---|---|---|---|---|
| 补 XcodeBuildMCP unavailable fallback、simulator cleanup、scheme discovery failure、输出 summary schema | Skill owner / workflow maintainer | 保留当前对象且进入 source 修改阶段 | 低到中 | 降低误触发和 handoff 歧义 | 可能增加少量维护成本 | P1 |
...
作为 支撑/实验链路 中的可复用方法节点，只承担原报告确认的职责：通过 Xcode/XcodeBuildMCP 验证 iOS/macOS 项目的 build、scheme 和模拟器运行。

### 9.2 推荐职责边界

- Own：通过 Xcode/XcodeBuildMCP 验证 iOS/macOS 项目的 build、scheme 和模拟器运行。
...
|---|---|---|---|---|
| Source 引用回源 | `rg 'test-xcode' skills agents src docs tests`，排除 generated mirrors | 明确消费者、调用路径和历史引用 | Skill owner | P1 |
| 结构契约检查 | 检查 required headings / output schema / failure modes | 缺口补齐且入口不过度膨胀 | Maintainer | P1 |
| Eval / fixture | 新增或规范 normalized eval，覆盖触发、near-neighbor、失败模式 | 至少覆盖原报告验证建议：先加 static smoke tests，覆盖 frontmatter、required headings、no direct unsafe mutation；再补 MCP mock eval。 | Test owner | P1 |
...
- test-xcode 当前建议为 优化，优先级 P1。
- 原报告核心风险：XcodeBuildMCP 不可用、scheme discovery 失败、simulator cleanup、真实设备 mutation 都需要明确降级。
- 当前 source 事实：存在：`skills/test-xcode/SKILL.md`，209 行；资源目录计数：references=0，scripts=0，evals=0，assets=0。
