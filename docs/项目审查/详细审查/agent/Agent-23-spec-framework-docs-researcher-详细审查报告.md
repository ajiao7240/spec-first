# spec-framework-docs-researcher 详细审查报告
...
|---|---|
| 对象类型 | Agent |
| 对象名称 | spec-framework-docs-researcher |
| 来源报告 | 2026-06-20-全量-skill-agent-优化建议.md |
| 原报告问题摘要 | 查阅官方框架文档和版本特定资料，提供 primary-source guidance。 |
| 审查日期 | 2026-06-20 |
| 审查结论等级 | 优化 |
| 优先级 | P1 |
...
- 事实 1：原报告位置为 docs/项目审查/2026-06-20-全量-skill-agent-优化建议.md:397。
- 事实 2：存在：`agents/spec-framework-docs-researcher.agent.md`，97 行。
- 事实 3：Agent frontmatter：model=inherit，tools=Read, Grep, Glob, Bash, WebFetch, WebSearch, mcp__context7__*；结构信号 output=有，confidence=弱/缺，dont-flag=弱/缺。
- 事实 4：原报告建议为：与 `spec-best-practices-researcher` 分工：framework-docs 负责 official/version-specific docs，best-practices 负责 broader patterns。补高时效浏览与 source attribution 规则。

### 2.2 推断

- 推断 1：该对象应进入“优化”处理路径。
  - 推断依据：原报告定位/风险/优化动作与当前 source 结构信号一致；主要风险为“与 `spec-best-practices-researcher` 分工：framework-docs 负责 official/version-specific docs，best-practices 负责 broader patterns。补高时效浏览与 source attribution 规则。”。
  - 可信度：高
...
| 待验证项 | 验证方式 | 影响范围 | 优先级 |
|---|---|---|---|
| 当前对象真实消费者与调用路径 | `rg 'spec-framework-docs-researcher' skills agents src docs tests`，并区分 source 引用、历史文档和 generated mirror | 决定保留、合并或退役 | P1 |
...
从 $yao-meta-skill 的元能力视角看，spec-framework-docs-researcher 的核心问题不是“是否还能写得更多”，而是边界、复用、治理证据是否与其复用等级匹配。当前建议：优化。

### 3.2 主要问题

- 与 `spec-best-practices-researcher` 分工：framework-docs 负责 official/version-specific docs，best-practices 负责 broader patterns。补高时效浏览与 source attribution 规则。
- Source 结构信号：输出格式 已有，confidence calibration 不足，What-you-dont-flag 不足。
...
从 skill-creator 视角看，spec-framework-docs-researcher 需要稳定“触发描述、执行步骤、输出契约、失败回退、验证样例”这五件事。当前最大缺口是：与 `spec-best-practices-researcher` 分工：framework-docs 负责 official/version-specific docs，best-practices 负责 broader patterns。补高时效浏览与 source attribution 规则。
...
spec-framework-docs-researcher 应被放在 Context/Knowledge Agent，服务主链路或支撑链路的明确节点；不应作为新的泛入口扩散。

### 5.2 架构定位判断

查阅官方框架文档和版本特定资料，提供 primary-source guidance。。结合原报告，当前更适合作为 Context/Knowledge Agent 中的条件能力，而不是替代公开 workflow 的 orchestrator。

### 5.3 与 spec-first 主链路的关系

它与 `需求输入 -> Graph -> Spec -> Plan -> Work -> Review -> Evidence -> Knowledge` 的关系是：提供 Context/Knowledge Agent 能力，但必须通过 source refs、artifact summary、verification 和 handoff 与上下游衔接。若缺少这些字段，就会停留在 prompt 建议而不是工程闭环。
...
三方一致认为：spec-framework-docs-researcher 的优化应先落在边界、输出契约、失败处理和验证证据上；不应在缺消费者证据时新增平行对象。
...
- 原报告的定位/风险/优化建议已经指向具体对象问题：与 `spec-best-practices-researcher` 分工：framework-docs 负责 official/version-specific docs，best-practices 负责 broader patterns。补高时效浏览与 source attribution 规则。
- 当前 source 事实显示：Agent frontmatter：model=inherit，tools=Read, Grep, Glob, Bash, WebFetch, WebSearch, mcp__context7__*；结构信号 output=有，confidence=弱/缺，dont-flag=弱/缺。
...
|---|---|---|---|---|---|---|
| 与 `spec-best-practices-researcher` 分工：framework-docs 负责 official/version-specific docs，best-practices 负责 broader patterns | Agent catalog owner / workflow orchestrator | 保留当前对象且进入 source 修改阶段 | 低到中 | 降低误触发和 handoff 歧义 | 可能增加少量维护成本 | P1 |
...
作为 Context/Knowledge Agent 中的专业判断角色，只承担原报告确认的职责：查阅官方框架文档和版本特定资料，提供 primary-source guidance。

### 9.2 推荐职责边界

- Own：查阅官方框架文档和版本特定资料，提供 primary-source guidance。
...
|---|---|---|---|---|
| Source 引用回源 | `rg 'spec-framework-docs-researcher' skills agents src docs tests`，排除 generated mirrors | 明确消费者、调用路径和历史引用 | Agent catalog owner | P1 |
...
- spec-framework-docs-researcher 当前建议为 优化，优先级 P1。
- 原报告核心风险：与 `spec-best-practices-researcher` 分工：framework-docs 负责 official/version-specific docs，best-practices 负责 broader patterns。补高时效浏览与 source attribution 规则。
- 当前 source 事实：存在：`agents/spec-framework-docs-researcher.agent.md`，97 行；Agent frontmatter：model=inherit，tools=Read, Grep, Glob, Bash, WebFetch, WebSearch, mcp__context7__*；结构信号 output=有，confidence=弱/缺，dont-flag=弱/缺。
- 视角 A 强调复用边界、治理证据和资源分层。
- 视角 B 强调触发描述、输出模板、失败处理和 eval。
- 视角 C 强调它必须服务 Context/Knowledge Agent，不能制造第二入口或第二真相源。
