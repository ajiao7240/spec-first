# spec-correctness-reviewer 详细审查报告
...
|---|---|
| 对象类型 | Agent |
| 对象名称 | spec-correctness-reviewer |
| 来源报告 | 2026-06-20-全量-skill-agent-优化建议.md |
| 原报告问题摘要 | 检查代码正确性、状态流、边界条件和错误传播。 |
| 审查日期 | 2026-06-20 |
| 审查结论等级 | 优化 |
| 优先级 | P2 |
...
- 事实 1：原报告位置为 docs/项目审查/2026-06-20-全量-skill-agent-优化建议.md:386。
- 事实 2：存在：`agents/spec-correctness-reviewer.agent.md`，53 行。
- 事实 3：Agent frontmatter：model=inherit，tools=Read, Grep, Glob, Bash；结构信号 output=有，confidence=有，dont-flag=有。
- 事实 4：原报告建议为：补 examples：logic bug、edge case、state mismatch、error propagation；强调 P2/P3 不能仅凭猜测，必须有 source/test/log 证据。

### 2.2 推断

- 推断 1：该对象应进入“优化”处理路径。
  - 推断依据：原报告定位/风险/优化动作与当前 source 结构信号一致；主要风险为“补 examples：logic bug、edge case、state mismatch、error propagation；强调 P2/P3 不能仅凭猜测，必须有 source/test/log 证据。”。
  - 可信度：中
...
| 待验证项 | 验证方式 | 影响范围 | 优先级 |
|---|---|---|---|
| 当前对象真实消费者与调用路径 | `rg 'spec-correctness-reviewer' skills agents src docs tests`，并区分 source 引用、历史文档和 generated mirror | 决定保留、合并或退役 | P2 |
...
从 $yao-meta-skill 的元能力视角看，spec-correctness-reviewer 的核心问题不是“是否还能写得更多”，而是边界、复用、治理证据是否与其复用等级匹配。当前建议：优化。

### 3.2 主要问题

- 补 examples：logic bug、edge case、state mismatch、error propagation；强调 P2/P3 不能仅凭猜测，必须有 source/test/log 证据。
...
从 skill-creator 视角看，spec-correctness-reviewer 需要稳定“触发描述、执行步骤、输出契约、失败回退、验证样例”这五件事。当前最大缺口是：补 examples：logic bug、edge case、state mismatch、error propagation；强调 P2/P3 不能仅凭猜测，必须有 source/test/log 证据。
...
spec-correctness-reviewer 应被放在 审查/验证 Agent，服务主链路或支撑链路的明确节点；不应作为新的泛入口扩散。

### 5.2 架构定位判断

检查代码正确性、状态流、边界条件和错误传播。。结合原报告，当前更适合作为 审查/验证 Agent 中的条件能力，而不是替代公开 workflow 的 orchestrator。
...
三方一致认为：spec-correctness-reviewer 的优化应先落在边界、输出契约、失败处理和验证证据上；不应在缺消费者证据时新增平行对象。
...
- 原报告的定位/风险/优化建议已经指向具体对象问题：补 examples：logic bug、edge case、state mismatch、error propagation；强调 P2/P3 不能仅凭猜测，必须有 source/test/log 证据。
...
|---|---|---|---|---|---|---|
| 补 examples：logic bug、edge case、state mismatch、error propagation；强调 P2/P3 不能仅凭猜测，必须有 source/test/log 证据 | Agent catalog owner / workflow orchestrator | 保留当前对象且进入 source 修改阶段 | 低到中 | 降低误触发和 handoff 歧义 | 可能增加少量维护成本 | P2 |
...
作为 审查/验证 Agent 中的专业判断角色，只承担原报告确认的职责：检查代码正确性、状态流、边界条件和错误传播。

### 9.2 推荐职责边界

- Own：检查代码正确性、状态流、边界条件和错误传播。
...
|---|---|---|---|---|
| Source 引用回源 | `rg 'spec-correctness-reviewer' skills agents src docs tests`，排除 generated mirrors | 明确消费者、调用路径和历史引用 | Agent catalog owner | P2 |
...
- spec-correctness-reviewer 当前建议为 优化，优先级 P2。
- 原报告核心风险：补 examples：logic bug、edge case、state mismatch、error propagation；强调 P2/P3 不能仅凭猜测，必须有 source/test/log 证据。
- 当前 source 事实：存在：`agents/spec-correctness-reviewer.agent.md`，53 行；Agent frontmatter：model=inherit，tools=Read, Grep, Glob, Bash；结构信号 output=有，confidence=有，dont-flag=有。
