...
`review-finding.v1` 是 code、document 和 app consistency reviews 共享的最小 finding envelope。它为 synthesis 提供共同结构，同时不抹平各 workflow 的领域扩展。
...
1. 父级 synthesis 先读取 structured findings，只有 evidence 不足时才打开 reviewer prose。
