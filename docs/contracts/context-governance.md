...
`.spec-first/graph/**`、`.spec-first/providers/**`、`.spec-first/impact/**`、`.spec-first/workspace/**`、`.spec-first/standards/**`、`.spec-first/app-audit/**` 和 `.spec-first/workflows/**` 默认也不是普通 source context。下游 workflow 应优先读取该目录下的 canonical summary、readiness facts、validated contract 或明确路径，而不是扫描整棵 `.spec-first/**`。
...
| `spec-standards` | `.spec-first/standards/**` 的 local baseline、preview、validation facts |
...
3. canonical readiness / standards / review facts 的 summary 或 validated contract。
