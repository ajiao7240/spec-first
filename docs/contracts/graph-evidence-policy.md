...
> - `evidence_posture=primary|fallback` 表示本轮 Plan 选择继续使用 GitNexus native capability，还是切到 direct source reads / ast-grep / git diff / code-review-graph。`fallback` 是 posture，不是 evidence grade。
