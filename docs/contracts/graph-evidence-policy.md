...
- Parent workspace 下 GitNexus group/registry evidence 只是一种 read-only query surface；消费规则见 `docs/contracts/workspace-gitnexus-consumption.md`。`group_missing` 不是 provider failure，dirty refresh blocked 也不等于 query unusable。
