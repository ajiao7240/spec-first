...
- **正文字数不少于 1.5 万字（15000 字节），正文配图 6-10 张**（不含封面）。
...
1. **先确认文章字数是否达到 1.5 万字**；不足时先扩写，再生成配图
...
const checks = {
  ...
  wordCount: Buffer.byteLength(body, 'utf8'),
...
if (checks.wordCount < 15000) { console.error(`FAIL: wordCount=${checks.wordCount} < 15000 (1.5万字)`); process.exitCode = 1; }
