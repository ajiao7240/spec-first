...
description: "（项目专属）微信公众号文章运营全流程流水线。从选题到发布串联：选题 → 写作检测 → 封面图 → 正文配图 → SVG 转换 → 图片完整性 → frontmatter 检查 → 排版预览 → 润色 → 推送。固化 spec-first 系列的 blueprint 风格规范，无需记忆 baoyu-skill 参数。非公开 spec-first workflow 入口，不通过 using-spec-first 路由推荐。Use when user says \"发布公众号文章\"、\"发文\"、\"wechat publish\"、\"走发布流程\"."
argument-hint: "[可选：文章路径，如 docs/11-文章系列/02-ai-coding-harness-publish.md。留空则进入选题步骤]"
---
...
MISSING=()
test -d "$HOME/.claude/skills/baoyu-post-to-wechat" \
  && echo "✓ baoyu-post-to-wechat" \
  || { echo "✗ baoyu-post-to-wechat 未安装"; MISSING+=("baoyu-post-to-wechat"); }
...
if [[ ${#MISSING[@]} -gt 0 ]]; then
  echo ""
  echo "✗ 缺少必需依赖: ${MISSING[*]}"
  echo "安装方式：baoyu-post-to-wechat 参考 https://github.com/JimLiu/baoyu-skills；svg-to-png.mjs 和 install-tech-blog-theme.mjs 应随本 skill 存在。"
  exit 1
fi
```
...
**注意：** frontmatter 检查（Step 4.7）放在排版预览**之前**，确保预览看到的是发布版本的标题结构。润色（Step 6）之后如果文件 frontmatter 被改动，回到 Step 6.5 重新检查。
...
1. 按文章结构选择 3-5 个关键位置
2. 每张图先写为 `docs/11-文章系列/pic/{slug}-{topic}.svg`
3. SVG 必须满足：
...
4. markdown 中引用同名 `.png`，不是 `.svg`
5. 进入 Step 4.5 批量转 PNG
...
# 健壮地判断 frontmatter title 和正文 H1
...
echo "frontmatter: $HAS_FRONTMATTER, frontmatter title: $HAS_FRONTMATTER_TITLE, body H1: $HAS_BODY_H1"

if [[ "$HAS_FRONTMATTER_TITLE" == "yes" && "$HAS_BODY_H1" == "yes" ]]; then
  echo "⚠ 重复风险：frontmatter title 和正文 H1 都存在，会导致编辑器标题与正文标题重复"
elif [[ "$HAS_FRONTMATTER_TITLE" == "no" && "$HAS_BODY_H1" == "no" ]]; then
  echo "⚠ 缺标题：既无 frontmatter title 也无正文 H1，wechat-article.ts 会用文件名做标题"
else
  echo "✓ 标题来源唯一（$([[ "$HAS_FRONTMATTER_TITLE" == "yes" ]] && echo "frontmatter" || echo "H1")）"
fi
```
...
if [[ -n "$H1_TITLE" ]]; then
  if grep -q "^---$" "$ARTICLE"; then
    ...
    # 没有 frontmatter，加上完整的 frontmatter 块
...
if (checks.imagePlaceholders < 1) process.exitCode = 1;
...
```bash
# 关掉残留的微信编辑器 tab，避免 wechat-article.ts 报 "Home page menu did not load"
cat > /tmp/_wechat_close_editor.ts << 'EOF'
  ...
  if (cdp) {
    ...
    for (const t of targetInfos) {
      if (t.type === 'page' && t.url?.includes('appmsg_edit')) {
        console.log('Closing stale editor tab:', t.url.slice(0, 80));
...
EOF
npx -y bun /tmp/_wechat_close_editor.ts
rm -f /tmp/_wechat_close_editor.ts
```
