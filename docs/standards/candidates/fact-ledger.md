...
fact: V2 acquisition outputs require a single-target task pack, evidence quality, source anchors, source matrix, lineage, owner queue and replay/eval boundary.
...
fact: docs/standards/candidates is proposal-only and cannot be hard project context.
...
fact: The standards index declares shared surface, team-standards capability and spec-first-maintainers owner role.
source_type: contract
provider_trust: source_confirmed
source_anchor:
  snapshot_id: 4d47b125
...
fact: The standalone skill has mode-specific reference loading and is not a public workflow.
...
fact: User documentation tells brownfield initialization to use a single slice and treat graph/code evidence as advisory.
source_type: explicit-doc
provider_trust: source_confirmed
source_anchor:
  snapshot_id: 4d47b125
