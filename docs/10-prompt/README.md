...
- `结构化项目角色契约.md` 是本目录的明确例外：它是当前项目角色与演化判断基线，但不是具体 runtime behavior contract

## 当前规范优先级

1. `docs/10-prompt/结构化项目角色契约.md`：角色与演化判断基线
2. `skills/`：当前 skill 合同
3. `agents/`：当前 agent 合同
4. `templates/claude/commands/spec/`：Claude 命令模板
5. `docs/02-架构设计/02-目录结构.md`：当前目录结构与运行态布局说明
6. `CLAUDE.md` / `AGENTS.md`：宿主侧治理与语言规则
