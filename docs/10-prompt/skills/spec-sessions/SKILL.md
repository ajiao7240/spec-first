...
```bash
Claude: /spec:sessions [question or topic]
Codex:  $spec-sessions [question or topic]
```
...
Dispatch `spec-first:research:session-historian` with the user's question as the task prompt. Omit the `mode` parameter so the user's configured permission settings apply. Include in the dispatch prompt:

- The user's question
- The current working directory
- The repo name and git branch from pre-resolved context
  only if they resolved to plain values — do not pass literal command strings
