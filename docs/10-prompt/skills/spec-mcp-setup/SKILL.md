...
- GitNexus `query_probe` 使用 indexed repo label，并写出最多 5 个 source-derived 的 bounded ordered `query_probe_policy.candidates[]`，同时保留 legacy `token` / `selected_from` 字段。候选优先跨栈入口/工作流信号，例如 main/launch/loading/home/login/router/navigation、controller、handler、service、repository、UI form/table/page/dashboard；Android Activity/ViewModel 只是平台信号之一。`postinstall`、`index`、`main`、`systemConfig` 等 lifecycle / config / type / schema / constants basename 只有在没有更好 source 候选时才可作为 fallback。`Health`、`Ping`、`Actuator`、`Status`、`Info`、`Error`、`Metrics` 等基础设施 token 不应压过业务 flow 候选，但 health-only repo 仍保留 low-confidence fallback。`Report`、`View`、`Screen`、`Layout`、`Modal`、`Advertise`、`Guide`、`Dialog`、`Adapter`、`Bean`、`Entity` 这类偏展示或弱证明 basename 必须降级，不能压过更强的 flow-bearing 候选。`spec-graph-bootstrap` 仍需执行自己的 consumer-side candidate limit，不能信任 producer 是唯一边界。
