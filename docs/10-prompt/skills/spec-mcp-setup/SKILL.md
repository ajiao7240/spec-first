...
- GitNexus `query_probe` 使用 indexed repo label，并写出最多 5 个 source-derived 的 bounded ordered `query_probe_policy.candidates[]`，同时保留 legacy `token` / `selected_from` 字段。候选优先跨栈入口/工作流信号，例如 main/launch/loading/home/login/router/navigation、controller、handler、service、repository、UI form/table/page/dashboard；对于 controller-heavy 且类名容易 definitions-only 的仓库，setup 可以从 tracked workflow source 文件中提取 bounded 方法级候选，并优先 `stepSave`、`validatePayload`、`booleanResult`、`failure`、`options`、`add`、`save`、`delete`、`create`、`cancel`、`submit`、`update` 等 flow-like method token。Android Activity/ViewModel 只是平台信号之一。`postinstall`、`index`、`main`、`systemConfig` 等 lifecycle / config / type / schema / constants basename 只有在没有更好 source 候选时才可作为 fallback。`Health`、`Ping`、`Actuator`、`Status`、`Info`、`Error`、`Metrics` 等基础设施 token 不应压过业务 flow 候选，但 health-only repo 仍保留 low-confidence fallback。`Report`、`View`、`Screen`、`Layout`、`Modal`、`Advertise`、`Guide`、`Dialog`、`Adapter`、`Bean`、`Entity` 这类偏展示或弱证明 basename 必须降级，不能压过更强的 flow-bearing 候选。`spec-graph-bootstrap` 仍需执行自己的 consumer-side candidate limit，不能信任 producer 是唯一边界。
...
- batch 模式下首次 Serena bootstrap 必须由 agent 传入 per-child language map（`--serena-language-for <child>=<language>[,<language>]` / `-SerenaLanguageFor`）；全局 `--serena-language` 不能套用到所有 child repo。缺少语言证据的 child 返回 `serena_language_required`，由 LLM 基于 child repo 证据选择语言后重试。
...
- 最终状态块第一段必须是 `Execution result` 汇总表，直接展示 `Harness runtime` 与 `Graph readiness` 判定，并列出 ready/pending graph providers；`Graph providers` 明细表必须同时展示 `Query` 与 `Bootstrap`，避免用户从 `Next` 反推 provider 状态。
