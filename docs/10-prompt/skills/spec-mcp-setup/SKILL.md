...
- 安装路径必须请求最新版本：npm/npx 使用 `@latest`，`uvx` 使用 `--upgrade`，Cargo 支持的工具使用 `--force`，Homebrew/winget handoff 优先 upgrade-before-install；`--verify-only` 仍保持只读，不升级。
...
- setup 完成后在状态块下提示先重启/新会话，再运行 `/spec:graph-bootstrap` 或 `$spec-graph-bootstrap`；同时允许 agent 在确认只需 deterministic script 时响应“继续完成”。
