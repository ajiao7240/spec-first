...
3. **Decide on evidence** -- check if the current PR description already contains evidence (a `## Demo` or `## Screenshots` section, a `user-attachments/assets/` video URL, or markdown image embeds). If evidence exists, preserve it unless the user's focus specifically asks to refresh or remove it. If no evidence exists, follow "Evidence for PR descriptions" in Step 6. Description-only updates may be specifically intended to add evidence.
4. **Write the new description** -- follow the writing principles in Step 6, driven by feature commits, final diff, and the evidence decision.
...
1. **Capture evidence now**:
   - For browser-visible changes, load the `feature-video` skill with a target description inferred from the branch diff. Use its returned GitHub asset URL to build a `## Demo` section.
   - For browser changes that only need still images, load `test-browser` or use `agent-browser` directly to capture screenshots and build a `## Screenshots` section.
   - For CLI or generated artifact changes, run the relevant command, capture the concrete output or artifact screenshot from the workspace, and embed that instead of inventing a visual placeholder.
...
If the chosen capture path fails or yields no shareable artifact, do not add a placeholder section. Summarize the skip reason in the final user report.

Place the evidence embed before the Spec First badge, typically after the summary or within the changes section. Do not label test output as "Demo" or "Screenshots".
...
- After: "Evidence capture now works for browser features, CLI tools, and generated artifacts."
- Scope: "Shipped with PR workflow cleanup because reviewers need one place that decides when evidence is worth the cost."
...
- **Lead with value**: The first sentence should tell the reviewer *what's now possible or fixed*, not what was moved around. "Fixes timeout errors during batch exports" beats "Updated export_handler.py and config.yaml." The subtler failure is leading with the mechanism: "Replaced the hardcoded PR body template" is technically purposeful but still doesn't tell the reviewer what changed for users. "Reviewers now get calibrated PR descriptions that match the real branch scope" does.
...
- If **yes**:
  ...
  3. Decide whether existing evidence should be preserved, refreshed, or omitted using "Evidence for PR descriptions."
  4. Write the description as if fresh, following Step 6's writing principles -- describe the PR's net result.
  5. Include the Spec First badge unless one is already present.
  6. Apply:
