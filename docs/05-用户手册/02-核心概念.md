...
- `.spec-first/specs/repo-profile.yaml` 与 `.spec-first/specs/README.md` 负责记录 repo-level shared spec seeds；它们是项目级共享输入，不进入当前 managed state，也不属于 `clean` 的删除范围
- `spec-repo-profile-refresh` 是 shared seed 的后续补全 skill：默认先预览建议，确认后才把允许字段写回 `.spec-first/specs/repo-profile.yaml`
