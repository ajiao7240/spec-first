...
Spec-First 的当前入口是 `spec-first doctor` 和 `spec-first init (--claude|--codex)`。前者检查环境和项目状态，后者把命令模板、workflow skills、agents、状态文件和项目级 `.developer` 安装到当前项目的 `.claude` 或 `.codex` 目录里。
...
- 用户入口是 `/spec:*`
- 命令模板来自项目本地 `.claude/commands/spec/` 或 `.codex/commands/spec/`
- workflow skills 来自项目本地 `.claude/skills/` 或 `.codex/skills/`
