...
多仓工作区中，从父 workspace 无参数运行时默认处理所有 child repos；`--repo <child>` 用于收窄到单个 child repo，`--all-repos` 是显式等价入口。父 workspace 只能写 advisory `.spec-first/workspace/*summary.json`；child repo 的 setup facts 和 `.serena/*` 必须写在 child repo 内。
...
- dirty refresh blocked 只表示当前 checkout 不适合刷新 index；不等于 GitNexus 旧索引完全不可查询。只读分析可以把 stale/advisory evidence 明确标注为限制后继续，但 dirty path 结论仍要直接读当前源码确认
...
- runtime drift 只能通过重新运行 `spec-first init --claude` 或 `spec-first init --codex` 修复，不手改 `.claude/`、`.codex/`、`.agents/skills/`
