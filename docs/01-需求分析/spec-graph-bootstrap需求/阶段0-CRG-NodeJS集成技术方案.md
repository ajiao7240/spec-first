...
参数约定：多参数命令用具名参数（`--pattern`、`--target`）；唯一主参数命令用位置参数（`crg flow <name>`），与主流 CLI（git/kubectl）惯例一致。
...
| `spec-first crg query` | `query_graph_tool` | `--pattern=<type> --target=<name> [--repo=<path>]` | 8 种查询模式（tests_for / callers_of 等） |
...
**native 模块策略**：`tree-sitter` 和 `better-sqlite3` 均为 native binding（node-gyp 编译）。缓解措施：两者均提供 prebuild binary，常见平台（macOS/Linux/Windows × x64/arm64）安装时直接下载预编译产物，不触发本地编译。降级方案：`sql.js`（纯 JS WASM，性能稍低）作为 `better-sqlite3` 的 fallback，详见 §十一风险。
...
| 13 | `detect_changes_tool` | 分析 git diff（指定 SHA 范围），对每个变更文件/函数输出风险评分（High/Medium/Low）+ 变更原因分类 | ✅ **集成** | P1 | `crg detect-changes` | Phase 4 刷新 + spec-review：风险评分变更分析，fingerprints stale 检测的依据 |
| 14 | `get_review_context_tool` | 针对 git diff 生成 review 上下文包：diff 摘要 + 影响半径分析 + 被修改函数的代码片段 + 相关测试列表 | ✅ **集成** | P1 | `crg review-context` | spec-review 场景：review-change.md 生成的核心数据源 |
| 15 | `get_affected_flows_tool` | 给定 git diff，返回受影响的执行流列表（哪些入口点的调用链经过了被修改的节点） | ✅ **集成** | P1 | `crg affected-flows` | spec-review 场景：变更影响了哪些执行流，review context pack 的补充数据 |
...
```text
集成（17 个）：P0 事实抽取核心（12）+ P0 graphify 借鉴新增（1：surprising-connections）+ P1 review/刷新（4）
不集成（8 个）：向量 embedding（1）+ CRG 自文档（1）+ 重构操作（2）+ wiki（2）+ 多仓库（2）
```
...
`tree-sitter-vue`、`tree-sitter-solidity`、`tree-sitter-kotlin`、`tree-sitter-swift` 等。Python 版支持的 19 语言可逐步补齐。
...
-- 社区表：3-Pass 社区检测产出的模块聚类（crg postprocess 写入）
CREATE TABLE communities (
    id          INTEGER PRIMARY KEY,
    name        TEXT NOT NULL UNIQUE,
    files       TEXT,           -- JSON array of file_path
    metadata    TEXT            -- JSON blob（density / independence / status / note 等，B2/B5 健康评估结果）
);
-- 建表顺序约束：communities 必须在 nodes 之前建（nodes.community_id 外键依赖）

-- 执行流表：入口点及其调用链摘要
CREATE TABLE flows (
    id          INTEGER PRIMARY KEY,
    name        TEXT NOT NULL UNIQUE,  -- 入口函数名
    file_path   TEXT,
...
Bash("spec-first crg query --pattern=callers_of --target=src/api/handler.js --repo=${TARGET_REPO}")
...
- `crg/parser.js`：支持 JS/TS/Python 三种语言，提取 Function/Class/Import/Test 节点和 CALLS/IMPORTS_FROM/TESTED_BY 边
- `crg/graph.js`：SQLite CRUD（基于 §七 schema）
...
### Step 2：覆盖全部 16 个子命令（预计 1-2 周）

**目标**：16 个子命令全部就绪，JSON 输出稳定

新增：

- `crg/communities.js`：目录分组社区检测（Leiden 简化版 fallback）
...
- 扩展语言支持至 v1 全部 11 种语言（去掉 CSS）

验收标准：

- 16 个子命令在 2 个真实项目上均返回合法 JSON，无 exit 2 意外退出
...
**目标**：spec-review 和 Phase 4 刷新所需子命令可靠运行
...
| **native 模块在受限环境编译失败** | 无 node-gyp / build-tools 的 CI 或容器无法安装 | better-sqlite3 fallback 到 `sql.js`（纯 JS WASM）；tree-sitter 有官方 WASM build；性能差距约 2-3x，不影响正确性；fallback 触发条件和性能影响在文档中显式说明 |
