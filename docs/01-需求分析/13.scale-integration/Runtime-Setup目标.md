...
> 2026-06-08 目标修正：裸 `$spec-mcp-setup` 的目标不是“装完 CLI + 首次生成后让用户自己维护”，而是“确认后完成安装、初始化，并启用 provider-native 的项目级自动刷新机制，使后续节点默认可用”。对 Graphify，这意味着在确认的 provider pack 内除 CLI wrapper + `graphify extract ... --no-cluster` 首次生成外，还应执行项目级 `graphify hook install` 作为自动刷新 setup；`graphify watch` 是长运行进程，不作为默认 setup 动作；Graphify SKILL/MCP 仍是用户宿主便利层，不默认安装。
...
| 快照式 CLI（无持久 daemon，产出 run-scoped artifact） | Graphify（CLI 产 `graphify-out/`） | Graphify 原生 CLI/MCP/hook 负责后续 refresh/use；无默认 steady-state daemon；裸 `$spec-mcp-setup` 确认 provider pack 后应安装项目级 hook 自动刷新 | Runtime Setup 做 first generation + project hook onboarding；不启动 `watch`，不代管 hook 内部刷新逻辑；下游缺/旧时 fallback 或提示用户修复 provider-native refresh |
...
- Graphify run-scoped 生成物默认写入 workspace-local provider artifact root，例如 `.spec-first/workspace/providers/graphify/graphify-out/`；缺少 `--requirement-workspace` 时 input scope 默认是 resolved project workspace。
- requirement workspace resolver 的显式 `--requirement-workspace <repo-relative-path>` 只是 input-scope override；绝对路径、`..` escape、缺失或 symlink escape 的显式 override 必须跳过 Graphify first generation 并输出 resolver reason_code，不能退回 repo-root `graphify-out/`。
...
- Graphify（快照式 CLI）：Runtime Setup 可安装 CLI（PyPI 包 `graphifyy`，bin `graphify`），并在用户 opt-in 后为当前 project workspace 执行首次 project-graph 生成。生成命令按场景分层（源码核实 `graphify` v0.8.x）：脚本/CI 用 headless `graphify extract <path> --out <workspace>`（代码 AST 确定性、无 LLM；语义聚类需 backend API key，可 `--no-cluster` 跳过），交互富媒体场景用 AI assistant 内 `/graphify .`（LLM/vision 驱动）。输出目录可经 `--out` 或 `GRAPHIFY_OUT` 环境变量（支持绝对路径）定向到 requirement workspace。它没有默认 steady-state daemon，裸 `$spec-mcp-setup` 确认 provider pack 后应继续执行项目级 `graphify hook install` 让后续 `graphify update` 增量刷新由 provider hook 触发；`--watch` 是长运行进程，不作为默认 setup 动作；Graphify SKILL/MCP 不默认安装。
...
[ ] Graphify
    ...
    推荐原因: 当前任务需要项目级架构导航
    安装位置: .spec-first/tools/graphify wrapper + .spec-first/cache/uv + .spec-first/workspace/providers/graphify/graphify-out
    首次生成: Graphify provider-native project graph generation
    后续刷新: Graphify project hook (`graphify hook install`) + CLI update; 不启动 watch
```
...
- host MCP config
- .codegraph/
- .spec-first/workspace/providers/graphify/graphify-out/
