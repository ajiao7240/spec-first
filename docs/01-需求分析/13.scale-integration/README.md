...
| v1.16 | Capability-aware 协同 | 父方案 + Runtime Setup 目标 + CodeGraph 子方案 | **「Runtime Setup 做 provider onboarding + first generation，消费不耦合」**：`spec-runtime-setup` 过 gate + 用户同意帮装 CodeGraph（prose capability class：`code-graph`；`provider-readiness.kind`：`code-structure`；经 `mcp-tools.json` opt-in entry + `install-mcp` 配 MCP + 首次 index）+ Graphify（`project-graph`，CLI；经 `provider-tools.json` + `install-helpers` 帮装，并在显式 setup/init 阶段完成当前需求 workspace 的首次 project-graph 生成）；消费侧只认能力类别、经 provider-native MCP/CLI 工具接口、advisory 回源、后续刷新归工具；memory 走 `docs/solutions/`、**GBrain 删除**；消费侧不注入编排面/不建 adapter/fusion | 已完成（plan：`docs/plans/2026-06-06-001-feat-capability-aware-provider-coordination-plan.md`；后续 Runtime Setup 生命周期重构计划：`docs/plans/2026-06-07-003-refactor-runtime-setup-lifecycle-plan.md`；focused provider/doc-contract tests 通过） |
