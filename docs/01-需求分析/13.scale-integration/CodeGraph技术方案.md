...
- **Graphify**：Runtime Setup 可负责安装 `graphify` CLI，并在显式 setup/init gate 后执行 Graphify 原生首次 project-graph 生成，随后为当前项目执行 `graphify hook install` 作为 provider-native 自动刷新 setup，并检测工具接口、artifact 与 hook readiness；`graphify-out/`、post-commit/post-checkout hook、后续刷新/查询由 Graphify 原生能力负责。Graphify SKILL/MCP 仍是用户宿主便利层，不默认安装；`graphify watch` 是长运行进程，不作为默认 setup 动作。
...
- **形态**：episodic 一次性生成静态文档（`gh` 实测 59.5k★ / MIT / Python；包名 `graphifyy`、bin 名 `graphify`；核源码产出 `GRAPH_REPORT.md` + `graph.json` + `graph.html`，自带 post-commit hook 用 `git diff HEAD~1` 增量重建、不调 LLM）。在 spec-first 的消费模型里，它默认不是 checked-in 架构文档，而是 workspace-local provider artifact root 下的 run-scoped 临时上下文。
- **安装协同**：`spec-runtime-setup` 可帮装 Graphify CLI wrapper（pinned `graphifyy==0.8.35`，过 gate；`0.8.35` 已用 `pip index versions graphifyy` 实测为 PyPI 当前发布版本，默认不装 latest）。首次生成命令按场景分层（源码核实）：脚本/CI 用 headless `graphify extract <path> --out .spec-first/workspace/providers/graphify/graphify-out --no-cluster`（代码 AST 确定性、无 LLM；语义聚类需 backend API key，本 setup 默认跳过），交互富媒体用 AI assistant 内 `/graphify .`。普通 setup 缺少 `--requirement-workspace` 时 input scope 默认是 resolved project workspace；显式 override 只用于收窄 input scope。确认后的 provider pack 目标还包括项目级 `graphify hook install`，让后续 `graphify update` 增量刷新由 provider hook 触发；spec-first 不在下游 workflow 运行期代生成、不启动 `--watch` 长运行进程、不代管 hook 内部刷新。
- **安装落点**：CLI wrapper 与 cache **workspace-local**（`.spec-first/tools/graphify` + `.spec-first/cache/uv`）；产物 **workspace-local provider artifact**（`.spec-first/workspace/providers/graphify/graphify-out/`，默认临时使用）；git hook **项目级**（`.git/hooks/`，确认后由 `$spec-mcp-setup` 执行 provider-native hook install，失败则 action-required）。MCP server 是**可选 extra**（`graphifyy[mcp]`），默认不装——所以默认无 MCP 配置层，比 CodeGraph 少一层。
