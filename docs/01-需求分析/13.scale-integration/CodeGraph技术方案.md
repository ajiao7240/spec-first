...
```text
CodeGraph = code-graph 能力工具：setup 里帮装(过 gate+用户同意),经 `mcp-tools.json` + `install-mcp` 配 MCP + 首次 index；大仓影响面增强,小仓默认 rg
Graphify  = project-graph 能力工具：setup 里帮装 CLI(过 gate)并做 provider-native 首次生成；后续刷新/查询走 Graphify 原生能力，产物默认是需求 workspace 下的 run-scoped 临时上下文
...
Graphify  = strategic project map，综合源码证据 + 可选 CodeGraph candidates，给当前需求 workspace 提供 run-scoped 高层导航
```

不是“源码服务于 CodeGraph”，而是 CodeGraph 服务于更快找到该读的源码，Graphify 服务于更快理解项目结构和变化影响，源码始终服务于验证结论是否成立。Graphify 可以消费源码证据和可选 CodeGraph candidates，但 CodeGraph 产物只能帮助 Graphify 缩小候选范围，不能成为 Graphify 的事实来源。Graphify 输出默认属于**每次需求的 workspace 下临时生成的 run-scoped 上下文**：可在本次需求、plan、review、debug 中作为架构导航使用，但不自动 check-in、不进入团队 source-of-truth、不替代 `docs/solutions/` 或项目文档。只有经过人工 promotion、补齐 source refs 与 invalidation condition 的内容，才可以沉淀为长期 docs/knowledge。
...
- **CodeGraph**：Runtime Setup 可负责 opt-in、MCP host config、`codegraph init`、readiness probe；之后 `.codegraph` 索引刷新、file watcher、MCP 工具面由 CodeGraph 自己维护。
- **Graphify**：Runtime Setup 可负责安装 `graphify` CLI，并在显式 setup/init gate 后执行 Graphify 原生首次 project-graph 生成，随后检测工具接口与当前 run-scoped artifact readiness；`graphify-out/`、post-commit hook、后续刷新/查询由 Graphify 原生能力负责。即便未来支持 Graphify provider-native hook onboarding，也必须 preview-first + explicit gate，默认不安装 hook。
...
- Graphify 是 CLI + artifact-backed provider：落 `provider-tools.json`，复用 `install-helpers.{sh,ps1}` 安装 CLI；Runtime Setup 在显式 setup/init gate 后可执行 Graphify 原生首次 project-graph 生成，并检测工具接口与当前 run-scoped artifact readiness。
- `spec-runtime-setup` detect 到缺失 → 过 install gate → 用户同意后装。**触发只在 setup workflow 里**，不在 plan/work/review 主动弹（见 §7 复发信号）。
 - provider-native hook onboarding 若未来接入，必须 preview-first + explicit gate，且刷新仍归 provider 自己；默认 setup 不默认装 hook。
...
| 代刷新 / silent 装 hook / 重建并行 drift 检测 / 自动 `graphify .` | 刷新和图谱生成归 provider 或用户显式 context-prep；spec-first 只旁观 freshness/readiness 信号 |
...
- **安装协同**：`spec-runtime-setup` detect → install gate（用户同意）→ `mcp-tools.json` opt-in MCP entry 经 `install-mcp` warmup + 配 MCP + 首次 `codegraph init`。`codegraph init` 已源码核实（本地 `src/bin/codegraph.ts:420-448`）：默认即建初始索引，`-i/--index` 为 deprecated no-op（仅向后兼容），**用 `init` 即可**。一处落地注意：`codegraph init` 在 watcher 可用环境基本非交互，但在 watcher-disabled 环境（WSL2 `/mnt/*`）会弹 hook-vs-manual 的 `@clack/prompts` 选择（可取消降级，不硬阻塞），setup 无人值守路径需处理。**装≠用**用既有 `provider-readiness.v1` 的 lifecycle 布尔位表达（`installed → configured → indexed → server_reachable → query_verified`，见 §5.2）。
...
- **形态**：episodic 一次性生成静态文档（`gh` 实测 59.5k★ / MIT / Python；包名 `graphifyy`、bin 名 `graphify`；核源码产出 `graphify-out/` 下 `GRAPH_REPORT.md` + `graph.json` + `graph.html`，自带 post-commit hook 用 `git diff HEAD~1` 增量重建、不调 LLM）。在 spec-first 的消费模型里，它默认不是 checked-in 架构文档，而是**当前需求 workspace 下的 run-scoped 临时上下文**。
- **安装协同**：`spec-runtime-setup` 可帮装 Graphify CLI（`uv tool install graphifyy`，过 gate；落地前实测 PyPI 当前版本，本地源码 pyproject 已到 0.8.35）。首次生成命令按场景分层（源码核实）：脚本/CI 用 headless `graphify extract <path> --out <workspace>`（代码 AST 确定性、无 LLM；语义聚类需 backend API key，可 `--no-cluster` 跳过），交互富媒体用 AI assistant 内 `/graphify .`。输出目录经 `--out` 或 `GRAPHIFY_OUT` env（支持绝对路径）定向到 run-scoped workspace。spec-first 不在下游 workflow 运行期代生成、不 silent 装其 post-commit hook、不代刷新（刷新走 `graphify update` 增量无 LLM / `--watch` / 用户显式 `graphify hook install`）。若未来支持 provider-native hook onboarding，也必须 preview-first + explicit gate，默认关闭。
- **安装落点**：CLI 本体**用户级**（`uv tool` global，一机一份）；产物 `graphify-out/` **需求 workspace 级**（`graph.json`/`GRAPH_REPORT.md`/`.graphify_python` 均绑当前 repo 与当前任务上下文，默认临时使用）；git hook **项目级**（`.git/hooks/`，可选、用户装）。MCP server 是**可选 extra**（`graphifyy[mcp]`），默认不装——所以默认无 MCP 配置层，比 CodeGraph 少一层。
...
| --- | --- | --- |
| 概览 | 只取 `GRAPH_REPORT.md` 的 **God Nodes / Surprising Connections / Suggested Questions** 三段，**不读全文**（官方明文 "Do NOT paste the full report"） | 若 Graphify MCP/CLI summary/query surface 可用，先通过接口取概览；只有无工具接口但已有 run-scoped `GRAPH_REPORT.md` 时，才有界读取这三段作 fallback |
...
```text
概览: 优先调用 Graphify MCP/CLI summary/query surface; 无接口但有 run-scoped GRAPH_REPORT.md 时,
...
- `spec-runtime-setup` 在 explicit install/init mode 下过 gate + 用户同意：CodeGraph 走 MCP opt-in 管线（配 MCP、首次 index），Graphify 走 CLI provider 管线（帮装 CLI，并为当前需求 workspace 执行 provider-native 首次 project-graph 生成）。
...
- Runtime Setup 按当前需求 workspace 完成 Graphify provider-native 首次 project-graph 生成后，workflow 优先通过 Graphify MCP/CLI 工具接口获取架构导航候选；`GRAPH_REPORT.md` 只作 backing store 或无接口时的有界摘要 fallback。长期沉淀必须人工 promotion 并补 source refs / invalidation condition。
- Runtime Setup 可记录 Graphify CLI 已安装但当前 artifact 缺失，并输出生成 run-scoped artifact 的 next_action；该状态应被视作“当前没有可消费 project-graph artifact”，不是 workflow 内自动生成的触发器。
...
>
> 落点：**CodeGraph = setup 帮装的 code-graph MCP（大仓增强，小仓默认 rg）；Graphify = setup 帮装 CLI 并执行 provider-native 首次生成，默认作为当前需求 workspace 下的 run-scoped 临时上下文；GBrain = 删除，memory 走 docs/solutions/。** install 从来不是 GitNexus 死因——把 install/first generation（走成熟 setup+gate）与 consumption（capability-aware 不耦合）拆开，既兑现「帮用户装好、用户选择是否同意」，又从结构上不重蹈 GitNexus 的消费耦合死法。
