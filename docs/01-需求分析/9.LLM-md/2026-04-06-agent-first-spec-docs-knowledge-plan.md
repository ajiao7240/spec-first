...
- `docs-local-path` 缺失、路径不存在、不是 git repo、working tree 脏、`pull --ff-only` 失败时给出明确错误

**Step 5: Commit**

```bash
git add package.json tests/integration/docs-repo-init.sh tests/smoke/cli.sh src/cli/knowledge.js src/cli/docs-runtime.js src/cli/commands/init.js templates/docs/project-readme.md templates/docs/knowledge-index.md
...
```bash
git add tests/smoke/cli.sh tests/integration/docs-repo-init.sh src/cli/commands/doctor.js src/cli/docs-config.js src/cli/knowledge.js
...
- Modify: `src/cli/commands/doctor.js`
- Test: `tests/unit/knowledge-skill-contracts.sh`
- Modify: `package.json`
...
Run: `bash tests/unit/knowledge-skill-contracts.sh`  
...
- `doctor` 对已绑定 docs repo 的项目执行 validator，并把失败升级为 `ERROR`
- shell tests 覆盖 invalid tree / missing owner / published-in-draft 三类失败场景
...
Run:
- `bash tests/unit/knowledge-skill-contracts.sh`
- `bash tests/smoke/cli.sh`
...
```bash
git add tests/smoke/cli.sh tests/unit/knowledge-skill-contracts.sh skills/spec-bootstrap/SKILL.md
...
在 `tests/unit/knowledge-skill-contracts.sh` 增加：
...
```bash
git add tests/unit/knowledge-skill-contracts.sh tests/smoke/cli.sh skills/spec-compound/SKILL.md skills/spec-compound-refresh/SKILL.md skills/spec-compound/assets/resolution-template.md skills/spec-compound/references/yaml-schema.md
...
```bash
git add tests/unit/knowledge-skill-contracts.sh tests/smoke/cli.sh skills/spec-plan/SKILL.md skills/spec-work/SKILL.md skills/spec-review/SKILL.md
...
- integration 覆盖 docs repo init、dirty working tree、missing localPath、`pull --ff-only` failure 与现有 e2e

**Step 5: Commit**

```bash
git add CHANGELOG.md docs/08-版本更新/README.md package.json tests/unit/docs-config.sh tests/unit/workspace.sh tests/unit/knowledge-skill-contracts.sh tests/smoke/cli.sh tests/integration/docs-repo-init.sh
...
Plan complete and saved to `docs/01-需求分析/9.LLM-md/2026-04-06-agent-first-spec-docs-knowledge-plan.md`. Two execution options:

**1. Subagent-Driven (this session)** - 我在当前会话按任务逐步实现、每步自检后继续

**2. Parallel Session (separate)** - 你开一个新会话，按 `superpowers:executing-plans` 批量执行这份计划
