...
> 方案性质：修订版架构与流程设计稿
> 适用范围：`spec-first` 主框架、`spec-graph-bootstrap`、`spec-brainstorm`、`spec-plan`、`spec-work`、`spec-review`、`spec-compound`、`spec-ideate`
> 参考来源：
...
| reference 元数据（路径、理由、排序分、质量状态） | `.context/spec-first/bootstrap/<slug>/analysis/reference-index.json` | `spec-graph-bootstrap` 初始化；`spec-review` / `spec-improve` 只产出候选更新 | `docs/contexts/<slug>/patterns/index.md`、plan 中的 references section、work 的消费上下文 | 把 `patterns/index.md` 当作机器真源；在 plan/work 中私自新增未登记 reference |
