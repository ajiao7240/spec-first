...
> 方案性质：修订版架构与流程设计稿
> 适用范围：`spec-first` 主框架、`spec-bootstrap`、`spec-brainstorm`、`spec-plan`、`spec-work`、`spec-review`、`spec-compound`、`spec-ideate`
> 参考来源：
...
4. 让 `spec-plan`、`spec-work`、`spec-review` 共享同一套 reference-first 与 history-first 语义
...
1. 是否保持 `spec-bootstrap` 只是增强器而不是 gatekeeper
...
- `spec-review/` 沿用当前项目已经存在的 scratch 路径命名，避免和现有 runtime / 文档约定冲突
...
- `spec-review/<run-id>/meta.json` 必须至少包含 `slug`、`spec_id`、可选 `work_run_id`
...
| `quick_links` | `.context/spec-first/bootstrap/<slug>/analysis/instruction-context.json` | `spec-bootstrap` | `instruction-context.md`、repo-root managed block、`docs/contexts/<slug>/README.md` 中的导航段 | 在多个 Markdown 文档中各自手工维护不同版本的 quick links |
| `high_risk_areas` | `.context/spec-first/bootstrap/<slug>/analysis/instruction-context.json` | `spec-bootstrap`；后续由 review / improve 只产出更新提案 | repo-root managed block、`work/<run-id>/meta.json`、review 选择 reviewer 的运行时上下文 | 直接在 prompt、README 或 plan 文档里手工改写并反向覆盖 JSON 真源 |
| `build/test/verify` 命令 | `.context/spec-first/bootstrap/<slug>/analysis/instruction-context.json`；若目标项目已有稳定脚本入口，应优先记录脚本入口而不是临时命令串 | `spec-bootstrap` 初始化；runtime 只消费 | repo-root managed block、`docs/contexts/<slug>/00-summary.md`、`work/<run-id>/verification.json` 的引用字段 | 在多个 workflow 中各自复制维护命令字符串 |
| reference 元数据（路径、理由、排序分、质量状态） | `.context/spec-first/bootstrap/<slug>/analysis/reference-index.json` | `spec-bootstrap` 初始化；`spec-review` / `spec-improve` 只产出候选更新 | `docs/contexts/<slug>/patterns/index.md`、plan 中的 references section、work 的消费上下文 | 把 `patterns/index.md` 当作机器真源；在 plan/work 中私自新增未登记 reference |
| verify hints / 结构性预检规则 | `.context/spec-first/bootstrap/<slug>/analysis/verify-hints.json` | `spec-bootstrap` | `work/<run-id>/preflight.json` 的引用字段、必要时的人类可读摘要 | 由 `spec-work` 在运行中默默改写 hint 真源 |
...
- 最低可用降级路径应为：读取当前 instruction file -> 做轻量 repo scan -> 明确提示“建议先运行 spec-bootstrap”
...
4. `spec-bootstrap` 在此场景下更偏“初始化上下文、约束、starter assets”，而不是 repo mining
...
1. `spec-bootstrap` 优先生成 `analysis.json`、`reference-index.json`、`verify-hints.json`、`pitfalls`
...
4. `spec-review` 与 `spec-compound` 更强调 pattern、pitfall、failure 的回流复用
...
- `spec-review` 与 `spec-improve` 只产出 reference 质量更新提案，不直接静默改写 `reference-index.json`
...
`spec-bootstrap` 不负责直接构建完整共享知识库，但负责初始化 repo-local 的知识路由上下文。
...
这里的 `Assembly` 专指 `spec-bootstrap` orchestrator 汇总 worker 产物、写入最终文档和结构化资产的阶段。
...
`.context/spec-first/spec-review/<run-id>/`
...
- `.context/spec-first/spec-review/<run-id>/fast-track-proposals.json`
...
- 它是 `proposal`，不是自动写回
- `spec-review` 只负责提出“建议改什么、为什么、证据是什么、建议写到哪里”
...
除通用 reviewer 外，`spec-review` 还应支持根据 bootstrap 与 knowledge 资产触发专项 reviewer。
...
- `spec-review/rule-candidates.json`
- `spec-review/pattern-candidates.json`
...
- `pitfalls_hit_rate`、`pitfalls_action_rate` 来自 `spec-review/<run-id>/findings.json` 与 candidate 结果
...
1. `first_pass_success_rate`
   一条需求在首次 `spec-work -> spec-review` 循环后，无需重大返工即可进入可接受状态的比例
...
- `first_pass_success_rate`
  来自 `work/<run-id>/verification.json`、`spec-review/<run-id>/findings.json` 与后续是否追加大规模 rework 的 metadata
- `spec_to_review_lead_time`
  来自 `spec_id` 对应 plan/work/review 的时间戳
- `blocking_finding_density`
  来自 `spec-review/<run-id>/findings.json` 的严重度统计
...
5. `spec-bootstrap` 初始化 `knowledge/sources.json`、`points.json` 与 `retrieval-policy.json`
...
1. `spec-review` 输出 `meta.json`
2. `spec-review` 输出 `rule-candidates.json`
3. `spec-review` 输出 `pattern-candidates.json`
4. `spec-review` candidate 增加 `suggested_destination`
5. Shared knowledge 引入双路召回
6. `spec-review` 增加专项 reviewer 路由
...
9. `spec-review` 能持久化 `meta.json`，并同时输出 findings、rule candidates、pattern candidates，且 candidate 带 `suggested_destination`
10. `spec-review` 支持基于 pitfall / high-risk area 的专项 reviewer 路由
