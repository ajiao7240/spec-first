# spec-graph-bootstrap v2 需求与实施方案

> 本文是 `spec-graph-bootstrap` v2 的正式需求与实施方案稿。
...
`spec-graph-bootstrap` v1 已经建立了 Stage-0 的基础能力，能够为目标仓库生成一组长期上下文资产，帮助后续阶段快速理解：
...
这与 `spec-graph-bootstrap v2` 的方向高度一致，尤其强化了三个结论：
...
`spec-graph-bootstrap` v2 的产品目标是：
...
| 胶水编程物料 | `spec-graph-bootstrap v2` 对应资产 |
|--------------|------------------------------|
...
`spec-graph-bootstrap` v2 的真正目标，不是把 Stage-0 文档写得更厚，而是：
...
本节定义 `spec-graph-bootstrap v2` 的完整技术实现规格，包括文件结构、工具链、数据格式、算法和错误处理。
...
**触发条件：** 用户在已有 `docs/contexts/<slug>/` 的仓库上再次运行 `spec-graph-bootstrap`。
