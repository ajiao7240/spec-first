# spec-graph-bootstrap v2 产物清单明细表

> 本文定义 `spec-graph-bootstrap` 在 v2 目标态下的长期资产与控制面产物清单。
...
也就是说，`spec-graph-bootstrap` 产物首先按“资产类型”组织，而不是按“前端任务类型”组织。
...
`spec-graph-bootstrap` v2 的核心，不是“多几份 markdown”，而是把 Stage-0 产物从“架构说明书”升级为“研发质量资产面”。
