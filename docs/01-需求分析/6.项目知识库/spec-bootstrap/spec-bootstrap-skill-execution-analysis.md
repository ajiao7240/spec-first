# spec-bootstrap 执行逻辑分析

> 分析对象：`skills/spec-bootstrap/SKILL.md`
> 参考实现：`skills/spec-bootstrap/references/prd-template.md`、`skills/spec-bootstrap/references/database-prd-template.md`
> 辅助参照：`templates/claude/commands/spec/bootstrap.md`、`docs/02-架构设计/03-agent-workflow-patterns.md`、`docs/05-用户手册/02-核心概念.md`

本文聚焦 `spec-bootstrap` 的**执行逻辑**：它如何从命令入口进入，如何做宿主就绪检查，如何分析项目，如何生成任务合同，如何并行执行 worker，以及失败时怎么恢复。

---

## 1. 一句话结论

`spec-bootstrap` 不是一个“生成文档的单一 prompt”，而是一套**Stage-0 上下文编排协议**。
...
`templates/claude/commands/spec/bootstrap.md` 只是命令入口，它不负责真正的分析逻辑，只负责把执行权交给 `skills/spec-bootstrap/SKILL.md`。
...
```text
┌────────────────────────────────────────────────────┐
│  /spec:bootstrap 或 $spec-bootstrap                │
└──────────────────────────────┬─────────────────────┘
                               │
                               v
┌────────────────────────────────────────────────────┐
...
1. 用户显式传入且满足 `[a-z0-9-]+`
2. 复用目标项目内已有 `docs/contexts/*/README.md` 的 `<!-- spec-bootstrap -->` 标记
...
`spec-bootstrap` 的任务合同不是口头说明，而是写入 `.context/spec-first/bootstrap/<slug>/tasks/<task-id>/prd.md` 的正式 PRD。
...
`spec-bootstrap` 的执行逻辑，实质上已经沉淀出一组可以复用的 workflow 模式：
