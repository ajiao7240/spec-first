...
| 归属 | 负责 | 不负责 |
| --- | --- | --- |
| Runtime Setup | 检测 host/runtime 状态；安装 skill / MCP / CLI / helper；配置 host/runtime；执行 provider-native 初次 init / bootstrap / first generation；验证工具接口；输出已安装与已初始化说明 | 在下游 workflow 运行期 lazy 安装；替 provider 做长期刷新；把 provider 输出晋升为 confirmed truth |
...
| 快照式 CLI（无持久 daemon，产出 run-scoped artifact） | Graphify（CLI 产 `graphify-out/`） | Graphify 原生 CLI/MCP/hook 负责后续 refresh/use；无默认 steady-state daemon，hook 必须 opt-in | Runtime Setup 只做 first generation；不代管后续刷新，下游缺/旧时 fallback 或提示用户用 Graphify 原生能力重跑 |
| 进程内 / 库式（无独立生命周期） | tree-sitter 类原语 | 变更检测归宿主调用方，非 provider 自建 watcher | 不作为 Runtime Setup 托管的 steady-state provider |

关键判断：生命周期二分（first-generation 属 setup / steady-state 属 provider）对 daemon 式是干净的；对快照式 CLI，setup 只拥有“首次生成”，默认形态下无托管的 steady-state，用户 opt-in hook 后由 provider hook（如 Graphify post-commit `git diff HEAD~1` 增量重建）代管，仍归 provider 原生而非 Runtime Setup。不要用“provider watcher 代管刷新”框架硬套快照式工具的默认形态。
...
- 目标 scope 必须明确，例如当前 repo、当前需求 workspace 或用户指定 workspace。
- 生成物默认是本地 run-scoped / workspace-scoped 上下文，不自动 check-in，不自动 promotion 到长期 docs。
- run-scoped 生成物必须写入解析后的 requirement workspace，例如 `.spec-first/workspace/requirements/<slug-or-run-id>/graphify-out/`；缺少明确 workspace scope 时应拒绝首次生成并输出 next_action。
- requirement workspace resolver 必须优先使用显式 repo-relative path，其次使用当前 plan/spec/task-pack 的 `spec_id` 或 workspace hint，再其次询问用户；无人值守且无法解析时必须跳过 Graphify first generation，不能退回 repo-root `graphify-out/`。
- 首次生成完成后，后续刷新归 provider 自己的 watcher / hook / command / MCP 能力。
...
Runtime Setup 应输出一份 setup-owned 工具说明，给后续 skill 读取。它可以由 machine-readable manifest 和 human-readable summary 组成，但两者必须来自同一组 deterministic facts。

下游 workflow 的 canonical consumer surface 是 `.spec-first/config/runtime-tooling-summary.md`。machine facts 可以新增 `.spec-first/config/runtime-tooling-manifest.json`，也可以并入既有 `tool-facts.json` / `runtime-capabilities.json`，但 summary 必须稳定存在并标明 machine facts source。summary 缺失时，下游不得从 registry 或 provider artifact 推断能力可用，只能 degraded fallback 到 direct source evidence。
...
| Runtime profile / overlay | registry profiles(当前 source 实用 minimal/recommended/platform) | 用户/团队 + setup | Runtime Setup plan/check/install mode |
...
- `team` / `user` 不是当前 registry enum，**也不在本批实现**:source 中 `team`/`user`/`overlay` 当前零消费者(仅 `recommended` 在用,helper 另用 `minimal/platform`),为 0 个真实用户预建配置层违反抗膨胀生死线。team/user 仅作为「未来 policy 输入方向」备注保留;真要做时它们是外层 policy overlay,解析后映射到现有 profile 或显式 item allowlist,且必须同步改 schema、loader、fixtures 和 tests,作为独立切片执行。
...
这道门槛也必须回溯校验存量：CodeGraph（新 first-generation `codegraph init` + global-npx + pre-1.0）与 Graphify（新 first-generation + `name-bin-mismatch` + single-maintainer + global-uv）按上面「边际成本陡升不进 recommended」的规则都命中陡升项，而当前 registry 把两者标为 `profile: recommended`。这是门槛立起来后必须解决的存量债：要么把两者降为 optional 并保留 explicit opt-in gate（CodeGraph 已有 `opt_in.explicit_consent_required`，Graphify 已有 consent env），要么在 registry/本文件显式记录「为何豁免进入 recommended」的三问答案。准入门槛是 plan/review 时由人回答的判断 checklist（Let the LLM decide），不生成校验脚本、不进 CI gate；落地测试只锁「门槛文档存在」，不构造准入校验器或 `admission-gate` schema。
...
[ ] CodeGraph
    ...
    安装位置: CLI 用户级 + MCP 项目级 + .codegraph 项目级索引
    首次初始化: codegraph init
    后续刷新: CodeGraph watcher/MCP

[ ] Graphify
    类型: CLI / project-graph
    推荐原因: 当前任务需要项目级架构导航
    安装位置: CLI 用户级 + .spec-first/workspace/requirements/<slug-or-run-id>/graphify-out
    首次生成: Graphify provider-native project graph generation
    后续刷新: Graphify CLI/MCP/hook
```
...
- 把 run-scoped 生成物自动 check-in 或 promotion 成长期 docs。
