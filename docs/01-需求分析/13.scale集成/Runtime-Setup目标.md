...
| Runtime profile / overlay | registry profiles(minimal/recommended/platform) + team/user policy overlay | 用户/团队 + setup | Runtime Setup plan/check/install mode |
...
- 禁用 provider 时改 profile / overlay，不改 workflow prose。
- `team` / `user` 不是当前 registry enum；它们是外层 policy overlay，解析后映射到现有 profile 或显式 item allowlist。若要把它们做成 registry enum，必须同步改 schema、loader、fixtures 和 tests。
