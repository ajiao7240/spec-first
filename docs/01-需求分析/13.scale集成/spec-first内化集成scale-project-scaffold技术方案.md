...
| KnowledgeBase/Graphify | `src/knowledge/KnowledgeBase.ts`、`GraphifyKnowledgeBase.ts` | `docs/solutions/` promotion 属 Knowledge Harness；project-graph 能力（如研发人员自生成的 Graphify 文档）当普通文档读，spec-first 不拥有其生命周期 | P2 harness / capability-aware 协同 | 不引入 SQLite 作为默认 source truth |
...
核心边界是 **「install 帮装、消费不耦合」**：`spec-runtime-setup` 在 explicit install mode 下帮用户装这些工具（detect → install gate → 用户同意 → install + configure MCP + 首次 index，与帮装 gh/jq 同构），命令存 `provider-tools.json` 数据（不硬编码、`--help` 复核）。但**刷新归工具自带 file-watcher，消费侧 capability-aware 不耦合**（详见 `CodeGraph技术方案.md` §3/§5）：
...
> 命令 snapshot 说明：上表命令与 `scale-engine/src/bootstrap/DependencyBootstrap.ts` 常量一致，但 CodeGraph 是 v0.9.8 pre-1.0、命令会漂移——必须存 `provider-tools.json` 数据并 `--help` 复核，不硬编码进 setup 脚本。
...
- 可显式 apply（过 install gate + 用户同意）：spec-first 自管 helper，以及 code-graph / project-graph 能力工具（CodeGraph/Graphify）的 install + configure MCP + 首次 index。
...
- 可选 L3/L4：研发人员自管的 project-graph 文档、`docs/solutions/` 历史决策仅作为 current-state / historical-decision candidates。
- 输出 PRD 时新增或强化：
...
- project-graph 文档、`docs/solutions/` 历史召回可用于语义一致性或历史事故线索，但未确认时只能进 residual risk / test candidate。
...
- 安装侧：填 `provider-tools.json`（CodeGraph/Graphify entry：`lifecycle_commands` + `safety` 复用 `helper-tools-registry.v1`）；扩 `install-helpers.{sh,ps1}` 读 registry 执行 install + configure MCP + 首次 index（双宿主 parity，过 install gate）；用 `provider-readiness.v1` lifecycle 布尔位表达「装≠用」ladder。
...
| Phase E：Capability-aware 协同（code-intelligence 能力工具） | —（project-scaffold 子方案只保留边界预留，不作为 v1.16 主要实施依据） | install 侧填 `provider-tools.json` + 扩 install-helpers 帮装（过 gate）；消费侧 capability-class 引导 + advisory + fallback（CodeGraph code-graph / Graphify project-graph；memory 走 docs/solutions） | v1.16 | setup 帮装过 gate；消费经原生 MCP 不注入编排面；不代刷、不建 adapter/fusion |
...
> CodeGraph 子方案归属说明（钉死，消除相位归属歧义）：`CodeGraph技术方案.md` 只属于 Phase E / v1.16，定位是 **「install 帮装、消费不耦合」**（setup 过 gate + 用户同意帮装 + 配 MCP + 首次 index；消费侧 capability-aware，不注入编排面、不代刷新），不是 provider 集成全链路、也不是「完全不碰安装」。Phase A 的通用 `provider-readiness.v1` schema / docs / fixture、missing/stale/degraded/fallback projection 由父方案与 project-scaffold 子方案承担；CodeGraph 子方案在其上**填 `provider-tools.json` 的具体 entry + 扩 install-helpers 读 registry**（这是 install 侧的真实增量），但消费侧**不写死工具名、不注入 routing/reminder/instruction block、不建 elaborate adapter/fusion**；memory 能力（如 GBrain）默认走 `docs/solutions/`、不集成。
