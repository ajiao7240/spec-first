...
本方案是 `spec-first` 的**横向基建层**，不是某个功能交付。它把 context / knowledge / memory / governance 沉淀为确定性 facts，目的是抬高**所有下游 workflow** 的输入质量，而不是交付某个面向用户的能力。

由此推出三条贯穿全文的判断口径：

- **服务对象是 workflow，不是 end feature**：每个能力的价值，体现在 `spec-prd / plan / work / review / debug / compound` 消费它之后行为变好，而不是 facts 本身被生产出来。
- **P0 排序是"先修最漏的柱子"，不是"功能优先级"**：四问（要解决什么 / 影响哪些文件服务文档 / 哪些验证真跑过 / 哪些该沉淀）是基建四根柱子的质量探针。P0 把证据柱（Q3：verification / honest-closeout）排在前面，是因为它当前最具体地缺、且最容易被滥用（谎报验证），不代表它比上下文/知识柱更重要。
- **成功标准是消费侧行为变化，不是 facts 产出**：一个 capability 只有当至少一个 named workflow 因消费它而产生可观察的行为变化时才算落地（验收口径见 §9.0）。只生产、无人消费、或消费了不改变行为，都视为未完成。
...
| 5 Context Intelligence | CodeGraph/Graphify/GBrain 先做 readiness/freshness/fallback | `scale-engine/src/codegraph/CodeIntelligence.ts` 明确 provider + fallback；`MemoryProviders.ts` 外部写入默认 disabled；`spec-mcp-setup` 只 owns setup facts | provider 只产 advisory/candidate facts，source-read 才确认 |
...
| Provider facts | CodeGraph / Graphify / GBrain / MCP / browser / shell 输出默认是 advisory，必须带 freshness、fallback、limitations 和 source-read requirements |
...
| 生成/本地 facts | `spec-mcp-setup` | `.spec-first/config/*.json`（`tool-facts`、`runtime-capabilities`） | 否 | 是 |
...
| Provider 生命周期阶段 | 各自布尔位 `installed` / `configured` / `initialized` / `indexed` | 不塞进 readiness enum |
...
┌──────────────────────────────▼─────────────────────────────┐
│ Optional Context Intelligence Providers                      │
│ CodeGraph / Graphify / GBrain / source-scan fallback         │
│ readiness, freshness, install plan, candidate facts only     │
└──────────────────────────────┬─────────────────────────────┘
                               │ projected through
┌──────────────────────────────▼─────────────────────────────┐
...
| Dependency Bootstrap | `src/bootstrap/DependencyBootstrap.ts` | `spec-mcp-setup` / `doctor` 的 install plan、explicit apply、post-check facts | P0 | 不默认全装 memory/knowledge/ui/full pack |
...
| Code Intelligence | `src/codegraph/CodeIntelligence.ts` | `provider-readiness.v1`、freshness、fallback、candidate files | P2 | provider 不拥有 finding/root-cause authority |
| Memory Providers | `src/memory/MemoryProviders.ts`、`GbrainRuntime.ts` | file-first memory + optional GBrain recall；外部写入 disabled/candidate-only | P2 | 不自动写长期记忆，不把 recall 当事实 |
| KnowledgeBase/Graphify | `src/knowledge/KnowledgeBase.ts`、`GraphifyKnowledgeBase.ts` | `docs/solutions/`、Graphify artifact summary、Knowledge Harness | P2 | 不引入 SQLite 作为默认 source truth |
...
| Rule Evolution | `src/evolution/RuleMaturity.ts` | shadow -> advisory -> required-evidence -> blocking candidate | P2 | blocking 必须有人审和误报证据 |
...
| Tools/skills policy | `.scale/tools.json`、`.scale/skills.json` | tool/skill readiness + domain hints + evidence-required | P2 | 不强制 Skill Radar 自动路由 |
| Code intelligence config | `.scale/code-intelligence.json` | CodeGraph/Graphify provider readiness + fallback | P2 | 不默认安装和刷新 |
...
| Product smoke | `.scale/product-smoke.json` | 真实跨边界 smoke probe 作为 verification profile 的可选 check（默认 `enabled:false`，空 probe = block，require command runtime evidence） | P2 | 不默认启用 example probe，不把空探针写成通过 |
| Output policy | `.scale/output-policy.json` | report-template / artifact handoff 思路接入现有 `artifact-summary`；`detectSecrets`、`allowRemoteScripts:false`、`defaultGitPolicy:review` 复用现有 redaction/secret-deny | P2 | 不引入平行 artifact 渲染管线，不绕过现有 summary-first |
...
| Host hooks | `.claude/settings.json`、`.codex/hooks.json` | 借鉴 configured dependency report 和 hook dependency consistency | P3 | 不复制 inline shell hook / blocking Stop gate |

> 盘点说明：上表已覆盖 `project-scaffold/.scale/` 下的全部 checked-in 治理文件（`resource-policy` / `tools` / `skills` / `code-intelligence` / `verification` / `governance.lock` / `product-smoke` / `output-policy` / `engineering-standards` / `frameworks` / `workspace` / `assets`，外加 `config.yaml` 作为这些文件的聚合入口）。其中 `assets.json` 判为 non-goal（已被现有 managed-state 覆盖），其余按 P1–P3 / advisory 内化。`config.yaml` 是 `.scale` 的总装配清单，spec-first 不复制，只在 §3.4 边界下按需读取对照。
...
src/
  cli/
    ...
    helpers/
      ...
      verification-profile.js
      verification-run-summary.js
...
| Profile | 默认状态 | 内容 | 适用 |
| --- | --- | --- | --- |
| `minimal` | 默认 | Node/npm/npx/jq/python/git、required MCP、agent-browser/ast-grep helper readiness | 所有 spec-first workflow |
| `recommended` | 显式选择 | CodeGraph、Graphify、GBrain readiness + install plan，不默认写长期记忆 | 需要 impact/context/memory 增强的团队 |
| `platform` | 显式选择 | provider install/apply/verify、host projection report、team policy | 团队级平台化治理 |

GBrain / Graphify / CodeGraph 不是唯一依赖。SCALE full pack 还涉及 RTK、UI skills、browser/web tools、skill registry、runtime hooks、memory fallback 等。`spec-first` 近期只把这些表达成 readiness facts，不把它们都变成默认安装项。
  ...
  "checks": [
    {
      ...
      "status": "passed",
      "exit_code": 0,
      "raw_log_ref": ".spec-first/workflows/spec-work/<run>/logs/typecheck.log"
    },
    {
      "id": "coverage",
      "status": "not-run",
...
`not-run` 必须有 `reason_code`。`passed` 必须有 command、exit_code 和 evidence/log ref。`skipped` 不能被最终总结写成 passed。
...
| 长期知识已沉淀 | `docs/solutions/**` source path 或 GBrain promote evidence；recall 不算沉淀 |
...
# 5. Knowledge / Context Intelligence：CodeGraph、Graphify、GBrain 与六层体系
...
```text
CodeGraph external-cli
Graphify artifact provider
fallback: internal-scan / rg / read
```

`scale-engine/src/memory/MemoryProviders.ts` 的关键点是 memory routing：
...
这两点应直接成为 `spec-first` provider 边界：
...
| Provider | 安装 | 启动/初始化 | 刷新策略 | spec-first 近期口径 |
| --- | --- | --- | --- | --- |
| CodeGraph | `npm install -g @colbymchenry/codegraph` 或等价命令，显式 apply | `codegraph init -i <project>` 生成项目索引；如需 MCP 才启动 `codegraph serve --mcp` | 可在 policy 允许时做受控 refresh；dirty/stale 必须降级 | optional provider；默认只检测 readiness/freshness/fallback |
| Graphify | `uv tool install graphify` 优先，pipx/pip 作为 fallback；显式 apply | `graphify install --platform codex`、`graphify hook install`；项目图谱由 `graphify update <project> --no-cluster` 生成 | 默认不自动刷新，只提示 refresh command；大项目刷新成本高 | artifact provider；missing/stale 只能作为 advisory gap |
| GBrain | `bun install -g github:garrytan/gbrain`，显式 apply | `gbrain init --pglite --no-embedding`；Windows 需注意 runtime shim/mirror | 不自动写长期记忆；recall 可读，promote 必须 review | optional memory provider；默认 external write disabled |

`spec-mcp-setup` 可以输出 install plan 和 readiness facts，但普通 plan/work/review/debug 不应自动安装或启动 provider。

> 命令 snapshot 说明：上表的安装/初始化命令是**当前 snapshot**，与 `scale-engine/src/bootstrap/DependencyBootstrap.ts` 常量一致，但不是 spec-first 的 contract。落地到 setup 脚本前必须通过 provider registry fixture + `--help` / status command 复核（与 `CodeGraph技术方案.md` §4.2 / §11 的纪律统一），不得把可能过期的 README 命令固化进 source。
...
| L2 Context Budget / Bundle | 选择上下文、记录 included/omitted、summary-first handoff | `spec-plan`、`spec-work`、`spec-code-review`、`spec-doc-review`、`spec-debug` | 各 workflow leaf reviewers/worker agents | `context-bundle.v1`、`artifact-summary.v1` | 不广播 raw logs、generated mirrors、全量 provider dump |
| L3 Code Intelligence | 缩小代码读取范围、影响面候选、affected tests candidate | `spec-plan`、`spec-work`、`spec-code-review`、`spec-debug`、`spec-app-consistency-audit` | `spec-correctness-reviewer`、`spec-testing-reviewer`、`spec-api-contract-reviewer`、`spec-security-reviewer`、`spec-performance-reviewer` | CodeGraph、Graphify、source-scan fallback | provider result 是 candidate；finding/root cause 必须 direct evidence |
| L4 Memory / Prior Decisions | 召回历史修复、RCA、已拒绝方案、团队经验 | `spec-sessions`、`spec-compound`、`spec-compound-refresh`、`spec-plan`、`spec-debug` | `spec-learnings-researcher`、session historian 类 agent | `docs/solutions/**`、session summaries、GBrain recall、out-of-scope rationale | recall 不等于事实；长期写入走 candidate -> review -> promote |
| L5 Skill / Tool Capability | 根据任务域提示可用 skill、MCP、CLI、browser/tool evidence | `spec-mcp-setup`、`spec-plan`、`spec-work`、`spec-code-review`、`spec-app-consistency-audit`、`spec-skill-audit` | `spec-agent-native-reviewer`、`spec-cli-readiness-reviewer`、UX/UI/browser 相关 reviewers | tool-facts、runtime-capabilities、skills/tool registry | 不强制 Skill Radar；setup facts 不替代语义判断 |
...
- 读取 provider registry / install profile。
- 输出 dependency readiness、provider readiness、freshness、install plan。
- 可显式 apply optional provider。
- 写 `.spec-first/config/tool-facts.json`、`runtime-capabilities.json`、setup scenario fingerprint。
- 不运行代码索引 watcher，不自动启动长期 daemon，不把 provider facts 当语义证据。
...
- 可选 L3/L4：Graphify/GBrain 仅作为 current-state / historical-decision candidates。
- 输出 PRD 时新增或强化：
...
- 使用 provider readiness 决定是否查询 CodeGraph/Graphify/GBrain。
- 把 provider 输出写成 candidate facts 和 source-read requirements。
...
- 使用 CodeGraph/source-scan 缩小读取候选，但仍 direct source read。
...
- Provider result 可以触发 investigation。
- Review finding 必须由 diff/source/test/log/contract/artifact 直接支撑。
- Graphify/GBrain 可用于语义一致性或历史事故召回，但未确认时只能进 residual risk / test candidate。
- 对文档 review，provider 输出只能作为 evidence path / premise challenge，不替代逐章文本审查。

## 6.6 `spec-debug`

接入方式：

- error/log -> direct repro -> CodeGraph path candidate -> source-read -> focused command/log confirmation。
- GBrain/session recall 只补历史线索。
...
- GBrain 写入是 optional promotion，不是默认 closeout。
- Graphify refresh 是 candidate next action，不在 compound 中静默执行。
...
目标：扩展 `doctor` / `spec-mcp-setup` 的 deterministic setup facts。

交付：

- `minimal/recommended/platform` profile。
- required harness runtime 与 optional provider readiness。
- provider install plan / explicit apply / post-check。
- configured dependency report：host hooks、MCP、helper、provider command、project artifact。

验收：

- 没有 explicit apply 时不安装、不改外部环境。
- missing/stale/degraded/fallback 有 reason_code。
- 普通 workflow 可在 provider unavailable 时继续使用 direct evidence。
...
- not-run reason、raw_log_ref、redaction status。
...
- `knowledge-harness.md`。
- `docs/solutions` 与 artifact summary 的 promotion rules。
- out-of-scope rationale。
- context bundle budget 增强。

验收：

- stable knowledge 与 session-local 过程产物可区分。
- GBrain/Graphify 写入都是 optional promotion。
- `spec-compound` 不变成 raw transcript archive。

## Phase E：Optional Context Intelligence Providers

目标：CodeGraph / Graphify / GBrain 作为可选增强。

交付：

- provider registry。
- readiness/freshness/fallback。
- CodeGraph/Graphify/GBrain adapters。
- context fusion summary。

验收：

- stale provider 不产生 confirmed context。
...
> 每个 capability 至少有一个 §6 中 named workflow，在同一 Phase 内，因消费它而产生**可观察的行为变化**；该变化必须能被一条具体断言捕获。

| 用例编号 | capability | named consumer | 可观察行为变化（断言） |
| --- | --- | --- | --- |
...
| `[CON-PROV-001]` | provider-readiness | `spec-plan`/`spec-debug` | provider `unavailable/stale` 时 workflow 明确走 fallback 并继续，而非阻塞或假装已确认 |
...
- **无消费方 = 不交付**：某 capability 找不到 named consumer，或 consumer 消费后断言无变化，则该 capability 停在 advisory，不计入 Phase 完成，避免基建空转。
- **消费侧验收与 contract test 并列**：contract test 证明 facts 正确（schema/边界），consumer gate 证明 facts 有用（行为变化）。两者都过才算该 capability 完成。
- **§6 接入矩阵是 consumer 的唯一登记处**：新增 capability 必须在 §6 登记至少一个 consumer，否则不进入路线。
...
- `[CT-005]` provider `confirmed_context` requires source refs。
...
- `spec-mcp-setup` verify-only 不安装。
...
- `[PV-001]` CodeGraph unavailable -> source-scan fallback。
- `[PV-002]` CodeGraph stale -> degraded + source-read required。
- `[PV-003]` Graphify artifact missing -> advisory gap。
- `[PV-004]` GBrain recall -> advisory unless source/docs confirms。
...
```bash
git diff --check -- CHANGELOG.md docs/01-需求分析/13.scale集成/spec-first内化集成scale-project-scaffold技术方案.md
```
...
依赖顺序（与 §8 对齐）：honest closeout（#4）的 deterministic 校验依赖 verification-run-summary（#3）的真实 exit_code，因此二者同属 Phase B、同列 P0，不拆到 P1 治理阶段。P0 内部按 1→5 的依赖顺序落地。
...
```text
1. CodeGraph readiness/query/context candidate
2. Graphify artifact readiness/fusion
3. GBrain recall/promotion
4. provider conflict/gap summary
```

理由：provider 能显著提升效率，但必须建立在 readiness、verification 和 source confirmation 之后。
