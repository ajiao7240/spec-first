...
| 5 Context Intelligence | CodeGraph/Graphify/GBrain 先做 readiness/freshness/fallback | `scale-engine/src/codegraph/CodeIntelligence.ts` 明确 provider + fallback；`MemoryProviders.ts` 外部写入默认 disabled；`spec-runtime-setup`（alias `spec-mcp-setup`）只 owns setup facts | provider 只产 advisory/candidate facts，source-read 才确认 |
...
| Code Intelligence | `src/codegraph/CodeIntelligence.ts` | 通用 `provider-readiness.v1` 槽位先服务 v1.11/v1.12；CodeGraph candidate files / adapters 归 v1.16 optional provider | P0 通用 readiness / P3 provider | provider 不拥有 finding/root-cause authority |
| Memory Providers | `src/memory/MemoryProviders.ts`、`GbrainRuntime.ts` | file-first memory boundary 服务 Knowledge Harness；optional GBrain recall 归 v1.16 provider pack | P2 harness / P3 GBrain | 不自动写长期记忆，不把 recall 当事实 |
| KnowledgeBase/Graphify | `src/knowledge/KnowledgeBase.ts`、`GraphifyKnowledgeBase.ts` | `docs/solutions/` promotion 属 Knowledge Harness；Graphify artifact summary 属 v1.16 optional provider | P2 harness / P3 Graphify | 不引入 SQLite 作为默认 source truth |
...
| Code intelligence config | `.scale/code-intelligence.json` | 通用 provider readiness/fallback 槽位进入 v1.11/v1.12；CodeGraph/Graphify 具体 provider 接入进入 v1.16 | P0 通用 readiness / P3 provider | 不默认安装和刷新 |
...
`spec-runtime-setup`（迁移期 alias `spec-mcp-setup`）可以输出 install plan 和 readiness facts，但普通 plan/work/review/debug 不应自动安装或启动 provider。
...
边界：Phase A 只建立通用 provider readiness 槽位、registry shape、missing/stale/fallback projection，以及“已配置但未验证”的 configured dependency facts；不交付 CodeGraph / Graphify / GBrain 的具体 adapter、query consumer 或 context fusion。这些属于 Phase E。
...
- 具体 provider registry entries。
- readiness/freshness/fallback。
- CodeGraph/Graphify/GBrain adapters。
- context fusion summary。

边界：Phase E 消费 Phase A 的通用 `provider-readiness.v1` 和 setup facts，只补具体 provider entries、adapter/query/fusion consumer；不反向改写 minimal baseline，也不把 provider 缺失升级为 ordinary workflow failure。
...
| Phase E：Optional Context Intelligence Providers | —（project-scaffold 子方案只保留边界预留，不作为 v1.16 主要实施依据） | Stage 0（provider-specific readiness foundation）+ Stage 1（CodeGraph）+ Stage 2（Graphify）+ Stage 3（GBrain）+ Stage 4（Context Fusion） | v1.16 | provider entries、provider adapters、freshness/fallback、context fusion |
...
> CodeGraph 子方案归属说明（钉死，消除相位归属歧义）：`CodeGraph技术方案.md` 只属于 Phase E / v1.16 Optional Provider Pack。Phase A 的通用 `provider-readiness.v1` schema / docs / fixture、通用 provider registry shape、missing/stale/degraded/fallback projection，以及 doctor 对这些 facts 的消费展示，均由父方案与 project-scaffold 子方案承担；CodeGraph 子方案只能把这些作为前置条件消费，不能反向成为 v1.11/v1.12 的实施 source。CodeGraph 子方案的 Stage 0 因此只表示 v1.16 内部的 provider-specific readiness foundation：具体 CodeGraph / Graphify / GBrain registry entries、provider-specific install plan / explicit apply / post-check。
...
理由：provider 能显著提升效率，但必须建立在 readiness、verification、governance/Knowledge Harness 基线和 source confirmation 之后。
