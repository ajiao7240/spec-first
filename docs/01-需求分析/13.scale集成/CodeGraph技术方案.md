...
> 若工具箱里存在 \<能力类别\> 能力（如用户自管的 MCP / 生成的架构文档），可在 \<对应节点\> 优先利用其产出作为 advisory candidate；缺失则走 fallback。任何此类输出都是 candidate，结论仍需 source/test/log/contract/user evidence 回源确认。
...
CodeGraph 官方（本地 clone `src/mcp/server-instructions.ts` 全文 + Context7 `/colbymchenry/codegraph` 一致）对 agent 的最佳实践是**按 intent 选工具 + 一次组合调用 + 信 banner**。它通过 MCP `initialize` 把这套 playbook 注入 agent system prompt——这正是「装成 MCP 即原生可见、不需 spec-first 注入」的机制本身（§4.1 消费协同）。官方的 10 个工具与推荐用法：
...
| --- | --- | --- |
| 概览 | 只取 `GRAPH_REPORT.md` 的 **God Nodes / Communities / Surprising Connections** 三段，**不读全文**（官方明文 "Do NOT paste the full report"） | Read 这三段拿架构骨架——默认档，零依赖，覆盖大多数「架构总览」场景 |
...
```text
概览: Read GRAPH_REPORT.md 的 God Nodes/Communities/Surprising Connections 三段(不读全文)
