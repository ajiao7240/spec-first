...
因此，本文中的 provider 集成必须排在 readiness、verification、honest closeout 和 Knowledge Harness 基线之后。
...
| Runtime setup | `skills/spec-mcp-setup/SKILL.md`、`mcp-tools.json`、setup scripts | `spec-mcp-setup` owns deterministic readiness facts，不 owns semantic code understanding |
...
| Readiness/freshness 检测 | 检查 installed/configured/reachable/fresh/repo aligned | `spec-mcp-setup` / setup helper | 应自动检测 |
...
- 统一表达 CodeGraph / Graphify / GBrain 的 installed/configured/available/freshness/fallback。
- 供 `spec-mcp-setup`、`doctor`、workflow skills 读取。
...
```json
{
  ...
  "candidate_trust": "advisory|evidence_candidate",
  "capabilities": [],
  "limitations": [],
  "fallback": {
    "available": true,
...
| --- | --- | --- | --- | --- | --- |
| P0 | `spec-mcp-setup` | 检测 CLI、索引、MCP config、freshness | 检测 artifact、refresh 建议 | 检测连接、citation/source 可达性 | 只产 readiness facts，不判断结论 |
| P1 | `spec-plan` | code impact candidates、related files、affected tests candidates | semantic/domain impact candidates | historical constraints / rejected rationale | 不自动扩大 scope |
| P1 | `spec-code-review` | changed symbols、callers/callees、blast-radius candidates | docs/code mismatch candidates、跨域关联 | 类似事故、历史规则候选 | finding 必须 direct evidence |
| P1 | `spec-debug` | 调用链、实现链、路径候选 | 业务流程语义候选 | 历史 RCA / workaround | root cause 必须由当前日志/源码/测试确认 |
| P2 | `spec-prd` | 必要时定位入口代码 | brownfield current-state / concept map | 历史背景、决策约束 | 只辅助 current-state，不编造 WHAT |
| P2 | `spec-work` | 缩小编辑候选、推荐验证候选 | 默认少用 | 默认少用 | 实现阶段避免上下文膨胀，不新增需求 |
| P2 | `spec-compound` | 非主要写入目标 | 可提示 graph refresh | verified learning 可导入 | `docs/solutions/` 仍是第一 durable store |
| P3 | `spec-doc-review` | 核查代码现状声明 | 核查语义现状声明 | 核查历史约束声明 | 最终仍需文本/源码直接证据 |
| P3 | `spec-app-consistency-audit` | 复杂 App impact 辅助 | 页面/模块语义辅助 | 历史产品/事故辅助 | 等 provider contract 稳定后再接 |
...
Adapter 输出同样使用双轴模型：`readiness_status` 表示 provider 机械新鲜度，只能取 `fresh/stale/degraded/not-run/unknown`；`candidate_trust` 表示这些候选如何被 workflow 消费，不得回填进 readiness 字段。

Adapter 输出应保持短、小、可追溯：

```json
{
  "provider": "codegraph",
  "readiness_status": "fresh",
  "candidate_trust": "evidence_candidate",
  "readiness": {
    "index_exists": true,
    "repo_aligned": "yes",
    "stale": false
...
## Stage 0：Provider Readiness Foundation

前置条件：

- 父方案 Phase A readiness baseline 已建立。
- `provider-readiness.v1` 有 schema / docs / fixture。
- `spec-mcp-setup` 能输出 provider missing/stale/degraded/fallback facts。

交付：

- provider registry source。
- install plan / explicit apply / post-check。
- host configured dependency report。
...
- `confirmed_context` requires direct source refs or confirmation evidence。
...
- host configured dependency report 区分 installed/configured/reachable/fresh。
...
```text
先做 readiness / freshness / fallback，
再让 workflow 消费 candidate facts，
再用 source-read / test / log / review 确认，
最后才做 durable knowledge promotion。
```
