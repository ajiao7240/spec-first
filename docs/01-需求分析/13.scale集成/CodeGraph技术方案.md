...
- **安装协同**：`spec-runtime-setup` detect → install gate（用户同意）→ `mcp-tools.json` opt-in MCP entry 经 `install-mcp` warmup + 配 MCP + 首次 `codegraph init`。`codegraph init -i` 已弃用；落地前继续用 `--help` 复核。**装≠用**用既有 `provider-readiness.v1` 的 lifecycle 布尔位表达（`installed → configured → indexed → server_reachable → query_verified`，见 §5.2）。
...
- **安装协同**：`spec-runtime-setup` 可帮装 Graphify CLI（`uv tool install graphifyy`，过 gate），并在显式 setup/init gate 后执行 Graphify 原生首次 project-graph 生成，随后检测当前 run-scoped artifact 与 CLI/MCP query surface。spec-first 不在下游 workflow 运行期代生成、不 silent 装其 post-commit hook、不代刷新。若未来支持 provider-native hook onboarding，也必须 preview-first + explicit gate，默认关闭。
