...
- Graphify 是 CLI provider：落 `provider-tools.json`，复用 `install-helpers.{sh,ps1}` 安装 CLI；生成 `GRAPH_REPORT.md` 仍是用户动作。
...
- `spec-runtime-setup` 在 explicit install mode 下过 gate + 用户同意：CodeGraph 走 MCP opt-in 管线（配 MCP、首次 index），Graphify 走 CLI provider 管线（只帮装 CLI；生成由用户触发）。
