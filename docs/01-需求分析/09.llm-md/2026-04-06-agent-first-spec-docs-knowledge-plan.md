...
grep -q "raw/snapshots/<timestamp>" "$REPO_ROOT/skills/spec-graph-bootstrap/SKILL.md"
...
```bash
git add package.json tests/unit/knowledge-schema.sh tests/unit/knowledge-validator.sh tests/fixtures/knowledge-invalid scripts/validate-knowledge-tree.js templates/docs/draft-page.md templates/docs/knowledge-page.md templates/docs/snapshot-manifest.json skills/spec-graph-bootstrap/SKILL.md skills/spec-compound/SKILL.md skills/spec-compound/assets/resolution-template.md skills/spec-compound/references/schema.yaml src/cli/commands/doctor.js
git commit -m "feat(knowledge): define raw draft knowledge schemas"
```

### Task 6: `spec-graph-bootstrap` 写入原始快照
...
```bash
grep -q 'raw/snapshots/' "$REPO_ROOT/skills/spec-graph-bootstrap/SKILL.md"
grep -q 'raw/snapshots/' "$TMP_DIR/.claude/skills/spec-graph-bootstrap/SKILL.md"
grep -q 'contexts/' "$TMP_DIR/.claude/skills/spec-graph-bootstrap/SKILL.md"
```
...
Expected: FAIL because `spec-graph-bootstrap` 目前只生成 context / PRD，不会明确沉淀 raw snapshot

**Step 3: Write minimal implementation**

调整 `skills/spec-graph-bootstrap/SKILL.md`：
- 先定位 docs repo 项目根
...
```bash
git add tests/smoke/cli.sh tests/unit/knowledge-schema.sh skills/spec-graph-bootstrap/SKILL.md
git commit -m "feat(spec-graph-bootstrap): persist code analysis snapshots to raw"
```
...
- 先复用 `init` / `doctor` / `spec-graph-bootstrap` / `spec-compound` / `spec-compound-refresh`
