...
| R-02 | P0 | For every behavior-bearing slice, `spec-work` must identify a compact coding-quality intent before or during first implementation: quality risks, existing patterns to follow, tests/checks to run or justify as not-run, and review focus when relevant. | user-stated; confirmed-source: feedback loop and testing clauses in `skills/spec-work/SKILL.md`; AE-02 |
...
Then it records or carries a compact quality intent naming state risk, existing patterns, tests/checks, and review focus before claiming completion.
