...
同时，真实使用中存在没有 Git 初始化的项目目录、解压源码包、临时代码目录或生成项目。GitNexus 官方支持 `analyze --skip-git`，但 spec-first 当前 graph target 与 freshness contract 仍以 Git repo 为默认边界。GitNexus-only 后应把非 Git 目录纳入显式 folder target，而不是把它伪装成 Git repo 或生成假的 commit / dirty 证据。
...
- R33. CRG 删除必须覆盖完整生命周期，而不是只删除用户文案：setup registry、dependency check、warmup install、optional live MCP host config、provider projection、bootstrap command allowlist、provider fingerprint、raw/normalized artifact contract、canonical graph facts、impact capabilities、review-pre-facts provider selector、workflow guidance、README/user manual、contract tests、shell tests 和 CI quality gate 都必须迁移或删除。
...
- R36. `skills/spec-mcp-setup/scripts/detect-tools.*`、`verify-tools.*`、`configure-host.*`、`install-mcp.*`、`uninstall-mcp.*` 和相关 setup tests 必须不再把 CRG 当作 required/optional setup subject；如果发现用户宿主残留 CRG MCP config，只能作为 cleanup advisory 或 explicit uninstall target，不能参与 baseline readiness。
...
- R42. `skills/spec-plan/SKILL.md`、`skills/spec-work/SKILL.md`、`skills/spec-debug/SKILL.md`、`skills/spec-doc-review/SKILL.md`、`skills/spec-graph-bootstrap/SKILL.md`、`skills/using-spec-first/SKILL.md` 和 Claude command templates 中的 graph refresh / evidence guidance 必须统一移除 active CRG role，保留 CRG 仅限历史迁移说明。
- R43. Tests 必须从“CRG 外部 provider 仍允许”改为“active source/runtime 不再依赖 CRG”：更新 `tests/unit/mcp-setup.sh`、`tests/unit/spec-graph-bootstrap.sh`、PowerShell parity tests、`graph-provider-consumption` tests、`runtime-tools-index` tests、`user-manual` tests、`no-graph-fast-path` tests、`no-crg-runtime` tests 和 CI quality gate covers，确保 source 不再出现 CRG active install/refresh/use path。
- R44. Package/install guard 必须继续防止退役内部 `src/crg/` runtime 回归，同时新增或改造 no-CRG-provider contract，拒绝 `mcp-tools.json`、setup writer、bootstrap scripts、runtime tool catalog、README 和 active workflow prose 重新引入 CRG provider。
...
- R46. Runtime capability catalog、runtime tools index、branch protection / quality gate covers、README managed blocks 和 generated documentation sources 不得继续把 CRG 描述为 current provider/tool fact source；如需提及，只能标记为 historical/retired/residual cleanup。
...
- AE5. **Covers R17, R18, R19.** Given a review diff changes shared behavior, when `$spec-code-review` runs with GitNexus ready, then it uses GitNexus impact/review evidence and canonical impact capability facts; CRG is not invoked or named as fallback.
...
- AE9. **Covers R33-R37.** Given setup and graph bootstrap source are inspected after migration, when provider lifecycle code is scanned, then there is no CRG install/warmup/host MCP/projection/command allowlist/fingerprint/bootstrap/status/query path.
...
- AE12. **Covers R46-R48.** Given an older checkout had CRG in generated runtime docs or `.spec-first/config/graph-providers.json`, when source setup/init regeneration runs, then active runtime/docs/config are GitNexus-only; if graph-bootstrap sees the stale CRG projection before setup, it fails closed without executing CRG.
...
| Runtime/catalog docs | `docs/catalog/runtime-capabilities.md`, runtime capability generator and runtime tools tests mention CRG as current provider fact source | Regenerate and test GitNexus-only catalog wording |
...
The implementation plan resolves the planning-owned questions as follows:
...
- Active CRG references are removed from setup, bootstrap, provider artifacts, workflow prose, runtime catalog and tests; historical docs may mention CRG only with retired/historical or residual cleanup framing.
