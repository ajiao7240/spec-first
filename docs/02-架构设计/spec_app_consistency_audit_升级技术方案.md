...
本文件描述的是终态蓝图，但 v0.1 只能实现最小可运行 contract spine。落地时必须把内容分成三层。
...
2. full claim_type evidence matrix
...
|---|---|---|---|
| 终态协议设计过完整，v0.1 容易被压垮 | 终态蓝图保留，v0.1 只实现最小可运行 Contract Spine | `3.2.2`、`19`、`20` 将 Validation Pass、standalone handoff、full matrix 延后 | 避免把默认 workflow 做成重型平台 |
...
| 精准降级契约太重 | v0.1 只做 required / opportunistic、basic coverage、minimal conclusion_cap | `4.6.2`、`19`、`20` 延后 full degraded matrix | 保持轻 contract，同时解决缺 PRD/Figma/Graph 的核心问题 |
| Evidence Gate / Auditor / Validation Pass 可能形成三重审判 | v0.1 只保留 Deterministic Evidence Gate + LLM Evidence Auditor，Validation Pass v0.2+ | `13.1`、`13.2`、`19` | 减少重复改写，仍保留证据质量控制 |
...
v0.1 只实现最小降级传播：
  ...
  - conclusion_cap for product_alignment / design_alignment / industry_compliance / rule-pack-only

v0.2+ 再实现：
  - expected / not_applicable 全量输入期望
  - full precise degraded_modes schema
  - full claim_type evidence matrix
...
11. interaction surface signals，例如 navigation / form / keyboard / dialog / permission / refresh / pagination / error state
...
```json
{
  ...
  "candidate_signals": [
    ...
    },
    {
      ...
      "confidence": 0.74,
      "subtypes": ["form_submit", "dialog", "keyboard"],
      "evidence": [
        {
          "source": "code",
          "file": "androidApp/src/main/kotlin/trade/TradeBuyScreen.kt",
...
7. default 模式默认最多选择 5 个领域专家，不计 LLM Evidence Auditor 和 Report Writer；depth:deep 可放宽，但必须写明原因。
...
6. Pre-gate candidate set
   ...
   - v0.1 后续必须依次经过 Deterministic Evidence Gate、LLM Evidence Auditor，再做 Final Partition
...
v0.1:
  暂缓 Validation Pass。
  只实现 Deterministic Evidence Gate + LLM Evidence Auditor + review_lifecycle。
...
本节展示的是终态 artifact tree。v0.1 只要求写出 `metadata.json`、`artifact-manifest.json`、`preflight.json`、`impact-facts.json`、`audit-plan.json`、`app-audit-context.json`、`issues.json`、`app-consistency-audit.summary.md` 和必要报告；其余 contract、expert review、handoff、regression 和 writeback-preview 产物按 `depth:deep` 或后续版本逐步打开。
...
9. codebase contract
10. kmp architecture contract
11. Agent IO contract minimal：compact findings + selected/skipped experts
12. strict issue schema 最小版
13. deterministic evidence gate 最小版
14. LLM evidence auditor 最小版
15. review_lifecycle minimal：normalize / deterministic_evidence_gate / llm_evidence_auditor
16. issues.json
17. app-consistency-audit.summary.md
18. headless envelope
19. report writer
20. report-only no-write
21. coverage_capabilities minimal：source / PRD / Figma / Graph / industry
22. conclusion_cap minimal：product_alignment / design_alignment / industry_compliance / rule-pack-only
23. summary/headless followups，不单独写 workflow-handoff-suggestions.json
24. interaction_surface_changed minimal signals：navigation / form / keyboard / dialog / permission / error-state 候选事实
25. secrets / network / remote execution / runtime validation 默认阻断或 deferred 的安全边界
26. CLI subprocess e2e test：覆盖 run-scoped path，不再只测扁平 .spec-first/app-audit/
27. 最小 eval / 回归集：trigger、boundary、mode、artifact、evidence、safety、precise-degradation-minimal
```
...
9. full claim_type evidence matrix
```
...
10. review_lifecycle minimal
11. planner_guardrails minimal：allowed_experts / must_select / max_selected_experts_default / skip_reason
12. global safety / redaction / data_sensitivity contract
13. agent catalog graduation policy
14. precise degraded_modes minimal：capability、conclusion_cap、legacy_aliases
15. input_expectations minimal：required / opportunistic
16. claim_type evidence requirement minimal：product_alignment / design_alignment / industry_compliance / rule-pack-only
```
...
9. planner 必须遵守 allowed_experts / must_select / max_selected_experts_default / activation_constraints，并为 selected/skipped experts 写 reason
```
...
2. v0.1 先在 impact-facts.json 中输出 interaction_surface_changed candidate signals。
3. v0.2 或 depth:deep 再输出完整 interaction-contract.json。
4. Mobile UX Expert 只在 interaction signals、screen/navigation/form/input/dialog/permission 证据存在时启用。
5. 静态无法确认的键盘、安全区、手势、字体放大、深色模式和系统权限问题，只输出 runtime_verification_suggestions。
...
2. 先输出 run-scoped impact-facts.json + audit-plan.json + app-audit-context.json + issues.json + report
