# spec-first 终态技术方案：PRD 迭代、大需求拆分、跨端协作与全流程闭环

## 0. 结论先行

这件事不应该做成一个孤立的“大需求拆分工具”，也不应该第一阶段就新增一个主流程 skill。

终态应该是：

```text
spec-brainstorm
  -> 支持 PRD Intake / Code-aware Normalization / Split Decision
  -> 根据需求复杂度输出不同需求资产

spec-plan
  -> 消费 requirements.md / Requirements Packet / Multi-Surface Requirements Packet
  -> 继续作为技术方案 source of truth

spec-write-tasks
  -> 只消费 plan
  -> 派生 task pack
  -> 保留 R / AE / PS / Contract / Surface / Owner refs

spec-work
  -> task-scoped execution

spec-code-review
  -> code review + requirement coverage + contract coverage + integration coverage

spec-compound
  -> 沉淀到 docs/solutions，不反向污染 requirements

spec-requirements
  -> 后续拟议维护型 skill，用于 validate / audit / repair / migrate / diff / update
```

最终主链路保持不变，但入口需要按 host 区分。`spec-write-tasks` 是 standalone skill，不是 command-backed workflow：

```text
Claude Code:
  /spec:brainstorm
    -> /spec:plan
    -> spec-write-tasks
    -> /spec:work
    -> /spec:code-review
    -> /spec:compound

Codex:
  $spec-brainstorm
    -> $spec-plan
    -> spec-write-tasks
    -> $spec-work
    -> $spec-code-review
    -> $spec-compound
```

变化的是 `spec-brainstorm` 的产物能力从“生成一个 requirements.md”升级为：

```text
根据需求复杂度，生成可规划、可协作、可追踪、可执行、可评审、可沉淀的需求资产。
```

---

# 1. 当前项目事实基线

## 1.1 `spec-brainstorm` 的职责边界

当前 `spec-brainstorm` 明确负责在 plan 前回答 **WHAT to build**，而 `spec-plan` 负责回答 **HOW to build**；它不实现代码，只探索、澄清、记录产品行为、范围边界和成功标准。它还明确要求按工作大小控制 ceremony，不增加无价值复杂度。([GitHub][1])

因此，本方案的第一原则是：

```text
Requirements Packet 是 spec-brainstorm 的一种输出模式，
不是新的主流程节点。
```

---

## 1.2 当前 requirements 已经具备 ID 化基础

当前 `requirements-capture.md` 已经要求 Standard / Deep scope 给 requirements 分配 R-ID，并使用 `A / F / AE` 等稳定 ID 描述 actors、flows、acceptance examples。它也明确 requirements document 应像轻量 PRD，但不要包含 implementation details，除非该 brainstorm 本身就是技术/架构决策。([GitHub][2])

所以终态方案沿用当前项目 ID 风格：

```text
R1, R2, R3
A1, A2
F1, F2
AE1, AE2
```

新增对象也保持一致：

```text
CAP1, CAP2
D1, D2
Q1, Q2
PS1, PS2
API1, DATA1, AUTH1, EVENT1, ERROR1
SURFACE-APP, SURFACE-H5
```

第一阶段不建议强行切换到 `R-001 / AE-001`，避免和现有 skill 文档风格分裂。

---

## 1.3 `spec-plan` 必须继续是技术方案事实源

当前 `spec-plan` 的定位是当 brainstorm/requirements 已经准备好时创建 implementation plan；它接收 requirements 文档，生成 plan，并在 plan 中建立 requirements trace、implementation units、test scenarios 和 verification。([GitHub][3])

所以终态仍然坚持：

```text
Requirements / PRD / Roadmap / Planning Slice
  只负责 WHAT、WHY、SCOPE、ACCEPTANCE、PLANNING INPUT

spec-plan
  负责 HOW、TECH DESIGN、FILES、UNITS、RISKS、TESTS、VERIFY
```

不能让 roadmap 替代 plan，也不能让 planning slice 直接变成 task。

---

## 1.4 `spec-write-tasks` 不能前置

当前 `spec-write-tasks` 是位于 `spec-plan` 和 `spec-work` 之间的可选派生层；它要求 plan 是 technical source of truth，task pack 是 derived artifact，不能改变 scope、acceptance criteria 或 non-goals。([GitHub][4])

因此必须禁止：

```text
Requirements Packet -> spec-write-tasks -> Tasks
```

必须保持：

```text
Requirements Packet -> spec-plan -> spec-write-tasks -> Tasks
```

---

## 1.5 Review 与 Knowledge 已有可扩展切入点

`spec-code-review` 已经包含 Requirements Completeness：当找到 plan 时，会检查 plan 中每个 requirement 和 implementation unit 是否在 diff 中有对应工作。这个能力正好可以扩展为 packetized requirements / multi-surface requirements 的 coverage review。([GitHub][5])

`spec-compound` 的职责是把解决过的问题沉淀到 `docs/solutions/`，形成可复用知识。([GitHub][6])

所以终态边界是：

```text
Review:
  验证 code 是否满足 plan / R / AE / Contract

Knowledge:
  沉淀经验到 docs/solutions
  不写回 requirements packet
```

---

# 2. 终态能力分层

`spec-brainstorm` 最终支持四类输入和三类输出。

## 2.1 输入类型

```text
Explore Mode
  用户只有想法，没有 PRD。

PRD Intake Mode
  用户已有产品需求文档，需要整理成可执行需求。

PRD Iteration + Code-aware Mode
  用户已有 PRD，并且这是对现有系统的需求迭代，需要结合代码事实。

PRD Update / Change Mode
  后续拟议由 spec-requirements 维护，处理 PRD v2 / packet 变更。
```

## 2.2 输出模式

```text
Single Requirements Mode
  小需求：输出一个 docs/brainstorms/*-requirements.md。

Requirements Packet Mode
  大需求：输出 thin entry + packet directory。

Multi-Surface Requirements Packet Mode
  跨端 / 跨团队 / 跨系统需求：输出 contract-first multi-surface packet。
```

---

# 3. 复杂度决策树

`spec-brainstorm` 进入 Phase 3 Capture Requirements 前，应先执行 Output Mode Selection。

```text
Step 1. 是否已有 PRD？
  否 -> Explore Mode
  是 -> PRD Intake Mode

Step 2. 是否是现有系统迭代？
  是 -> PRD Iteration + Code-aware Mode

Step 3. 是否小需求？
  是 -> Single Requirements Mode

Step 4. 是否大需求？
  是 -> Requirements Packet Mode

Step 5. 是否跨端 / 跨团队 / 跨系统？
  是 -> Multi-Surface Requirements Packet Mode

Step 6. 每个 surface 是否本身也足够大？
  是 -> Parent Packet + Child Packets，后置高级能力
  否 -> 一个 Parent Multi-Surface Packet + lane planning
```

判断口诀：

```text
单点需求：单文件
大需求：packet
跨端闭环：multi-surface packet
跨端且每端都很大：parent + child packets
只是主题相关、可独立上线：多个 spec_id
```

---

# 4. Source of Truth 规则

## 4.1 小需求

```text
docs/brainstorms/*-requirements.md
  = requirements source of truth
```

## 4.2 大需求

```text
docs/brainstorms/*-requirements.md
  = thin entry，只做兼容入口

docs/brainstorms/*-requirements/
  = requirements packet source of truth
```

## 4.3 技术方案

```text
docs/plans/*-plan.md
  = technical source of truth
```

第一阶段不拆 Plan Packet，避免破坏当前 `source_plan` / `source_plan_hash` 机制。

## 4.4 任务

```text
docs/tasks/*-tasks.md
  = derived task pack
```

task pack 永远由 plan 派生，不直接由 requirements 派生。

## 4.5 知识

```text
docs/solutions/**/*.md
  = reusable knowledge
```

Knowledge 不写回 requirements packet。

---

# 5. 小需求终态结构

适用：

```text
单点需求
单端
单模块
单 workflow
不需要 roadmap
不需要复杂拆分
后续 plan 不会过大
```

目录：

```text
docs/brainstorms/
  2026-05-02-001-topic-requirements.md

docs/plans/
  2026-05-02-001-topic-plan.md

docs/tasks/
  2026-05-02-001-topic-tasks.md

docs/solutions/
  ...
```

要求：

```text
小需求不强行 packet 化。
一个 requirements.md 能讲清楚，就保持单文件。
```

---

# 6. 大需求 Requirements Packet 终态结构

适用：

```text
PRD 较大
requirements 超过 15–20 条
多个 capability
多个 flow
多个 acceptance examples
需要 roadmap / MVP / phase
后续 plan 会膨胀
但不明显跨端 / 跨团队
```

目录：

```text
docs/brainstorms/
  2026-05-02-001-topic-requirements.md

  2026-05-02-001-topic-requirements/
    00-index.md
    manifest.json
    trace-ledger.json

    05-source/
      source-documents.md
      raw-prd-summary.md
      source-coverage.md

    06-prd/
      standard-prd.md
      prd-delta.md
      code-alignment.md
      prd-open-gaps.md
      split-decision.md

    10-intent/
      product-intent.md
      scope-boundaries.md
      non-goals.md

    20-capabilities/
      CAP1-xxx.md
      CAP2-xxx.md

    30-flows/
      F1-xxx.md
      F2-xxx.md

    40-requirements/
      R1-xxx.md
      R2-xxx.md

    50-acceptance/
      AE1-xxx.md
      AE2-xxx.md

    60-decisions/
      D1-xxx.md

    70-open-questions/
      Q1-xxx.md

    75-roadmap/
      roadmap.md
      roadmap-items.yaml
      drafts/

    80-planning-slices/
      PS1-xxx.md
      PS2-xxx.md
```

---

# 7. Multi-Surface Requirements Packet 终态结构

适用：

```text
一个业务需求涉及多个 surface / team / system：

APP
H5
Admin
中台组
行情组
权限
配置
埋点
灰度
消息
数据同步
```

判断标准：

```text
是否需要多个团队共同完成一个业务闭环？
是否存在共享接口 / 字段 / 权限 / 配置 / 埋点？
是否需要端到端验收才算完成？
```

目录：

```text
docs/brainstorms/
  2026-05-02-001-cross-surface-feature-requirements.md

  2026-05-02-001-cross-surface-feature-requirements/
    00-index.md
    manifest.json
    trace-ledger.json

    05-source/
      source-documents.md
      raw-prd-summary.md
      source-coverage.md

    06-prd/
      standard-prd.md
      prd-delta.md
      code-alignment.md
      prd-open-gaps.md
      split-decision.md

    10-intent/
      product-intent.md
      scope-boundaries.md
      non-goals.md

    20-capabilities/
      CAP1-business-entry.md
      CAP2-order-flow.md
      CAP3-market-data-display.md
      CAP4-admin-config.md

    25-surfaces/
      surface-map.md
      APP.md
      H5.md
      ADMIN.md
      BACKEND-MIDDLE.md
      BACKEND-MARKET.md

    30-flows/
      F1-user-app-flow.md
      F2-user-h5-flow.md
      F3-admin-config-flow.md
      F4-backend-sync-flow.md
      F5-end-to-end-business-flow.md

    35-integration-contracts/
      API1-business-query-contract.md
      API2-submit-action-contract.md
      DATA1-shared-field-dictionary.md
      AUTH1-permission-contract.md
      ERROR1-error-code-contract.md
      EVENT1-tracking-contract.md

    40-requirements/
      R1-global-business-rule.md
      R2-app-display-requirement.md
      R3-h5-display-requirement.md
      R4-admin-config-requirement.md
      R5-middle-backend-service-requirement.md
      R6-market-backend-data-requirement.md

    50-acceptance/
      AE1-app-basic-flow.md
      AE2-h5-basic-flow.md
      AE3-admin-config-effective.md
      AE4-backend-contract-compatible.md
      AE5-end-to-end-cross-surface-flow.md

    60-decisions/
      D1-source-of-truth-for-config.md
      D2-market-data-owner.md
      D3-app-h5-consistency-policy.md

    70-open-questions/
      Q1-market-data-refresh-frequency.md
      Q2-admin-permission-scope.md
      Q3-h5-fallback-rule.md

    75-roadmap/
      roadmap.md
      roadmap-items.yaml
      drafts/

    80-planning-slices/
      PS1-contract-foundation.md
      PS2-middle-backend-service.md
      PS3-market-backend-adapter.md
      PS4-admin-config.md
      PS5-app-implementation.md
      PS6-h5-implementation.md
      PS7-integration-acceptance.md
```

核心原则：

```text
Contract-first
Lane-based
Integration-verified
```

---

# 8. PRD 迭代 + Code-aware Normalization

已有 PRD 且是需求迭代时，不能只把 PRD 改写成“更漂亮的文档”。

终态链路是：

```text
Raw PRD
  -> 05-source/source-coverage.md
  -> 06-prd/standard-prd.md
  -> 06-prd/prd-delta.md
  -> 06-prd/code-alignment.md
  -> 06-prd/prd-open-gaps.md
  -> 06-prd/split-decision.md
  -> Requirements Packet / Multi-Surface Packet
  -> spec-plan
  -> spec-write-tasks
  -> spec-work
  -> spec-code-review
  -> spec-compound
```

## 8.1 `05-source/`

职责：记录原始材料和覆盖关系。

```text
05-source/
  source-documents.md
  raw-prd-summary.md
  source-coverage.md
```

### `source-documents.md`

记录：

```text
PRD 标题
版本
来源
日期
hash
相关原型 / 接口草案 / 埋点文档 / 会议纪要
```

### `source-coverage.md`

记录：

```text
PRD section -> compiled requirement / acceptance / contract / planning slice
```

示例：

```markdown
| Source Section | Compiled Output | Status | Notes |
|---|---|---|---|
| APP 展示 | R2, AE1, PS5 | covered | - |
| H5 展示 | R3, AE2, PS6 | covered | - |
| 行情字段 | R6, DATA1, PS3 | partial | 枚举不完整 |
| 灰度策略 | Q3 | open | PRD 未说明 |
```

---

## 8.2 `06-prd/standard-prd.md`

职责：输出结合代码事实整理后的标准 PRD。

推荐结构：

```markdown
# Standard PRD: <Topic>

## 1. 背景与问题
## 2. 目标与非目标
## 3. 用户与场景
## 4. 本次迭代范围
## 5. 现有能力与差异分析
## 6. 产品需求
## 7. 多端 / 多系统职责
## 8. 接口 / 数据 / 权限 / 配置契约
## 9. 用户流程
## 10. 验收标准
## 11. 兼容性与迁移
## 12. 风险与依赖
## 13. 开放问题
## 14. PRD 到 Requirements Trace
```

---

## 8.3 `06-prd/prd-delta.md`

职责：说明这次迭代相对当前系统的变化。

```markdown
# PRD Delta

## 新增
## 修改
## 删除 / 下线
## 复用
## 兼容
```

---

## 8.4 `06-prd/code-alignment.md`

职责：结合代码事实说明现状、复用点、影响面、PRD 与代码不一致的地方。

```markdown
# Code Alignment

## Existing Implementation Summary
## Reuse Candidates
## Impacted Areas
## Mismatch Between PRD and Code
```

示例：

```markdown
| PRD Claim | Code Fact | Risk | Action |
|---|---|---|---|
| PRD 说已有行情状态 | 代码里只有部分枚举 | 字段语义不完整 | 进入 Q1 |
| PRD 说 Admin 可配置 | 代码里配置不支持灰度 | 影响上线 | 进入 R4 / Q2 |
```

---

## 8.5 `06-prd/split-decision.md`

这是 PRD 迭代场景的关键文件。

职责：

```text
决定是否拆
怎么拆
为什么这样拆
哪些保持同一个 spec_id
哪些 deferred
默认 plan 哪一批
是否需要 multi-surface
是否需要 parent + child packets
```

模板：

```markdown
# Split Decision

## 1. Decision Summary

- Mode: Multi-Surface Requirements Packet
- Parent spec_id: 2026-05-02-001-cross-surface-feature
- Default planning target: Minimal Integration Loop
- Split strategy: Parent packet with lane-based planning slices

## 2. Why Not Single Requirements

## 3. Why Not Multiple Independent Specs

## 4. Selected Split Strategy

## 5. Internal Slices

| Slice | Scope | Owner | Reason |
|---|---|---|---|
| PS1 | Contract Foundation | 中台组 | 先收敛接口、字段、权限、错误码 |
| PS2 | Middle Backend Service | 中台组 | 提供统一业务聚合接口 |
| PS5 | APP Implementation | APP Team | 原生端展示与交互 |
| PS6 | H5 Implementation | H5 Team | Web 端展示与交互 |

## 6. Deferred Scope

## 7. Planning Guidance
```

---

# 9. Roadmap 设计

借鉴 CodeStable 的 roadmap 设计，但放到 spec-first 的 Requirements Packet 内，而不是新增独立主流程。CodeStable 的 roadmap 把大需求拆成概设、接口契约和子 feature 清单，并要求先模块拆分、再接口契约、再拆子 feature；同时用 `items.yaml` 管依赖、状态和最小闭环。([GitHub][7])

spec-first 采用：

```text
75-roadmap/
  roadmap.md
  roadmap-items.yaml
  drafts/
```

## 9.1 `roadmap.md`

```markdown
# Topic Roadmap

## 1. Background
## 2. Scope and Non-goals
## 3. Capability / Module Split
## 4. Cross-slice Contracts / Shared Protocols
## 5. Planning Slice List
## 6. Minimal Loop
## 7. Delivery Rationale
## 8. Observations
## 9. Changelog
```

## 9.2 `roadmap-items.yaml`

普通大需求：

```yaml
roadmap: 2026-05-02-001-topic
created: 2026-05-02
default_planning_target: MVP

items:
  - id: PS1
    title: Foundation
    description: 建立最小能力边界和入口
    requirement_refs: [R1]
    acceptance_refs: [AE1]
    depends_on: []
    status: planned
    minimal_loop: true
    plan: null
    notes: null

  - id: PS2
    title: Expert Routing
    description: 根据 change surface 和 review intent 选择专家
    requirement_refs: [R2, R3]
    acceptance_refs: [AE2, AE3]
    depends_on: [PS1]
    status: planned
    minimal_loop: false
    plan: null
    notes: null
```

跨端需求：

```yaml
roadmap: 2026-05-02-001-cross-surface-feature
created: 2026-05-02
default_planning_target: minimal_integration_loop

items:
  - id: PS1
    title: Contract Foundation
    description: 定义 API / DATA / AUTH / ERROR / EVENT 契约
    surface_refs: [APP, H5, ADMIN, BACKEND-MIDDLE, BACKEND-MARKET]
    requirement_refs: [R1]
    acceptance_refs: [AE5]
    contract_refs: [API1, DATA1, AUTH1, ERROR1, EVENT1]
    depends_on: []
    status: planned
    minimal_loop: true
    owner: BACKEND-MIDDLE
    participants: [APP, H5, ADMIN, BACKEND-MARKET]

  - id: PS5
    title: APP Minimal Display
    description: APP 消费中台接口，展示最小状态
    surface_refs: [APP]
    requirement_refs: [R2]
    acceptance_refs: [AE1]
    contract_refs: [API1, DATA1, ERROR1, EVENT1]
    depends_on: [PS1, PS2]
    status: planned
    minimal_loop: true
    owner: APP
```

第一阶段状态保持克制：

```text
planned | blocked | deferred | dropped
```

后续再扩展：

```text
planned -> in-plan -> in-tasks -> done
```

---

# 10. `manifest.json` 终态 Schema

## 10.1 普通 packet

```json
{
  "schema_version": "requirements-packet/v1",
  "spec_id": "2026-05-02-001-topic",
  "mode": "packetized",
  "entry": "00-index.md",
  "thin_entry": "../2026-05-02-001-topic-requirements.md",
  "source": {
    "type": "prd-iteration-code-aware",
    "documents": [
      {
        "id": "SRC1",
        "title": "产品需求文档",
        "path": "05-source/source-documents.md",
        "hash": "sha256:..."
      }
    ],
    "coverage": "05-source/source-coverage.md"
  },
  "prd": {
    "standard_prd": "06-prd/standard-prd.md",
    "delta": "06-prd/prd-delta.md",
    "code_alignment": "06-prd/code-alignment.md",
    "open_gaps": "06-prd/prd-open-gaps.md",
    "split_decision": "06-prd/split-decision.md"
  },
  "roadmap": {
    "path": "75-roadmap/roadmap.md",
    "items": "75-roadmap/roadmap-items.yaml",
    "default_planning_target": "MVP"
  },
  "ids": {
    "requirements": {
      "R1": "40-requirements/R1-foundation.md"
    },
    "acceptance_examples": {
      "AE1": "50-acceptance/AE1-basic-flow.md"
    },
    "planning_slices": {
      "PS1": "80-planning-slices/PS1-foundation.md"
    }
  },
  "split": {
    "strategy": "single-parent-packet",
    "reason": "large requirement with multiple planning slices",
    "child_packets": []
  }
}
```

## 10.2 Multi-surface packet

```json
{
  "schema_version": "requirements-packet/v1",
  "spec_id": "2026-05-02-001-cross-surface-feature",
  "mode": "multi-surface-packetized",
  "entry": "00-index.md",
  "thin_entry": "../2026-05-02-001-cross-surface-feature-requirements.md",
  "source": {
    "type": "prd-iteration-code-aware",
    "documents": [
      {
        "id": "SRC1",
        "title": "产品需求文档",
        "path": "05-source/source-documents.md",
        "hash": "sha256:..."
      }
    ],
    "coverage": "05-source/source-coverage.md"
  },
  "prd": {
    "standard_prd": "06-prd/standard-prd.md",
    "delta": "06-prd/prd-delta.md",
    "code_alignment": "06-prd/code-alignment.md",
    "open_gaps": "06-prd/prd-open-gaps.md",
    "split_decision": "06-prd/split-decision.md"
  },
  "surfaces": {
    "map": "25-surfaces/surface-map.md",
    "items": {
      "APP": "25-surfaces/APP.md",
      "H5": "25-surfaces/H5.md",
      "ADMIN": "25-surfaces/ADMIN.md",
      "BACKEND-MIDDLE": "25-surfaces/BACKEND-MIDDLE.md",
      "BACKEND-MARKET": "25-surfaces/BACKEND-MARKET.md"
    }
  },
  "contracts": {
    "API1": "35-integration-contracts/API1-business-query-contract.md",
    "DATA1": "35-integration-contracts/DATA1-shared-field-dictionary.md",
    "AUTH1": "35-integration-contracts/AUTH1-permission-contract.md",
    "ERROR1": "35-integration-contracts/ERROR1-error-code-contract.md",
    "EVENT1": "35-integration-contracts/EVENT1-tracking-contract.md"
  },
  "roadmap": {
    "path": "75-roadmap/roadmap.md",
    "items": "75-roadmap/roadmap-items.yaml",
    "default_planning_target": "minimal_integration_loop"
  },
  "split": {
    "strategy": "single-parent-multi-surface-packet",
    "reason": "one business loop with shared contracts and cross-surface acceptance",
    "child_packets": []
  }
}
```

---

# 11. `spec-plan` 终态消费协议

## 11.1 输入分类

`spec-plan` 支持：

```text
normal requirements.md
packetized requirements
multi-surface packetized requirements
```

## 11.2 Packet Intake 流程

普通 packet：

```text
读取 thin entry
读取 manifest.json
读取 00-index.md
读取 06-prd/standard-prd.md
读取 06-prd/prd-delta.md
读取 06-prd/code-alignment.md
读取 06-prd/split-decision.md
读取 75-roadmap/roadmap.md
读取 75-roadmap/roadmap-items.yaml
读取 selected PS
读取 selected R / AE / D / Q
查询 Graph evidence
生成 plan
```

Multi-surface packet 额外读取：

```text
25-surfaces/surface-map.md
selected surface files
35-integration-contracts/*
cross-surface AE
```

## 11.3 Plan frontmatter

```yaml
---
title: Cross Surface Feature Plan
type: feat
status: active
date: 2026-05-02
spec_id: 2026-05-02-001-cross-surface-feature
origin: docs/brainstorms/2026-05-02-001-cross-surface-feature-requirements.md
origin_mode: multi-surface-packetized
origin_manifest: docs/brainstorms/2026-05-02-001-cross-surface-feature-requirements/manifest.json
planning_target: minimal_integration_loop
surfaces:
  - APP
  - H5
  - ADMIN
  - BACKEND-MIDDLE
  - BACKEND-MARKET
---
```

## 11.4 Plan 正文

普通 packet：

```markdown
## PRD Source Scope
## Requirements Packet Scope
## Split Decision Followed
## Requirements Trace
## Acceptance Trace
## Deferred Requirements
## Graph Evidence
## Implementation Units
## Risks
## Verification
```

Multi-surface packet：

```markdown
## PRD Source Scope
## Requirements Packet Scope
## Split Decision Followed
## Orchestration Overview
## Integration Contracts
## Lane Plans
## Dependency Graph
## Requirements Trace
## Contract Trace
## Cross-Surface Acceptance Trace
## Contract Compliance Plan
## Risks
## Rollout / Gray / Monitoring
```

---

# 12. `spec-write-tasks` 终态协议

## 12.1 第一阶段输出

继续输出单个 task pack：

```text
docs/tasks/
  2026-05-02-001-cross-surface-feature-tasks.md
```

内部按 lane 分 section。

## 12.2 Task Pack Contract 增强

```json
{
  "schema_version": "task-pack/v1",
  "spec_id": "2026-05-02-001-cross-surface-feature",
  "source_plan": "docs/plans/2026-05-02-001-cross-surface-feature-plan.md",
  "source_plan_hash": "sha256:...",
  "source_requirements_mode": "multi-surface-packetized",
  "source_requirements_manifest": "docs/brainstorms/2026-05-02-001-cross-surface-feature-requirements/manifest.json",
  "split_strategy": "single-parent-multi-surface-packet",
  "tasks": [
    {
      "task_id": "T005",
      "title": "Implement APP minimal display",
      "surface": "APP",
      "owner": "APP",
      "participants": [],
      "source_unit": "U5",
      "planning_slice_refs": ["PS5"],
      "requirement_refs": ["R2"],
      "acceptance_refs": ["AE1"],
      "contract_refs": ["API1", "DATA1", "ERROR1", "EVENT1"],
      "blocked_by": ["T001", "T002"],
      "wave": 3,
      "files": [],
      "test_focus": "APP display and error state",
      "done_signal": "AE1 passes and APP consumes API1 without local business derivation"
    }
  ]
}
```

## 12.3 后续高级输出：Lane Task Packs

当需要跨团队分发独立文件时，启用：

```text
docs/tasks/
  2026-05-02-001-cross-surface-feature-tasks.md

  2026-05-02-001-cross-surface-feature-tasks/
    APP-tasks.md
    H5-tasks.md
    ADMIN-tasks.md
    BACKEND-MIDDLE-tasks.md
    BACKEND-MARKET-tasks.md
    INTEGRATION-tasks.md
```

这些 lane task pack 共享同一个：

```text
spec_id: 2026-05-02-001-cross-surface-feature
```

不拆业务身份，只拆执行包。

---

# 13. 多团队协作模型

不拆多个 `spec_id` 时，多团队通过 lane 分发：

```text
一个 parent spec_id
  -> 多个 surface lane
  -> 多个 integration contracts
  -> 一个 orchestration plan
  -> 多个 lane plan units
  -> 一个 master task pack
  -> 多个 lane task sections / lane task packs
  -> 各团队执行自己的 lane
  -> integration review 收敛
```

## 13.1 Team Handoff Matrix

```markdown
| Team | Lane | Receives | Must Follow | Done Signal |
|---|---|---|---|---|
| APP | PS5 / U5 | APP lane tasks | API1, DATA1, ERROR1, EVENT1 | AE1 pass |
| H5 | PS6 / U6 | H5 lane tasks | API1, DATA1, ERROR1, EVENT1 | AE2 pass |
| Admin | PS4 / U4 | Admin lane tasks | AUTH1, config contract | AE3 pass |
| 中台组 | PS2 / U2 | Middle lane tasks | API1, AUTH1, DATA1 | API1 ready |
| 行情组 | PS3 / U3 | Market lane tasks | DATA1 | market fields ready |
| Feature Owner / QA | PS7 / U7 | Integration tasks | all contracts | AE5 pass |
```

---

# 14. Review 终态协议

## 14.1 普通 packet review

```markdown
## PRD Delta Coverage

| Delta | Evidence | Result |
|---|---|---|
| 新增 API response 字段 | API1 implemented | pass |
| 新增状态展示 | UI diff present | pass |

## Packetized Requirements Completeness

| Ref | Source | Evidence | Result |
|---|---|---|---|
| R1 | 40-requirements/R1.md | plan U1 + diff | met |
| AE1 | 50-acceptance/AE1.md | test evidence | partial |

## Split Decision Compliance

| Rule | Result | Notes |
|---|---|---|
| MVP only | pass | Phase 2 deferred |
```

## 14.2 Multi-surface review

```markdown
## Lane Review

| Lane | Requirement | Evidence | Result |
|---|---|---|---|
| APP | R2 | APP consumes API1 | pass |
| H5 | R3 | missing error state | partial |

## Contract Compliance

| Contract | Consumers | Result | Notes |
|---|---|---|---|
| API1 | APP, H5 | partial | H5 missing error handling |
| DATA1 | APP, H5, Middle | pass | field semantics aligned |
| EVENT1 | APP, H5 | missing | H5 tracking absent |

## Cross-Surface Acceptance Coverage

| AE | Scope | Result | Notes |
|---|---|---|---|
| AE5 | APP/H5/Admin/Middle/Market | partial | integration test incomplete |
```

---

# 15. Knowledge 终态协议

Knowledge 写入：

```text
docs/solutions/**/*.md
```

建议 frontmatter：

```yaml
source_spec_id: 2026-05-02-001-cross-surface-feature
source_plan: docs/plans/2026-05-02-001-cross-surface-feature-plan.md
source_requirements_manifest: docs/brainstorms/2026-05-02-001-cross-surface-feature-requirements/manifest.json
source_refs:
  - R1
  - D2
  - AE5
  - API1
```

沉淀内容包括：

```text
PRD 标准化经验
Code-aware delta 分析经验
大需求 split decision 模式
contract-first 跨端协作模式
lane-based task 分发模式
integration review 模式
字段语义治理经验
```

---

# 16. 后续拟议维护型 skill：`spec-requirements`

## 16.1 定位

`spec-requirements` 当前不是公开 workflow 入口，也不是已实现的 slash / dollar command。它是后续拟议的需求资产维护 skill 概念；只有在重复的 packet 维护痛点被验证后，才应进入 `spec-plan` 重新设计。

```text
skills/spec-requirements/
  SKILL.md
  scripts/
    validate-requirements-packet.js
    audit-requirements-packet.js
    repair-requirements-packet.js
    migrate-legacy-requirements.js
    diff-prd.js
    update-from-prd.js
```

## 16.2 拟议能力 API

```text
spec-requirements validate <packet-dir>
spec-requirements audit <packet-dir>
spec-requirements repair <packet-dir>
spec-requirements migrate <legacy-requirements.md>
spec-requirements diff-prd <prd-v1> <prd-v2>
spec-requirements update-from-prd <packet-dir> <prd-v2>
```

当前不存在 `/spec:requirements` 或 `$spec-requirements` 入口；不要在用户引导中把它写成当前可运行 command。

## 16.3 能力

```text
validate:
  校验 manifest / required files / IDs / DAG / contract refs

audit:
  审查 coverage / dangling refs / roadmap gaps / contract gaps

repair:
  修复 manifest / trace-ledger / missing refs

migrate:
  旧巨型 requirements.md -> Requirements Packet

diff-prd:
  对比 PRD v1 / v2，输出影响 R / AE / PS / Contract

update-from-prd:
  根据 PRD v2 更新 packet，并标记 stale plan / stale tasks
```

---

# 17. 校验脚本终态

## 17.1 `validate-requirements-packet.js`

检查：

```text
manifest.json 是否存在
00-index.md 是否存在
thin entry 是否存在
manifest 路径是否存在
ID 是否重复
roadmap.md 是否存在
roadmap-items.yaml 是否存在
depends_on 是否引用不存在 item
depends_on 是否有循环
minimal_loop 是否存在
multi-surface 模式下是否存在 25-surfaces/
multi-surface 模式下是否存在 35-integration-contracts/
contract_refs 是否能找到对应 contract
source-coverage 是否覆盖 PRD section
```

不检查：

```text
需求拆得是否合理
技术方案是否正确
roadmap 优先级是否最佳
owner 是否组织上正确
```

语义判断仍然由 LLM + human 确认。

---

# 18. 版本路线图

## V1：PRD Intake + Requirements Packet MVP

目标：

```text
解决大 PRD / 大 requirements 撑爆 plan 的问题。
```

范围：

```text
spec-brainstorm:
  - Output Mode Selection
  - PRD Intake
  - Code-aware Normalization
  - split-decision.md
  - Requirements Packet
  - Multi-Surface Packet 最小结构

spec-plan:
  - packet intake
  - 读取 manifest / roadmap / selected PS
  - 输出仍为 docs/plans/*-plan.md
```

不做：

```text
Plan Packet
Lane Task Packs
Parent + Child Packets
spec-requirements
PRD v1/v2 diff
自动状态回写
```

---

## V2：Task Trace + Multi-Team Handoff

目标：

```text
让多个团队可以在同一个 spec_id 下领取自己的 lane tasks。
```

范围：

```text
spec-write-tasks:
  - task contract 增加 surface / owner / participants
  - 增加 planning_slice_refs
  - 增加 contract_refs
  - 增加 blocked_by / wave
  - 单 tasks.md 内按 lane 分 section

spec-work:
  - task-scoped context
  - 不读取完整 packet
```

---

## V3：Review Coverage 闭环

目标：

```text
让 Review 不只审代码，还审需求、验收、契约、跨端集成。
```

范围：

```text
spec-code-review:
  - PRD Delta Coverage
  - Packetized Requirements Completeness
  - Split Decision Compliance
  - Contract Compliance
  - Lane Review
  - Cross-Surface Acceptance Coverage

spec-compound:
  - source_spec_id
  - source_refs
  - contract-first / lane-based / integration-review 知识沉淀
```

---

## V4：`spec-requirements` 拟议维护层

目标：

```text
让需求资产可以长期维护，而不是一次性生成。
```

范围：

```text
spec-requirements:
  - validate
  - audit
  - repair
  - migrate
  - diff-prd
  - update-from-prd
```

---

## V5：Lane Task Packs

目标：

```text
在多仓库、多团队场景下，每个团队拿到独立 task pack。
```

范围：

```text
docs/tasks/
  xxx-tasks.md
  xxx-tasks/
    APP-tasks.md
    H5-tasks.md
    ADMIN-tasks.md
    BACKEND-MIDDLE-tasks.md
    BACKEND-MARKET-tasks.md
    INTEGRATION-tasks.md
```

仍然共享同一个 `spec_id`。

---

## V6：Plan Packet

目标：

```text
当 plan 自身也膨胀时，把 plan 也 packet 化。
```

目录：

```text
docs/plans/
  xxx-plan.md
  xxx-plan/
    00-plan-index.md
    plan-manifest.json
    10-overview/
    20-plan-slices/
    30-coverage/
    40-task-pack-input/
```

触发条件：

```text
plan 超过 800 行
implementation units 太多
write-tasks 难以消费单文件 plan
多团队 plan 需要独立 review
```

---

## V7：Parent + Child Packets

目标：

```text
当一个 multi-surface parent 也装不下，每个 surface 自身也是大需求时启用。
```

目录：

```text
docs/brainstorms/
  xxx-requirements/
    child-packets/
      app-requirements/
      h5-requirements/
      admin-requirements/
      middle-backend-requirements/
      market-backend-requirements/
```

启用条件：

```text
每个 surface 都需要独立 plan / tasks / review
各 surface 周期很长
跨仓库复杂度高
单 parent packet 已经难以维护
```

---

# 19. 最终开发任务拆分

## Task Group 1：Brainstorm Output Mode

```text
修改 skills/spec-brainstorm/SKILL.md
新增 Output Mode Selection
新增 PRD Intake Mode
新增 PRD Iteration + Code-aware Mode
新增 Packet Mode
新增 Multi-Surface Packet Mode
```

## Task Group 2：Packet Templates

```text
新增 templates 或 references：
requirements-packet-template.md
multi-surface-packet-template.md
standard-prd-template.md
split-decision-template.md
roadmap-template.md
roadmap-items-template.yaml
planning-slice-template.md
integration-contract-template.md
```

## Task Group 3：Validation Script

```text
新增 validate-requirements-packet.js
校验 manifest / paths / IDs / roadmap DAG / contracts
```

## Task Group 4：Plan Intake

```text
修改 skills/spec-plan/SKILL.md
支持 origin_mode
支持 origin_manifest
支持 packet intake
支持 multi-surface intake
新增 Requirements Packet Scope / PRD Source Scope / Split Decision Followed
```

## Task Group 5：Task Contract

```text
修改 skills/spec-write-tasks/SKILL.md
修改 task-pack schema
增加 surface / owner / participants / contract_refs / planning_slice_refs / blocked_by / wave
```

## Task Group 6：Review Coverage

```text
修改 skills/spec-code-review/SKILL.md
增加 Packetized Requirements Completeness
增加 PRD Delta Coverage
增加 Contract Compliance
增加 Lane Review
增加 Cross-Surface Acceptance Coverage
```

## Task Group 7：Knowledge Refs

```text
修改 skills/spec-compound/SKILL.md
支持 source_spec_id / source_requirements_manifest / source_refs
```

## Task Group 8：Future Maintenance Skill

```text
后续如有重复维护痛点，再通过 spec-plan 设计 skills/spec-requirements/
拟议能力包括 validate / audit / repair / migrate / diff-prd / update-from-prd
```

---

# 20. 失败模式与防护

| 失败模式                         | 表现              | 防护                                    |
| ---------------------------- | --------------- | ------------------------------------- |
| 所有需求都 packet 化               | 小需求变重           | Output Mode Selection                 |
| 先生成巨型 PRD 再拆                 | 第一事实源已污染        | 先建模，直接落盘                              |
| standard-prd 停止在文档           | plan 仍不知如何消费    | 必须生成 split-decision / PS              |
| roadmap 替代 plan              | HOW 前置到需求阶段     | Roadmap 只控阶段、边界、约束                    |
| planning slice 变 task        | 绕过 plan         | PS 只喂给 spec-plan                      |
| write-tasks 直接吃 requirements | scope 被 task 发明 | write-tasks 只消费 plan                  |
| Multi-surface 无 contract     | 联调漂移            | 必须有 35-integration-contracts          |
| 多团队各干各的                      | 单端完成，整体失败       | Integration AE + Contract Compliance  |
| Review 只审代码                  | 需求覆盖断链          | Requirements / AE / Contract Coverage |
| Knowledge 写回 requirements    | 事实源污染           | Knowledge 只写 docs/solutions           |
| manifest 变规则引擎               | 僵硬复杂            | manifest 只做结构账本                       |

---

# 21. 最终铁律

建议写入方案文档或 skill 规则：

```text
1. spec_id 代表业务闭环，不代表团队。
2. 小需求保持单文件。
3. 大需求直接生成 Requirements Packet，不先生成巨型 requirements.md。
4. 已有 PRD 的迭代需求必须做 Code-aware Normalization。
5. 标准 PRD 不是终点，必须进入 split decision。
6. 跨端需求必须 Multi-Surface + Contract-first。
7. Roadmap 控制 plan scope，但不替代 spec-plan。
8. Planning Slice 只喂给 spec-plan，不直接变 task。
9. spec-plan 始终是技术方案 source of truth。
10. spec-write-tasks 只消费 plan。
11. spec-work 只读取 task-scoped context。
12. spec-code-review 必须审 requirement / acceptance / contract coverage。
13. spec-compound 只沉淀 knowledge，不写回 requirements。
14. manifest 只做结构账本，不做规则引擎。
15. Parent + Child Packets、Plan Packet、Lane Task Packs 都是后置高级能力。
```

---

# 22. 最终一句话

终态方案不是“做一个更复杂的 PRD 模板”，而是：

```text
让 spec-first 能把真实公司里的原始 PRD、需求迭代、大需求、跨端协作，
编译成能被 Plan / Tasks / Work / Review / Knowledge 全链路消费的需求资产。
```

最终形态：

```text
小需求：
  requirements.md

大需求：
  requirements packet + roadmap + planning slices

PRD 迭代：
  source + standard-prd + delta + code-alignment + split-decision + packet

跨端需求：
  multi-surface packet + surfaces + contracts + lane planning + integration acceptance

多团队执行：
  一个 spec_id + 多个 lane / owner / task sections / integration review

长期维护：
  spec-requirements validate / audit / repair / migrate / diff / update
```

这条路线最符合 spec-first 的核心理念：

```text
AI coding is not a prompt problem — it is a workflow problem.
Spec > Code.
Systems > Prompts.
```

[1]: https://raw.githubusercontent.com/sunrain520/spec-first/master/skills/spec-brainstorm/SKILL.md "raw.githubusercontent.com"
[2]: https://raw.githubusercontent.com/sunrain520/spec-first/master/skills/spec-brainstorm/references/requirements-capture.md "raw.githubusercontent.com"
[3]: https://raw.githubusercontent.com/sunrain520/spec-first/master/skills/spec-plan/SKILL.md "raw.githubusercontent.com"
[4]: https://raw.githubusercontent.com/sunrain520/spec-first/master/skills/spec-write-tasks/SKILL.md "raw.githubusercontent.com"
[5]: https://raw.githubusercontent.com/sunrain520/spec-first/master/skills/spec-code-review/SKILL.md "` to `testing_gaps`. If `maintainability`, append the same to `residual_risks`. Record the demotion count for Coverage. The finding does not appear in the Stage 6 findings table. (Use title only -- the compact return omits `why_it_matters`, and report-only mode skips artifact files entirely. Soft-bucket entries are FYI items; readers who want depth can open the per-agent artifact when one exists.)
   \- **Headless and autofix modes:** Suppress the finding entirely. Record the suppressed count in Coverage as \"mode-aware demotion suppressions\" so the user can see what was filtered.

Demotion is intentionally narrow. The conservative scope (testing/maintainability + P2/P3 + advisory) is the starting point; do not expand the rule by guessing which other personas overproduce noise. If real review runs show another persona consistently emitting weak signal, expand with evidence.

7\. **Confidence-first gate.** After dedup, promotion, and demotion have shaped the primary set, suppress remaining findings below anchor 75. Exception: P0 findings at anchor 50+ survive the gate -- critical-but-uncertain issues must not be silently dropped. Record the suppressed count by anchor (so Coverage can report \"N findings suppressed at anchor 50, M at anchor 25\"). The gate runs late deliberately: anchor-50 findings need a chance to be promoted by step 3 (cross-reviewer corroboration) or rerouted by step 6c (mode-aware demotion to soft buckets) before any drop decision.
8\. **Partition the work.** Build three sets:
   \- in-skill fixer queue: only `safe_auto -> review-fixer`
   \- residual actionable queue: unresolved `gated_auto` or `manual` findings whose owner is `downstream-resolver`
   \- report-only queue: `advisory` findings plus anything owned by `human` or `release`
9\. **Sort.** Order by severity (P0 first) -> anchor (descending) -> file path -> line number.
10\. **Collect coverage data.** Union residual_risks and testing_gaps across reviewers.
11\. **Preserve Spec-First agent artifacts.** Keep the learnings, agent-native, schema-drift, and deployment-verification outputs alongside the merged finding set. Do not drop unstructured agent output just because it does not match the persona JSON schema.

\### Stage 5b: Validation pass (externalizing modes only)

Independent verification gate. Spawn one validator sub-agent per surviving finding using `references/validator-template.md`. The validator's job is to re-check the finding against the diff and surrounding code with no commitment to the original persona's analysis. Findings the validator rejects are dropped; findings the validator confirms flow through unchanged.

\**When this stage runs:**

\| Mode | Runs Stage 5b? | Where |
\|------|---------------|-------|
\| `headless` | Yes, eagerly | Between Stage 5 and Stage 6 |
\| `autofix` | Yes, eagerly | Between Stage 5 and Stage 6 |
\| `interactive`, walk-through routing (option A) — per-finding phase | No -- the user is the per-finding validator | n/a |
\| `interactive`, walk-through routing (option A) — Auto-resolve with best judgment on the rest handoff | No -- the best-judgment path dispatches the fixer immediately; the fixer's apply/fail outcome is the validation | n/a |
\| `interactive`, best-judgment routing (option B) | No -- the best-judgment path dispatches the fixer immediately; the fixer's apply/fail outcome is the validation | n/a |
\| `interactive`, File-tickets routing (option C) | Yes, on all pending findings | Before tracker dispatch |
\| `interactive`, Report-only routing (option D) | No -- nothing is being externalized | n/a |
\| `report-only` | No -- read-only mode externalizes nothing | n/a |

When Stage 5b does not run, the merged finding set from Stage 5 flows through to Stage 6 unchanged. When it runs, the steps below execute on the relevant set.

The best-judgment path skips Stage 5b deliberately. Running validators before fixer dispatch is duplicate research: the fixer re-checks each finding while applying or proposing the fix, and items where the cited evidence no longer matches the code are routed to the `failed` bucket during the fix attempt. The user reviews via diff and the post-run failure-handling question, not via a pre-dispatch validator gate.

\**Steps:**

1\. **Select findings to validate.**
   \- **headless/autofix:** All survivors of Stage 5.
   \- **interactive File-tickets (option C):** All pending findings regardless of recommended action. Option C externalizes every finding as a ticket, so every finding needs validation.
2\. **Apply dispatch budget cap.** If the selected set exceeds 15 findings, validate the highest-severity 15 (P0 first, then P1, then P2, then P3, breaking ties by anchor descending). Drop the remainder and record the over-budget count for the Coverage section. The blunt drop is intentional; a review producing 15+ surviving findings is already in territory where a second wave would not change the user's triage approach.
3\. **Spawn validators in parallel.** One sub-agent per finding, dispatched concurrently using the validator template. Each validator receives:
   \- The finding's title, severity, file, line, suggested_fix, original reviewer name, and confidence-first anchor
   \- `why_it_matters` when available — loaded from the per-agent artifact file at `/tmp/spec-first/spec-code-review/{run_id}/{reviewer_name}.json`; omit when the file is absent or the artifact write failed. The validator proceeds without it, using the diff and cited code directly.
   \- The full diff
   \- Read-tool access to inspect the cited code, callers, guards, framework defaults, and git blame
4\. **Collect verdicts.** Each validator returns `{ \"validated\": true | false, \"reason\": \"<one sentence>\" }`.
   \- `validated: true` -> finding survives unchanged into the next phase (Stage 6 for headless/autofix, dispatch for interactive)
   \- `validated: false` -> finding is dropped; record the validator's reason in Coverage
   \- Validator failure (timeout, dispatch error, malformed JSON) -> drop the finding with reason \"validator failed\"; conservative bias is correct
5\. **Use mid-tier model for validators.** Same model class (sonnet) the persona reviewers use. Validators are read-only — same constraints as persona reviewers. They may use non-mutating inspection commands (Read, Grep, Glob, git blame, gh).
6\. **Record metrics for Coverage.** Total dispatched, validated true count, validated false count (with reasons), failures, and over-budget drops.

\**Why per-finding parallel dispatch (not batched):** Independence is the point. A single batched validator looking at all findings together pattern-matches across them and recreates the persona-bias problem. Per-finding parallel dispatch preserves fresh context per call. Per-file batching is a plausible future optimization for reviews with many findings clustered in few files; not implemented today.

\### Stage 6: Synthesize and present

Assemble the final report using **pipe-delimited markdown tables for findings** from the review output template included below. The table format is mandatory for finding rows in interactive mode — do not render findings as freeform text blocks or horizontal-rule-separated prose. Other report sections (Applied Fixes, Learnings, Coverage, etc.) use bullet lists and the `---` separator before the verdict, as shown in the template.

1\. **Header.** Scope, intent, mode, reviewer team with per-conditional justifications.
2\. **Findings.** Rendered as pipe-delimited tables grouped by severity (`### P0 -- Critical`, `### P1 -- High`, `### P2 -- Moderate`, `### P3 -- Low`). Each finding row shows `#`, file, issue, reviewer(s), confidence, and synthesized route. Omit empty severity levels. Never render findings as freeform text blocks or numbered lists.
3\. **Requirements Completeness.** Include only when a plan was found in Stage 2b. For each requirement (R1, R2, etc.) and implementation unit in the plan, report whether corresponding work appears in the diff. Use a simple checklist: met / not addressed / partially addressed. Routing depends on `plan_source`:
   \- **`explicit`** (caller-provided or PR body): Flag unaddressed requirements as P1 findings with `autofix_class: manual`, `owner: downstream-resolver`. These enter the residual actionable queue.
   \- **`inferred`** (auto-discovered): Flag unaddressed requirements as P3 findings with `autofix_class: advisory`, `owner: human`. These stay in the report only — no autonomous follow-up. An inferred plan match is a hint, not a contract.
   Omit this section entirely when no plan was found — do not mention the absence of a plan.
4\. **Applied Fixes.** Include only if a fix phase ran in this invocation.
5\. **Residual Actionable Work.** Include when unresolved actionable findings were handed off or should be handed off.
6\. **Pre-existing.** Separate section, does not count toward verdict.
7\. **Learnings & Past Solutions.** Surface spec-learnings-researcher results: if past solutions are relevant, flag them as \"Known Pattern\" with links to docs/solutions/ files.
8\. **Agent-Native Gaps.** Surface spec-agent-native-reviewer results. Omit section if no gaps found.
9\. **Schema Drift Check.** If spec-schema-drift-detector ran, summarize whether drift was found. If drift exists, list the unrelated schema objects and the required cleanup command. If clean, say so briefly.
10\. **Deployment Notes.** If spec-deployment-verification-agent ran, surface the key Go/No-Go items: blocking pre-deploy checks, the most important verification queries, rollback caveats, and monitoring focus areas. Keep the checklist actionable rather than dropping it into Coverage.
11\. **Coverage.** Suppressed count by anchor (e.g., \"N findings suppressed at anchor 50, M at anchor 25\"), mode-aware demotion count (interactive/report-only) or suppression count (headless/autofix), validator drop count and reasons (when Stage 5b ran), validator over-budget drops (when the 15-cap fired), residual risks, testing gaps, failed/timed-out reviewers, and any intent uncertainty carried by non-interactive modes.
12\. **Verdict.** Ready to merge / Ready with fixes / Not ready. Fix order if applicable. When an `explicit` plan has unaddressed requirements, the verdict must reflect it — a PR that's code-clean but missing planned requirements is \"Not ready\" unless the omission is intentional. When an `inferred` plan has unaddressed requirements, note it in the verdict reasoning but do not block on it alone.

Do not include time estimates.

\**Format verification:** Before delivering the report, verify the findings sections use pipe-delimited table rows (`| # | File | Issue | ... |`) not freeform text. If you catch yourself rendering findings as prose blocks separated by horizontal rules or bullet points, stop and reformat into tables.

\### Headless output format

In `mode:headless`, replace the interactive pipe-delimited table report with a structured text envelope. The envelope follows the same structural pattern as spec-doc-review's headless output (completion header, metadata block, findings grouped by autofix_class, trailing sections) while using spec-code-review's own section headings and per-finding fields.

\```
Code review complete (headless mode).

Scope: <scope-line>
Intent: <intent-summary>
Reviewers: <reviewer-list with conditional justifications>
Verdict: <Ready to merge | Ready with fixes | Not ready>
Artifact: /tmp/spec-first/spec-code-review/<run-id>/

Applied N safe_auto fixes.

Gated-auto findings (concrete fix, changes behavior/contracts):

\[P1][gated_auto -> downstream-resolver][needs-verification] File: <file:line> -- <title> (<reviewer>, confidence-first <N>)
  Why: <why_it_matters>
  Suggested fix: <suggested_fix or \"none\">
  Evidence: <evidence[0]>
  Evidence: <evidence[1]>

Manual findings (actionable, needs handoff):

\[P1][manual -> downstream-resolver] File: <file:line> -- <title> (<reviewer>, confidence-first <N>)
  Why: <why_it_matters>
  Evidence: <evidence[0]>

Advisory findings (report-only):

\[P2][advisory -> human] File: <file:line> -- <title> (<reviewer>, confidence-first <N>)
  Why: <why_it_matters>

Pre-existing issues:
\[P2][gated_auto -> downstream-resolver] File: <file:line> -- <title> (<reviewer>, confidence-first <N>)
  Why: <why_it_matters>

Residual risks:
\- <risk>

Learnings & Past Solutions:
\- <learning>

Agent-Native Gaps:
\- <gap description>

Schema Drift Check:
\- <drift status>

Deployment Notes:
\- <deployment note>

Testing gaps:
\- <gap>

Coverage:
\- Suppressed: <N> findings below anchor 75 (P0 at anchor 50+ retained)
\- Mode-aware demotion suppressions: <N> findings suppressed (testing/maintainability advisory P2-P3)
\- Validator drops: <N> findings rejected by Stage 5b validator
  \- <file:line> -- <reason>
\- Validator over-budget drops: <N> findings exceeded the 15-cap and were not validated
\- Untracked files excluded: <file1>, <file2>
\- Failed reviewers: <reviewer>

Review complete
\```

\**Detail enrichment (headless only):** The headless envelope includes `Why:`, `Evidence:`, and `Suggested fix:` lines. After merge (Stage 5), read the per-agent artifact files from `/tmp/spec-first/spec-code-review/{run_id}/` for only the findings that survived dedup and confidence-first gating.
   \- **Field tiers:** `Why:` and `Evidence:` are detail-tier -- load from per-agent artifact files. `Suggested fix:` is merge-tier -- use it directly from the compact return without artifact lookup.
   \- **Artifact matching:** For each surviving finding, look up its detail-tier fields in the artifact files of the contributing reviewers. Match on `file + line_bucket(line, +/-3)` (the same tolerance used in Stage 5 dedup) within each contributing reviewer's artifact. When multiple artifact entries fall within the line bucket, apply `normalize(title)` to both the merged finding's title and each candidate entry's title as a tie-breaker.
   \- **Reviewer order:** Try contributing reviewers in the order they appear in the merged finding's reviewer list; use the first match.
   \- **No-match fallback:** If no artifact file contains a match (all writes failed, or the finding was synthesized during merge), omit the `Why:` and `Evidence:` lines for that finding and note the gap in Coverage. The `Suggested fix:` line can still be populated from the compact return since it is merge-tier.

\**Formatting rules:**
\- The `[needs-verification]` marker appears only on findings where `requires_verification: true`.
\- The `Artifact:` line gives callers the path to the full run artifact for machine-readable access to the complete findings schema. The text envelope is the primary handoff; the artifact is for debugging and full-fidelity access.
\- Findings with `owner: release` appear in the Advisory section (they are operational/rollout items, not code fixes).
\- Findings with `pre_existing: true` appear in the Pre-existing section regardless of autofix_class.
\- The Verdict appears in the metadata header (deliberately reordered from the interactive format where it appears at the bottom) so programmatic callers get the verdict first.
\- Omit any section with zero items.
\- If all reviewers fail or time out, emit `Code review degraded (headless mode). Reason: 0 of N reviewers returned results.` followed by \"Review complete\".
\- End with \"Review complete\" as the terminal signal so callers can detect completion.

\## Quality Gates

Before delivering the review, verify:

1\. **Every finding is actionable.** Re-read each finding. If it says \"consider\", \"might want to\", or \"could be improved\" without a concrete fix, rewrite it with a specific action. Vague findings waste engineering time.
2\. **No false positives from skimming.** For each finding, verify the surrounding code was actually read. Check that the \"bug\" isn't handled elsewhere in the same function, that the \"unused import\" isn't used in a type annotation, that the \"missing null check\" isn't guarded by the caller.
3\. **Severity is calibrated.** A style nit is never P0. A SQL injection is never P3. Re-check every severity assignment.
4\. **Line numbers are accurate.** Verify each cited line number against the file content. A finding pointing to the wrong line is worse than no finding.
5\. **Protected artifacts are respected.** Discard any findings that recommend deleting or gitignoring files in `docs/brainstorms/`, `docs/plans/`, or `docs/solutions/`.
6\. **Findings don't duplicate linter output.** Don't flag things the project's linter/formatter would catch (missing semicolons, wrong indentation). Focus on semantic issues.

\## Language-Aware Conditionals

This skill uses stack-specific reviewer agents when the diff clearly warrants them. Keep those agents opinionated. They are not generic language checkers; they add a distinct review lens on top of the always-on and cross-cutting personas.

Do not spawn them mechanically from file extensions alone. The trigger is meaningful changed behavior, architecture, or UI state in that stack.

\## After Review

\### Mode-Driven Post-Review Flow

After presenting findings and verdict (Stage 6), route the next steps by mode. Review and synthesis stay the same in every mode; only mutation and handoff behavior changes.

\#### Step 1: Build the action sets

\- **Clean review** means zero findings after suppression and pre-existing separation. Skip the fix/handoff phase when the review is clean.
\- **Fixer queue:** final findings routed to `safe_auto -> review-fixer`.
\- **Residual actionable queue:** unresolved `gated_auto` or `manual` findings whose final owner is `downstream-resolver`.
\- **Report-only queue:** `advisory` findings and any outputs owned by `human` or `release`.
\- **Never convert advisory-only outputs into fix work or ticket handoff.** Deployment notes, residual risks, and release-owned items stay in the report.

\#### Step 2: Choose policy by mode

\**Interactive mode**

\- Apply `safe_auto -> review-fixer` findings automatically without asking. These are safe by definition.
\- **Zero-remaining case:** if no `gated_auto` or `manual` findings remain after the `safe_auto` pass, skip the routing question entirely. Emit a one-line completion summary phrased so advisory and pre-existing findings (which are not handled by this flow) are not implied to be cleared. When no advisory or pre-existing findings remain in the report, `All findings resolved — N safe_auto fixes applied.` is accurate. When advisory and/or pre-existing findings do remain, use the qualified form `All actionable findings resolved — N safe_auto fixes applied. (K advisory, J pre-existing findings remain in the report.)`, omitting any zero-count clause. Follow the summary with the existing end-of-review verdict, then proceed to Step 5 per the gating rule there.
\- **Tracker pre-detection:** before rendering the routing question, consult `references/tracker-defer.md` for the session's tracker tuple `{ tracker_name, confidence, named_sink_available, any_sink_available }`. The probe runs at most once per session and is cached for the rest of the run. `named_sink_available` drives the option C label (inline tracker name only when the named sink can actually be invoked). `any_sink_available` drives whether option C is offered at all (it can still be offered when the named tracker is unreachable but GitHub Issues via `gh` works).
\- **Verify question-tool pre-load (checklist, Claude Code only).** Before firing the routing question in Claude Code, confirm `AskUserQuestion` is loaded (per Interactive mode rules at the top of this skill). If not yet loaded this session, call `ToolSearch` with query `select:AskUserQuestion` now. Do not proceed to the routing question without this verification. Rendering the question as narrative text because the schema isn't loaded yet is a bug, not a valid fallback. On Codex this checklist does not apply — there is no `ToolSearch` preload step to perform. (If `request_user_input` is unavailable in the current Codex runtime mode, use the numbered-list fallback described below.)
\- **Routing question.** Ask using the platform's blocking question tool (`AskUserQuestion` in Claude Code or `request_user_input` in Codex). Stem: `What should the agent do with the remaining N findings?` — use third-person voice referring to \"the agent\", not first-person \"me\" / \"I\". Options:

  \```
  (A) Review each finding one by one — accept the recommendation or choose another action
  (B) Auto-resolve with best judgment — apply per-finding fixes the agent can defend, surface the rest
  (C) File a [TRACKER] ticket per finding without applying fixes
  (D) Report only — take no further action
  \```

  Render option C per `references/tracker-defer.md`: when `confidence = high` AND `named_sink_available = true`, replace `[TRACKER]` with the concrete name and keep the full label (e.g., `File a Linear ticket per finding without applying fixes`). When `any_sink_available = true` but either `confidence = low` or `named_sink_available = false` (GitHub Issues via `gh` is working as the fallback), use the generic label `File an issue per finding without applying fixes` — this is a whole-label substitution, not a `[TRACKER]` token swap. When `any_sink_available = false`, **omit option C entirely** and add one line to the stem explaining that no issue tracker is configured for this checkout (Linear, GitHub Issues, etc., were probed and unavailable). The three remaining options (A, B, D) survive.

  The numbered-list text fallback applies when `ToolSearch` explicitly returns no match for the platform's question tool or the tool call errors (including Codex runtime modes where `request_user_input` is unavailable). It does not apply when the agent simply hasn't loaded the tool yet — in that case, load it now (see the verification checklist above). When the fallback applies, present the options as a numbered list and wait for the user's reply — never silently skip the question.

\- **Dispatch on selection.** Route by the option letter (A / B / C / D), not by the rendered label string. The option-C label varies by tracker-detection confidence (`File a [TRACKER] ticket per finding without applying fixes` for a named tracker, `File an issue per finding without applying fixes` as the generic fallback, or omitted entirely when no sink is available — see `references/tracker-defer.md`), and options A / B / D have a single canonical label each. The letter is the stable dispatch signal; the canonical labels below are shown for documentation only. A low-confidence run that rendered option C as the generic label routes to the same branch as a high-confidence run that rendered it with the named tracker.
  \- (A) `Review each finding one by one` — load `references/walkthrough.md` and enter the per-finding walk-through loop. The walk-through accumulates Apply decisions in memory; Defer decisions execute inline via `references/tracker-defer.md`; Skip / Acknowledge decisions are recorded as no-action. `Auto-resolve with best judgment on the rest` exits the loop and dispatches one fixer pass on the union of already-accumulated Apply decisions plus remaining undecided findings. When the user works through every finding without invoking that shortcut, dispatch one fixer subagent for the accumulated Apply set at end of loop (Step 3). Emit the unified completion report after dispatch.
  \- (B) `Auto-resolve with best judgment — apply per-finding fixes the agent can defend, surface the rest` — dispatch the fixer subagent (Step 3) immediately on the full pending action set (`gated_auto` + `manual` + `advisory`). No Stage 5b validator pre-pass. No bulk-preview approval gate. The fixer applies items with concrete `suggested_fix`, no-ops on advisory items, and routes items where the fix cannot be applied cleanly, or where cited evidence no longer matches the code, to a `failed` bucket with a one-line reason.

    \**After the fixer returns, the order is:**
    1\. If `failed` is empty, emit the unified completion report and proceed to Step 5 per its gating rule.
    2\. If `failed` is non-empty, fire the post-run failure-handling question before emitting the report. Stem: `N findings could not be auto-resolved. What should the agent do with them?` Options:
       \- `File tickets for these` — route the failed set through `references/tracker-defer.md` Interactive mode. Omit this option when `any_sink_available = false`, and say no issue tracker is configured for this checkout.
       \- `Walk through these one at a time` — re-enter the walk-through loop scoped to the failed set. Items with `suggested_fix` recommend Apply; items without `suggested_fix` recommend Defer and do not offer Apply.
       \- `Ignore — leave them in the report` — record the failed list as residual actionable work.

    After the user's choice executes, emit the unified completion report reflecting any tickets filed or additional fixes applied during walk-through re-entry.
  \- (C) `File a [TRACKER] ticket per finding without applying fixes` (or the generic `File an issue per finding without applying fixes` when the named-tracker label is not used) — first run Stage 5b validation on every pending finding. Drop validator-rejected findings with their reasons recorded in Coverage. Then load `references/bulk-preview.md` with every surviving finding in the file-tickets bucket. On `Proceed`, route every finding through `references/tracker-defer.md`; no fixes are applied. On `Cancel`, return to this routing question. Emit the unified completion report.
  \- (D) `Report only — take no further action` — do not enter any dispatch phase. Emit the completion report, then proceed to Step 5 per its gating rule (`fixes_applied_count > 0` from earlier `safe_auto` passes). If no fixes were applied this run, stop after the report.

\- The walk-through's completion report, the best-judgment / File-tickets completion report, and the zero-remaining completion summary all follow the unified completion-report structure documented in `references/walkthrough.md`. Use the same structure across every terminal path.

\**Autofix mode**

\- Ask no questions.
\- Apply only the `safe_auto -> review-fixer` queue.
\- Leave `gated_auto`, `manual`, `human`, and `release` items unresolved.
\- Prepare residual work only for unresolved actionable findings whose final owner is `downstream-resolver`.

\**Report-only mode**

\- Ask no questions.
\- Do not build a fixer queue.
\- Do not write run artifacts.
\- Stop after Stage 6. Everything remains in the report.

\**Headless mode**

\- Ask no questions.
\- Apply only the `safe_auto -> review-fixer` queue in a single pass. Do not enter the bounded re-review loop (Step 3). Spawn one fixer subagent, apply fixes, then proceed directly to Step 4.
\- Leave `gated_auto`, `manual`, `human`, and `release` items unresolved — they appear in the structured text output.
\- Output the headless output envelope (see Stage 6) instead of the interactive report.
\- Write a run artifact (Step 4). Do not file tickets or externalize work — the caller owns that.
\- Stop after the structured text output and \"Review complete\" signal. No commit/push/PR.

\#### Step 3: Apply fixes with one fixer

\- Spawn exactly one fixer subagent for the current fixer queue in the current checkout. That fixer applies all approved changes and runs the relevant targeted tests in one pass against a consistent tree.
\- Do not fan out multiple fixers against the same checkout. Parallel fixers require isolated worktrees/branches and deliberate mergeback.
\- **Homogeneous queue** (autofix, headless, walk-through Apply set): every item is `safe_auto -> review-fixer` or every item carries a concrete `suggested_fix`. Apply each item. If a walk-through Apply item lacks `suggested_fix`, route it to `failed` with reason `no fix proposed by reviewer`.
\- **Heterogeneous queue** (best-judgment path): the queue may mix `gated_auto`, `manual`, and `advisory`. Apply items with `suggested_fix`, no-op advisory items, and route anything that cannot be applied cleanly to `failed` with a one-line reason.
\- If any applied finding has `requires_verification: true`, run the relevant targeted verification before reporting it as applied.
\- Do not start a mutating review round concurrently with browser testing on the same checkout. Future orchestrators that want both must either run `mode:report-only` during the parallel phase or isolate the mutating review in its own checkout/worktree.

\#### Step 4: Emit artifacts and downstream handoff

\- In interactive, autofix, and headless modes, write a per-run artifact under `/tmp/spec-first/spec-code-review/<run-id>/` containing:
  \- synthesized findings (merged output from Stage 5)
  \- applied fixes
  \- residual actionable work
  \- advisory-only outputs
  Per-agent full-detail JSON files (`{reviewer_name}.json`) are already present in this directory from Stage 4 dispatch.
\- Also write `metadata.json` alongside the findings so downstream skills (e.g., `spec-polish-beta`) can verify the artifact matches the current branch and HEAD. Minimum fields:
  \```json
  {
    \"run_id\": \"<run-id>\",
    \"branch\": \"<git branch --show-current at dispatch time>\",
    \"head_sha\": \"<git rev-parse HEAD at dispatch time>\",
    \"verdict\": \"<Ready to merge | Ready with fixes | Not ready>\",
    \"completed_at\": \"<ISO 8601 UTC timestamp>\"
  }
  \```
  Capture `branch` and `head_sha` at dispatch time (before any autofixes land), and write the file after the verdict is finalized. This file is additive -- pre-existing artifacts that predate this field are still valid, and downstream skills fall back to file mtime when it is missing.
\- In autofix mode, the run artifact is the handoff. Orchestrators read the artifact's residual actionable work and route it as appropriate. The skill itself does not file tickets or prompt the user in autofix.
\- Interactive mode may offer to externalize residual actionable work via `references/tracker-defer.md` (named tracker -> GitHub Issues via `gh`), but it is not required to finish the review.

\#### Step 5: Final next steps

\**Interactive mode only.** After the fix-review cycle completes (clean verdict or the user chose to stop), offer next steps based on the entry mode. Reuse the resolved review base/default branch from Stage 1 when known; do not hard-code only `main`/`master`.

\**The gate is total fixes applied this run, not routing option.** Track `fixes_applied_count` across the whole Interactive invocation. This counter includes both the `safe_auto` fixes applied automatically before the routing question (see Step 2 Interactive mode) AND any Apply decisions executed by routing option A (walk-through) or option B (Auto-resolve with best judgment). Routing options C (File tickets) and D (Report only) add zero to this counter; neither does a walk-through that ends with only Skip / Defer / Acknowledge, and neither does an Auto-resolve with best judgment run that applies no fixes.

Step 5 runs only when `fixes_applied_count > 0`. If the counter is zero — no `safe_auto` fixes were applied AND the routing path produced no additional Apply — skip Step 5 entirely and exit after the completion report. Asking \"push fixes?\" when nothing changed in the working tree is incoherent.

Common outcomes:

\- `safe_auto` produced fixes AND the user picked any routing option → Step 5 runs (counter > 0 from the safe_auto pass alone).
\- No `safe_auto` fixes AND the user picked option C or D → Step 5 skipped.
\- No `safe_auto` fixes AND walk-through / Auto-resolve with best judgment finished with zero Applies → Step 5 skipped.
\- Zero-remaining case (no `gated_auto` / `manual` after `safe_auto`) with at least one `safe_auto` fix → Step 5 runs; the routing question was never asked but the counter is > 0.

\- **PR mode (entered via PR number/URL):**
  \- **Push fixes** -- push commits to the existing PR branch
  \- **Exit** -- done for now
\- **Branch mode (feature branch with no PR, and not the resolved review base/default branch):**
  \- **Create a PR (Recommended)** -- push and open a pull request
  \- **Continue without PR** -- stay on the branch
  \- **Exit** -- done for now
\- **On the resolved review base/default branch:**
  \- **Continue** -- proceed with next steps
  \- **Exit** -- done for now

If \"Create a PR\": first publish the branch with `git push --set-upstream origin HEAD`, then use `gh pr create` with a title and summary derived from the branch changes.
If \"Push fixes\": push the branch with `git push` to update the existing PR.

\**Autofix, report-only, and headless modes:** stop after the report, artifact emission, and residual-work handoff. Do not commit, push, or create a PR.

\## Fallback

If the platform doesn't support parallel sub-agents, run reviewers sequentially. Everything else (stages, output format, merge pipeline) stays the same.

\---

\## Included References

\### Persona Catalog

@./references/persona-catalog.md

\### Subagent Template

@./references/subagent-template.md

\### Diff Scope Rules

@./references/diff-scope.md

\### Findings Schema

@./references/findings-schema.json

\### Review Output Template

@./references/review-output-template.md
"
[6]: https://raw.githubusercontent.com/sunrain520/spec-first/master/skills/spec-compound/SKILL.md "raw.githubusercontent.com"
[7]: https://raw.githubusercontent.com/liuzhengdongfortest/CodeStable/main/cs-roadmap/SKILL.md "raw.githubusercontent.com"
