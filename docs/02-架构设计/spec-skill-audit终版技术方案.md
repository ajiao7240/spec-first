...
对 `spec-first` 来说，项目自身 CLAUDE.md 已经明确：`skills/`、`agents/`、`templates/` 是 source of truth，`.claude/`、`.codex/`、`.agents/skills/` 是由 `spec-first init --claude|--codex` 管理的 generated runtime assets，不能手改生成产物，否则会制造 source/runtime drift。([GitHub][5])
...
- suggest rerunning `spec-first init --claude` or `spec-first init --codex`
...
```text
正确建议：
  rerun spec-first init --claude
  rerun spec-first init --codex
  ...
  "findings": [
    {
      ...
      "recommendation": "Run spec-first init --claude. Do not edit generated runtime assets directly."
    }
  ]
}
```
