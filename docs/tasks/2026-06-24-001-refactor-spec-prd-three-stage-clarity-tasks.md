...
```json
{
  ...
  "tasks": [
    ...
    },
    {
      ...
      "done_signal": "User docs explain the Product Expert Lens -> Requirements Grill -> Standard PRD Write-In -> Readiness Lens flow, CHANGELOG records all source changes, fresh-source eval is passed or honestly not-run with reason — and when not-run, the delivery must explicitly declare 'product-judgment capability is unverified, naming-only residual risk remains' as a review_gate-blocking note (not a soft record that auto-passes), because fresh-source eval is the only semantic gate and `$spec-prd` default-inline + frequent missing dispatch makes not-run the most likely path; the user docs / changelog record the Downstream Consumption Contract as a documented handoff boundary (closeout three-items are same-session handoff; cross-session downstream infers readiness from persistent PRD sections; the spec-plan Planning Recheck re-confirm dependency is an out-of-scope cross-workflow note, not assumed fixed); and any runtime mirror changes come only from `spec-first init`.",
      "parallelizable": false,
