...
5. **源码定向闭环**：生成 task pack 前做 bounded source orientation，按 `CRG -> Serena/LSP -> direct repo reads` 自动降级，只为 `files`、`context_refs`、`risk_note` 和 wave 判断提供候选证据，不替代 plan，也不替 LLM 做语义拆分。

这五点都是“脚本准备事实、LLM 做判断”的延伸：脚本只证明 identity / freshness / structure 是否可信，CRG/Serena/direct reads 只提供候选源码证据，LLM 仍判断是否需要 task pack、怎么拆任务、何时回到 plan。
...
| bounded source orientation 降级链不明确 | P2 | 复杂 plan 需要源码候选证据提升 task 边界，但 CRG 不可用时容易退化成完整读源码或纯 plan 猜测 | 建立 `CRG -> Serena/LSP -> direct repo reads` 的自动降级链，只产出候选证据和 limitations |
...
- CRG、Serena/LSP 和 direct repo reads 负责提供 bounded source orientation 的候选证据
...
### 8.5.1 Bounded Source Orientation 的自动降级链
...
source plan
  -> CRG ready?
      yes: workflow-context + locate + explain + optional path/impact
      no:
        -> Serena MCP available?
            yes: activate project + LSP quick index + symbol/file queries
            no:
              -> targeted direct repo reads
  -> LLM prunes evidence
...
- CRG 优先：用于 query-first 的代码事实候选，适合发现 entrypoints、依赖边、shared surfaces、影响面和测试入口。
- Serena/LSP 降级：当 CRG missing、degraded、stale 或 parser coverage 不足时，自动激活目标项目并使用 LSP 快速索引做 symbol/file orientation，例如 overview、symbol lookup、references 和局部 pattern search。
- direct repo reads 兜底：当 CRG 与 Serena 都不可用，或语言/文件类型无法被索引时，只读取 plan 指向的候选文件、目录索引、测试入口和局部实现模式。
...
- 把 CRG 或 Serena 返回的第一个候选当成最终修改点。
- 因为 LSP references 存在就自动扩大 scope。
- 在 task pack 阶段写逐步 patch 策略。
- 用源码发现的新需求改写 plan 范围；发现 plan 缺 scope / contract / acceptance 时，应返回 `return-to-plan` 或生成 `draft-only`。

`orientation_evidence` 可以记录 provider 和 limitation，但不应成为 executable handoff 的硬门槛。graph 或 LSP 不可用时，正确行为是记录 `provider: direct-repo-reads` 与 limitation；只有当 plan 本身也不足以确定 task 边界时，才停止回到 plan。
...
orientation:
  provider: crg | serena-lsp | direct-repo-reads | mixed | skipped
...
- bounded source orientation 产生的候选源码证据：CRG、Serena/LSP 或 targeted direct repo reads 的文件、符号、测试入口和 limitation
...
- 从 readiness ledger 或工具调用结果暴露 Serena/LSP 是否可作为降级 orientation provider
- 返回机器可读 validation result，供 `spec-write-tasks`、`spec-work` 和 CRG hook 共用

### 10.2 LLM 负责

- 判断要不要生成 tasks
- 按 `CRG -> Serena/LSP -> direct repo reads` 降级链选择和剪裁 bounded source orientation 证据
- 合并或拆分 task
- 识别任务波次
- 标记 parallelizable
- 压缩 task 语言
...
**最佳方案：采用 CRG-first、Serena/LSP-second、direct-read-last 的自动降级链。**
...
2. 若 CRG ready，优先使用 `workflow-context`、`locate`、`explain`，必要时补 `path` / `impact`，只收集候选文件、符号、测试入口和共享风险。
3. 若 CRG unavailable / degraded / stale，且 Serena MCP 可用，自动 `activate_project(<repo>)`，等待 LSP 快速索引后使用 symbol overview、symbol lookup、references 和局部 pattern search 做降级定位。
4. 若 CRG 与 Serena 都不可用，退回 targeted direct repo reads，只读 plan 已指向或少量 query 发现的候选路径。
...
- Serena/LSP 是轻量快速降级，适合 CRG 尚未 build、parser coverage 不足或 graph stale 时避免全仓搜索。
- direct repo reads 兜底能保证工具缺失时仍可工作，不把 task pack 生成绑定到某个 MCP。
- 三层都只提升输入质量，不替代 LLM 的任务拆分和范围判断。

验收信号：

- CRG ready 时，task pack 的 `context_refs` 和 `files` 能引用 CRG 辅助发现的候选代码面。
- CRG 不可用但 Serena 可用时，仍能生成带 LSP limitation 的 bounded orientation evidence。
- 两者都不可用时，输出明确 `provider: direct-repo-reads`，不会假装有 graph / LSP 证据。
...
- `spec-write-tasks` 生成时支持 bounded source orientation 自动降级链：`CRG -> Serena/LSP -> direct repo reads`，并记录 provider 与 limitation
...
| CRG 不可用时源码定向退化成猜测 | `files`、`context_refs` 和 wave 判断质量下降 | 自动降级到 Serena/LSP 快速索引，再降级到 targeted direct repo reads，并记录 provider limitation |
...
- bounded source orientation 的 `CRG -> Serena/LSP -> direct repo reads` 自动降级链
