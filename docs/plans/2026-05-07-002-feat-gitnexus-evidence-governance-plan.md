...
`docs/plans/.../execution-log` 必须先记录以下命令的实际输出，再进入 D1 路径选择。当前 source-of-truth pin 为 `gitnexus@1.6.3`（npm `dist-tags.latest`，对齐 LOOP-045 stable 决策）：

```bash
npx -y gitnexus@1.6.3 analyze --help
npx -y gitnexus@1.6.3 --version
```
...
- 如 stable 1.6.3 不支持，再针对 RC 通道最新版（参考 npm `dist-tags.rc`）做对照 spike，便于 D1 路径 B 决策。
...
| 路径 | 触发条件 | 实施要点 |
|---|---|---|
| **A. spec-first 接管 + provider 不写入** | SP1 证实 stable `1.6.3` 已支持 `--skip-agents-md` 或等效 flag | 引入新 managed marker `<!-- spec-first:gitnexus-prose:start -->` / `:end -->`；由 `src/cli/instruction-bootstrap.js` 渲染；`runtime-tools-index.js` 把识别规则从"外部 provider block 保留"改为"managed slice 重生成"；GitNexus bootstrap argv 加 `--skip-agents-md`。 |
| **B. 显式切到 RC 通道后再走路径 A** | SP1 证实 stable `1.6.3` 不支持但 RC 通道（npm `dist-tags.rc`，当前 `1.6.4-rc.82+`）已支持 | 先做显式决策：是否接受 RC 通道维护成本（每次升 RC 需回归测试；历史 SIGSEGV 在 LOOP-045 中已出现）；如同意，通过 `mcp-tools.json` provider registry 把 pin 从 `1.6.3` 切到选定 RC 版本，**在 CHANGELOG 与 plan execution log 显式声明通道反转的理由**，再按路径 A 落地；不接受则降级到路径 C。 |
...
如 stable `1.6.3` 不支持 `--skip-agents-md`，对照 spike 当前 `dist-tags.rc`（参考 `npm view gitnexus dist-tags`）：

```bash
npx -y gitnexus@<rc-tag> analyze --help
```

输出抄录到 plan execution log；驱动 D1 路径选择。**禁止默认升级到 RC 通道**：路径 B 必须显式声明通道反转决策（推翻 LOOP-045 stable 锁定）。
...
| 风险 | 缓解 |
|---|---|
| SP1 显示 stable `1.6.3` 不支持 `--skip-agents-md` | 先对照 spike RC 通道；若 RC 支持，路径 B 必须显式记录"推翻 LOOP-045 stable 锁定"的决策与回归测试范围；若 RC 也不支持，降级到路径 C（二步重建）。三条路径已预定义，不阻塞推进。 |
| 路径 B 重新进入 RC 通道带来 SIGSEGV / FTS 等历史回归 | LOOP-045 (`8be33a5d`) 与 v1.6.0 (`2026-04-28`) 历史已记录 RC 通道 SIGSEGV；路径 B 决策必须附带 `gitnexus@<rc-tag> analyze` 在 spec-first 自身仓库的回归 spike 输出，并把 RC pin 维护人显式写入 plan execution log。 |
