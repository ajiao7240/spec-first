...
| **B. 显式切到 RC 通道后再走路径 A** | 当前已选路径：spec-first `v1.8.0` 把 source-of-truth pin 显式设为 `gitnexus@1.6.4-rc.100` | 接受 RC 通道维护成本；每次升 RC 必须回归测试。`mcp-tools.json` provider registry 是唯一 pin 来源；CHANGELOG 与 plan execution log 必须显式声明通道反转理由和生效 pin。 |
...
输出抄录到 plan execution log；驱动 D1 路径选择。**禁止静默升级或漂移 RC 通道**：当前路径 B 以 `gitnexus@1.6.4-rc.100` 为准，任何未来 RC 变更都必须显式声明通道、理由、回归范围和生效 pin。
...
| 风险 | 缓解 |
|---|---|
| 当前 `gitnexus@1.6.4-rc.100` 不支持 `--skip-agents-md` 或回归不通过 | 降级到路径 C（二步重建），并在 plan execution log 中记录失败命令、exit code、stderr 摘要和是否继续保留 RC pin。 |
...
**通道治理状态**：当前路径 B 锁定在 `gitnexus@1.6.4-rc.100`。下一次 pin 变更（无论 RC 升降级或回归 stable）必须重新跑 SP1 spike，把输出抄录到本日志，并在 CHANGELOG 显式声明通道、理由、生效 pin、回归范围。
