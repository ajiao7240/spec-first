...
把 `spec-graph-bootstrap` 阶段 2 生成的 Stage-0 产物接入 `spec-plan`、`spec-work`、`spec-review` 三个 workflow，以**最小侵入、最稳降级**为核心约束。
...
| `spec-review` | `# Code Review` 主标题之后、`## When to Use` 之前 | scope 确定前读取，供 reviewer 参考 |
...
**已知降级损失**：`context-packs/review-change.md` 对 spec-review 很重要，但不在 Level 2 固定集中，因为它是 review 语义专属文件，不符合"跨 stage 普遍有用"的设计约束。当 yaml 不可用时，spec-review 会缺少变更风险上下文，这是已知的、可接受的 v1 降级损失。v2 可考虑引入 stage 差异化的 Level 2 降级集合。
...
### 4.3 修改 `skills/spec-review/SKILL.md`
