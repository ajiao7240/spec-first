...
- Suggested reads 必须经过 sensitive / generated / self-output 过滤；不得建议读取 `.env*`、credential/secret/private-key 文件、`.spec-first/**`、`.claude/**`、`.codex/**`、`docs/contexts/**` 或其他生成索引输出。
...
- 为 graph rebuild 增加 shrink guard 和 last-known-good 指针。
...
- Modify: `docs/contracts/crg-cli-v1.schema.json`
- Test: `tests/contracts/crg-cli-v1.test.js`
- Test: `tests/e2e/crg-all-commands.sh`
...
- 将 `change-surface`、verification summary 中仍有价值的逻辑迁移到 `src/crg/workflow-context` 或 `src/crg/changes`。
...
### Unit 8: Add input detection, freshness, and shrink guard

**Goal:** 在 CRG build 前建立 Graphify-style input detection 与图健康保护。
...
- graph rebuild 后比较 node/edge count 与 last-known-good。
- 异常缩小进入 degraded，不静默 promote。

**Test scenarios:**

- `.spec-first/graph/` 与显式 report projection 不会被索引。
- sensitive 文件被跳过并只记录 count。
- 无显式 prune 时 node_count 大幅下降不会覆盖 last-known-good。
