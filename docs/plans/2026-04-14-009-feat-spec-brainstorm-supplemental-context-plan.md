...
把 `spec-brainstorm` 从当前的 repo-context-only 工作流，升级为受当前产品边界约束的 `Supplemental Context` 版本：保留上游 `ce-brainstorm` 中与方法论质量直接相关、且不依赖 Slack 的能力，同时新增 `Local Docs`、`Feishu Chat`、`Feishu Doc`、`GitHub URL`、`Web URL`、`Docs URL` 的路由与 adapter contract，并补齐最小 contract / smoke 回归。
...
- `Feishu`、`GitHub`、`Web/Docs URL`、本地文档共同组成当前增强方向
...
- R7. `Feishu` adapter 的运行时依赖必须与 `spec-mcp-setup` 飞书 MCP 计划解耦：本计划负责 brainstorm adapter contract，本地是否具备 MCP 由独立准备工作决定。
...
- 不包含 `spec-mcp-setup` 对飞书 MCP 的检测与安装实现，该部分由既有计划单独推进
...
- `docs/plans/2026-04-14-008-feat-feishu-mcp-setup-integration-plan.md`
  飞书 MCP 的 host-level 准备工作已经被独立建模为 `spec-mcp-setup` 任务。因此 `feishu-chat-researcher` / `feishu-doc-reader` 在本计划中应被视为上层消费方，而不是重复承接 host setup 责任。

### External References

- 本次未追加外部在线研究。
- 原因：当前工作主要是 repo-owned workflow contract 设计与 asset 组织，关键外部依赖信息已经在 `docs/业界分析/14...` 与飞书 MCP 现有计划中被沉淀，不需要再用开放式研究替代本仓事实。
...
- **外部 supplemental context 必须沿用上游 Slack context 的 opt-in 精神，但将触发条件推广为 source-driven。**
  - 理由：上游 `ce-brainstorm` 对 Slack 明确采用“opt-in、never auto-dispatch”。当前产品虽然移除了 Slack，但 GitHub / Web / Docs / Feishu doc/chat 同样属于 repo 外部上下文，若按 topic 自动抓取会扩大权限、成本和行为不确定性。因此这里固定为：显式 source 或显式请求才 dispatch；否则只提示可用能力，不做自动外部读取。

- **`feishu-doc-reader` 支持 MCP-first / browser-fallback，`feishu-chat-researcher` 只做 MCP-first。**
  - 理由：飞书文档可以在授权后通过浏览器读取做降级，而聊天检索的结构化搜索、时间窗、讨论聚合更依赖 MCP/API，不适合 browser-only 充当主实现。
...
- **`feishu-doc-reader` 的 browser fallback 是否要限制为“用户显式给出链接”场景？**
  - 原因：这属于执行时权限/可读性策略细化，可在实现时根据可用 MCP 与登录态细节再定。
...
User request / origin topic
  ...
  -> Existing and Supplemental Context Scan
       ...
       -> explicit feishu chat ask? -> feishu-chat-researcher
       -> explicit feishu doc link? -> feishu-doc-reader
       ...
       -> no stable source?
            ...
            -> no  -> 向用户说明当前环境不支持 supplemental skill discovery；
                      列出仍可使用的 source 类型（local docs / Feishu / GitHub / Web URL / Docs URL）；
                      继续执行 brainstorm，不阻塞
...
| Feishu chat | `feishu-chat-researcher` | none | MCP/API-first |
| Feishu doc | `feishu-doc-reader` | `agent-browser` | browser fallback 仅作读取降级 |
...
- [x] **Unit 4: 落地 Feishu adapters，并与 host-level MCP 准备工作解耦**

**Goal:** 让 `spec-brainstorm` 具备可消费的飞书聊天/文档 adapter contract，同时不把 `spec-mcp-setup` 的职责重新塞回 brainstorm。
...
**Files:**
- Add: `agents/research/feishu-chat-researcher.md`
- Add: `agents/research/feishu-doc-reader.md`
- Modify: `skills/spec-brainstorm/SKILL.md`
- Test: `tests/unit/spec-brainstorm-contracts.test.js`
- Test: `tests/smoke/cli.sh`
...
**Test scenarios:**
- Happy path: 主 skill 和 agent 文本都声明 Feishu Chat / Feishu Doc 的 source role 与 digest 输出
- Edge case: 文档 reader 明确区分 MCP-first 与 browser fallback，不把 browser 写成默认主路径
- Error path: 飞书 MCP 不可用时，contract 要求显式返回“工具不可用/权限不足”，不伪造读取结果
- Integration: runtime 安装包含两个 Feishu agents
...
| Feishu adapters 落地后，宿主环境还没有飞书 MCP | 明确依赖独立的 `spec-mcp-setup` 飞书计划；文案允许 browser fallback 或显式降级 |
...
- 根据仓库治理规则，本次也应同步更新 `docs/08-版本更新/README.md`，内容应列出新增的 supplemental source 类型（Feishu Chat / Feishu Doc / GitHub URL / Web URL / Docs URL / Local Docs）作为用户可见的 workflow 增强。
- 计划期不要求修改 `docs/业界分析/13...` / `14...`，除非实现中发现 source-of-truth 与代码结构再次产生新的阻断性冲突。
- 若 `spec-mcp-setup` 的飞书 MCP 计划尚未落地，不阻断本次 asset 开发，但会影响 `feishu-*` adapters 的真实可运行性验收；需要在实施时作为前置依赖记录。
...
- Supporting design: [docs/业界分析/14.spec-brainstorm-增强版-飞书MCP接入技术方案-2026-04-14.md](docs/业界分析/14.spec-brainstorm-增强版-飞书MCP接入技术方案-2026-04-14.md)
