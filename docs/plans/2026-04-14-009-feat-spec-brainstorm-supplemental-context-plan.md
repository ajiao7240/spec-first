...
- **保持 source skill 使用内部命名，不为 runtime 名称改 source frontmatter。**
  - 理由：`spec-plan`、`spec-work`、`spec-code-review` 等 command-backing workflow 的 source 都使用内部命名；Codex runtime 会在安装时改成 `spec-brainstorm`。如果直接把 source 改成 public name，容易与现有 smoke / transform 约定冲突。
...
- **Origin document:** [业界学习/04-能力映射/skill-映射方案-2026-04-15/13.spec-brainstorm-追平后增强版-外部上下文架构方案-2026-04-14.md](/Users/kuang/xiaobu/spec-first-doc/业界学习/04-能力映射/skill-映射方案-2026-04-15/13.spec-brainstorm-追平后增强版-外部上下文架构方案-2026-04-14.md)
