...
> 2026-04-20 更新：本计划已吸收同日补强方案中的有效要求。后续开发以本文件为唯一主计划文档。
...
本计划的核心目标不是引入新的 orchestration 层，而是让 runtime 产出更诚实、更统一、更可解释的决策输入，让 `spec-plan`、`spec-work`、`spec-review` 在不同仓库形态下都能读到正确上下文边界，并让 `stage0-context` 成为三类场景下统一的运行时入口。
