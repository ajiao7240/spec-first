...
- Graphify：first generation 为 Graphify provider-native project-graph generation,`steady_state.refresh_mode=cli-mcp-hook-on-demand`、`hook_default=false`。**执行前先核实 Graphify 是否有可被 setup 非交互定向到指定 workspace 的 generation 命令(见 B2)**:有则 scope 为 run-scoped workspace 并写明 `requirement_workspace_path` / `artifact_root` 的 repo-relative 解析规则;无则 first_generation 退化为 install-only + next_action,不在 setup 代跑生成。
