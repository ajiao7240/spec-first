...
- 扩展 `skills/spec-code-review/SKILL.md`
...
5. `workspace-registry.json` 的定位不能只看当前目录；必须从 `target || cwd` 开始按祖先目录逐级向上探测，首个命中的候选目录视为 `workspaceRoot`
6. 若某个候选目录存在 `workspace-registry.json`，但 JSON 损坏、关键字段缺失或 `schema_version` 不支持，必须锁定该目录进入 workspace degrade 路径，禁止继续向上搜索或直接掉回单仓 fallback
...
1. `workspace-registry.json` 与 `workspace-routing.json` 都必须带 `schema_version`
2. loader 必须在读取后做 schema/关键字段校验，而不是只判断文件是否存在
3. schema 不兼容时返回 `workspace_registry_schema_unsupported` 或等价 `workspace_routing_schema_unsupported`
...
同时补一条 freshness 语义，避免实现时各模块自己解释：

1. workspace mode 需要聚合 workspace overview 与 selected child contexts 的 `freshness_status`
2. 若任一 selected context 为 `stale` 且没有更强的 contract 失败，保留当前 level，并将 `fallback_reason` 置为 `freshness_stale`
3. `freshness_stale` 是正交信号，不改变 workspace / single-repo 的入口决策
...
- `workspace-registry.json` / `workspace-routing.json` 的 `schema_version` 不兼容
- nested repo 去重
- 识别 child repo 后停止递归
...
- 从非 git 的嵌套子目录启动时，ancestor lookup 能正确命中 `workspaceRoot`
- registry 损坏时返回 workspace degrade，而不是静默回退为 directory fallback
...
- workspace registry/routing 损坏或 schema 不兼容时，degrade 行为一致
- `skills/spec-plan/SKILL.md`、`skills/spec-work/SKILL.md`、`skills/spec-code-review/SKILL.md` 的 Stage-0 文案与真实 loader 行为一致
