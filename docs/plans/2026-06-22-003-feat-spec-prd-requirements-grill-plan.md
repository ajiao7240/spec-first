...
深度集成的含义不是“在 PRD 写完后再审一遍”，而是在需求文档阶段完成对后续研发最有影响的问题确认：术语是否与既有 glossary/代码一致、用户所说的系统行为是否与当前代码一致、场景是否覆盖入口/状态/异常/权限、每个 owner answer 是否带推荐答案和后果、每个决议是否落回 PRD-local section。这样后续 `spec-plan`、task pack 和 `spec-work` 消费的是质量更高的需求输入，而不是把 WHAT 缺口延迟到研发阶段。
...
- R27. `grill-with-docs` 的 `CONTEXT.md` 与 ADR 持久化模型必须改写为 PRD-local persistence：术语决议优先写入 `Glossary`，避免词可写入 `_Avoid_`/说明性 prose；硬决策、取舍和后果写入 `Decision Notes`、`Evidence And Assumptions` 或 `Scope Boundaries`。只有当决策 hard-to-reverse、surprising without context 且 real trade-off 三条件同时满足时，才把项目级 ADR 或 glossary promotion 作为 follow-up suggestion，不作为本 PRD 默认产物。
...
- KTD18. **Deep integration means method parity, not artifact parity.** `spec-prd` should preserve `grill-with-docs`' questioning rigor and source/code contradiction checks, but translate its `CONTEXT.md`/ADR updates into PRD-local `Glossary`, `Decision Notes`, `Evidence And Assumptions`, `Scope Boundaries` and follow-up suggestions.
