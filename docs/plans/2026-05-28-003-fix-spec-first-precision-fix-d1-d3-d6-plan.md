...
- **mcp-setup final-response 输出层(runtime 层提示)**:当 `quarantined_paths.length > 0` 时,在 final response 末尾输出固定一行
  ...
  这是 runtime 输出而非 SKILL.md prose,让 LLM 在调用 mcp-setup 时即看到 artifact 存在,排除"LLM 不知道文件存在"的 Tier 1 验证失败模式;参考 Anthropic Skills 的"按需告知" runtime 模式
...
本 plan 不改 SKILL.md prose。Tier 1 完成后用轻量 learning 观察 LLM 是否能在不被预先 instructed 的情况下自主消费 additive 字段;该观察是后续是否升级 fingerprint/schema 的决策输入,**不是本 PR 的 release gate**,**不预设 pass/fail 阈值**——参考 Anthropic SDK `ToolChoiceAuto` "let the model decide" 哲学,让真实数据说话,而非用预设数字劫持结论。

- **样本范围**:在 spec-first 本仓 + kaz-mvp 各至少 5 次真实 workflow 调用(plan / work / debug / code-review 任意组合)
- **记录维度(每次调用记录一行)**:
  - 调用了哪个 workflow,场景特征(single-repo / multi-repo / clean / dirty / 有无 quarantine / 有无 label conflict)
  - LLM 是否主动引用以下任一字段:`quarantined_paths[]` / `repo_label_resolution.conflict` / `repo_label_resolution.candidates[]`(仅 conflict=true 时存在) / `dirty_paths_sample[]`
  - 引用后的决策动作(走 clean 入口?限定 bounded read?忽略字段?)
  - 决策质量主观评分(1-5,基于结果是否解决了原问题)
- **反例同样有价值**:LLM **忽略**字段时记录"为什么忽略"(字段不可见?LLM 不理解?字段无信号价值?)——比单纯统计"引用率"更有信息量
- **决策时机**:观察期末(1-2 个月后)读这份记录,基于实际数据判断:
  - LLM 自主消费良好 → fingerprint 抽象继续延后,可能根本不需要
  - LLM 频繁忽略某字段 → 评估根因,加最小 SKILL.md prose 还是改字段设计
  - LLM 引用了但决策质量低 → 字段信号不够,可能需要更结构化(此时再考虑原 plan 2026-05-28-002 的 fingerprint 方向)
