...
- 其他 5 项(D2 / D4 / D5 / D7 / D8)要么需要更大的抽象设计,要么 ROI 偏低,延后到 Tier 1 验证后再决策
...
- **D6** `dirty_paths_breakdown` 只是 `setup_owned_count / non_graph_metadata_count / graph_affecting_count` 三计数,LLM 拿不到具体路径,无法做 bounded direct read 精准降级

---

## Requirements

- R1. **D1 修复**:multi-repo workspace 模式下,`mcp-setup` 在识别到顶层 stale 产物时写入 `.spec-first/workspace/parent-artifact-quarantine.json`(新 schema `parent-artifact-quarantine.v1`),并在 `mcp-setup-summary.json` 增加 `parent_workspace_pollution_count`
- R2. **D1 配套**:`spec-first clean --workspace-orphans` read-only 子命令,基于 quarantine 文件列出待清理路径,**不自动删除**(实际删除留待后续 PR)
- R3. **D3 修复**:`provider-status.json.gitnexus` 增加 `repo_label_resolution` 字段(三源 candidates + selected + selected_source + conflict bool + 冲突时 next_action)
- R4. **D6 修复**:`graph-facts.json` 增加 `dirty_paths_sample[]`(上限 ≤30 条,优先 `graph_affecting=true`,每条含 `path` + `graph_affecting` bool + `build_module`;`build_module` 字段在本 plan **预留但固定为 null**——schema 提前占位,后续 PR 可在不改 schema 的前提下填值)
...
- `dirty_paths_sample[].build_module` 字段在本 plan **暂不填**(总是 null),原因:build_module 推导需要 build-target scan,后者已被 scope out

### Deferred to Follow-Up Work

- D2 / D4 / D5 / D7 / D8 修复:见原 plan `docs/plans/2026-05-28-002-feat-spec-first-scenario-adaptive-milestone-plan.md` 的 U6–U12,等 Tier 1 验证 1–2 月观察期后再决策是否进入实施
- `parent-artifact-quarantine.json` preview-first 实际删除升级:本 plan 完成后独立 PR
- `dirty_paths_sample[].build_module` 字段填值:依赖 build-target scan 实现,本 plan 不做
...
- `src/cli/commands/clean.js` — `spec-first clean` 命令入口,当前已有 `--claude` / `--codex` 子参数;新增 `--workspace-orphans` 参数走同一注册模式
...
- **`dirty_paths_sample[]` 上限 30**:基于 origin 文档 A T1.3 决议;超过 30 时按 `graph_affecting=true` 优先保留,其他截断;`build_module` 字段固定 null(留给后续 PR 填值)
...
- **Q: U1 quarantine 写入时机是 verify-tools 还是 install-mcp?** verify-tools 阶段。理由:install-mcp 仍可能修改顶层产物,quarantine 应在所有 setup 写入完成后产出,这是 verify-tools 的语义边界
- **Q: D6 dirty_paths_sample 是否需要 build_module 字段?** 本 plan 暂不实现(总是 null),延后到 build-target scan 出现
- **Q: clean --workspace-orphans 是否本期就支持实际删除?** 不,只 read-only 列举。实际删除是 destructive 行为,UX 设计需要更慎重(preview + confirm),独立 PR
- **Q: 是否同时升级 SKILL.md 让 LLM 知道这些新字段?** 不。让 LLM 自主消费 additive 字段是本 Tier 1 验证的一部分:如果 LLM 在不改 prose 的情况下也能正确使用这 3 个字段,说明 fingerprint 抽象可能根本不需要

### Deferred to Implementation

- 跨平台 stat 行为差异在 macOS / Linux / Windows / WSL / NFS 下的具体表现:U1 实施时跨平台测试
- `mcp-setup-summary.json.parent_workspace_pollution_count` 是 quarantine_paths 数组长度,还是按 reason_code 分组计数:U1 实施时按最简实现
- `repo_label_resolution.next_action` 文案:U2 实施时按 origin 文档 A T1.4 模板写
...
- Modify: `src/cli/commands/clean.js`(新增 `--workspace-orphans` 参数,read-only 列举)
...
- Quarantine 文件 schema(`parent-artifact-quarantine.v1`):
  ...
  next_action: "Confirm before deletion; spec-first clean --workspace-orphans is the dedicated cleanup command (read-only in current version)."
  ```
- 检测路径(扫描列表固定):
...
- `mcp-setup-summary.json.parent_workspace_pollution_count = quarantined_paths.length`(最简实现)
- `clean --workspace-orphans`:
  - 无 `parent-artifact-quarantine.json` 时提示 "先跑 mcp-setup"
  - 有 quarantine 文件时打印路径列表 + 每条 reason_code,固定输出 "Use --confirm to actually delete (not yet implemented in this release)"
  - 不实际删除(本期 read-only)
...
- `src/cli/commands/clean.js` 现有 `--claude` / `--codex` 参数注册模式
...
- CLI: `spec-first clean --workspace-orphans` 有 quarantine 文件 → 打印路径 + reason_code + "not yet implemented" 提示
- Cross-platform: PowerShell 产出的 quarantined_paths[].path 无反斜杠
- additive: 旧 mcp-setup-summary.json reader 看到新增 `parent_workspace_pollution_count` 字段不报错(schema additive)
...
- `provider-status.json.gitnexus.repo_label_resolution`:
  ...
  repo_label_resolution: {
    ...
    conflict: <bool>                          // candidates 中有 ≥2 个非 null 值不一致 → true
  }
  ```
...
**Test scenarios:**
- Happy path: `.gitnexus/meta.json.remoteUrl=https://gitlab.example.com/group/web.git` + `git remote get-url origin=https://gitlab.example.com/group/web.git` + dir basename=`web` → conflict=false,selected_source=`gitnexus_meta_remote_url_basename`
- Error path: `.gitnexus/meta.json.remoteUrl=...kaz-app.git` + git remote `kaz-mvp.git` + dir=`kaz-mvp` → conflict=true,candidates 含 `kaz-app` 与 `kaz-mvp` 两个不同值
- Edge case: `.gitnexus/meta.json` 不存在 + git remote=`web.git` + dir=`web` → conflict=false,candidates[0].value=null
- Edge case: 无 `.gitnexus/` + 无 git remote → 仅 directory_basename 可用,conflict=false
  ...
  dirty_paths_sample: [
    {
      ...
      build_module: null    // 本 plan 固定 null,留给后续 PR
    },
    ...
  ],
...
- 现有 `dirty_paths_breakdown` 三计数字段保留(additive,不替换)
- `build_module` 字段:本 plan 固定 null。理由:推导 build_module 需要 build-target scan(已 scope out)
...
- **Interaction graph:** mcp-setup verify-tools → 写 quarantine.json + summary.parent_workspace_pollution_count → `spec-first clean --workspace-orphans` 读 quarantine 列举;graph-bootstrap bootstrap-providers → 写 provider-status.repo_label_resolution + graph-facts.dirty_paths_sample
...
| `dirty_paths_sample[]` 在大量 dirty 文件时性能 | Low | Low | 上限 30 + 稳定排序;`git status --porcelain` 已是常用命令 |
| LLM 不消费新字段(本 plan 不改 SKILL.md prose) | Medium | Low | Tier 1 验证一部分;具体观察标准见下方 Documentation/Operational Notes 节 |
...
- **docs/05-用户手册/** 不新增章节(用户感知通过 CHANGELOG + artifact 直接可见)
- **docs/contracts/** 不新增 schema 文档(三个字段都是 additive,在 source-of-truth 注释中描述)
- **docs/solutions/** Tier 1 落地后追加一条 learning,记录"为什么没做完整 fingerprint 方案"以及未来何种证据出现时考虑升级
...
### Tier 1 验证标准(1–2 月观察期)

本 plan 不改 SKILL.md prose,验证 LLM 是否能在不被预先 instructed 的情况下自主消费 additive 字段。具体观察方法:

- **样本范围**:在 spec-first 本仓 + kaz-mvp 各至少 5 次真实 workflow 调用(plan / work / debug / code-review 任意组合)
- **记录维度**:LLM 是否主动引用以下任一字段做决策——
  - `parent-artifact-quarantine.json.quarantined_paths[]`
  - `provider-status.json.gitnexus.repo_label_resolution.conflict`
  - `graph-facts.json.dirty_paths_sample[]`
- **成功标准**:**引用率 ≥60%**(10 次调用中至少 6 次主动消费新字段)→ 视为 LLM 自主消费成功,fingerprint 抽象可继续延后
- **失败处理**:引用率 < 60% → 评估根因(字段不可见 / LLM 不理解 / 字段无价值),决定是否需要在 SKILL.md 增加最小 prose 引导,或升级到更结构化方案
- **观察记录**:用 `docs/solutions/` 追加一条 learning 记录观察结果,作为后续是否进入原 plan(2026-05-28-002)的实证依据
...
- **完整 roadmap 备查:** `docs/plans/2026-05-28-002-feat-spec-first-scenario-adaptive-milestone-plan.md`(原 milestone plan,Tier 1 验证后决策是否继续推进)
