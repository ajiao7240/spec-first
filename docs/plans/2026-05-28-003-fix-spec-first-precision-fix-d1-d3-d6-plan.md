...
- R3. **D3 修复**:`provider-status.json.gitnexus` 增加 `repo_label_resolution` 字段(三源 candidates + selected + selected_source + conflict bool);不写 `next_action`,由 LLM 基于 facts 判断修复建议
...
本 plan 不改 SKILL.md prose。Tier 1 完成后用轻量 learning 观察 LLM 是否能在不被预先 instructed 的情况下自主消费 additive 字段;该观察是后续是否升级 fingerprint/schema 的决策输入,不是本 PR 的 release gate。
...
- **观察方式**:人工记录每次 workflow 是否主动引用上述字段、引用发生在哪个决策点、字段是否改变了降级/修复建议
- **判断口径**:如果多次真实 workflow 中字段反复可见但未被引用,再评估字段不可见 / LLM 不理解 / 字段价值不足 / 是否需要最小 SKILL.md prose 引导
- **观察记录**:用 `docs/solutions/` 追加一条 learning 记录观察结果,作为后续是否进入原 plan(2026-05-28-002)的实证依据;除非后续另设采集 artifact,不声明精确引用率指标
