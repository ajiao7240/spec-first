...
- R8. 扩展既有 Graphify 只读 query probe（`skills/spec-mcp-setup/scripts/install-helpers.sh:1058-1070` 的 `probe_graphify_query_if_available` 与 PowerShell 孪生 `Invoke-GraphifyQueryProbe`），而非新建机制：(a) verify-only/refresh 路径上 gated 执行 probe，修复当前每次 fact 刷新把 `lifecycle.query_verified` 静默回退为 false 的缺陷；(b) renderer 在 installed && artifact_exists && query_verified=false 时输出 Graphify advisory next_action（落既有 `next_actions` 字段，镜像 CodeGraph 在 `provider-readiness-renderer.cjs:481-483` 的先例）；(c) 增加确定性陈旧事实：比对图谱 artifact 内嵌 `built_at_commit` 与 `git rev-parse HEAD`，不匹配时 readiness_status 按确定性证据降为 `stale` 并给出 advisory next_action；(d) setup-side Graphify CLI 解析前置自愈：`spec-mcp-setup` helper 在 install、first generation、verify-only probe、hook 检查前统一解析 `graphify` 命令，复用 renderer 已有 `~/.local/bin` provider-standard path 口径；若 CLI 只在标准路径可执行但不在 `PATH`，在当前 helper 进程内 prepend 该目录或使用绝对命令路径继续执行，不修改用户 shell profile，不新增 reason_code。probe 保持只读、`run_with_timeout` 有界、无条件 return 0、不阻塞 setup 与 doctor；verify-only 高频路径上的 probe 须使用独立短超时（默认 ≤30 秒，可经 env 覆盖），不得沿用 install 段的 900 秒默认值（机制见 U6(a)：调用点 env 覆盖，不改函数签名）。
- R9. 脚本只产机械事实（退出码、输出非空、commit 比对结果）；「召回质量是否足够」的语义判断留给 LLM/用户，不实现质量评分脚本（Scripts prepare, LLM decides）。
- R10. focused tests 锁住核心边界：新增 `tests/unit/project-graph-consumption-contracts.test.js`（pin 合同承重句——含 readiness_status 消费映射句与三层接力链/禁止跳层抬升句——与消费方 SKILL 引用循环）；`tests/unit/capability-aware-provider-contracts.test.js` 的 `WORKFLOW_SKILLS` 扩展为 7 个 skill 并新增合同引用断言；`tests/unit/ai-coding-harness-contracts.test.js` referenced-files 列表加入新合同；`tests/unit/mcp-setup.sh` 与 `tests/unit/mcp-setup-powershell-contracts.test.js` 增加 verify-only probe 断言；`tests/unit/dependency-readiness-baseline.test.js` 增加 refresh 持久性与 advisory next_action 行为用例。
- R11. 文档同步：`CHANGELOG.md`（user-visible）、`docs/contracts/ai-coding-harness.md` Evidence Harness map、`package.json` files 白名单加入新合同、父方案登记行；U5 确定修改 `skills/spec-work/references/shipping-workflow.md` closeout prose，并同步检查 `docs/05-用户手册/04-workflows-artifacts-map.md` 是否需要对应更新；`CLAUDE.md` 与 `AGENTS.md` 的 `## graphify` 指引小节须与合同对齐——该小节随会话注入（CLAUDE.md:305-313 / AGENTS.md:309 区域），当前包含 query-first 强制与「修改代码后运行 `graphify update .`」的运行期刷新指令，与合同的 candidate-only、readiness gate、「不运行期刷新图谱」非目标直接冲突，不对齐则单一真源在 host 注入层被击穿；对齐为 candidate-only 口径并移除运行期强制刷新句（host 入口文档可保留具名命令，R2 的 provider-neutral 约束只作用于 workflow SKILL prose）。
...
- A5. Graphify CLI 路径检测与 setup 自愈复用既有 helper/renderer 口径（`docs/contracts/provider-readiness.md:22-23` 已含 `~/.local/bin` 检测语义）：renderer 仍可输出 PATH next_action 给人工 shell 使用；install/probe/hook helper 则在当前进程内用 resolved command path 自愈并继续，不新增 `cli_not_on_path` 类 reason_code 词表，也不静默修改用户 shell profile。
...
- current_revision: `387abe4a`（分支 `leo-2026-06-03-ceupdate`）
...
- source_reads_completed:
  - `skills/spec-plan/SKILL.md:90-92`、`skills/spec-debug/SKILL.md:90-92`、`skills/spec-code-review/SKILL.md:93-95`：三个 `## Capability-Class Evidence Boundary` 段形状一致（advisory candidate、readiness_status/lifecycle 检查、fallback、provider_untrusted 记录、never-block），spec-code-review 变体额外指明记录进 Coverage。
  ...
  - `skills/spec-mcp-setup/scripts/install-helpers.sh:1058-1070,1135,1151,1169-1170,1264-1265`：`probe_graphify_query_if_available` 已存在——只读、`run_with_timeout` 有界、输出丢弃（`>/dev/null`，exit-code liveness）、无条件 return 0；仅在 MODE=install 的首次生成路径执行；`verify-tools.sh:534` 走 `--verify-only` 时 probe 不执行，导致 query_verified 从未设置的 env var 重算为 false；:1265 的 `PROVIDER_JSON=$(node provider-readiness-renderer.cjs --source helper ...)` 是每次运行（含 verify-only）都经过的渲染收口点。
  - `skills/spec-mcp-setup/scripts/install-helpers.ps1:1034-1051`：PowerShell 孪生 `Invoke-GraphifyQueryProbe` 存在，双宿主修改面确认。
  - `skills/spec-mcp-setup/scripts/provider-readiness-renderer.cjs:368,346-418,481-483`：env flag 进 `lifecycle.query_verified`；helperProviderEntries 是 advisory next_action 的零新脚本挂点；CodeGraph 已有 query_verified=false 时的 next_action 先例。
  ...
  - `docs/contracts/provider-readiness.md:3,12,16,21-23`：readiness 是 advisory setup fact 非 workflow truth；禁止语义信任字段进 readiness contract；lifecycle 位是 display/passthrough 不自行决定 workflow health——约束 P3 advisory 不得变 doctor 阻塞项。
  ...
  - 运行时探针（只读）：四种代表性 graphify 查询全部未召回 `CodeGraph技术方案.md`（图内 30 节点）与 `provider-readiness.md`（图内 3 节点），召回被 CLAUDE.md:305/AGENTS.md:309 的 graphify 小节自指占据；`graphify path` 报 ambiguous 且 no path——origin §8.2 确认且细化为「检索/种子选择失败而非索引缺失」，直接支撑 candidate-only 纪律。
- key_files_for_implementation:
  ...
  - `skills/spec-plan/SKILL.md`、`skills/spec-debug/SKILL.md`、`skills/spec-code-review/SKILL.md`
...
- **Requirements:** R5, R10
- **Dependencies:** U1
- **Files:** `skills/spec-plan/SKILL.md`（:90-92）、`skills/spec-debug/SKILL.md`（:90-92）、`skills/spec-code-review/SKILL.md`（:93-95）、`tests/unit/capability-aware-provider-contracts.test.js`、`tests/unit/project-graph-consumption-contracts.test.js`
- **Approach:** 各 boundary 段开头追加一句「Follows `docs/contracts/project-graph-consumption.md`」式引用（匹配 Scenario Capability 声明块的现行引用惯例），保留既有 workflow 专属句不动；唯一措辞修订：把既有「stale 与缺失/unknown 并列触发 fallback」短语对齐为合同 readiness_status 消费映射（见 Scope Boundaries 例外条款）。capability-aware 测试新增合同路径断言；新合同测试的引用循环加入这三个路径。
- **Test scenarios:** 三个 skill 均含合同路径；既有 token 断言全部不回退；负向断言（无 `graphify`/`codegraph_` 字面量）保持通过；stale 短语修订有直接测试锁——对被修订 skill 断言 `not.toContain('absent, stale, unknown, or unverified')`，防止旧措辞回流。
...
- **Files:**
  - `skills/spec-mcp-setup/scripts/install-helpers.sh`（verify-only 路径 :1264-1265 渲染收口点前 gated 调用 `probe_graphify_query_if_available`）
  - `skills/spec-mcp-setup/scripts/install-helpers.ps1`（`Invoke-GraphifyQueryProbe` 同步）
  - `skills/spec-mcp-setup/scripts/provider-readiness-renderer.cjs`（helperProviderEntries :346-418 增加 Graphify advisory next_action 分支，镜像 :481-483 CodeGraph 先例；built_at_commit 比对结果进 readiness_status/stale 与 next_actions）
  - `tests/unit/mcp-setup.sh`（:61-67 Graphify 断言簇）、`tests/unit/mcp-setup-powershell-contracts.test.js`（:121-135）、`tests/unit/dependency-readiness-baseline.test.js`（install 路径用例 :521-566 旁增 verify-only 用例）
- **Approach:** (a0) 在 sh/ps1 helper 先补 Graphify command resolver（可与 renderer `knownCommandCandidates()` 口径保持同构，优先 `PATH`，再查 `~/.local/bin/graphify[.cmd/.exe]`）。resolver 命中 PATH 外标准位置时，只在当前 helper 进程内自愈：sh 通过 `PATH="$(dirname "$resolved"):$PATH"` 或 `GRAPHIFY_CMD="$resolved"` 供后续调用；ps1 通过 `$script:graphifyCommand = <resolved>` 或等价 PATH prepend 供后续调用。install、project skill install、first generation、code-only fallback、hook、verify-only probe 全部使用同一 resolved command，不再各自调用裸 `graphify`/`command -v graphify`/`Test-CommandExists 'graphify'`。这回答当前实测限制：CLI 不在 PATH 时，mcp-setup 自身应感知并在本次 setup/verify 流程内修复，而不是只靠 renderer 输出人工 next_action。(a) verify-only 分支在渲染前调用既有 probe 函数，gate 条件=resolved CLI 可达（复用 A5）且 `graphify-out/graph.json` 存在；artifact 路径不依赖 install 段 resolver（verify-only 路径上不执行），从既有全局派生：`repo_root=${SPEC_FIRST_PROVIDER_REPO_ROOT:-$PWD}`、`artifact_abs=${repo_root}/${GRAPHIFY_ARTIFACT_ROOT_DEFAULT}`（install-helpers.sh:211 已存在该常量）。超时机制：probe 函数体内部硬编码 `run_with_timeout "$DEFAULT_STAGE_TIMEOUT_SECONDS"`（install-helpers.sh:1064），不改函数签名，由 verify-only 调用点以局部 env 覆盖实现解耦——`DEFAULT_STAGE_TIMEOUT_SECONDS="${SPEC_FIRST_PROBE_TIMEOUT_SECONDS:-30}" probe_graphify_query_if_available ...`；install 路径调用不变仍用 900s。ps1 侧 `Invoke-GraphifyQueryProbe`（install-helpers.ps1:1034-1051）当前完全没有超时包装，须补等价有界机制（如 `Wait-Job -Timeout`）达成双宿主 parity，超时同样取 `SPEC_FIRST_PROBE_TIMEOUT_SECONDS` 默认 30s。保持函数无条件 return 0。落地顺序：(a0) 必须先于或同于 (a)/(b) 交付，否则当前 `~/.local/bin/graphify` 不在 PATH 的环境会继续无法持久 query_verified；(a) 先于或与 (b) 同 commit 交付，避免 (b) 先上线导致每次 verify-only 都触发 advisory。(b) renderer 在 installed && artifact_exists && query_verified=false 时 push advisory next_action（提示重跑 setup 或 provider 自带 update 入口）；(b) 不写 readiness_status——它仍由既有逻辑与 (c) 的确定性陈旧判定决定，两者职责不重叠：(b) 只增 next_actions 条目，(c) 只依据 built_at_commit 比对结果置 `stale`；若 entry 已因其他原因处于更差档位（`degraded`/`unknown`），(c) 不得把它「提升」回 stale——stale 仅在原状态为 fresh/正常时降档写入。(c) 的提取与比对统一放在 renderer（Node 侧，已能读文件系统与执行 git）：用流式/首段读取做有界提取（读文件头部块匹配 `"built_at_commit"`，首匹配即停，不 `JSON.parse` 29.7MB 全量），sh/ps1 不再各自实现提取逻辑，消除双宿主孪生漂移面；与 `git rev-parse HEAD` 比对，不一致时置 readiness_status=`stale` 并附 next_action。陈旧 advisory 的文案须诚实表述（「图谱落后 HEAD，探索档仍可用，结论须回源」），不催促运行期刷新（与「不运行期自动刷新」非目标一致）。所有输出仍是 advisory setup facts，不进 doctor 阻塞路径。
...
- **Approach:** 对齐 `CLAUDE.md`/`AGENTS.md` 的 graphify 指引小节：移除「修改代码后运行 `graphify update .`」运行期刷新指令与无条件 query-first 强制，改写为 candidate-only + readiness gate 口径并指向合同路径（host 入口文档保留具名命令合法，R2 约束不及于此）；该小节若属 spec-first managed block 则改 source slice 经生成同步，若为手工段落则直接修订。CHANGELOG 按仓库现行格式以 developer profile（leokuang）记录，标注 `(user-visible)`（workflow 边界与 setup advisory 行为对用户可见）。skill prose 变更按 `docs/contracts/workflows/fresh-source-eval-checklist.md` 执行 fresh-source eval（把磁盘上的目标 SKILL 源文件注入全新 read-only subagent 评估边界语义是否可执行）；宿主不支持时记录未执行原因。源变更全部完成后运行 `spec-first init` 刷新 mirrors，确认 source/runtime 无 drift。README 仅在用户可见行为描述受影响时小幅同步。
...
- 既有边界与测试：`skills/spec-plan/SKILL.md:90-92`、`skills/spec-debug/SKILL.md:90-92`、`skills/spec-code-review/SKILL.md:93-95`、`tests/unit/capability-aware-provider-contracts.test.js:8-45`、`tests/unit/context-governance-contracts.test.js:16-102`、`tests/unit/scenario-capability-matrix-contracts.test.js:46-138`
- setup/probe 源码：`skills/spec-mcp-setup/scripts/install-helpers.sh:1058-1070,1135,1151,1169-1170,1264-1265`、`install-helpers.ps1:1034-1051`、`provider-readiness-renderer.cjs:346-418,368,481-483`、`verify-tools.sh:534`
- contracts/schema：`docs/contracts/provider-readiness.md:3,12,16,21-23`、`provider-readiness.schema.json:27,30-51,75-78`、`docs/contracts/workflows/spec-work-run-artifact.schema.json:27,230-352`、`src/cli/helpers/spec-work-run-artifact.js:56-70,96,871-877,1138`、`src/cli/helpers/setup-facts.js:298,323`
