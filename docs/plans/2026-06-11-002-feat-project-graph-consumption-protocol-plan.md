...
- **Files:**
  ...
  - `skills/spec-mcp-setup/scripts/provider-readiness-renderer.cjs`（graphify next_actions 分支增加 query_verified=false advisory 条目，镜像 CodeGraph 的 query 探针 next_action 先例；**built_at_commit 陈旧判定已降级 Deferred，本期不动 renderer 的 readiness_status 计算逻辑**）
