...
为 `spec-graph-bootstrap` 引入两档刷新模型：`incremental`（显式 `--incremental` operator escape hatch / expert opt-in 触发，委托 provider 原生增量命令）与 `full`（默认）。**速度目标仅用于 clean single-repo 显式 opt-in 场景**：本计划验证 single-repo `commit` 后的 canonical readiness 刷新是否能从 ~25 秒降到 ~3-5 秒；documented expert opt-in 必须同时通过 U4 correctness gate 与最低速度收益 gate（`B_incremental ≤ B_full / 3`）。只有 speed claim gate（最低速度收益 + 累计耗时 ≤ 5s）通过时，README / 用户手册才可宣传 ~3-5 秒收益。`--all-repos` 多仓 bootstrap 保持 `DEFAULT_REFRESH_MODE_ALL_REPOS=full` 不变；`--all-repos --incremental` 在本计划内不支持，多仓优化属独立 validation 计划。
...
| **验收 gate**（U4） | A→B 差分对照在 disposable temp clone 跑；correctness gate + 最低速度收益 gate 全绿 → **incremental flag-gated operator opt-in 上线**；speed claim gate 再决定是否宣传 ~3-5 秒；**default 切换需独立更宽 rollout plan**（A→B→C→D 链 / rename / parser degradation 等） |
...
计划完成（~0 min）        incremental flag-gated operator escape hatch / expert opt-in 上线，
                          本计划交付完成；若 correctness 或最低速度收益不过，
                          `--incremental` 仅保留 validation/experimental，不文档化为 opt-in。
...
- **体验闭环顺序：** 本计划只交付成熟度 L1：把显式 `$spec-graph-bootstrap --incremental` 做成可信、可回退、可验证的刷新入口。L2 notify-only stale detector 是下一层体验增强，用于提示何时该跑该入口，但不改变 refresh ownership。只有 L1+L2 都稳定后，才值得讨论 L3 clean-only auto-refresh opt-in policy；CI / scheduled refresh 属 L4 独立治理。
- **速度宣传边界：** correctness gate 通过只说明 provider 增量路径在合成 diff 上等价；documented expert opt-in 还必须通过 minimum speed-benefit gate（duration ratio）。只有 U4 speed claim gate 同时满足 duration ratio 与 `≤5s` 时，README / 用户手册才可宣传 ~3-5 秒收益。若 correctness 通过但 minimum speed-benefit gate 失败，validation doc 记录实测耗时，`--incremental` 仅保留 validation / experimental，不进入 README / 用户手册；若 ratio 通过但 absolute ≤5s 失败，用户文档只保留 expert opt-in，不写速度承诺。
