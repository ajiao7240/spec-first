...
为 `spec-graph-bootstrap` 引入两档刷新模型：`incremental`（显式 `--incremental` operator escape hatch / expert opt-in 触发，委托 provider 原生增量命令）与 `full`（默认）。**速度目标仅用于 clean single-repo 显式 opt-in 场景**：本计划验证 single-repo `commit` 后的 canonical readiness 刷新是否能从 ~25 秒降到 ~3-5 秒；只有 U4 speed claim gate 通过时，README / 用户手册才可宣传该速度收益。Correctness gate 单独决定 `--incremental` 是否可作为 documented expert opt-in 上线，不等同于速度承诺。`--all-repos` 多仓 bootstrap 保持 `DEFAULT_REFRESH_MODE_ALL_REPOS=full` 不变，多仓优化属独立 validation 计划。
...
| **验收 gate**（U4） | A→B 差分对照在 disposable temp clone 跑，correctness gate 全绿 → **incremental flag-gated operator opt-in 上线**；speed claim gate 单独决定是否宣传 ~3-5 秒；**default 切换需独立更宽 rollout plan**（A→B→C→D 链 / rename / parser degradation 等） |
...
- R6. `last_indexed_commit` 的**来源、写入时机与合法性**（U2 Approach 复用本条，不重复展开）：
  ...
  - **写入时机（clean-readiness-aware + carry-forward）**：当前 `write_provider_status()` 用 `jq -n | write_file_atomic` 原子重写整个 status.json。本计划必须**先读旧 status.json** 捕获 `$prior_last_indexed_commit` / `$prior_requires_clean_full_refresh`，最终字段 emit 表达式：
    ...
    - **`requires_clean_full_refresh`**（**新字段**）：`if provider_refresh_process_started_and_exited_nonzero then true else if (worktree_dirty == false and refresh_mode == "full" and graph_ready and query_ready) then false else $prior_requires_clean_full_refresh end`——只由脚本能确定的 provider refresh process 事实驱动：`bootstrap` / `incremental` / fallback full 进程已启动且非零退出时置 `true`；clean worktree + 完成 full + readiness 通过时清除；preflight / shape gate / projection stale / missing command / base ref invalid / status-only / query probe 失败都不额外置 `true`。若用户遭遇持续死锁（provider native storage 损坏），`spec-first doctor` 输出必须提示：`requires_clean_full_refresh=true` 且连续 full 失败时，手动删除 per-provider status.json 可重置状态（R11 文档要求同步）。
...
- **ADV-004 / 3-5s 速度目标无 baseline 实测**（adversarial）— **Superseded by 2026-05-13 follow-up fix**：active U4 已拆分 correctness gate 与 speed claim gate；`≤5s` 只决定是否宣传 ~3-5 秒，不阻断 correctness-backed expert opt-in。原始推荐：U1 之前跑一次 throwaway timing probe，把实测值写入 A3 / Problem Frame；若实测 > 5s，决定 (a) 调整 Summary 目标，或 (b) 是否撤销本计划。
...
U4           (~1.5-2 d)   文档同步 + opt-in 正确性基线 benchmark（disposable temp clone）：
                          ...
                          - correctness gate 全绿（node capture/equivalence 必过；
                            edge 可 N/A 但阻断未来 default switch）
                            → explicit operator opt-in 计划完成
                          - speed claim gate（duration ratio + absolute ≤ 5s）只决定
                            是否宣传 ~3-5 秒，不阻断 correctness-backed opt-in
...
- 若 U0a 或 U0b 为 fail / inconclusive，实施必须停在 U1 前：不得修改 `docs/contracts/graph-provider-consumption.md`、README / 用户手册、CLI flags、provider projection、bootstrap scripts、PowerShell parity 或 canonical artifact contract。允许产物只有 validation note，以及收窄到单 provider / 保持 experimental-undocumented / 重写计划的后续决策。
...
- **Provider command failure recovery**：只有 provider refresh process 已启动且非零退出（incremental primary 或 fallback full）才写 `requires_clean_full_refresh=true`；preflight / shape gate / projection stale / missing command / base ref invalid / status-only / query probe 失败都不额外置 true。后续 incremental preflight 必须降级 full，直到 clean successful full 清除该字段。
...
- Edge: query-unverified carry-forward — full 成功 → incremental command 成功但 query probe 失败 → 下次跑 → per-provider status.json 的 `last_indexed_commit` 仍为**首次 full 时的 HEAD**（不被中间失败擦除），本轮写 `readiness_source=incremental-update` 作为 command source，但 `status=query-unverified` / `query_ready=false`；下次 `--incremental` 可启动。
...
- **query-unverified command source 测试**：incremental command 成功但 query probe 失败时，per-provider status 写 `readiness_source=incremental-update` + `status=query-unverified` + `query_ready=false`，且 `last_indexed_commit` 不推进；consumer test 断言只看 `readiness_source` 不能判定 readiness success。
...
- **requires_clean_full_refresh 状态机测试**：Step 2 provider refresh process 已启动且失败时置 `true` 且 `last_indexed_commit` 不推进；Step 3 clean full 清为 `false` 且推进 `last_indexed_commit`；Step 4 incremental 可启动。
...
**Verification:** parity 测试通过；核心常量（`refresh_mode`、`incremental-update`、`incremental-fallback-full`、`incremental-base-ref-missing`、`incremental-base-ref-unset`、`incremental-base-status-untrusted`、`incremental-base-ref-not-ancestor`、`dirty-refresh-non-canonical`、`DEFAULT_REFRESH_MODE_SINGLE_REPO`、`DEFAULT_REFRESH_MODE_ALL_REPOS`）在两个脚本中都存在；`command_shape_supported` 的 incremental kind 分支 parity 对齐；**R1 CLI flag 分流两档（`--all-repos` 命中 ALL_REPOS / 无 flag 命中 SINGLE_REPO）parity 对齐**（env-driven child propagation parity 已 deferred）；`requires_clean_full_refresh` 状态机 lifecycle parity（provider refresh process started + failed 置 true → clean full 清 false → incremental 可启动）；artifact retention parity 明确覆盖 dirty blocked preflight 不覆盖 canonical artifacts、both-failed skip normalized rewrite、aggregate provider-status / graph-facts carry-forward、`last_indexed_commit` clean success 才推进、query-unverified 保留 `readiness_source=incremental-update` 但不推进 readiness、以及不生成 `refresh_mode_summary` / `refresh_modes_by_provider`。
...
7. **Per-provider pass matrix**：
   ...
   |---|---|---|---|---|---|---|---|
   | gitnexus | 必过 | 必过 | 必过 或 `N/A`（preflight 决定） | 必过 或 `N/A` | 必过 | 必过 | speed claim gate |
   | code-review-graph | 必过 | 必过 | 必过 或 `N/A`（preflight 决定） | 必过 或 `N/A` | 必过 | 必过 | speed claim gate |

   - **Correctness gate（阻断 documented opt-in）**：每个 enabled provider 必须至少具备 node capture + node equivalence 的 anchor-level 证据，`readiness_source=incremental-update` 且 query probe no degradation；若 node extraction 不可用，validation 失败或 inconclusive，不能以 meta stats / query probe 通过 opt-in correctness。
   ...
   - **Speed claim gate（只决定是否宣传 ~3-5 秒，不阻断 correctness-backed opt-in）**：`Duration ratio (B_incr ≤ B_full / 3)` 与 **所有 provider 累计** `B_incremental_total_duration ≤ 5s` 同时通过时，validation doc 才可写"达到 ~3-5 秒速度目标"；若 correctness gate 通过但 speed claim gate 失败，`--incremental` 仍可作为 documented expert opt-in，但 README / 用户手册不得宣传 3-5 秒速度收益，只能记录实测耗时与 caveat。

8. **Rollout 决策**：本计划 U4 的角色是 **explicit operator escape hatch / expert opt-in 正确性基线**（incremental 开关功能可用、对一组合成 diff 表现等价）；
   - **不**自动触发默认切换。Correctness gate 全绿 → incremental flag 可作为 documented operator opt-in 上线，Plan 视为完成；speed claim gate 只控制是否宣传 ~3-5 秒。
...
- **Error propagation：** incremental 命令失败自动 fallback full；fallback full 也失败按现有 `workflow_mode=degraded-fallback` 处理；dirty preflight 固定 command-level machine-readable combo 为 `workflow_mode=blocked` + `overall_status=action-required` + `reason_code=dirty-refresh-non-canonical` + `canonical_artifacts_preserved=true`，且不执行 provider command。`overall_status` 只属于 `graph-bootstrap-result.v1` stdout / command result / optional report，不进入 canonical `.spec-first/graph/provider-status.json` 或 `.spec-first/graph/graph-facts.json`；canonical 判断仍靠 `workflow_mode`、`status`、`query_ready`、`source_revision`、`worktree_dirty`、`worktree_status_hash`。**不新增 workflow-level error category**（workflow_mode 仍是现有 `primary` / `degraded-fallback` / `blocked` / `no-source`）；**新增若干 incremental 相关 reason_code**（`incremental-refresh-failed-fallback-full`、`incremental-base-ref-missing`、`incremental-base-ref-unset`、`incremental-base-status-untrusted`、`incremental-base-ref-invalid-format`、`incremental-base-ref-not-ancestor`、`incremental-and-full-failed`、`incremental-command-unavailable`、`dirty-refresh-non-canonical`、`clean-full-refresh-required`、`fingerprint-spec-first-changed`、`fingerprint-projection-changed`、`fingerprint-provider-changed`），由 consumption 契约 + contract test 扩展声明（不新增 JSON schema 文件，沿用 prose + Jest 验证模式）。
...
- **速度宣传边界：** correctness gate 通过只说明 `--incremental` 可作为 documented expert opt-in；只有 U4 speed claim gate 同时满足 duration ratio 与 `≤5s` 时，README / 用户手册才可宣传 ~3-5 秒收益。若 speed gate 失败，validation doc 记录实测耗时，用户文档只保留 expert opt-in，不写速度承诺。
