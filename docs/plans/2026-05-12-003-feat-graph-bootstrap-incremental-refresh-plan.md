...
- Test: `tests/unit/mcp-setup-powershell-contracts.test.js`（断言 PowerShell projection 生成同等 `commands.incremental` argv，并更新 command hash / key-ordering 断言）
...
- **并发保护**：同一仓库并发跑 bootstrap（如用户两个 terminal / spec-work 间接触发）会互相覆盖 `status.json`。本 unit 加 `flock -n "$PROVIDERS_DIR/<id>/.bootstrap.lock"` 保护每个 provider 的写入临界区；获取锁失败返回 `reason_code=bootstrap-concurrent-run`，不执行 provider 命令。PowerShell parity（U3）用 `System.Threading.Mutex`。
