...
completion_posture: diagnostic-only
---
...
为 `spec-graph-bootstrap` 引入两档刷新模型：`incremental`（显式 `--incremental` clean single-repo diagnostic / validation-only 触发，委托 provider 原生增量命令）与 `full`（默认）。实施后的 U4 corrected runs 覆盖 correctness 证据，但未通过最低速度收益 gate（`B_incremental ≤ B_full / 3`），因此本计划不把 `--incremental` 交付为 speed-backed documented expert opt-in，也不在 README / 核心用户手册宣传 ~3-5 秒收益。`--all-repos` 多仓 bootstrap 保持 `DEFAULT_REFRESH_MODE_ALL_REPOS=full` 不变；`--all-repos --incremental` 在本计划内不支持，多仓优化属独立 validation 计划。

**Rollout 策略**：本计划最终交付 **clean single-repo diagnostic / validation-only `--incremental` path**；U4 benchmark 是 diagnostic correctness evidence，不自动触发默认切换，也不把增量刷新定位为普通 workflow 用户默认路径。Speed-backed documented expert opt-in 与 default 切换到 incremental 都需要独立 rollout plan（含连续增量链 A→B→C→D 测试 / rename benchmark / parser degradation fuzz 等更宽 gate）来支撑。本计划只覆盖 bootstrap 自身的两档模型，不捆绑其他 workflow 的治理或披露改动。

**2026-05-14 closure revision**：实施后的 corrected U4 benchmark 证明两个 enabled provider 在 split disposable runs 中具备 node/edge capture 与同 commit full equivalence，但 stable pinned providers 未达到 minimum speed-benefit gate（`B_incremental <= B_full / 3`）。本计划最终交付范围因此显式修订为 **clean single-repo diagnostic / validation-only `--incremental` path**：默认仍为 `full`，README / 核心用户手册不宣传 `--incremental`，speed-backed documented expert opt-in 与 provider speed/pin investigation 拆为 follow-up。本修订不改变已落地 runtime contract；它只收口本计划 completion posture，避免把未达标速度收益包装成交付事实。
...
| **核心机制** | bootstrap 按 fingerprint 决策 `incremental`（委托 `gitnexus analyze` / `code-review-graph update --base`）vs `full`；incremental 命令失败自动 fallback 到 full；dirty worktree 直接 provider-non-mutating block |
| **已闭环的实施级 bug** | R6 `last_indexed_commit` carry-forward 避免原子 rewrite 擦除；R6 `^[0-9a-f]{40}$` 格式校验防 argv injection；R7 dirty worktree 不执行 provider 命令，避免 dirty filesystem 污染 native storage / base ref |
| **验收 gate**（U4） | corrected disposable runs 覆盖 correctness 证据；最低速度收益 gate 未通过。本计划按 2026-05-14 closure revision 完成为 diagnostic-only path；speed-backed documented opt-in 与 default 切换都需独立 follow-up plan。 |
| **速度目标范围** | single-repo only；`--all-repos` 仍走 full 默认，`--all-repos --incremental` 不支持，不在本计划速度承诺内 |
| **实施前必做** | A4 协调（`2026-05-09-003` 串行 / 并行 ack；**A3 sanity check 已在 planning 阶段通过，无需重跑**）+ U0a extraction surface discovery + U0b provider-native proof（U1/U2 前必须通过） |
| **显式 Scope 剥离** | dirty incremental opt-in / `spec-work` 披露契约 / 统一 redaction helper / notify-only stale detector / hook-triggered rebuild policy / `--all-repos` 多仓默认切换 → 全部 follow-up plan |
...
文档分层规则：由于 U4 speed-benefit gate 未通过，README / README.zh-CN / 核心用户手册不宣传 `--incremental`；clean single-repo operator 的 diagnostic / validation-only 边界保留在 `spec-graph-bootstrap` skill、graph provider consumption contract 与 validation 文档中。fallback 处理、issue 上报字段、redaction checklist、benchmark caveats 放到 graph-bootstrap 专项 validation / troubleshooting 文档或 graph provider consumption contract，不抬到 onboarding 主路径。
...
官方 GitNexus 本地路径与本计划的边界一致：stale 后显式运行 `npx gitnexus analyze`；该命令默认使用 provider 内部的 cache-aware incremental analysis，`--force` 才强制 full rebuild。GitNexus 的 Claude Code hook 采用 post-commit / post-merge stale detection + prompt 模式，不在 hook 内直接运行 `analyze`。code-review-graph 提供独立的 `update [--base <ref>]` 增量命令和 `build` 全量命令。spec-first 不需要自建增量算法，也不把 provider rebuild 挂到普通 consumer workflow；只要在 `$spec-graph-bootstrap` 内按 fingerprint 决策 incremental vs full、委托 provider 原生命令、失败时 fallback。

---

## Requirements

- R1. `spec-graph-bootstrap` 必须支持两档刷新模式：`incremental`（显式 `--incremental` flag 触发的 operator opt-in）与 `full`（默认、显式 `--force` / `--full`、fingerprint 非 repo-snapshot 部分变化、或 `requires_clean_full_refresh=true` 时从 incremental preflight 降级）。**Dirty worktree 不进入任一 provider refresh mode；直接走 provider-non-mutating blocked preflight**（R7）。默认值由两个独立常量控制：`DEFAULT_REFRESH_MODE_SINGLE_REPO` 与 `DEFAULT_REFRESH_MODE_ALL_REPOS`，初始均为 `full`；本计划交付 incremental flag-gated operator opt-in，**不**自动切换默认（rollout plan 独立）。**运行时分流规则**：
  - CLI flag `--all-repos` 存在且未显式传 `--incremental` → 走 `DEFAULT_REFRESH_MODE_ALL_REPOS`。
  - CLI flag `--all-repos --incremental` 同时存在 → 本计划范围内 unsupported scope，provider command 前 blocked，`reason_code=incremental-all-repos-unsupported`；不允许把未经验证的 multi-repo incremental 当作 explicit opt-in。
  - 否则 → 走 `DEFAULT_REFRESH_MODE_SINGLE_REPO`。
  - `workspace-graph-targets.v1` 是 advisory summary，不参与本分流判定。
  - 测试矩阵（U2/U3）：(a) 直接 `--all-repos` 走 ALL_REPOS 常量；(b) 无 flag 走 SINGLE_REPO 常量；(c) `--all-repos --incremental` blocked 且不执行 provider command。
...
- R6. `last_indexed_commit` 的**来源、写入时机与合法性**（U2 Approach 复用本条，不重复展开）：
  ...
  - **写入时机（clean-readiness-aware + carry-forward）**：当前 `write_provider_status()` 用 `jq -n | write_file_atomic` 原子重写整个 status.json。本计划必须**先读旧 status.json** 捕获 `$prior_last_indexed_commit` / `$prior_requires_clean_full_refresh`，最终字段 emit 表达式：
    ...
    - **`requires_clean_full_refresh`**（**新字段**）：`if (worktree_dirty == false and final_full_attempt_succeeded and graph_ready and query_ready) then false else if provider_refresh_process_started_and_exited_nonzero then true else $prior_requires_clean_full_refresh end`——只由脚本能确定的 provider refresh process 事实驱动：`final_full_attempt_succeeded` 覆盖正常 `full` 与 incremental 失败后的成功 fallback full（`refresh_mode=incremental-fallback-full`）；因此 fallback full 成功会视为 clean full recovery 并清除该标记。`bootstrap` / `incremental` / fallback full 进程已启动且最终未得到 clean full readiness 时置 `true`；preflight / shape gate / projection stale / missing command / base ref invalid / status-only / query probe 失败都不额外置 `true`。若用户遭遇持续 provider native storage 损坏，R11 覆盖的 troubleshooting / consumption 文档必须提示：`requires_clean_full_refresh=true` 且连续 clean full 失败时，可手动删除 per-provider `status.json` 重置状态；本计划不新增 `spec-first doctor` 代码路径。
  - **R7 P0 修复（dirty full 也会污染索引）**：dirty worktree 下不论 incremental 还是 full，`gitnexus analyze` / `code-review-graph build` 都会从 filesystem 读 working tree 包括未提交修改。所以 dirty 场景不执行 provider refresh command，避免 provider native storage 被 dirty filesystem 污染；也不推进 `last_indexed_commit`，不因 dirty 本身设置 `requires_clean_full_refresh=true`。
...
- R7. Dirty worktree 下任何 refresh request（`--incremental`、`--full`、`--force` 或 default）都必须在 provider command 前 provider-non-mutating block，**无 opt-in 逃生路径**。`gitnexus analyze` / `code-review-graph build` 从 filesystem 读 working tree，dirty 修改会被一并索引；如果执行 provider command，会污染 native storage 并可能让下一次 incremental base 与索引内容错位。所以本计划保护机制是组合：
  - dirty 任何刷新 → 不执行 provider refresh/status/query command，不写 provider raw log，不重写 normalized envelopes，不触碰 provider native storage
  - dirty 任何刷新 → 不推进 `last_indexed_commit`，不因为 dirty 本身设置 `requires_clean_full_refresh=true`
  - dirty 任何刷新 → **不覆盖 canonical graph artifacts**（per-provider status、aggregate provider-status、graph-facts、normalized envelopes 全部保持上一轮 snapshot）；只在命令级 `graph-bootstrap-result.v1` / stdout / optional report 返回 provider-non-mutating diagnostic：`workflow_mode=blocked` + `overall_status=action-required` + `reason_code=dirty-refresh-non-canonical` + `canonical_artifacts_preserved=true`，避免 downstream 误读为当前 HEAD 的 clean readiness
  - 该 blocked 状态由本计划承担（不再 defer 到 follow-up plan）；`--allow-dirty` opt-in 仍不引入。
...
- R9. `readiness_source` 枚举扩展（per-provider `provider-status.v1`）必须向后兼容：
  - 现有值保持不变：
    - `cold-run`（**所有 clean full 模式场景都填此值**：首次 bootstrap、显式 `--force` / `--full`、fingerprint 变化触发的强制 full、`requires_clean_full_refresh=true` 触发的 clean full。语义= "本次跑了完整 provider commands"；"cold" 不等于"首次"；dirty worktree blocked preflight 不填该值）
    - `skipped`、`preflight-blocked`
  - 新增值：`incremental-update`（**指本次用 incremental 命令形式调用 provider**；provider 内部是否真走增量由 provider 决定，不由本字段断言；下游不得据此推论 graph 只含 diff。该值允许与 `status=query-unverified` / `query_ready=false` 共存，表示 command source 不是 readiness success；consumer 必须继续用 `status=ready && query_ready=true` 判断可消费 readiness；`last_indexed_commit` 仍只在 clean worktree + graph_ready + query_ready 全过时推进；下一次 `--incremental` 看到 prior status `query_ready=false` 必须按 R6 provenance gate 降级 full，`reason_code=incremental-base-status-untrusted`）、`incremental-fallback-full`（incremental 失败后已重跑 full 并成功）
...
- R10. Aggregate `graph-facts.v1` 顶层**不新增 refresh mode 字段**：不使用单值 `refresh_mode`，也不新增 `refresh_modes_by_provider` / `refresh_mode_summary` 便利字段。per-provider `.spec-first/providers/<id>/status.json.refresh_mode` 是唯一 truth source；aggregate `provider-status.json.providers[]` 可以镜像 per-provider 字段用于现有 provider 列表消费，但 `graph-facts.v1` 不承载新的 refresh-mode convenience surface。若未来出现明确 consumer 需要 aggregate map，再按独立 contract change 添加。
...
- A2. GitNexus `analyze` 的内部 cache-aware incremental 判断是可信的；本计划不自行判断文件变更集，也不把 GitNexus Enterprise / hook 形态的 auto-reindexing 外推为本地 OSS CLI 默认行为。
...
- 不新增会直接执行 provider rebuild 的 git hook / watcher / daemon / CI 自动触发。若未来对齐 GitNexus Claude Code hook 体验，只能先做独立 follow-up 的 **notify-only stale detector**：检测 post-commit / post-merge stale 并提示 `$spec-graph-bootstrap --incremental`，不得在 hook 内静默运行 `gitnexus analyze` / `code-review-graph build`。
...
- Notify-only post-commit / post-merge stale detector（对齐 GitNexus Claude Code hook 的提醒模式）属于独立 ergonomics plan；本计划不实现 hook，也不允许 hook 内直接 rebuild provider index。
...
- **ADV-004 / 3-5s 速度目标无 baseline 实测**（adversarial）— **Superseded by 2026-05-13 latest doc-review fix**：active U4 已拆分 correctness gate、minimum speed-benefit gate 与 speed claim gate；documented expert opt-in 需要 correctness + duration ratio，`≤5s` 只决定是否宣传 ~3-5 秒。原始推荐：U1 之前跑一次 throwaway timing probe，把实测值写入 A3 / Problem Frame；若实测 > 5s，决定 (a) 调整 Summary 目标，或 (b) 是否撤销本计划。
...
- **ADV-006 / `requires_clean_full_refresh` 死锁陷阱**（adversarial）— **Superseded by 2026-05-13 follow-up fix**：active R6 已改为确定性条件：只有 provider refresh process 已启动且非零退出才置 `true`；preflight / shape gate / projection stale / base ref invalid / query probe 失败不额外置 `true`。原始推荐：(a) 加 doctor 风格诊断说明该状态位的设置原因；(b) 加显式 `--reset-incremental-state` flag 一次性清空 per-provider status.json + native storage；(c) README / 用户手册介绍 `--incremental` 时同步说明 reset 路径。
...
- **PL-003 / U0 benchmark-first ordering**（product-lens）— **Superseded by 2026-05-13 follow-up fix**：active plan 已拆成 U0a extraction surface discovery 与 U0b minimal provider-native proof helper；U0a 不通过时不创建 helper、不进入 U1/U2。原始推荐：在 U1 之前插入 U0（~0.5 天 throwaway）：temp clone 内跑 `analyze` / `analyze --force` / `update --base` / `build`，用 `.gitnexus/meta.json` stats + query probe 粗对比验证 provider 原生增量假设；U0 通过后才进 U1/U2。U0 失败则本计划直接作废或收窄到单 provider，避免 U2 沉没成本。
- **SG-002 / `refresh_mode_summary` 是投机性聚合字段**（scope-guardian）— **Superseded by 2026-05-13 follow-up fix**：active R10 已进一步收敛为 `graph-facts.v1` 不新增任何 refresh-mode convenience 字段；consumer 需要精确判断时回读 per-provider status 或 aggregate provider-status provider 条目。原始推荐：从 R10 与 graph-facts.v1 移除 `refresh_mode_summary` 字段；保留 `refresh_modes_by_provider` 作为唯一 aggregate truth source；如未来有 consumer 需要 summary 语义，届时再按需添加。
- **COH-001 / 顶层 `refresh_mode` vs `refresh_modes_by_provider` 命名错位**（coherence）— **Superseded by 2026-05-13 follow-up fix**：active R10 已不新增 `refresh_modes_by_provider`，命名错位随 aggregate map 移除而消解。原始推荐：在 Phase 4 加一句明确双层契约："Per-provider `status.json` 写单值 `refresh_mode`；aggregate `graph-facts.json` 写 `refresh_modes_by_provider` map。Consumer 必须读 per-provider status 取 truth，aggregate map 仅供概览。"
...
spec-graph-bootstrap [--incremental | --full | (default)]
  ...
  │    dirty worktree?                   → blocked/action-required
  │                                         (reason_code=dirty-refresh-non-canonical, R7;
  │                                          不执行 provider command)
  │    flag --all-repos + --incremental? → blocked/unsupported-scope
  ...
  │
  │    若期望 = incremental, 依次 preflight（reason_code 见 R1/R6/System-Wide Impact；readiness_source 见 R9）:
  ...
  │
  │    [字段映射] reason_code 是过程审计字段（preflight 分支诊断）；readiness_source 是 provider command source 字段（R9 enum）。
  │    对应关系：full 命令路径 → readiness_source=cold-run；
  │    incremental 命令路径 → readiness_source=incremental-update；
  │    incremental 失败后 fallback 成功 → readiness_source=incremental-fallback-full。
  │    readiness_source 不代表 readiness success；consumer 必须同时检查 status=ready && query_ready=true。
  │
  ...
  │        readiness_source=incremental-update（command source），但 status=query-unverified / query_ready=false；
  │        不推进 last_indexed_commit；下一次 --incremental 因 prior query_ready=false
  │        触发 incremental-base-status-untrusted 并降级 full
  ...
  │    graph-facts.v1 不新增 refresh mode convenience 字段（R10）
  │    last_indexed_commit：clean worktree + readiness 三级证据全过时按 R6 写入当前 HEAD
  │    clean successful canonical refresh 才更新 freshness 字段到当前；
  │    dirty preflight 不覆盖 canonical artifacts；只在 graph-bootstrap-result.v1 / stdout / optional report
  │    返回 workflow_mode=blocked + overall_status=action-required + reason_code=dirty-refresh-non-canonical；
  │    fallback both-failed attempt 固定 workflow_mode=degraded-fallback；
  │    fallback both-failed 时 canonical freshness 指针 carry-forward；readiness 三级证据契约保持不变
```

字段映射：preflight / fallback 分支产出的 `reason_code` 是过程审计字段；`readiness_source` 是 provider command source 字段，不是 readiness success 字段。结果映射为：full command source → `cold-run`；incremental command source → `incremental-update`；incremental 失败后 fallback 成功 → `incremental-fallback-full`。`readiness_source=incremental-update` 可与 `status=query-unverified` / `query_ready=false` 共存；downstream 必须继续以 `status=ready && query_ready=true` 判断可消费 readiness，不把 `reason_code` 当成 `readiness_source`。query-unverified 的 prior status 不可信，下一次 `--incremental` 必须降级 full。
...
| `.spec-first/graph/graph-facts.json`（`graph-facts.v1`） | **不覆盖** `source_revision` / `generated_at` / `timing.*` / `repo_snapshot.*` / `worktree_status_hash` / 其他 canonical freshness 指针；不新增 refresh mode convenience 字段 | 保留上一次 readiness 通过时的指针；当前失败细节从 per-provider status 与 aggregate provider-status 的 provider 条目读取 |
...
核心语义：**canonical freshness 指针（`source_revision` / `generated_at` / `timing.*` / `worktree_status_hash` / `last_indexed_commit` / prior successful normalized envelopes）在两次 attempt 都失败时 carry-forward**；**当前 attempt readiness 字段必须报告 failed（`status=failed` / `graph_ready=false` / `query_ready=false`）**；**本轮 log + degraded-fallback 报告必须覆盖**（否则 triage 读到上一轮日志）；**provider native storage 不由 spec-first 回滚，provider refresh process 已启动且失败后靠 `requires_clean_full_refresh=true` 强制 clean full recovery**。

**Dirty preflight 保留矩阵（provider-non-mutating block）**：

| Artifact | 是否覆盖 | 说明 |
|---|---|---|
| provider native storage（`.gitnexus/` / `.code-review-graph/`） | **不触碰** | dirty worktree 不执行 provider command，避免 dirty filesystem 污染索引 |
| `.spec-first/providers/<id>/raw/*.log` | **不覆盖** | 没有 provider attempt，不生成本轮 raw log |
| `.spec-first/providers/<id>/status.json` | **不覆盖** | per-provider canonical status 保持上一轮 snapshot；不推进 `last_indexed_commit`，不因 dirty 本身设置 `requires_clean_full_refresh=true` |
| `.spec-first/graph/provider-status.json` / `.spec-first/graph/graph-facts.json` | **不覆盖** | aggregate canonical artifacts 保持上一轮 snapshot；不得写入 `overall_status` 或 dirty blocked diagnostic |
| normalized envelopes（`architecture-facts.json` / `impact-capabilities.json`） | **不覆盖** | consumer-facing readiness 快照保持上一轮值 |
| `graph-bootstrap-result.v1` stdout / command result / optional `.spec-first/graph/bootstrap-report.md` | **覆盖 / 返回本轮诊断** | `workflow_mode=blocked`、`overall_status=action-required`、`reason_code=dirty-refresh-non-canonical`、`canonical_artifacts_preserved=true`；这是命令级结果，不是 canonical readiness snapshot |
...
Pre-flight   (~2 min)     A4 ack（与 2026-05-09-003 协调决策，默认假设串行即可）
    ...
    │
    ▼
U0a          (~0.25 d)    Provider extraction surface discovery：
                          - disposable temp clone 内只做 provider surface discovery
                          - 证明两个 enabled provider 都能导出 node anchors
                          - 未通过时不创建 helper，不进入 documented `--incremental` 实施
    │
    ▼
U0b          (~0.25-0.5 d) Minimal provider-native proof helper：
                          - 创建最小 extract-graph-anchors.sh
                          - 手动跑 provider-native full/incremental
...
U4           (~1.5-2 d)   文档同步 + opt-in 正确性基线 benchmark（disposable temp clone）：
                          ...
                          - 四个独立比对：node capture / node equivalence /
                            edge capture / edge equivalence
                          - correctness gate 全绿（node + edge capture/equivalence 必过）
                          - minimum speed-benefit gate（duration ratio）通过
                            → explicit operator opt-in 计划完成
                          - speed claim gate（duration ratio + absolute ≤ 5s）再决定
                            是否宣传 ~3-5 秒
                          - validation doc 记录脱敏命令元数据 / 耗时 / diff / 决策
    │
    ▼
计划完成（~0 min）        2026-05-14 closure revision 后，本计划完成为 clean single-repo
                          diagnostic / validation-only `--incremental` path；correctness 证据已记录，
                          但 minimum speed-benefit gate 未通过，所以不文档化为 speed-backed opt-in。
...
**关键 blocker**：A4（P0 协调 2026-05-09-003）、U0a provider extraction surface discovery、U0b provider-native proof helper 是进入 U1/U2 前的 blockers；A3 sanity check 已通过（CRG 2.3.3 支持 `update --base`）。

**最佳实践成熟度分层（用于约束后续自动化讨论）**：

| Level | 能力 | 触发 / 写入边界 | 晋级门槛 |
|---|---|---|---|
| L0 | automatic freshness-check | plan/work/debug/review 等 consumer 自动只读检查；不写 canonical graph/provider artifacts | 已由 `2026-05-12-002` policy 定义 |
| L1（本计划交付） | explicit `$spec-graph-bootstrap --incremental` | 用户 / operator 显式进入 `$spec-graph-bootstrap`；可写 canonical artifacts；dirty preflight blocked | U0a/U0b provider proof + U4 correctness gate |
| L2（紧随 follow-up） | notify-only stale detector | post-commit / post-merge 后只读检测并提示；不得运行 provider rebuild；默认先 opt-in | L1 可用，且 detector 不写 canonical artifacts、不阻塞提交路径 |
| L3（独立后续） | opt-in clean-only auto-refresh policy | 仅 clean single-repo、显式 opt-in policy；可自动调用 bootstrap；必须有 lock / timeout / cancellation / failure circuit breaker / audit artifacts | L1+L2 稳定，且连续链、rename、parser degradation、失败熔断验证通过 |
| L4（独立后续） | CI / scheduled policy refresh | PR head / default branch / nightly 等显式 CI policy；写入边界与 artifacts retention 独立定义 | CI 权限、成本、provider cache、artifact publication 与 redaction policy 明确 |

本计划只交付 L1；L2/L3/L4 作为后续计划，不得作为 U1-U4 的 hidden scope。

**推荐紧随其后的 follow-up（不属于本计划交付）**：`notify-only stale detector`。该 follow-up 的目标不是刷新 graph，而是把 commit / merge 后的 stale 发现前移，避免后续 `$spec-plan` / `$spec-work` / review 才突然发现 GitNexus 不能作为 primary evidence。最小契约：

- **触发点**：post-commit / post-merge 后，或当前 host 已能可靠观察到 `git commit` / `git merge` 完成事件时；默认应先 opt-in，不作为本计划安装的默认 hook。
- **只读检查**：比较当前 `HEAD`、canonical `graph-facts.json.source_revision`、`worktree_dirty`、`worktree_status_hash` 与 provider projection freshness；不得运行 `gitnexus analyze`、`code-review-graph build/update`、`$spec-graph-bootstrap` 或任何 provider repair。
- **写入边界**：不得写 `.spec-first/graph/*`、`.spec-first/providers/*`、`.spec-first/impact/*`、`.gitnexus/*`、`.code-review-graph/*`；如后续需要 cooldown / advisory marker，必须在独立 follow-up 中定义非 canonical 路径和 retention。
- **提示策略**：clean single-repo 且 stale 时提示 `Run: $spec-graph-bootstrap --incremental`；dirty worktree 时提示 provider refresh 会被 blocked，要求先 commit / stash / clean 后再显式刷新；no-source / no graph-heavy 场景不强推刷新。
- **成功标准**：提交路径不被 provider rebuild 阻塞；stale 提示含 current HEAD、graph source revision、建议命令和 dirty 状态；不会让后续 workflow 误以为 graph 已刷新。
...
**Goal:** 在投入 U1/U2 bootstrap 改造前，先用 U0a 证明两个 enabled provider 都有可行 node anchor extraction surface；只有 U0a 通过后，才用 U0b 创建最小 helper 并证明 provider-native incremental 至少能跑通 A→B→C 连续增量链的最低 node capture / equivalence 证据。
...
**Files:**
- U0a Create/Modify: `docs/validation/2026-05-12-graph-bootstrap-incremental-benchmark.md`（第一节记录 extraction surface discovery 结论；允许结论为 pass / fail / inconclusive）
- U0b Create: `tests/benchmark/extract-graph-anchors.sh`（仅在 U0a pass 后创建，先以 proof spike 所需的最小 node anchor extraction 支持落地；U4 可继续扩展 edge / impact 输出）
- Modify: `CHANGELOG.md`（U0a/U0b 即使 fail / inconclusive，只要写入 tracked validation note，也必须按仓库治理记录）

**Approach:**
- **U0a — Provider extraction surface discovery（不创建 helper）：**
  - 在 disposable temp clone / isolated worktree 内运行，禁止污染当前 repo canonical `.spec-first/graph/` artifacts。
  - 只验证两个 enabled provider 是否存在可用 node anchor extraction surface（CLI / MCP / serialized facts 均可；stats-only / query-probe-only 不算）。
  - extraction surface 必须绑定 temp clone provenance：记录 `repo_path`、`source_revision`、provider data dir 或 repo selector、CLI/MCP request metadata、以及输出文件路径；MCP surface 必须显式 target temp clone，或返回可核验的 repo root / source revision。缺任一 provenance 关键项即记为 `inconclusive`，不得算 U0a pass。
  - validation doc 记录每个 provider 的 surface、命令元数据、exit code、可导出的字段、稳定性观察和 `pass | fail | inconclusive` 结论；不得粘贴 raw stdout/stderr。
  - 任一 enabled provider 无法产出 node anchors → U0a fail / inconclusive；不创建 `tests/benchmark/extract-graph-anchors.sh`，不进入 U0b / U1 / U2。可选出路只有收窄到单 provider、保持 experimental / undocumented，或重写计划。
- **U0b — Minimal provider-native proof helper（仅 U0a pass 后执行）：**
  - 创建最小 `tests/benchmark/extract-graph-anchors.sh`，只覆盖 node anchors（`path` / `kind` / `name` / `span`）；edge anchor 可先记录 `extractable: yes|no`，不作为 U0b 通过条件，但会约束 U4/default-switch。
  ...
  - U0b 未通过时，U1/U2 不进入 documented `--incremental` 实施；不得用 stats-only / query probe 代替 node anchor proof。
  - validation doc 只记录脱敏命令元数据、exit code、duration、anchor counts、diff 摘要和结论；禁止粘贴 provider 原始 stdout/stderr 或未脱敏 URL。

**Test scenarios:**
- Happy U0a: 两个 provider 均可导出 node anchors，validation doc 记录 discovery pass，允许进入 U0b。
- Happy U0b: A→B→C incremental node delta 与 full checkpoint 粗等价。
- Error U0a: 任一 provider 只能给 stats / query probe、不能给 node anchors → U0a fail / inconclusive，不创建 helper，本计划不得把 `--incremental` 作为 correctness-backed opt-in 对外记录。
- Error U0b: surface 存在但 A→B→C 粗等价失败 → U0b fail / inconclusive，停止进入 U1/U2。

**Verification:**
- U0a validation doc 第一节含每个 provider 的 extraction surface、pass/fail/inconclusive、脱敏 metadata 和是否允许创建 helper 的明确结论。
- U0a validation doc 必须逐项记录 temp clone provenance（`repo_path`、`source_revision`、provider data dir / repo selector、CLI/MCP request metadata、输出路径）；任一 enabled provider provenance 缺失时，U0a 结论只能是 fail / inconclusive。
- U0b `tests/benchmark/extract-graph-anchors.sh` 在 temp clone 中对 GitNexus 和 CRG 均能输出合法 JSON。
- validation doc 第一节追加 U0b anchor diff 摘要和是否允许继续 U1/U2 的明确结论。
- 若 U0a 或 U0b 为 fail / inconclusive，实施必须停在 U1 前：不得修改 `docs/contracts/graph-provider-consumption.md`、README / 用户手册、CLI flags、provider projection、bootstrap scripts、PowerShell parity 或 canonical artifact contract。允许产物只有 validation note、对应 `CHANGELOG.md` 记录，以及收窄到单 provider / 保持 experimental-undocumented / 重写计划的后续决策。
...
**Precondition:** Open Questions / P0 Blocker 段的 fast-reuse 协调决策已在 PR 描述中被显式 ack（默认假设串行亦算 ack），且 U0a extraction surface discovery 与 U0b provider-native proof helper 均已通过。A3 的 CRG `update --base` sanity check 已在 planning 阶段完成（CRG 2.3.3 支持 `update [--base BASE]`），PR 描述引用该结论即可；仅在 CRG pin 升级或行为变化时才复跑。
...
- Modify: `docs/contracts/graph-provider-consumption.md`（**本计划决策：不新增 JSON schema 文件**；`src/cli/contracts/graph/` 目录当前不存在，canonical 验证面维持 prose contract + Jest assertion；在 consumption 契约中新增 truth table 列出所有新字段：`readiness_source` 全枚举值、per-provider 顶层 `refresh_mode` / `fallback_from_incremental` / `last_indexed_commit` / `requires_clean_full_refresh`、`command_results[]` 扩展的 `refresh_mode` + `attempt_role`、以及 graph-facts 不新增 refresh-mode convenience 字段的 consumer 使用规则）
...
- Test: `tests/unit/spec-graph-bootstrap-contracts.test.js`（**已存在** Jest 契约测试；扩展 readiness_source 枚举 + per-provider `refresh_mode` / `command_results[].refresh_mode` / `requires_clean_full_refresh` 字段断言；断言 `graph-facts.v1` 不新增 `refresh_mode` / `refresh_modes_by_provider` / `refresh_mode_summary`）

**Approach:**
- CLI flag：新增 `--incremental` / `--full` / `--force`（`--force` 是 `--full` 别名；dirty worktree 按 R7 在 provider command 前 blocked，**无 --allow-dirty opt-in**）。
- Flag scope guard：`--all-repos --incremental` 是本计划明确不支持的组合；必须在 provider command 前 blocked，`reason_code=incremental-all-repos-unsupported`，且不写 canonical provider/graph artifacts。`--all-repos` 未带 `--incremental` 时仍按 `DEFAULT_REFRESH_MODE_ALL_REPOS=full` 执行。
...
- **扩展 `command_shape_supported()` 与分层校验策略**（关键实施约束，避免破坏向后兼容）：
  ...
  - **校验分层（必须严格遵守）**：
    ...
    - **可选 kind（`incremental`）**：**不**加入启动前无条件循环；仅在 `resolve_refresh_mode()` 决议为 `incremental` 并准备执行时校验。处理分支：
      - 字段缺失 / `null` → refresh_mode 降级为 `full`，`reason_code=incremental-command-unavailable`（**不** `emit_blocked`；新增 reason_code 加入 consumption contract / System-Wide Impact / contract tests）
...
- Provider 执行按 refresh_mode 选 argv；incremental command exit != 0 时 fallback 重跑 full。
- **CRG base ref 来源与 carry-forward 写入语义**：按 R6 执行（来源、prior status provenance 校验、写入时机 + 旧值 carry-forward + clean-readiness-aware + `requires_clean_full_refresh` 状态位、合法性含 40-hex 格式校验）。U2 实施要点：
...
- Canonical artifacts 写入按 schema 层级：
  - `.spec-first/providers/<id>/status.json`（`provider-status.v1`，per-provider）顶层：
    ...
    - `requires_clean_full_refresh`（布尔；provider refresh process 已启动且失败时置 true，clean full 成功清除；dirty preflight 不因 dirty 本身置 true）
  ...
  - `.spec-first/graph/graph-facts.json`（`graph-facts.v1`）不新增 refresh-mode convenience 字段；consumer 需要判断 provider refresh mode 时读取 per-provider status 或 aggregate provider-status 的 provider 条目；不新增 `refresh_mode_summary`
  ...
  - **Artifact retention 写入路径（避免 helper 重写擦除 prior truth）**：
    ...
    - dirty preflight blocked 时，per-provider status、aggregate provider-status、graph-facts 与 normalized envelopes 都不覆盖，只返回 command-level diagnostic。
    - incremental + fallback full 都失败时，**跳过 normalized envelopes 重写**（`architecture-facts.json` / `impact-capabilities.json` 保持上一轮 readiness 通过内容），并 carry-forward canonical freshness fields：`source_revision`、`repo_snapshot.worktree_dirty`、`worktree_status_hash`、`generated_at` / `timing.*`、prior successful normalized envelopes。
    - 当前 attempt 的诊断事实仍要写入对应层级：both-failed 写 per-provider / aggregate status（`status=failed`、`graph_ready=false`、`query_ready=false`、`workflow_mode=degraded-fallback`）；dirty preflight 不写 per-provider / aggregate canonical artifacts，只在 `graph-bootstrap-result.v1` stdout / command result / optional report 返回 blocked diagnostic（`workflow_mode=blocked` + `overall_status=action-required` + `reason_code=dirty-refresh-non-canonical` + `canonical_artifacts_preserved=true`）。两者都不得让 failed/dirty attempt 看起来是当前 HEAD 的 clean query-ready readiness。
- Readiness 三级证据在 incremental 下仍必过；incremental command 成功但 query probe 失败时走现有 `query-unverified` 路径，不触发 fallback，写 `readiness_source=incremental-update` 作为 command source，但保持 `status=query-unverified` / `query_ready=false` 且不推进 `last_indexed_commit`。
- **Provider command failure recovery**：只有 provider refresh process 已启动且非零退出且最终没有 clean full readiness 时才写 `requires_clean_full_refresh=true`；incremental primary 失败但 fallback full 成功并通过 readiness 时，视为 clean full recovery，必须清除 `requires_clean_full_refresh=false`。preflight / shape gate / projection stale / missing command / base ref invalid / status-only / query probe 失败都不额外置 true。query-unverified 后的下一次 `--incremental` 由 R6 prior status provenance（`query_ready=false`）降级 full，`reason_code=incremental-base-status-untrusted`；clean successful full 后才恢复 incremental。
- **Dirty worktree 处理**：按 R7 执行——provider command 前 blocked，`reason_code=dirty-refresh-non-canonical`，不读取任何 opt-in flag（R7 无 opt-in 路径）；dirty run 不发布正常 ready/fresh canonical graph facts，也不覆盖 canonical graph artifacts；只在命令级 result / stdout / optional report 写 dirty-scoped diagnostic（`workflow_mode=blocked` + `overall_status=action-required` + `canonical_artifacts_preserved=true`）。
...
- Happy (R1 CLI flag 分流矩阵)：
  ...
  - (c) `--all-repos --incremental` → provider command 前 blocked，`reason_code=incremental-all-repos-unsupported`，断言未调用 provider command、未写本轮 raw log、未重写 canonical artifacts。
...
- Edge: query-unverified carry-forward — full 成功 → incremental command 成功但 query probe 失败 → per-provider status.json 的 `last_indexed_commit` 仍为**首次 full 时的 HEAD**（不被中间失败擦除），本轮写 `readiness_source=incremental-update` 作为 command source，但 `status=query-unverified` / `query_ready=false`；下次 `--incremental` 必须因 prior status provenance 不可信（`query_ready=false`）降级 full，`reason_code=incremental-base-status-untrusted`。
- Edge: fingerprint.spec_first.mcp_tools_hash 变化 → `full`，`reason_code=fingerprint-spec-first-changed`。
- Edge: dirty worktree + 任意 refresh flag/default → provider command 前 blocked，命令级 result / stdout / optional report 含 `workflow_mode=blocked` + `overall_status=action-required` + `reason_code=dirty-refresh-non-canonical` + `canonical_artifacts_preserved=true`；断言未调用 provider command、未写本轮 raw log、未重写 per-provider status、未重写 aggregate provider-status / graph-facts、未重写 normalized envelopes、未推进 `last_indexed_commit`、未因 dirty 本身设置 `requires_clean_full_refresh=true`。
- Edge (requires_clean_full_refresh 完整状态机 lifecycle，验证 provider failure recovery 能解锁)：
  - Step 1 — 从 clean worktree + 已跑通 full 的基线开始（`requires_clean_full_refresh=false`）。
  - Step 2 — clean worktree 下跑 `--incremental`，mock incremental primary 与 fallback full 两个 provider refresh process 均已启动且非零退出；断言 per-provider status.json 写入 `requires_clean_full_refresh=true`，`last_indexed_commit` **未**推进（保持 Step 1 基线值）。
  - Step 2 额外断言：both-failed run 不更新正常 ready/fresh canonical graph facts；aggregate/report 反映 degraded-fallback，canonical freshness 指针仍指向 Step 1 clean 成功值。
  - Step 3 — 保持 clean worktree，再跑 `--incremental` → preflight 见到 `requires_clean_full_refresh=true` 仍降级 full（`reason_code=clean-full-refresh-required`）；断言本次 full 成功且 `requires_clean_full_refresh` 被清为 `false`，`last_indexed_commit` 推进到当前 HEAD。
  - Step 4 — 再次跑 `--incremental` → 正常进 incremental（`refresh_mode=incremental`，`readiness_source=incremental-update`），证明 provider failure recovery 状态位能正确解锁。
...
- Aggregate provider-status: 两个 provider 一个 incremental 一个 fallback-full → `.spec-first/graph/provider-status.json.providers[]` 镜像 per-provider `refresh_mode` 差异；`graph-facts.v1` 不新增 refresh mode convenience 字段。
- Aggregate provider-status: 一个 provider incremental 成功 + 一个 provider incremental+full 都失败 → failed provider 条目 `refresh_mode=failed`；workflow_mode=degraded-fallback。
- Error: incremental 命令 exit != 0 → 重跑 full；若 fallback full 成功并通过 readiness，写 `fallback_from_incremental=true`、`readiness_source=incremental-fallback-full`、`reason_code=incremental-refresh-failed-fallback-full`，并清除 `requires_clean_full_refresh=false`，证明下一次 `--incremental` 不会被 `clean-full-refresh-required` 阻断。
- Error: incremental command 成功但 query probe 失败 → `readiness_source=incremental-update`、`status=query-unverified`、`query_ready=false`，不 fallback，不推进 `last_indexed_commit`。
...
- canonical artifacts 字段扩展符合 consumption contract（含 per-provider `refresh_mode`、`requires_clean_full_refresh`、`command_results[]` 扩展字段，并断言 `graph-facts.v1` 不新增 refresh-mode convenience 字段）。
- fingerprint 决策矩阵全分支在 contract test 覆盖（含 dirty-refresh-non-canonical blocked preflight、base-ref-unset、base-status-untrusted 新分支）。
- **query-unverified command source 测试**：incremental command 成功但 query probe 失败时，per-provider status 写 `readiness_source=incremental-update` + `status=query-unverified` + `query_ready=false`，且 `last_indexed_commit` 不推进；consumer test 断言只看 `readiness_source` 不能判定 readiness success；下一次 `--incremental` 必须按 prior status provenance 降级 full，`reason_code=incremental-base-status-untrusted`。
- **dirty canonical preservation 测试**：dirty preflight 后 per-provider status、aggregate provider-status、graph-facts 与 normalized envelopes 与 baseline byte-for-byte 不变；`overall_status=action-required` 只出现在 `graph-bootstrap-result.v1` stdout / command result / optional report，不进入 canonical graph artifacts。
...
- **requires_clean_full_refresh 状态机测试**：Step 2 provider refresh process 已启动且最终失败时置 `true` 且 `last_indexed_commit` 不推进；Step 3 clean full 清为 `false` 且推进 `last_indexed_commit`；Step 4 incremental 可启动；另测 incremental primary 失败但 fallback full 成功时立即清为 `false`，下一次 incremental 不被 `clean-full-refresh-required` 阻断。
...
**Verification:** parity 测试通过；核心常量（`refresh_mode`、`incremental-update`、`incremental-fallback-full`、`incremental-base-ref-missing`、`incremental-base-ref-unset`、`incremental-base-status-untrusted`、`incremental-base-ref-not-ancestor`、`dirty-refresh-non-canonical`、`incremental-all-repos-unsupported`、`DEFAULT_REFRESH_MODE_SINGLE_REPO`、`DEFAULT_REFRESH_MODE_ALL_REPOS`）在两个脚本中都存在；`command_shape_supported` 的 incremental kind 分支 parity 对齐；**R1 CLI flag 分流三档（`--all-repos` 命中 ALL_REPOS / 无 flag 命中 SINGLE_REPO / `--all-repos --incremental` blocked）parity 对齐**（env-driven child propagation parity 已 deferred）；`requires_clean_full_refresh` 状态机 lifecycle parity（provider refresh process started + 最终失败置 true → clean full 清 false → incremental 可启动；fallback full 成功立即清 false）；artifact retention parity 明确覆盖 dirty blocked preflight 不覆盖 canonical artifacts、both-failed skip normalized rewrite、aggregate provider-status / graph-facts carry-forward、`last_indexed_commit` clean success 才推进、query-unverified 保留 `readiness_source=incremental-update` 但不推进 readiness且下次 incremental 降级 full、以及不生成 `refresh_mode_summary` / `refresh_modes_by_provider`。
...
- Modify: `docs/contracts/graph-evidence-policy.md`（Refresh Trigger Policy 表下加一行 post-commit 运维建议；**必须的 guidance 文本**："commit / merge 之后 canonical `graph-facts.json` 可能 stale；consumer 可自动检测并 handoff。**clean single-repo** operator 在需要 query / review 且接受 expert opt-in 边界时可显式运行 `$spec-graph-bootstrap --incremental`，对齐 GitNexus notify-only stale detection + explicit `analyze` 模型。`--all-repos` 仍走 full 默认，`--all-repos --incremental` unsupported；dirty worktree 会在 provider command 前 blocked，不执行增量或全量 provider refresh；普通 workflow 不静默 rebuild provider index。"）
...
- Modify: `CHANGELOG.md`
- Modify: `docs/validation/2026-05-12-graph-bootstrap-incremental-benchmark.md`（U0a/U0b 第一节基础上追加完整 benchmark）
- Modify: `tests/benchmark/extract-graph-anchors.sh`（从 U0b node proof 扩展到 U4 node/edge anchor helper）
...
5. **Provider 原生 graph 提取（benchmark-only helper）**：normalized artifact（`architecture-facts.json` / `impact-capabilities.json`）当前是 readiness envelope，**不含**完整 node/edge facts。U0a 已证明 extraction surface，U0b 已创建 `tests/benchmark/extract-graph-anchors.sh` 的最小 node proof；U4 在此基础上扩展 edge / impact 输出（或记录 `N/A`）。
   ...
   **GitNexus incremental caveat**：官方 `analyze` 默认增量不是"只处理 diff 文件"的承诺；它可能仍遍历必要的全局信息，但通过 parse cache / changed-file storage update 避免 full rebuild 成本。因此 U4 speed gate 必须看实测 duration ratio 与 absolute duration，不能从 changed file count 或 `readiness_source=incremental-update` 推断速度收益。
   ...
   **U4.0 Preflight Gate（实施时机：U2/U3 完成后、U4 主体 step 1 之前；用于复核 U0a/U0b 已证明的 extraction surface 是否能通过正式 `--incremental` 路径工作。U0a/U0b 已经是 U1/U2 前的 blocker，U4.0 不再承担首次证明职责）**：
   ...
   - extraction surface provenance 是 U4.0 mandatory gate：每次提取必须记录 `repo_path`、`source_revision`、provider data dir 或 repo selector、CLI/MCP request metadata、输出文件路径；MCP surface 必须显式 target disposable temp clone，或返回可核验的 repo root / source revision。缺失 provenance 时该 provider 的 U4.0 结论只能是 `inconclusive`，不得用 stats-only / query probe 补位。
   - 确认每个 provider 有可行 extraction surface 能产出 **edge anchor 集合**。edge extraction 是 documented opt-in correctness pass 的 mandatory gate，因为 cross-file IMPORTS/CALLS drift 会直接影响 graph-heavy review / impact evidence。某 provider 的 edge anchor 无法从 CLI / MCP 提取 → 记录为 `extractable: no`，U4 validation 对该 provider fail / inconclusive；不得把 `--incremental` 作为 correctness-backed opt-in 对外记录。
   - 任一 enabled provider 的 node 或 edge extraction 不可用 → U4 validation inconclusive / fail；不得把 stats-only + query probe 标记为 opt-in correctness pass，`--incremental` 最多保持 experimental / undocumented。
...
6. **对照（拆成 node + edge 两组独立比对）**：
   ...
   - **比对 (iii) — Incremental edge capture**：`A_full_anchors.edges` vs `B_incremental_anchors.edges` 的对称差集 = step 2 预先列出的 A→B **edge delta**（IMPORTS/CALLS 级联更新——cross-file 场景的关键信号）
   - **比对 (iv) — Incremental edge equivalence**：`B_incremental_anchors.edges` vs `B_full_anchors.edges` 的对称差集 = **∅**

   四个比对相互独立；node 与 edge 四项对每个 enabled provider 都必须通过，否则 validation fail / inconclusive。stats-only / query-probe-only 弱证据不能让 opt-in correctness pass 成立。
...
7. **Per-provider pass matrix**：
   ...
   |---|---|---|---|---|---|---|---|
   | gitnexus | 必过 | 必过 | 必过 | 必过 | 必过 | 必过 | minimum speed-benefit gate |
   | code-review-graph | 必过 | 必过 | 必过 | 必过 | 必过 | 必过 | minimum speed-benefit gate |

   - **Correctness gate（阻断 documented opt-in）**：每个 enabled provider 必须具备 node capture + node equivalence + edge capture + edge equivalence 的 anchor-level 证据，`readiness_source=incremental-update` 且 query probe no degradation；若 node 或 edge extraction 不可用，validation 失败或 inconclusive，不能以 meta stats / query probe 通过 opt-in correctness。
   ...
   - **Minimum speed-benefit gate（阻断 documented opt-in）**：每个 enabled provider 的 `Duration ratio (B_incr ≤ B_full / 3)` 必须通过。若 correctness gate 通过但最低速度收益失败，`--incremental` 最多保持 validation / experimental，不作为 documented expert opt-in 对外记录。
   - **Speed claim gate（只决定是否宣传 ~3-5 秒）**：minimum speed-benefit gate 与 **所有 provider 累计** `B_incremental_total_duration ≤ 5s` 同时通过时，validation doc 才可写"达到 ~3-5 秒速度目标"；若最低速度收益通过但 absolute ≤5s 失败，`--incremental` 可作为 documented expert opt-in，但 README / 用户手册不得宣传 3-5 秒速度收益，只能记录实测耗时与 caveat。

8. **Rollout 决策**：本计划 U4 的角色是 **explicit operator escape hatch / expert opt-in 正确性基线**（incremental 开关功能可用、对一组合成 diff 表现等价）；
   - **不**自动触发默认切换。Correctness gate + minimum speed-benefit gate 全绿 → incremental flag 可作为 documented operator opt-in 上线，Plan 视为完成；speed claim gate 只控制是否宣传 ~3-5 秒。
   ...
   - **2026-05-14 closure revision**：本次实施后的 corrected runs 满足 diagnostic-only correctness 证据，但 minimum speed-benefit gate 失败；因此 Plan 以 diagnostic-only posture 完成，不把 `--incremental` 上线为 documented speed-backed opt-in。
...
- validation doc 必须包含 `redaction_checklist_completed: yes`，并通过最小 lint / checklist：禁止 credentialed URL、token/API key、`Authorization:`、`Cookie:`、私有 registry URL、用户名绝对路径片段和大段 fenced stdout/stderr；该 lint 只覆盖 tracked validation docs，不替代后续统一 redaction helper。
- CHANGELOG 按仓库现行格式登记。
...
- **Canonical artifact 扩展：** `readiness_source` enum 增加 2 个值；`refresh_mode`（值域 `{full, incremental, incremental-fallback-full, failed}`）/ `fallback_from_incremental` / `last_indexed_commit` / `requires_clean_full_refresh` 为新可选字段（per-provider 顶层）；aggregate `provider-status.json.providers[]` 可镜像 per-provider 字段；现有 `command_results[]` 元素扩展可选 `refresh_mode` + `attempt_role`（不引入独立 `bootstrap_attempts[]`，避免 schema double ledger）；`graph-facts.v1` 不新增 `refresh_mode` / `refresh_modes_by_provider` / `refresh_mode_summary`。`readiness_source=incremental-update` 是 command source，不是 readiness success；consumer 必须继续检查 `status=ready && query_ready=true`。所有现有 consumer 对 `readiness_source` 无硬分支，新值透明。
- **Error propagation：** incremental 命令失败自动 fallback full；fallback full 也失败按现有 `workflow_mode=degraded-fallback` 处理；dirty preflight 固定 command-level machine-readable combo 为 `workflow_mode=blocked` + `overall_status=action-required` + `reason_code=dirty-refresh-non-canonical` + `canonical_artifacts_preserved=true`，且不执行 provider command；`--all-repos --incremental` 固定 command-level unsupported-scope diagnostic，`reason_code=incremental-all-repos-unsupported`，且不执行 provider command。`overall_status` 只属于 `graph-bootstrap-result.v1` stdout / command result / optional report，不进入 canonical `.spec-first/graph/provider-status.json` 或 `.spec-first/graph/graph-facts.json`；canonical 判断仍靠 `workflow_mode`、`status`、`query_ready`、`source_revision`、`worktree_dirty`、`worktree_status_hash`。**不新增 workflow-level error category**（workflow_mode 仍是现有 `primary` / `degraded-fallback` / `blocked` / `no-source`）；**新增若干 incremental 相关 reason_code**（`incremental-refresh-failed-fallback-full`、`incremental-base-ref-missing`、`incremental-base-ref-unset`、`incremental-base-status-untrusted`、`incremental-base-ref-invalid-format`、`incremental-base-ref-not-ancestor`、`incremental-and-full-failed`、`incremental-command-unavailable`、`incremental-all-repos-unsupported`、`dirty-refresh-non-canonical`、`clean-full-refresh-required`、`fingerprint-spec-first-changed`、`fingerprint-projection-changed`、`fingerprint-provider-changed`），由 consumption 契约 + contract test 扩展声明（不新增 JSON schema 文件，沿用 prose + Jest 验证模式）。
...
| Risk | Mitigation |
|---|---|
| `code-review-graph update --base` 在 pinned 2.3.3 下不可用或语义不等价 | U0a/U0b 在 U1/U2 前验证 provider-native A→B→C node chain；必要时走 `2026-05-09-005` pin 升级流程或只对 GitNexus 启用 incremental。 |
| GitNexus `analyze`（不带 `--force`）实际退化为全量 | benchmark 核验耗时；如耗时未显著下降，保留 full 默认；`readiness_source=incremental-update` 的字段语义已在 R9 限定为"incremental 命令形式 / command source"，readiness success 仍由 `status=ready && query_ready=true` 判断，且不声称 provider 内部一定做了 graph diff。 |
...
| `--all-repos` 多仓场景未经 benchmark 验证 | 本计划保持 `DEFAULT_REFRESH_MODE_ALL_REPOS=full`；显式 `--all-repos --incremental` 在 provider command 前 blocked，`reason_code=incremental-all-repos-unsupported`。`--all-repos` incremental default switch 需要独立多仓 validation；`--incremental` 只是 clean single-repo explicit operator escape hatch，不是未验证多仓默认切换的 mitigation。 |
...
| dirty worktree 下执行 provider refresh 会把未提交内容写入 native storage，造成 canonical freshness 与索引内容错位 | R7 修复：dirty 任何 refresh request 都在 provider command 前 blocked，不执行 incremental/full provider command，不推进 last_indexed_commit，不因 dirty 本身设置 requires_clean_full_refresh；只写 blocked/action-required diagnostic 并 carry-forward clean freshness 指针。 |
| 未来若把 post-commit / post-merge hook 从 notify-only 升级为直接 provider rebuild，可能阻塞提交路径、超时、中途被 kill，留下不一致 provider storage | 本计划不做 hook-triggered rebuild；follow-up 只能先设计 notify-only stale detector。任何真正 auto-refresh policy 必须单独规划 repo lock、timeout/cancellation、failure circuit breaker、audit artifacts 与 explicit opt-in。 |
...
| 混合失败语义：GitNexus 成功 / CRG 全失败时 aggregate graph-facts 若新增 summary 会压扁 per-provider truth | R10 不在 `graph-facts.v1` 新增 refresh-mode convenience 字段；consumer 需要精确判断时回读 per-provider status 或 aggregate provider-status 的 provider 条目。 |
...
- **Rollout 范围（critical 边界）：** 本计划最终交付 **clean single-repo diagnostic / validation-only `--incremental` path**。U4 benchmark 证明 incremental 在合成 diff 上可与 full 等价，但未证明稳定速度收益；普通 workflow 用户不会自动获得 post-commit 加速，README / 核心用户手册不宣传该入口。**Speed-backed documented opt-in** 与 **Default 切换到 incremental** 都需独立 follow-up rollout plan（覆盖 A→B→C→D 连续增量链 / 文件 rename / parser degradation fuzz / 长 commit 链累积误差等更宽 gate）。`DEFAULT_REFRESH_MODE_*` 在本计划范围内保持 `full` 不变。
- **GitNexus 官方对齐：** GitNexus 本地 CLI 的 stale recovery 是显式运行 `npx gitnexus analyze`；Claude Code hook 是 post-commit / post-merge notify-only stale detection，不直接重建索引。本计划对应为 consumer 自动 freshness-check / handoff，真正 refresh 仍归 `$spec-graph-bootstrap`。
- **体验闭环顺序：** 本计划完成的是 L1 的 diagnostic-only 骨架：显式 `$spec-graph-bootstrap --incremental` 可用于验证和 provider 行为调查，但尚未晋级为 speed-backed operator opt-in。L2 notify-only stale detector 仍是下一层体验增强；只有 speed-backed opt-in 与 L2 都稳定后，才值得讨论 L3 clean-only auto-refresh opt-in policy；CI / scheduled refresh 属 L4 独立治理。
- **速度宣传边界：** corrected U4 证明的是合成 diff 上的等价性，不证明稳定速度收益。README / 用户手册不得宣传 `--incremental` 或 ~3-5 秒收益；speed-backed documented opt-in 需要 future validation 重新满足 minimum speed-benefit gate，speed claim 仍需同时满足 duration ratio 与 `≤5s`。
...
- **用户沟通：** README / 用户手册不宣传 `--incremental`。diagnostic-only 边界、fallback 处理、redaction checklist 和 repeated-chain caveats 下沉到 graph-bootstrap 专项 validation / troubleshooting 或 consumption contract。`$spec-graph-bootstrap --help` 含 flag 文档，但不暗示普通 workflow 自动采用增量刷新。
...
- **External docs / official provider references：**
  - code-review-graph `/tirth8205/code-review-graph` — `update` / `build`
  - GitNexus `/abhigyanpatwari/gitnexus` — `analyze` / `analyze --force`
  - GitNexus `AGENTS.md` — `analyze` / `--force`、Keeping the Index Fresh、hook notify-only 语义：<https://github.com/abhigyanpatwari/GitNexus/blob/main/AGENTS.md>
  - GitNexus `RUNBOOK.md` — stale index 修复路径 `npx gitnexus analyze`，必要时 `--force`：<https://github.com/abhigyanpatwari/GitNexus/blob/main/RUNBOOK.md>
  - GitNexus PR #205 — post-commit / merge stale detection 从直接 reindex 收敛为 notify-only：<https://github.com/abhigyanpatwari/GitNexus/pull/205>
- **实施证据：**
