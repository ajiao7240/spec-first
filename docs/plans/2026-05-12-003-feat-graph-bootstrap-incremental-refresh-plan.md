...
- R10. Aggregate `graph-facts.v1` 顶层**不使用单值 `refresh_mode`**（mixed provider 场景会被压扁）。改为：
  ...
  - 可选 `refresh_mode_summary ∈ { full, incremental, mixed }`（**计算规则**：所有 provider 值相同 → 该值；任一不同或存在 `failed` → `mixed`。consumer 读到 `mixed` 必须回 per-provider 读精确事实）
...
- 不改变 provider 命令执行的 env 传递、secret redaction 现状（existing argv positions）。Raw log 路径在 incremental 场景下**新增** `update.log` / `fallback-analyze.log`（GitNexus fallback） / `fallback-build.log`（CRG fallback）；现有 `analyze.log` / `build.log` / `status.log` / `query.log` 路径不变；新 log 继承现有 log 的 redaction 现状（当前 provider 调用不引入 secret 源，argv 不携带 credentials）；provider id allowlist（`bootstrap-providers.sh` L726-729 的 `gitnexus|code-review-graph`）保证日志路径安全。**incremental 场景新增的 argv position（`--base <sha>`）携带 `.spec-first/providers/<id>/status.json.last_indexed_commit` 值，属于新增 user-influenced 攻击面，由 R6 的 40-hex 格式校验治理，不在既有 env / redaction 边界内**。未来若发现 provider 输出含 secret，统一 redaction helper 作为独立 security plan 处理（扩大 scope，非本计划）。
...
- `src/cli/contracts/graph/provider-status.v1.schema.json`（或等价路径）—— schema 扩展点。
...
spec-graph-bootstrap [--incremental | --full | (default)]
  ...
  │    full 也失败?
  │      → workflow_mode=degraded；保留上一次 artifacts
  │
  └─ Phase 4: 写 canonical artifacts
       ...
       per-provider refresh_mode ∈ {full, incremental}
...
**Files:**
- Modify: `skills/spec-mcp-setup/mcp-tools.json`（新增 incremental 命令模板，见 A3）
- Modify: `skills/spec-mcp-setup/scripts/write-provider-config.sh`（`provider_commands` 函数 ~L580 扩展 `incremental` argv）
- Modify: `skills/spec-mcp-setup/scripts/write-provider-config.ps1`（PowerShell parity）
- Modify: `src/cli/contracts/graph-providers.v1.schema.json`（或等价）：`commands` 新增可选 `incremental` 字段
- Test: `tests/unit/mcp-setup.sh`（扩展 L1102 区域 "provider commands are config-defined arrays" 断言，加 `commands.incremental` argv 精确形状）
- Test (new schema/doc layer only): 若需 Jest 覆盖 schema enum 与 readiness_source 枚举，放入 `tests/unit/graph-provider-consumption-contracts.test.js`，不新增 Jest 驱动的 bash 行为测试

**Approach:**
- `mcp-tools.json` 新增：
  - gitnexus: `["npx","-y","<package>","analyze"]`（无 `--force`）
  - code-review-graph: `["uvx","<package>","update","--base","<ASCII sentinel>"]`（sentinel 原样写入，不展开）
...
- **sentinel 运行时替换与合法性校验是 U2 的职责**，U1 不涉及；U1 只保证 projection 把 incremental argv 正确落地。
- schema `commands.incremental` 为可选字段，老 runtime 向后兼容。
...
- **扩展 `command_shape_supported()` 与校验循环**（关键实施约束）：
  - 当前 `bootstrap-providers.sh` L638-699 的 `command_shape_supported()` jq 函数仅覆盖 `bootstrap / status / query_probe` 三个 kind；L732-738 校验循环同样硬编码这三个 kind。本 unit 必须同时扩展两处，加入 `incremental` kind 的精确 argv shape：
  ...
  - **jq alternation 规范（关键细节，现有 crg_shape 用 exact equality tail 匹配，必须扩展为 alternation）**：
    ```jq
  ...
  - 未扩展两处任一即视为实施失败；contract test 必须断言扩展后的 shape gate 拒绝非法 incremental argv（例如 `bash -c` / 未知 subcommand / 含 shell metacharacters / 非 hex 的 base 值）。
...
- **并发保护**（A1）：同一仓库并发跑 bootstrap（如用户两个 terminal / spec-work 间接触发）会互相覆盖 `status.json`。本 unit 加 `flock -n "$PROVIDERS_DIR/<id>/.bootstrap.lock"` 保护每个 provider 的写入临界区；获取锁失败返回 `reason_code=bootstrap-concurrent-run`，不执行 provider 命令。PowerShell parity（U3）用 `System.Threading.Mutex`。
- **Raw log 路径扩展与 `bootstrap_attempts[]` 契约**（锁定 array 方式，避免同 kind 覆盖）：
  ...
  - 新增路径：`update.log`（CRG incremental 的 update 命令输出）、`fallback-build.log`（incremental 失败后 fallback full 的 build / analyze 输出）
  ...
  - 必须满足：incremental 失败 + fallback full 成功时，`bootstrap_attempts[]` 保留两条，`update.log` 与 `fallback-build.log` 都可追溯不互相覆盖。
...
- Raw log: incremental 失败后 fallback full 成功 → `bootstrap_attempts[]` 保留两条（incremental + fallback），两个 attempt 的 `raw_log` 分别指向 `update.log` 与 `fallback-build.log`；`raw_logs` 仅保持现有 backward-compat latest-kind alias，不新增 `raw_logs.incremental` / `raw_logs.fallback` 子键。
...
- Aggregate: 一个 provider incremental 成功 + 一个 provider incremental+full 都失败 → `refresh_modes_by_provider.<failed-id>=failed`；`refresh_mode_summary=mixed`；workflow_mode=degraded。
...
- Error: incremental + full 都失败 → `workflow_mode=degraded`，`reason_code=incremental-and-full-failed`。
...
5. **对比 B_incremental vs B_full**（**语义 anchor 归一化**，不用 provider 内部 node id——后者可能含 hash/timestamp/run-uuid 等 provider 实现细节，每次 build 不稳定）：
   - **Node 归一化为 `(repo_relative_path, symbol_kind, name, start_line, end_line)`**；两次运行的 node anchor 集合对称差集 = 预期的 A→B 符号变更
   - **Edge 归一化为 `(relation_type, from_anchor, to_anchor)`**；两次运行的 edge anchor 集合对称差集 = 预期的 import/call 级联
   - **Query / impact 结果**比对 `result_class` + 结果集合的 anchor 归一化，不比对 provider 内部 id
   - Readiness 三级证据两次都满足
   - Flow / community count 允许抖动（±1% 或实测 baseline 后定的阈值）
   - B_incremental 耗时 ≤ B_full 的 1/3（目标值；阈值实测后写入 validation doc）
6. **Rollout 决策**：
   - 三类 diff 全部通过 → 独立单行 PR 把 `DEFAULT_REFRESH_MODE_SINGLE_REPO` 从 `full` 改为 `incremental`；`DEFAULT_REFRESH_MODE_ALL_REPOS` 保持 `full`（R1）
   - 任一类 diff（尤其 (b) cross-file）不通过 → 保留 `full` 默认，validation doc 记录失败模式，incremental 保持 opt-in
...
- **Error propagation：** incremental 失败自动 fallback full；full 失败按现有 `workflow_mode=degraded` 处理。**不新增 workflow-level error category**（workflow_mode 仍是现有 `primary` / `degraded-fallback` / `blocked` / `no-source` / `action-required`）；**新增若干 incremental 相关 reason_code**（`incremental-refresh-failed-fallback-full`、`incremental-base-ref-missing`、`incremental-base-ref-unset`、`incremental-base-ref-invalid-format`、`incremental-base-ref-not-ancestor`、`incremental-and-full-failed`、`dirty-worktree-incremental-not-allowed`、`bootstrap-concurrent-run`、`fingerprint-spec-first-changed`、`fingerprint-projection-changed`、`fingerprint-provider-changed`），由 consumption 契约 + contract test 扩展声明（不新增 JSON schema 文件，沿用 prose + Jest 验证模式）。
...
| 与 `2026-05-09-003` fast-reuse 的 `resolve_refresh_mode` ownership 冲突 | U0 前置决策节点显式 resolve，进入 U1/U2 依赖链。 |
...
| incremental 失败后 raw log 按 kind 覆盖，审计证据丢失 | U2 引入 `bootstrap_attempts[]` 与独立 log 路径（`update.log` / `fallback-build.log`）；Scope Boundaries 明确松绑该约束。 |
