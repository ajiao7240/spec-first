...
按 `/spec:doc-review` 综合 reviewer 反馈(spec-feasibility-reviewer / spec-adversarial-document-reviewer / spec-product-lens-reviewer)+ **审查报告 §8 v1 校准吸收**,把 origin §6 9 项 finding **再次收敛为 Phase A / Phase B 两阶段执行**——避免 6 unit 单 plan 执行触发对抗反例 + 评分焦虑驱动 + 低 ROI 路径风险。
...
- **Phase B**(defer 到后续 batch review,**不在本 plan ship-now scope**):U3(Key Principles 部分删 + **Cache-Friendly 改名 Context Handoff Layout 而非删除**,§8 校准修订)+ U5(description 改写)+ U6(拆 references)。与其他 5 核心 skill batch 跑新版 `审查skill.md`(含 Step 1.5)结果一起进入统一治理 plan,杠杆 ≥5x 单 skill 深修
...
**Goal**:消除 fresh-source eval 加载 examples.json 第 5 例时找不到 owner anchor 的误报路径(审查报告 §8.1 校准:owner 真实存在于邻近 skill / contract,不是 stale governance,而是 ownership 文档化缺失)
...
**审查报告校准**(§8.1):v0 评估 "local bundle grep 0 命中" 是范围错误(只扫 `skills/spec-work/`)。全仓 grep(含 src/cli/contracts/ + 邻近 skill source)证明 owner 真实存在,**不是 stale governance**——是 ownership 文档化在邻近 skill 而非 spec-work 私有。删除第 5 例是因 spec-work 不实现 staging/security engine,边界由 owner skill 维护;不是因 owner 不存在。
...
**Dependencies**:**U2**(若 U2 在 §Workflow Contract Summary 末尾加了自指否定句,U7 可放 U2 之后或之前——逻辑上 U7 是 H3 子标题段,U2 是末尾 1 行 prose,两者位置不冲突且语义互补:U2 声明 "不实现 staging/security engine"(自指否定),U7 声明 "orchestrates execution + LLM decides ..."(自指肯定 + 工具分工))

**Files**(repo-relative):
- `skills/spec-work/SKILL.md`(修改:§Workflow Contract Summary 末尾后,§Examples As Context 之前,加 ~5 行 H3 子段)

**Approach**:按 v1 §6.2 P1-001-light-boundary + 审查报告 §0/§8.2——加 3-6 行轻量 Execution Boundary 段:
...
- U1 / U2 / U7 不依赖 U4 的行号稳定(它们改的是 frontmatter / Workflow Contract Summary / examples.json,不涉及 Phase 2 / Phase 0 段)
...
| 触发条件 | 回滚动作 |
|---|---|
| advisory 信号偏低 且诊断为 U7 light-boundary 段触发 cross-skill anchor(尽管严格控制,仍有残余风险) | 回滚 U7 单元:删除 §Workflow Contract Summary 后的 §Execution Boundary H3 段;保留 U2 自指否定句;U1/U4 不回滚 |
| advisory 信号偏低 且诊断为 U2 声明触发 cross-skill stale anchor | 回滚 U2 单元:删除自指否定句;保留 U7(light-boundary 段语义独立);U1/U4 不回滚 |
| advisory 信号明显偏低 且诊断为多 unit 互相干扰 | 同时回滚 U2 + U7(保留 U1 + U4)。U1 回滚需重新添加 examples.json 第 5 例(从 git history `git show HEAD~N:skills/spec-work/evals/examples.json` 恢复) |
...
- [ ] User 确认 §8 校准吸收(U7-light-boundary 进 Phase A + U3 Cache-Friendly 改名而非删除 + U1 Approach owner mapping)
