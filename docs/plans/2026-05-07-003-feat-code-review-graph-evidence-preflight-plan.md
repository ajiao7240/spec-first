...
- 将 CRG evidence 注入 reviewer context，而不是新增一个 CRG reviewer agent。
...
- D1. 增加 preflight stage，而不是 agent。`code-review-graph` 仍然是 provider/tool；orchestrator 收集它的 evidence。
...
- 风险：新增 CRG agent 与 reviewer personas 重复。
  - 缓解：不新增 agent；由 orchestrator 拥有 evidence block。
...
实施者应从 U1 和 U2 开始，因为它们定义 durable review-stage boundary。随后通过 U3 和 U4 把 evidence 串入 reviewer context 与 reporting。U5 默认应保持 no-op，除非实施时发现当前 setup/bootstrap docs 会误导新的 downstream behavior。
