...
- R10. 用户级同步失败必须显式报告；不得静默吞掉权限、global developer profile 或用户 instruction 文件写入失败。
...
- R15. Codex 用户级同步必须检测 effective Codex home 下非空 `AGENTS.override.md` 对 `AGENTS.md` 的遮蔽；本计划不写 override 文件，因此 override 活跃时不得宣称 Codex 用户级语言 guidance 已生效。
- R16. Claude Code v1 用户级目标选择必须显式记录 `~/.claude/CLAUDE.md` 与 `~/.claude/rules/` 的取舍；如选择前者，必须说明这是当前版本的 deliberate target choice，而不是否认 user-level rules surface 存在。

---

## Assumptions

- A1. Claude Code 的用户级自然语言规则入口按当前官方文档可使用 `~/.claude/CLAUDE.md`，同时官方文档也提供 `~/.claude/rules/` 作为 user-level rules surface。本计划 v1 deliberate choice 是写 `~/.claude/CLAUDE.md`：它在官方表格中对应 "User instructions"、每个 session 加载，且最接近单个 language-only managed block 的维护模型；不在本轮写 `~/.claude/rules/spec-first-language.md`，避免同时维护两个 Claude 用户级 instruction surface。实现必须通过单一解析 helper（建议 `resolveClaudeUserInstructionPath()`）得到最终写入路径，不能在调用点硬编码 `os.homedir()/.claude`；v1 helper 固定返回 `{ absolutePath: ~/.claude/CLAUDE.md, displayPath: "~/.claude/CLAUDE.md", basis: "claude-user-instructions-claude-md-v1" }`。当前 docs/help/tests 不承诺 `CLAUDE_CONFIG_DIR` 会重定位 user memory / `CLAUDE.md`；若未来 Claude Code 官方文档或实测行为表明 rules 文件更适合 machine-maintained personal preference block，再通过单独 plan/ADR 扩展或迁移 helper 与测试。
...
- Codex 路径遵守 `CODEX_HOME`，并在非空 `AGENTS.override.md` 遮蔽时报告 action-required 而非宣称生效；Claude 路径由 `resolveClaudeUserInstructionPath()` 统一解析到 `~/.claude/CLAUDE.md`，并在测试中锁定 `basis: "claude-user-instructions-claude-md-v1"`。
...
- key_findings:
  ...
  - `resolveGlobalDeveloperWriteAction` / `writeGlobalDeveloperFile` provide existing global profile merge/write primitives, but `sync_user_language` persistence must be applied by the run-level user sync aggregate rather than repeated from parent/child project plans.
...
- Codex official docs identify `AGENTS.md` as the guidance surface for Codex, with user-level global guidance under Codex home; they also state `AGENTS.override.md` takes precedence over `AGENTS.md` at the same global level, so an active override can shadow this plan's `AGENTS.md` target.
- Claude Code official memory docs identify `CLAUDE.md` as the user/project instruction surface and describe memory files as behavioral guidance rather than hard enforcement; the same docs also describe `.claude/rules/` and `~/.claude/rules/` as rules surfaces, which this v1 plan deliberately does not target.
...
- KTD9. **Separate marker upsert from marker removal with pair-aware corruption handling.** Writing a managed block and deleting a managed block have different absence semantics: upsert appends when markers are absent, while removal is a no-op when markers are absent. Both helpers must operate only on complete, well-formed marker pairs. A lone start/end marker is treated as user-owned corrupted text and preserved byte-for-byte; upsert appends one fresh complete managed block, and removal leaves the corrupted text untouched while returning a diagnostic such as `corrupted-marker-preserved`. Do not encode both behaviors in a single ambiguous helper whose no-marker behavior depends on caller assumptions, and do not try to "repair" corrupted markers by deleting content between an orphan marker and a later valid block.
- KTD10. **Model user-language sync as one run-level aggregate plan.** `userLanguageSyncPlan` belongs to the current `spec-first init` invocation, not to a repo-local `operationPlan`, child project plan, or individual host/project plan. The plan contains per-host operations (`codex` / `claude` write, remove, noop, skipped, failed) plus the single global developer profile sync-preference write when the run changes or must persist `sync_user_language`. It is built once after language/host/preference resolution, is applied once at run level, and owns the user-sync preview/output/exit-code summary. This avoids repeated all-repos writes and prevents global cleanup/profile-persistence failures from being hidden inside a local project success.
...
- KTD14. **Detect Codex global override shadowing.** Codex enable/maintain targets remain `$CODEX_HOME/AGENTS.md`, not `AGENTS.override.md`. If `$CODEX_HOME/AGENTS.override.md` exists and is non-empty, the operation may still preview/maintain the managed block in `AGENTS.md`, but the per-host result must be `action-required` with reason code `codex-global-override-active`; output must say Codex will read the override first and must not claim the new language block is active until the override is removed or reconciled.
- KTD15. **Choose Claude `CLAUDE.md` for v1, not rules files.** Claude v1 writes the single language-only managed block to `~/.claude/CLAUDE.md` because the official docs label it user instructions and it matches the existing project `CLAUDE.md` block model. `~/.claude/rules/` remains a documented alternative for future migration if user-level rule files prove a better fit; this plan must not write both surfaces in one release.
- KTD16. **Profile persistence is part of sync completion.** A run may not report user-language sync enabled/disabled as complete unless the relevant global developer profile write and user-file aggregate both reached their intended state. The sync-preference write must be applied by the same top-level run-level user sync executor as the user-file aggregate, not by `applyProjectInitPlan`, `parentPlan`, `childPlans`, or any per-platform project plan. If user files succeed but profile persistence fails, report non-zero with `user-language-profile-write-failed`; if cleanup succeeds but persisting `sync_user_language=false` fails, do not claim the opt-out preference is stored because a later run could otherwise re-enable from the stale profile.
...
- Should Codex sync write `AGENTS.override.md` when an override exists? No. This plan owns `AGENTS.md` only; an active override becomes an action-required shadowing diagnostic, not a second write target.
- Should Claude v1 use `~/.claude/CLAUDE.md` or `~/.claude/rules/`? Use `~/.claude/CLAUDE.md` for v1 because it is the documented User instructions location and matches the single managed-block model; keep rules files as a future documented alternative.
...
flowchart TB
  ...
  H --> K[build repo-local init plans]
  F --> M[build one run-level all-host removal plan]
  R --> M
  J --> K
  M --> K
  K --> L[dry-run preview or apply repo-local plans, then user aggregate once at run level]
  L -->|success or no-op| O[done]
...
- Generalize the existing marker-replacement logic into two marker-pair-parameterized helpers with explicit, non-overlapping semantics:
  - `upsertMarkerBlock(existing, block, startMarker, endMarker)`: replace one complete well-formed start/end pair; append a fresh complete block when no complete pair exists; preserve all bytes outside the replaced/appended region.
  - `removeMarkerBlock(existing, startMarker, endMarker)`: remove only one complete well-formed managed block; return input byte-for-byte unchanged when no complete pair exists.
- Implement marker handling with a pair-aware scan, not independent `indexOf(start)` / `indexOf(end)` lookups. If content contains a lone start marker or lone end marker, treat that marker as ordinary user text, preserve it byte-for-byte, and return a non-fatal diagnostic such as `corrupted-marker-preserved`. The idempotency invariant is "exactly one complete spec-first user-language managed block after repeated upserts", not "exactly one start marker in the whole file"; orphan marker text may remain until a human removes it.
...
- Edge case: corrupted user-language start marker without end marker appends a fresh complete block without deleting user content; a subsequent write must replace the complete managed block while preserving the orphan start marker as user text. The assertion is exactly one complete user-language block, not exactly one start marker in the whole file.
- Edge case: corrupted marker text plus a later complete user-language block -> upsert replaces only the complete block and preserves corrupted marker text before/after it; removal deletes only the complete block and preserves corrupted marker text.
...
- Modify: `src/cli/commands/init.js` (the profile merge/branch-preservation logic prescribed below lives here, but sync-preference persistence is applied by the run-level `userLanguageSyncPlan`, not by per-project `globalDeveloperWrite`)
...
- Split global profile persistence into two responsibilities:
  - existing project/profile identity writes (`name`, `lang`, `hosts`, `initialized_at`, `version`) may continue through the existing `globalDeveloperWrite` path;
  - `sync_user_language` persistence is owned by the run-level user sync aggregate and must be planned/applied once per `spec-first init` invocation, even in dual-host or all-repos runs.
- Update global profile merge logic so an explicit sync preference change produces a run-level sync-preference profile operation even when `name`, `lang`, and `hosts` are unchanged. Host-only identity updates must preserve the existing `sync_user_language` value, and explicit name/lang identity overwrites must preserve or update it according to the run-level user sync result without letting a later project plan erase the just-persisted sync preference.
- Treat interactive consent (the one-time prompt answered yes/no) as an explicit preference change for the run-level sync-preference profile operation, not only the `--sync-user-language` / `--no-sync-user-language` flags — otherwise re-init that grants consent can fall through the current per-project `preserve` branch and never persist.
...
- Integration: existing profile plus unchanged `name`/`lang`/`hosts` still writes the run-level sync-preference profile operation when the run passes `--sync-user-language` or `--no-sync-user-language`.
- Integration: host-only profile updates preserve the stored `sync_user_language` value.
- Integration: dual-host and all-repos init apply the sync-preference profile operation at most once total; parent/child project plans must not each write `sync_user_language`.
...
- Codex target resolution must also inspect `$CODEX_HOME/AGENTS.override.md`. When that file exists and is non-empty, `AGENTS.md` is shadowed by Codex's documented discovery order. Do not write or modify `AGENTS.override.md`; instead mark the Codex operation `action-required` / `codex-global-override-active`, include the override display path in diagnostics, and ensure preview/apply output does not claim Codex will load the `spec-first:user-language` block. The implementation may still maintain the managed block in `AGENTS.md` for future use after the user removes/reconciles the override, but the host-level result remains non-success until the shadowing condition is gone.
...
- Claude target resolution must not hardcode `os.homedir()/.claude` at call sites; resolve through a single `resolveClaudeUserInstructionPath()` helper in `src/cli/helpers/global-config-dir.js` or `src/cli/user-language-sync.js`. The helper is a firm requirement of U4, not optional, and returns at least `{ absolutePath, displayPath, basis }` so docs/output/tests can consume the same decision. For v1, official Claude Code docs identify `~/.claude/CLAUDE.md` as User instructions and `~/.claude/rules/` as user-level rules; this plan deliberately chooses `~/.claude/CLAUDE.md` and records `basis: claude-user-instructions-claude-md-v1`. Treat `CLAUDE_CONFIG_DIR` as out of scope for user memory / `CLAUDE.md` until official docs or implementation verification proves otherwise.
...
- The run-level plan should also contain the sync-preference profile operation when the run needs to persist `sync_user_language=true|false`. This operation reads/merges the current global developer profile once, preserves identity fields and `hosts`, and is reported with the same run-level result as the user-file operations.
...
- Apply order: apply repo-local project/runtime plans through the existing flow; apply the user-language aggregate exactly once from the top-level init runner, never from `applyProjectInitPlan` or per-child workspace logic. The aggregate owns both user-file operations and the sync-preference profile operation, so a run-level result can fail loud when either side fails. For explicit opt-out/removal flows, avoid the inconsistent state where `sync_user_language=false` is persisted but old blocks remain active: either apply the all-host removal plan before persisting profile false, or keep a stored-false residual-cleanup mechanism that reruns removal-only cleanup on later init attempts until no block remains.
...
- Treat global developer profile persistence failures as user-language sync failures when the current run changed or relied on `sync_user_language`. The top-level run result must carry a non-zero exit code and a reason code such as `user-language-profile-write-failed`; diagnostics must distinguish "user file block written/removed" from "preference persisted".
...
- Error path: Codex sync with a non-empty temp `CODEX_HOME/AGENTS.override.md` reports `codex-global-override-active`, does not modify the override file, and does not claim the `AGENTS.md` managed block is active guidance; apply mode returns non-zero even if repo-local init succeeds.
- Dry-run: Codex sync with a non-empty `AGENTS.override.md` previews the shadowing diagnostic and writes no user files/profile.
...
- Regression: `CLAUDE_CONFIG_DIR` does not affect v1 Claude user instruction targeting; the helper still returns `~/.claude/CLAUDE.md` with `basis: "claude-user-instructions-claude-md-v1"` unless a future documented change deliberately updates this contract.
...
- Edge case: all-repos init with parent plus two child repos and an explicit sync preference change -> the global developer profile `sync_user_language` write is called once total, not once per project plan.
...
- Error path: profile write failure after successful user-file writes returns `user-language-profile-write-failed`, exits non-zero, and does not claim sync enabled/disabled is complete.
...
- If the sync-preference profile operation fails, render it in the same output section as user-file sync failures with `user-language-profile-write-failed`; do not let a successful per-project `globalDeveloperWrite` summary mask that failure.
...
- For global developer profile write failures, output must distinguish file-side effects from preference persistence: e.g. "user-language block written, but preference was not persisted" for enable, or "managed blocks removed, but disabled preference was not persisted" for opt-out. These cases are non-zero and must use `user-language-profile-write-failed` or an equivalent stable reason code.
- For Codex override shadowing, output must distinguish "managed block present in `$CODEX_HOME/AGENTS.md`" from "Codex will load it." If non-empty `AGENTS.override.md` is present, print action-required with the override display path and do not render the Codex operation as successful sync.
...
- Error path: simulated global developer profile write failure during `--sync-user-language` exits non-zero, does not claim stored consent, and distinguishes any user-file write result from preference persistence.
- Error path: simulated global developer profile write failure during `--no-sync-user-language` exits non-zero, does not claim disabled preference was stored, and prevents later stale-profile re-enable from being mistaken for a cleanup success.
- Error path: active Codex `AGENTS.override.md` prints `codex-global-override-active`, exits non-zero in apply mode, and does not claim the `AGENTS.md` block is loaded guidance.
...
- Document:
  ...
  - Codex `AGENTS.override.md` can shadow `AGENTS.md`, so sync may require user action even after the managed block is written;
  - Claude v1 writes `~/.claude/CLAUDE.md`, while `~/.claude/rules/` is a documented alternative left out of scope for this release;
...
- Docs mention Codex override shadowing and the Claude `CLAUDE.md` v1 target choice without implying hooks/config enforce language.
...
| Codex `AGENTS.override.md` shadows the target `AGENTS.md`, making a "successful" write ineffective | Detect non-empty `$CODEX_HOME/AGENTS.override.md`, do not modify it, mark Codex sync action-required with `codex-global-override-active`, and do not claim the user-language block is active guidance |
| Claude config/home behavior drifts or is misunderstood | Route all Claude user instruction targeting through `resolveClaudeUserInstructionPath()` and lock v1 to `~/.claude/CLAUDE.md` with `basis: "claude-user-instructions-claude-md-v1"`; do not treat `CLAUDE_CONFIG_DIR` as user-memory relocation unless future docs or verification deliberately change the contract |
| Claude `~/.claude/rules/` is a valid user-level rules surface but not the v1 target | Record the deliberate v1 choice for `~/.claude/CLAUDE.md`, test the helper basis, and leave rules-file support to a separate migration decision instead of writing both surfaces |
...
| User files change but global developer profile persistence fails | Treat profile write failure as part of user-language sync failure, return non-zero, and avoid claiming opt-in/opt-out preference was stored |
...
- Implementation scope: `spec-first init` now supports run-level user-language sync with explicit/interactive consent, stored true maintenance, stored false all-host cleanup, dry-run preview, Codex override action-required, Claude/Codex user instruction targets, and all-repos summary reporting.
- Verification: `npm run typecheck`, `npm run test:unit`, `npm run test:smoke`, and `npm run test:integration` passed after review fixes.
- Review: single-agent report-only fallback review found two cleanup-state issues and both were fixed: apply-time recomputation prevents stale preview data from recreating removed cleanup targets, and explicit opt-out now persists `sync_user_language=false` even when residual cleanup fails so future `init` runs continue removal-only cleanup.
- Generated runtime: not regenerated and not hand-edited; this change modifies source, docs, and tests only.
