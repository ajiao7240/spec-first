...
本计划把 `spec-write-tasks` 从当前 A- / 90 分推进到 **evidence-complete 目标态**：每个非满分维度都有可复查证据，确定性审计分数到达其诚实上限（在 `--target` 审计与 KTD6/A4「不改 scorer」前提下约为 92），剩余分差逐一归因到具名的 scorer capability gap。具体补齐 executable output eval、语义质量事实分析、进一步 entrypoint 瘦身、input/output/workflow 证据、runtime/cross-host portability 证据、高风险 task-pack doc-review 的 bounded continuation 证明，并把超大 source plan 的 large-plan handling 收敛为 follow-up/docs-only 边界（规模触发阈值 + 中间产物持久化 + 宽单元 fan-out 自检），不在本轮扩成 analyzer 行为。核心原则是增强证据闭环与拆分纪律，而不是把任务拆分语义硬编码进脚本，也不是把审计数字本身当成目标。
...
- **Validation focus:** 新增 repo-level runner 和 analyzer 的单测、eval fixture execution report、至少一个 recorded actual task-pack output + adjudication、official `.skill` package smoke、可用宿主的 runtime sync smoke、task-pack fixture validation、`spec-skill-audit` 复跑与 changelog/plan hygiene。
...
- R2. 为 `skills/spec-write-tasks/evals/output-quality-cases.json` 提供可执行 output eval runner，至少能在无 provider credential 的本地环境跑结构化 `deterministic_assertions`，并生成带 freshness metadata 的 scorecard report；prose `objective_assertions` 只作为 reviewer narrative，不能被 runner 当作可执行断言。
...
- `docs/validation/spec-write-tasks/output_quality_scorecard.md` 记录 owner、review cadence、output contract、rollback boundary、missing evidence、本次 run 结果、input fixture hashes、runner script hash、source revision、command 和 stale-check status。
- 至少一个 recorded actual task-pack output（带 source hash）经过 human/model adjudication；若没有，该维度必须在 closeout 中归为 output-quality residual，而不是 evidence-complete。
...
- U9 需包含 downstream task-pack consumer outcome check：用代表性 task pack 证明 handoff 对 `spec-work` / doc-review 消费者没有 execution-blocking ambiguity，不能只用 audit score、runner report 或 maintainer evidence 作为成功代理。
...
- KTD2. **Add executable fixture assertions and recorded output evidence before adding more cases.** Current fixture count is enough; the gap is execution evidence and adjudication, not fixture volume. The runner may only execute structured `deterministic_assertions`; prose `objective_assertions` remains reviewer narrative. At least one recorded actual task-pack output with source hash and human/model adjudication is required for the output-quality evidence leg to be called complete. **注意 runner 的价值是 maintainer/reviewer 证据与 adjudication：`scoreEvalReadiness` 不检测 runner，`eval_readiness` 仍封顶 4（具名 audit-tool-gap），runner 不会提升该维度的审计分。**
...
- **Resolved by latest doc-review:** U2/U3 tools are maintainer-only repo-level tools, not packaged skill runtime assets. Keep JSON/MD reports only under `docs/validation/spec-write-tasks/**`, with freshness metadata and stale checks; if reuse across other skills is desired later, route it as a separate feature.
- **Resolved by latest doc-review:** U4/U5 will not create `input-output-contract.md` or `workflow-branching.md` in this plan. Move displaced `SKILL.md` detail into existing `execution-handoff-contract.md` and `task-quality-guide.md`, and add tests that prevent duplicate branch/input-output definitions across those existing owners.
...
- Define report freshness metadata: input fixture hashes, generated output hashes where applicable, runner/analyzer script hash, source revision, command, generated_at, and stale-check status.
...
- Add at least one recorded actual task-pack output under `docs/validation/spec-write-tasks/recorded-output/` with source plan hash, generation command/context, output hash, and human/model adjudication status. If implementation cannot produce this artifact, closeout must classify output quality as a named residual rather than evidence-complete.
- Use deterministic execution mode names: `fixture-backed`, `recorded-fixture`, `model-executed`, `human-adjudicated`, `missing-evidence`.
- Produce JSON and Markdown reports with pass/fail deterministic assertions, baseline risks, with-skill expectations, evidence status, missing evidence, and freshness metadata from U1.
...
- Freshness: changing a fixture, recorded output, runner script, or source revision without regenerating the report causes the stale check to fail.

**Verification:**
- Running the script locally produces both reports and exits non-zero only for structural, stale, or deterministic assertion failures, not for honestly declared missing provider evidence.
...
- Include freshness metadata consistent with U1 so committed analysis reports cannot be mistaken for current evidence after fixture/script/source changes.
...
### U8. Defer Large-Plan Handling To Follow-Up Evidence
...
**Files:**
- Modify: `docs/plans/2026-06-22-002-refactor-spec-write-tasks-quality-to-100-plan.md`
- Test: `tests/unit/spec-write-tasks-contracts.test.js`
...
- 在本计划 closeout 中把 large-plan handling 记录为 deferred follow-up，不计入 evidence-complete 成功条件。
...
**Verification:**
- evidence-complete closeout can succeed without large-plan behavior changes, and any remaining large-plan work is explicitly tracked as deferred scope rather than hidden inside U3/U4.
...
| Committed reports become stale evidence | Store fixture hashes, runner/analyzer hashes, source revision, command and stale-check status; fail validation when reports drift from inputs. |
...
- **Split large-plan handling (R11/KTD9/U8) into its own plan:** Accepted after doc-review. This plan now records large-plan handling as follow-up scope only; it does not add analyzer behavior, SKILL entrypoint pointers, or `task-quality-guide.md` thresholds until a representative large-plan fixture / recorded output demonstrates the need and expected behavior.
...
- Add or update report docs under `docs/validation/spec-write-tasks/**` as source-maintainer evidence, with freshness metadata and stale-check status.
...
- **Recommended next artifact:** Run `spec-write-tasks` on this plan to produce an executable task pack, because the work has eight evidence-closure units plus one explicit follow-up boundary unit and touches repo-level scripts, validation reports, tests, package evidence and skill prose.
