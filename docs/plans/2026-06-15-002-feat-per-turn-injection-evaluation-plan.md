...
referenced_reviews:
  - path: docs/项目审查/2026-06-15-行业对标研究档案.md
    role: origin
    scope: in
    addresses_findings: ["BENCH-borrow-per-turn-injection"]
  - path: docs/项目审查/2026-06-15-项目Review与优化方案.md
    role: origin
    scope: in
    addresses_findings: ["P1-per-turn-injection"]
---
...
**关键正面发现:** spec-first **已有此机制的右尺寸 native 版本**——`templates/claude/hooks/spec-plan-guard` 用 Claude `UserPromptExpansion` 事件,在 `/spec:plan` 展开时注入 best-effort 注意力提醒,**显式声明无硬保护、无状态**。这说明 spec-first-native 的「抗漂移注入」形态是:**按命令展开注入、无状态、best-effort、诚实降级**——而不是 Trellis 的有状态每轮注入。

**初步推荐(待 U1-U3 评估确认):** 不采纳 Trellis 式有状态 per-turn 注入;若抗漂移确为真实痛点,走轻量 native 变体——扩展既有 `UserPromptExpansion` guard 模式到更多 workflow(Claude-only,Codex 经 SessionStart 降级 + 响亮声明)。若痛点未被证据支持,带理由 defer。
...
- current_revision: leo-2026-06-11-plan-update 分支
- worktree_dirty: true
...
- limitations: 「抗 context 压缩漂移」是否为 spec-first 真实高频痛点,**无 spec-first 侧数据**(Evaluation Harness 待数据);Codex 未来是否新增 per-turn hook 事件未知(advisory:当前 hooks.json 仅 SessionStart)
...
- hook 基建已有:Claude `session-start`(注入 using-spec-first bootstrap)+ `spec-plan-guard`(`UserPromptExpansion`,/spec:plan 展开时 best-effort 注意力提醒);Codex `session-start`(hooks.json 仅 SessionStart)。
- 无「当前 active workflow/phase」持久状态;进度由 git 派生。
- `spec-plan-guard` 是 native 右尺寸先例:无状态、按命令展开、显式 best-effort、no hard protection。
...
**Approach:** 检索 docs/solutions、docs/项目审查、docs/12-bug分析 是否有相关失败记录;评估 session-start bootstrap(已每会话注入)是否已大部分覆盖。**若无证据支持痛点,按角色契约 §10 aspirational 推进义务,倾向 defer 而非造能力。**
**Verification:** 给出 痛点 confirmed / advisory / 无证据 的分级结论。
...
**Approach:** 决策矩阵:痛点证据 × 双宿主代价 × 边界契合度。若 defer,在 review-closure 中把 finding 标 deferred_findings + 重估条件(如「当出现 ≥N 条 context-drift 失败记录时重启」)。
**Verification:** 决策落盘,P1 闭环状态明确。
...
2. **抗漂移若被证据支持** → 走 U3 轻量 native 变体(扩展 UserPromptExpansion guard,Claude-only + Codex 响亮降级)。
3. **抗漂移若无证据** → 带理由 defer,记重估条件。这是合法闭环(reasoned deferral),不是搁置。
