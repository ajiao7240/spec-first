...
- **R3.** task-governance-signals 的信号采集**分两个来源**，分别对应两个 workflow 阶段（架构性约束，不可混）：
  - **plan-declared 来源（`spec-plan` Phase 0.6，pre-code / pre-plan 默认无 git diff、也还没有 Implementation Units）**：从 planning context 采集——用户请求、origin requirements/plan、Phase 0.4 直接源码扫描得到的候选路径/模块、cross-module 路径前缀(`>=2` 顶层目录)、request/origin 文本关键词命中。不得依赖尚未写出的 Implementation Units `Files` 清单。
...
- **R5.** 新增 `resource-governance-lens.v1` source contract（schema + docs）。字段与默认 policy 由 spec-first 自有 schema/docs/helper 常量定义；project-scaffold `resource-policy.json` 只作为参考输入，不成为 source-of-truth。默认覆盖大文件阈值（`maxGitFileSizeBytes=5MB`）、generated output / retained runtime 目录白名单、raw log retention/redaction、owner/module path hints、staging-scope（staged/generated runtime 风险与 `git add .` 风险推断）。输出 advisory facts + reason_code，**不 blocking**。
...
- git-diff 信号 producer（`--numstat` + 纯 JS cross-module/keyword）与 plan-declared planning-context 信号采集实现，`signal_source` 正确区分两来源；空 diff / pre-code / base-ref 解析失败有 reason_code，不崩溃。
...
- `resource-governance-lens.v1` schema 落地并经校验；resource advisory 项（含 §4.8 全五项，含 owner/module-path-hint）产出 + reason_code；`subject_path`/`offending_path` 可指向 generated runtime，但 `evidence_ref` 仍只能指向允许的 source/log/artifact 证据。
...
- current_revision: `a0a754d4`（branch `leo-2026-06-03-ceupdate`）
...
- limitations: 未实际运行 helper（尚未实现）；plan-declared 信号的 planning-context 输入口径需实现时与 `spec-plan` Phase 0.4/0.6 实际可用信息对齐；spec-code-review Stage 3 preflight 的确定性化需实现时与现有口算字段逐项对齐
...
- plan-declared 信号的「planning-context 输入」口径：实现时对齐 `spec-plan` Phase 0.4/0.6 实际可用信息，只消费用户请求、origin 文档、候选路径/模块和文本关键词；如果未来需要基于已写出的 Implementation Units `Files` 清单复核 depth，应作为 Phase 4 后的二次 reclassify，而不是 Phase 0.6 初判输入。
- `candidate_level` 的分桶阈值（多少 declared path/source ref/target area、是否 cross-module → 建议哪档 depth）：实现时给**透明分桶**（非 opaque score），并写进 docs 让 LLM 可质疑；阈值是 advisory 建议非裁决。
...
**Goal:** 落 `task-governance-signals.v1` schema + docs，并实现 deterministic 信号 producer：plan-declared 来源（planning context 声明面）+ git-diff 来源（`--numstat`），产出 `candidate_level`（plan depth 语言）+ signals + reason_codes，不裁决、不输出 score/confidence。
...
**Approach:**
- schema canonical 以父方案 §4.1 为准，**去除 `score`，去除 `confidence`**；`candidate_level` ∈ `lightweight|standard|deep`；signals 含 `signal_source`、`file_count`、`line_delta`、`declared_path_count`(plan-declared)、`source_ref_count`(plan-declared)、`target_area_count`、`cross_module`、`top_dirs`、`critical_path_hits`、`keyword_hits`；顶层含 `risk_domains`、`recommended_artifacts`、`recommended_gate_lenses`（取自 U1 taxonomy）、`reason_codes`。
...
- plan-declared 信号：从传入的 planning context（用户请求、origin 文档、Phase 0.4 候选路径/模块、文本）算 declared_path_count/source_ref_count/target_area_count/cross_module/keyword_hits；`signal_source:"plan-declared"`，不得读取尚未生成的 plan Implementation Units，也不得把 draft plan `Files` 清单当 Phase 0.6 输入。
...
- candidate_level 用**透明分桶**（如 declared_path_count/source_ref_count/target_area_count + cross_module 组合 → 建议档），分桶规则写进 docs 让 LLM 可质疑；明确是 candidate 非 final。
...
- Happy path（plan-declared）: 给定用户请求 + origin/source-scan 候选路径 → declared_path_count/source_ref_count/cross_module 正确，`signal_source:"plan-declared"`，无 git 依赖，且不依赖 Implementation Units。
...
- Edge case: blocking stage 出现但无 approver/evidence → schema 允许枚举值但 docs 标注 v1.14 不产生该值（contract test 断言没有注册 `rule-maturity` internal helper / producer；task/resource producers 若 emit maturity observation，也只能 emit shadow/advisory）。
- Test expectation: 无自动 promotion 逻辑（grep 确认无 promotion/approve 函数）。

**Verification:**
- valid/invalid fixture 经 schema-validator；contract test 锁定 v1.14 为 schema/docs-only，且不存在会输出 required-evidence/blocking 的 producer。
...
- Read-for-reference: `skills/spec-plan/SKILL.md` Phase 0.4/0.6（确认 Phase 0.6 可用的 planning context）；`skills/spec-plan/references/plan-template.md` 只用于未来 draft-plan 二次 reclassify，不作为 Phase 0.6 输入来源。
...
- 明确 Phase 0.6 是 pre-code / pre-plan：candidate_level 来自 plan-declared planning-context 信号（请求、origin、候选路径/模块、关键词），不是 git diff，也不是尚未写出的 Implementation Units。
...
**Goal:** 落 `resource-governance-lens.v1` schema + docs，并实现 advisory resource lens producer：从 git diff / staged diff / status facts + spec-first-owned resource policy defaults 算 large file / generated output / raw log / staging-scope advisory facts + reason_code，不 blocking。
...
**Approach:**
- 输入：git-diff 文件清单（复用 U2 `git-diff-signals.js`）、staged 文件清单（`git diff --cached --name-status`）、worktree status（`git status --porcelain`）+ spec-first-owned `DEFAULT_RESOURCE_POLICY`（大文件阈值 5MB、generated/retained 目录白名单、staging-scope 模式、**owner/module-path-hint map**）。project-scaffold `resource-policy.json` 只作为参考样例；若实现显式 `--policy-json <path>`，必须先把 override schema 写进 `resource-governance-lens.md`，不能把外部 `.scale` policy 当 canonical。
- 输出 advisory items：`{lens_family, dimension, severity, reason_code, subject_path, evidence_ref}`。**维度命名澄清（修 FLAG 1）**：`resource` 不是 U1 gate-lens taxonomy 的 family（它是父方案 §4.8 的独立轴）。resource-lens 用自有 `dimension` 名（`large-file`/`generated-output`/`raw-log`/`staging-scope`/`owner-hint`），仅在需要归类时把 item 映射到 U1 的 `summary`/`verification` family，**不声称取自 taxonomy 的 `resource` family**。severity 只到 advisory，不 blocking。`subject_path` / `offending_path` 可指向 generated runtime 或大文件目标；`evidence_ref` 必须指向允许的 source/log/artifact 或 deterministic git-status summary，继续复用普通 evidence boundary，避免 generated runtime path 被当成可信 evidence ref。
- 检测项（覆盖父方案 §4.8 全五项）：large file added（`>maxGitFileSizeBytes`）、generated output committed/staged（命中 generated mirror / 非 retained 白名单的生成物）、raw log 体积+redaction、**owner / module path hint（来自 spec-first-owned DEFAULT_RESOURCE_POLICY `owners`/`modules` map，对命中文件标 ownership advisory）**、staging-scope（staged/generated runtime 越界；`git add .` 只能基于 status 形状推断为 advisory，不能声称观测到用户命令）。
...
- Happy path: generated output committed/staged（`.claude/` 入暂存）→ advisory + reason_code，`.claude/...` 落 `subject_path`，`evidence_ref` 指向允许的 git-status summary / log artifact。
- Edge case: raw log 超阈值 → advisory；retained 白名单目录不误报。
- Edge case: owner/module map 命中变更文件 → ownership advisory（覆盖 §4.8 第三项）。
- Edge case: staged/generated runtime 或 broad status shape → staging-scope advisory；`git add .` 风险只作为 inferred reason_code。
- 越界: 所有 severity 只到 advisory，无 blocking 字段值（schema 约束）；item 不声称 `resource` 为 gate-lens family；generated runtime 不进入 `evidence_ref`。
...
**Approach:**
- spec-work closeout：Phase 4 区域调用 resource-governance-lens → 命中项作 advisory facts（large file / generated output / raw log / staging-scope），与 honest-closeout claims 并列；reason_code 优先；明确不 blocking；展示时保留 `subject_path` 与 `evidence_ref` 分离，避免把 generated runtime 目标路径当 evidence ref。
...
- `[CT-006]` generated runtime path 不进 evidence refs（复用 target-repo 校验）；resource-lens 只能把 generated runtime 放进 `subject_path`/`offending_path`。
...
- 两 signal_source 区分正确；plan-declared 来源不依赖 Implementation Units；中文关键词命中；无 score/confidence 输出。
...
**Resource Lens Tests：**
- large file / generated output / raw log / owner-hint / staging-scope 各检测项 + reason_code；覆盖 `git diff --cached --name-status` 与 `git status --porcelain` 输入；retained 白名单不误报；severity 只到 advisory；item 不声称 `resource` 为 gate-lens family；generated runtime 目标路径只出现在 `subject_path`/`offending_path`，不进入 `evidence_ref`。
...
- v1.14 不注册 rule-maturity producer/helper；task/resource producers 不输出 `blocking` / `required-evidence` stage；无自动 promotion 逻辑（grep + contract 断言）。
...
批 1（CON-TASK-001）:
  ...
  task-governance-signals.js  ──uses──► git-diff-signals.js（work/review 阶段才有 diff）
    产出: {candidate_level: lightweight|standard|deep,
           signals{signal_source, declared_path_count, source_ref_count, cross_module, keyword_hits, ...},
...
批 2（CON-RES-001）:
  spec-work closeout Phase 4 / spec-code-review Stage 3
    │ 调用 resource-governance-lens（git-diff + staged diff + status + spec-first-owned defaults）
    ▼
  resource-governance-lens.js
    产出: advisory items[{lens_family, dimension, severity:advisory, reason_code, subject_path, evidence_ref}]
