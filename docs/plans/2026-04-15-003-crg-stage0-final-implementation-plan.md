...
3. `spec-plan / spec-work / spec-review` 接入 evaluator
...
- 让 `spec-plan / spec-work / spec-review` 不再只靠 prompt 约定消费 Stage-0
...
4. Implementation Unit 4：接入 `spec-plan / spec-work / spec-review`
...
**Patterns to follow:**
- 沿用 CommonJS 风格
- 沿用当前 `spec-plan/spec-work/spec-review` 的 stage 命名：`plan/work/review`
...
**Goal:** 让 `spec-plan / spec-work / spec-review` 的 Stage-0 预载 contract 统一切到 evaluator 输出，而不是继续以 `injection-index.yaml` 作为唯一真源。
...
1. `spec-plan / spec-work / spec-review` 三个 workflow 的 Stage-0 预载 contract 都以同一 evaluator 输出格式为真源。
...
- Create: `.spec-first/workflows/spec-review/`
