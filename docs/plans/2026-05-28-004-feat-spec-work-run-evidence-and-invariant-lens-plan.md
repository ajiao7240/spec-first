...
3. `trigger-degraded-provider`：**本次 closeout 中 LLM 实际消费过**的 graph evidence 中，存在 `evidence_grade ∈ {stale, query-unverified}` 或 `evidence_posture=fallback`。未消费 graph evidence 的 spec-work 不命中此 trigger。
4. `trigger-deferred-follow-up`：closeout 输出含 `deferred_follow_up[]` 非空。
5. `trigger-handoff`：handoff 给 `spec-code-review` / `spec-compound` / release（首版由 LLM 在 closeout payload 中自报；后续可能引入 deterministic 信号——见 ## Open Questions C1）。
6. `trigger-compaction-resume`：本会话经历过 compaction 或 resume cue（host 暴露信号时按信号判定；否则由 LLM 自报）。
7. `trigger-long-task`：本次任务规模或会话时长足以让 closeout 时上下文已经被压缩或可能出现指令遗忘——**首版不引入数字阈值**，由 LLM 语义判断；本 plan 落地 1-2 周后按真实 run.json 数据再决定是否引入 deterministic 阈值。
8. 全部未命中：`no-trigger-matched`，可跳过写 run.json，但 final response 仍需说明 reason_code。

阈值不进 schema enforce——保持语义判断空间。`trigger-long-task` 第一版**不写具体数字**，避免凭空给阈值反而把决策刚性化（参见 ## Open Questions C-trigger）。
...
1. **plan-template.md 引入 frontmatter 显式声明字段**：新 plan 在 frontmatter 中通过 `implements_schemas: [<repo-relative path>, ...]` 字段显式列出本 plan 实际实施的 schema 路径。**lint 只扫这个字段，不做 free-text scan**——free-text scan 会大量误判（任何 plan 提到一个 schema 都会被关联）。Legacy plan 没有这个字段时 lint 跳过 schema flag 检查并 warn（不 fail），允许逐步回填。
2. 写 lint test：扫 `docs/plans/*.md` frontmatter，检查 (a) `status` enum；(b) 对 `status: completed` 的 plan，遍历 `implements_schemas[]` 中每条路径，读对应 schema 顶层 `x-spec-first-workflow-integrated`，若任一为 false 则 fail，要求改 `partially-shipped` 或为该 schema 移出 implements_schemas。
3. escape hatch：`partially_shipped_reason: <code>` frontmatter 字段允许 owner 显式声明半成品原因（仅在 status=partially-shipped 时有意义；不允许与 status=completed 共存——否则就是滥用）。
4. 修改 `plan-template.md`：`status: active` 注释改为 `status: active  # active | partially-shipped | completed | superseded`；新增 `implements_schemas: []` 字段说明（可空数组）；可选 "Completion Criteria" 小节给出"列出本 plan 完成时仍然 deferred 的 schema flag / contract status / runtime asset"模板。
5. 修正 `2026-05-11-001`：frontmatter status 改 `partially-shipped`，新增 `implements_schemas: ['docs/contracts/workflows/spec-work-run-artifact.schema.json']` 与 `partially_shipped_reason: workflow-integrated-deferred-to-2026-05-28-004`；正文末尾 "## Status 说明" 段补充修正原因（U3 schema flag 在本方案前长期为 false，由 `2026-05-28-004` 收口）。
6. 跑 lint 全绿。
...
1. **C-trigger**：`trigger-long-task` 首版不引入数字阈值（B1 决定），完全靠 LLM 语义判断。落地 1-2 周后看真实 run.json 数据：(a) `trigger-long-task` 命中频率是否合理；(b) 是否需要回引入 deterministic 阈值；(c) 阈值具体数值用统计中位数 / P90 决定。本周期不预设。
2. **C1**：`trigger-handoff` 缺乏 deterministic 信号——首版由 LLM 在 closeout payload 中自报 `handoff_intent` 字段（`spec-code-review` / `spec-compound` / `release` / `none`）。后续如可从 final response 结构、命令调用记录或 next_action 字段中提取确定性信号，再迁移；不预设迁移时间。
3. **C2**：fresh-source eval 具体执行路径——首版接受两种验证方式（详见 U-A 验证段）；选择交给 Stage 4 实施期，按当时宿主能力决定。如端到端 host-driven workflow 验证不可行，至少做 producer-level fixture 验证。
4. **C3**：`partially-shipped` 新值是否影响 `spec-plan` SKILL 中关于 status 的判断逻辑？Stage 5 落地前必须 grep `skills/spec-plan/` 中所有 `status` 字段判断，如有"假设 status 只有 active/completed/superseded"的硬编码逻辑，同步修正。本 plan 不预测影响面。
5. **C4**：producer 调用延迟若 > 500ms 是否需要 async 路径？暂不优化，按现有 atomic write + schema validate 实测延迟决定；如真成 closeout 体感问题，下个周期单独评估。
6. **C-compaction**：`trigger-compaction-resume` 在 host 不暴露 compaction 信号时如何判定？暂作为 LLM 自报字段（"本 session 经历过 compaction？"），后续如有 host-side 信号再 deterministic 化。
7. **C-lens-noise**：Invariant lens 的 phrase 同义词维护是否会随 prose 风格演变成噪音？试点期 4 条 invariant 容量小，问题暴露后再决定 alias 策略；如真成噪音源，下个周期评估改为 markdown 锚点 + 标签注释式守护。
8. **C-escape-abuse**：`partially_shipped_reason` escape hatch 是否会被滥用为"绕过 lint 的 completed"？lint 强制 status≠completed 才能写 reason，escape 只是允许 partially-shipped 不再被 schema flag 反向阻塞；滥用风险通过 PR review 兜底。
