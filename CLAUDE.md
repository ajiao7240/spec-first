...
# 2026-04-21 正在发布 `spec-first@1.5.7`；收口 compat/bootstrap/doctor/init 修复与文档归档，release snapshot 待 publish 完成后更新
```
...
- **workflow 轻量协作约束收口**（2026-04-20）：`spec-brainstorm`、`spec-plan`、`spec-work`、`spec-work-beta`、`spec-debug` 已补 `Restated Understanding / Current Core Goal / Scope / Non-goals / Verification-as-Done` 轻量锚点；`spec-plan`、`spec-work`、`spec-work-beta`、`spec-review` 已补 freshness-driven reload contract（`freshness_stale` 不再被误写成 `L0`）；`spec-work` 新增 `docs/contracts/workflows/spec-work-run-artifact.schema.json` 作为 `run.json` machine-truth contract，显式 handoff 标识收口为 `work_run:<run-id>` / `work_artifact_dir:<artifact_dir>`；`spec-review` 新增 `Requirement Completion / Plan-Diff Fidelity / Code Intrinsic Quality` 三轴 verdict，并在 `missing plan` 下只输出 `Code Intrinsic Quality`
- **workspace / topology / coding-guidelines 收口**（2026-04-20）：bootstrap 编译器新增 monorepo topology 视角与数据库路由 contract，`stage0-context` / `spec-plan` 消费面同步接入 workspace 级 selection context；CLI `init`/`clean` 新增 coding-guidelines managed block 写入与清理，并统一 help 面板样式（含图标与仓库地址）；对应 tests、docs/contracts、docs/solutions 与 prompt mirrors 已同步更新
