...
- 常见入口锚点：环境/MCP→`/spec:mcp-setup`；更新/runtime 修复→`/spec:update`；bug/失败→`/spec:debug`；代码/文档评审→`/spec:code-review`/`/spec:doc-review`；需求/计划/任务/执行→`/spec:brainstorm`/`/spec:plan`/`spec-write-tasks`/`/spec:work`；可度量优化→`/spec:optimize`
