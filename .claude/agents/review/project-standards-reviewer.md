...
2. For each changed file, check its ancestor directories up to the repo root for standards files. A file like `plugins/spec-first/AGENTS.md` applies to all changes under `plugins/spec-first/`.
...
- **Broken cross-references** -- agent names that are not fully qualified (e.g., `learnings-researcher` instead of `spec-first:research:learnings-researcher`). Skill-to-skill references using slash syntax inside a SKILL.md where the standards say to use semantic wording. References to tools by platform-specific names without naming the capability class.
