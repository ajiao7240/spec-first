...
初始化完成后，在 Claude Code 中直接使用工作流命令：

After init, use workflow commands directly in Claude Code:

```text
/spec:brainstorm  ← 需求探索与澄清 | Explore and clarify requirements
/spec:plan        ← 生成实施方案   | Generate implementation plan
/spec:work        ← 按计划执行     | Execute the plan
/spec:review      ← 结构化评审     | Structured multi-agent review
/spec:compound    ← 沉淀知识资产   | Compound project knowledge
```
