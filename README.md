...
- `/spec:graph-bootstrap` / `$spec-graph-bootstrap` 是 graph-informed Phase 0-4 入口，负责阶段2最小闭环产物生成
...
如果 `doctor` 提示 `legacy managed state`，或者 `clean` 明确拒绝处理 legacy 安装，不要先手动删除 `.claude/`、`.codex/`、`.agents/skills/` 下的目录，直接重新运行对应平台的 `spec-first init` 即可。当前升级策略是 hard-cut：

- `init` 是唯一受支持的 legacy 升级入口
- `init` 会先执行 managed hard reset，再按当前版本全量重建运行时
- `clean` 只负责移除当前受管资产，不承担 legacy 迁移
...
# 图引导入口（Phase 0-4）| Graph-Informed Entry (Phase 0-4)
...
当前并行期中，如需运行 graph-informed 的阶段2最小闭环，可额外使用 `/spec:graph-bootstrap` 或 `$spec-graph-bootstrap`。它已覆盖 Phase 0-4 事实抽取与文档生成，但默认稳定入口仍是 `bootstrap`。
...
| 能力层 | 内置 `45` 个 skills、`54` 个 agents 和 `4` 个 agent support files | Ships with `45` skills, `54` agents, and `4` agent support files |
...
| `spec-first init` | 初始化运行时 | 向当前项目同步 commands、skills、workflow skills、agents、agent support files 与开发者元数据；`--claude` / `--codex` 指定平台；也是唯一支持的 legacy 升级入口 |
| `spec-first clean` | 清理运行时 | 移除 Spec-First 管理的当前项目资产，保留非受管内容；不负责 legacy state 迁移 |
