...
For the canonical definitions of `query_ready`, `definitions-only`, `dirty-advisory`, `graph-affecting-blocked`, `stale`, `session-local`, `setup-inferred`, and other GitNexus readiness/capability lifecycle terms used across setup, bootstrap, startup snapshot, and downstream workflows, see the **Capability State Vocabulary** section in `docs/contracts/gitnexus-capability-catalog.md`. Other docs, skills, and runtime prose must use those terms; do not introduce synonyms.
...
- Scenario-adaptive facts are now part of the setup/bootstrap evidence path. Setup can write `.spec-first/workspace/scenario-fingerprint-setup.json`; graph bootstrap can merge it into `.spec-first/workspace/scenario-fingerprint.json`, `.spec-first/workspace/graph-targets.json`, and `.spec-first/workspace/graph-bootstrap-summary.json.quality_signals`. These artifacts are advisory deterministic context: scripts prepare them, while the current workflow decides whether to use graph evidence, bounded direct source reads, `spec-first clean --workspace-orphans`, or a setup/bootstrap handoff.
