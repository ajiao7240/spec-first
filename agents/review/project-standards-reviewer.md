...
2. For each changed file, check its ancestor directories up to the repo root for standards files. A standards file in a parent directory governs all changes under that subtree.
...
- **Broken cross-references** -- agent names that are not fully qualified (e.g., `learnings-researcher` instead of the project's fully qualified agent name). Skill-to-skill references using slash syntax inside a SKILL.md where the standards say to use semantic wording. References to tools by platform-specific names without naming the capability class.
