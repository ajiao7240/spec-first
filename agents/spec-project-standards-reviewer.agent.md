...
- **Broken cross-references** -- agent references that do not match installed flat `spec-*.agent.md` identities. Skill-to-skill references using slash syntax inside a SKILL.md where the standards say to use semantic wording. References to tools by platform-specific names without naming the capability class.
