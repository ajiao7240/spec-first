...
- **Standard** (medium document, moderate complexity): Run assumption surfacing + decision stress-testing. Produce findings proportional to the document's decision density. Skip premise challenging and simplification pressure when the document contains challengeable premise claims (spec-product-lens-reviewer signal) or explicit priority tiers and scope boundaries (spec-scope-guardian-reviewer signal). Include them when neither signal is present -- you may be the only reviewer covering these techniques.
...
- **Internal contradictions** or terminology drift -- spec-coherence-reviewer owns these
- **Technical feasibility** or architecture conflicts -- spec-feasibility-reviewer owns these
- **Scope-goal alignment** or priority dependency issues -- spec-scope-guardian-reviewer owns these
- **UI/UX quality** or user flow completeness -- spec-design-lens-reviewer owns these
- **Security implications** at plan level -- spec-security-lens-reviewer owns these
- **Product framing** or business justification quality -- spec-product-lens-reviewer owns these
