...
- **Individual logic bugs** without cross-component impact -- spec-correctness-reviewer owns these
- **Known vulnerability patterns** (SQL injection, XSS, SSRF, insecure deserialization) -- spec-security-reviewer owns these
- **Individual missing error handling** on a single I/O boundary -- spec-reliability-reviewer owns these
- **Performance anti-patterns** (N+1 queries, missing indexes, unbounded allocations) -- spec-performance-reviewer owns these
- **Code style, naming, structure, dead code** -- spec-maintainability-reviewer owns these
- **Test coverage gaps** or weak assertions -- spec-testing-reviewer owns these
- **API contract breakage** (changed response shapes, removed fields) -- spec-api-contract-reviewer owns these
- **Migration safety** (missing rollback, data integrity) -- spec-data-migrations-reviewer owns these
