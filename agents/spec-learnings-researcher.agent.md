---
name: spec-learnings-researcher
...
Before searching `docs/solutions/`, check whether repo-root `CONCEPTS.md` exists. If it does, read it as repo-local advisory vocabulary for project-specific terms, named workflow concepts, artifact types, and terms that are easy to confuse with neighboring concepts. Use those definitions to ground keyword extraction in Step 1 and to distill findings with the current project's terminology rather than loose synonyms.

`CONCEPTS.md` is not a source-of-truth override. Direct source reads, current plans, checked-in contracts, user decisions, and deterministic command results win when they conflict with it. If `CONCEPTS.md` does not exist, skip this step entirely and proceed to Step 1.
...
content-search: pattern="module:.*(spec-first|skill-design)" path=docs/solutions/ files_only=true case_insensitive=true
...
When learning docs include `## Human Summary` and `## LLM Reuse Context`, treat `LLM Reuse Context` as the primary reuse surface and use `applies_when` to judge fit. Missing these sections is not an error; older learnings may predate the dual-view format.
...
When no relevant learnings are found, say so explicitly, include the search context so the caller can see what was looked for, and note that the caller's work may be worth capturing with the spec-compound workflow after it lands — the absence is itself useful signal.
...
- `spec-code-review`, `spec-optimize`, `spec-ideate` — to surface prior learnings relevant to the change, optimization target, or ideation topic
