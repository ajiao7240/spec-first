...
The managed bootstrap blocks in `CLAUDE.md` and `AGENTS.md` are the using-spec-first core decision set, generated by `spec-first init` after choosing the target host and injected at session start. They carry the load-bearing routing decision logic (when to enter a workflow, how to route, priority order, anti-rationalization red flags, intent→entrypoint map) so it is present from session start rather than depending on the model reading this full SKILL. The block and this SKILL are independently maintained (the block is bilingual compressed bullets; this SKILL is the full English prose policy); they are kept in sync by test-asserted invariants (Route Map identifier set + key tokens), not by single-source generation. The block is a faithful core subset, not the full decision tree — details, edge cases, and the complete Route Map live here.
