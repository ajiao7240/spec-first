...
Why a separate skill: several callers need the same writing logic without the single-PR interactive scaffolding that lives in `git-commit-push-pr`. `spec-pr-stack`'s splitting workflow runs this once per layer as a batch; `git-commit-push-pr` runs it inside its full-flow and refresh-mode paths. Extracting keeps one source of truth for the writing principles.

**Naming rationale:** `spec-pr-description`, not `git-pr-description`. Stacking and PR creation are GitHub features; the "PR" in the name refers to the GitHub artifact. Using the `spec-` prefix matches this project's naming convention for workflow-adjacent skills.
...
- `spec-pr-description` → current-branch, no focus, auto-detect base
- `spec-pr-description emphasize the benchmarks` → current-branch, focus = "emphasize the benchmarks"
- `spec-pr-description base:origin/develop` → current-branch, base pinned to `origin/develop`
- `spec-pr-description base:origin/develop emphasize perf` → same + focus
- `spec-pr-description pr:561` → PR #561, no focus
- `spec-pr-description #561 do a good job with the perf story` → PR #561, focus = "do a good job with the perf story"
- `spec-pr-description https://github.com/foo/bar/pull/561 emphasize safety` → PR #561 in foo/bar, focus = "emphasize safety"
...
5. **Spec-First badge** -- append a badge footer separated by a `---` rule. Skip if the existing body (for `pr:` input) already contains the badge.
...
**Model slug:** Replace spaces with underscores. Append context window and thinking level in parentheses if known. Examples: `Opus_4.7_(1M,_Extended_Thinking)`, `Sonnet_4.7_(200K)`, `GPT-5.5_(High)`.
...
```bash
BODY_FILE=$(mktemp "${TMPDIR:-/tmp}/spec-pr-body.XXXXXX") && cat > "$BODY_FILE" <<'__SPEC_PR_BODY_END__' && echo "$BODY_FILE"
<the composed body markdown goes here, verbatim>
__SPEC_PR_BODY_END__
```

The quoted sentinel `'__SPEC_PR_BODY_END__'` keeps `$VAR`, backticks, `${...}`, and any literal `EOF` inside the body from being expanded or clashing with the terminator. Keep `echo "$BODY_FILE"` chained with `&&` so a failed `mktemp` or write never yields a success exit status with a path to a missing file.
