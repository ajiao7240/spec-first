...
代码结构不能单独产生 `confirmed` trust；code structure cannot produce `confirmed` by itself. `provider_untrusted` facts can prioritize inspection, but they cannot be promotion evidence until a current source anchor exists.
