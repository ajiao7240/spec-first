...
- **Body:**
  - Plain-English problem statement — reads the persona-produced `why_it_matters` from the merged reviewer return first. If detail is unavailable and the review return named a parent-owned artifact cache, fall back to `<review-artifact-dir>/{reviewer}.json`, using the same `file + line_bucket(line, +/-3) + normalize(title)` matching headless mode uses (see SKILL.md Stage 6 detail enrichment). Falls back to the merged finding's `title`, `severity`, `file`, and `suggested_fix` (when present) when no detail match is available — these fields are guaranteed in the merge-tier return.
...
- **Length cap:** when the composed body would exceed a tracker's body length limit, truncate with `... (continued in spec-code-review run artifact: <review-artifact-dir>)` and include the finding_id in both the truncated body and the metadata block so the artifact is discoverable.
