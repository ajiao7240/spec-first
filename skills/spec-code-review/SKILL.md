...
3. If `.spec-first/standards/standards-candidates.json` exists for the target repo, include that path, `.spec-first/standards/standards-preview.md` when present, and the latest standards validation result when available in a separate `<standards-baseline-paths>` block. These artifacts are optional context: only `confirmed` standards may become hard review criteria and hard findings; `observed` / `imported` / `suggested` remain advisory context, `conflict` remains risk context, and `unknown` remains question context. If validation failed, is missing, reports `trust_level=degraded`, reports `consumption_boundary=advisory_only`, or carries `workspace-advisory-only`, the baseline is degraded/advisory only and must not produce hard project-standards findings. `glue-map.json` may support reuse-first review questions but is not a workflow state machine. See `docs/examples/standards-glue-consumption-examples.md` for concrete examples; examples do not authorize hard findings from advisory or degraded baselines.
