...
- **Compile task pack with `spec-write-tasks`** -> Load the standalone `spec-write-tasks` skill with the plan path. Do not invoke it through slash commands, `$spec-*` commands, or any command-backed workflow surface. If it writes an executable task pack with matching `spec_id` and verifiable `source_plan_hash`, follow the skill's returned `next_action`: when it resolves to `review-task-pack` on an interactive host, the skill continues directly into current-host headless doc-review of the task pack before work, so report that review outcome and then offer to proceed with the current host's work entrypoint using the task-pack path; otherwise offer the work entrypoint with the task-pack path directly. If it returns `skip`, `return-to-plan`, `draft-only`, unverifiable identity/hash, or a non-executable task pack, do not offer task-pack execution; follow the returned recommendation instead.
