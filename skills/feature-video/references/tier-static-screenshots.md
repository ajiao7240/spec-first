...
If `agent-browser` is not installed, inform the user: "`agent-browser` is not installed. Run the current host's MCP setup entrypoint (`/spec:mcp-setup` on Claude Code, `$spec-mcp-setup` on Codex) to install required dependencies." Then skip to the CLI or fallback sections below.
