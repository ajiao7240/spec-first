...
If `agent-browser` is not installed, inform the user: "`agent-browser` is not installed. Run `/spec:mcp-setup` to install required dependencies." Then fall back to a lower tier (static screenshots or skip).
...
- Wait for `--load networkidle` after navigation, then a short fixed buffer for any post-fetch render. A fixed `wait 2000` alone is not enough on SPAs that fetch data after paint; screenshots can capture the empty shell.
