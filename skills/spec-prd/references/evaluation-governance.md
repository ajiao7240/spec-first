...
Focused Jest tests (`tests/unit/spec-prd-contracts.test.js`) check the source package contract, the compressed 8-file source topology (`SKILL.md` + 5 references + 2 scripts), the 170-line/15000-char entrypoint ceiling, both scripts against good/bad fixtures, and the human template mirror's evidence-tag enum. These are file-backed fixture checks, not provider-backed model execution.
