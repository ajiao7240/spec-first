...
- When the user asks to check something → use `agent-browser` to screenshot or inspect the page; if it is missing, ask them to run the current host's MCP setup entrypoint (`/spec:mcp-setup` on Claude Code, `$spec-mcp-setup` on Codex)
