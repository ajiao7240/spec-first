...
Process findings from all agents through this pipeline. Order matters.
...
- Do not modify the caller skills (`spec-brainstorm`, `spec-plan`)

## Iteration Guidance

On subsequent passes, re-dispatch personas and re-synthesize. The auto-fix mechanism and confidence gating should prevent the same findings from recurring once fixed. If findings are repetitive across passes, recommend completion.
