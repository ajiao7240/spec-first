---
name: spec-work-beta
description: "[BETA] Execute work with external delegate support. Same as spec-work but includes experimental Codex delegation mode for token-conserving code implementation."
...
For stale graph + lightweight work such as docs-only prose, narrow typo fixes, first project trial, or a small local bug with direct source evidence, disclose limitations and continue with bounded direct reads or session-local live MCP pointers. For stale graph + graph-heavy work such as shared helper/API/route/provider contract/core workflow/cross-module changes, review-pre-facts changes, high-risk review follow-up, or implementation choices that depend on execution flows or blast radius, recommend `$spec-graph-bootstrap` / `/spec:graph-bootstrap` before claiming graph-backed impact evidence. Delegation prompts must carry the same stale-graph limitations and must not ask workers to run GitNexus analyze, code-review-graph build, provider repair, index rebuild, default git hooks, watchers, or daemons. `detect_changes` / impact results are review evidence when graph facts are fresh or explicitly session-local; they are not refresh triggers.
...
After extracting tokens from arguments, resolve the delegation state using this precedence-first chain:
...
If it shows an unresolved command string, read `.spec-first/config.local.yaml` from the repo root using the native file-read tool (e.g., Read in Claude Code, read_file in Codex). If the file does not exist, all settings fall through to defaults.
...
4. **Choose Execution Strategy**
   ...
   **Host capability matrix**
   ...
   | Codex delegation (`codex exec`) | Follow `references/codex-delegation-workflow.md`; this path forces serial execution from the delegation routing gate. | No parallel overlap. Batching is sequential and never split across units that share files. | The orchestrator owns git operations and PR creation; Codex verifies inside the delegated batch per the reference workflow. |
...
2. **Incremental Commits**
   ...
   **Parallel subagent mode:** Commit ownership is split by isolation mode (see Phase 1 Step 4):
   ...
   - **Codex delegation:** `references/codex-delegation-workflow.md` controls execution and verification; the orchestrator still owns git operations and PR creation.
...
3. **Follow Existing Patterns**
   ...
   - Follow project coding standards from `AGENTS.md`, directory-scoped equivalents, and `CLAUDE.md` only if the repo still keeps a compatibility shim; use `docs/contracts/`, nearby source, and tests to prefer existing capabilities over reimplementation
