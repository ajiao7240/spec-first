...
On acceptance:
- Resolve the repo root: `git rev-parse --show-toplevel`. Write `work_delegate_consent: true` and `work_delegate_sandbox: <chosen-mode>` to `<repo-root>/.spec-first/config.local.yaml`
- To write: (1) if file or directory does not exist, create `<repo-root>/.spec-first/` and write the YAML file; (2) if file exists, merge new keys preserving existing keys
