...
- Write `work_delegate_consent: true` and `work_delegate_sandbox: <chosen-mode>` to `<repo-root>/.spec-first/config.local.yaml`
- If the file exists, merge the new keys while preserving existing keys
...
**Delegation invocation:** For each batch, execute these as **separate shell tool calls** (not combined into one):

**Step A — Launch (background, separate call):**

Write the prompt file, then launch:
...
Quoting is critical for the `-c` flag: use single quotes around the entire key-value pair and double quotes around the TOML string value inside.

**Step B — Poll (foreground, separate calls):**

After the launch call returns, poll for the result file:
...
Repeat until the output is `DONE`, then read the result file and proceed to classification.
