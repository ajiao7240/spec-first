...
For GitNexus, those setup-owned facts may include `graph-providers.json.providers.gitnexus.native_capabilities` and `runtime-capabilities.json.gitnexus_capability_discovery`. They are setup-inferred availability/discovery facts only: they help `$spec-plan` choose read-only native surfaces such as `query`, `context`, `impact`, `api_impact`, `route_map`, `shape_check`, `cypher`, `tool_map`, `list_repos`, or workspace advisory group orientation, but they do not prove current live MCP exposure, query success, or canonical `query_ready` graph evidence.
