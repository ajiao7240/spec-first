...
2. Present a compact confirmation block naming:
   ...
   - CodeGraph install/init: pinned dependency install from `mcp-tools.json`, `codegraph init`, `codegraph status`, one bounded `codegraph sync` if status reports `Pending Changes`, `.codegraph/codegraph.db`, and host MCP command `codegraph serve --mcp`;
