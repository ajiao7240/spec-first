...
- v1.6.0 2026-04-18 23:18:18 Codex: docs(plans): 新增 004 Karpathy guidelines 多 skill 拆解方案，按 `spec-brainstorm/spec-plan/spec-work/spec-work-beta/spec-debug/spec-review/resolve-pr-feedback` 分层分配四类行为原则，并将 001 已完成的 `spec-review` change-discipline 改造收口为先行子集 (user-visible)
...
- v1.6.0 2026-04-18 20:40:00 kuang: feat(stage0-runtime-preload): 新增内部 CLI `stage0-context`，将 `compileWorkspaceContext()` 的 Stage-0 / verification summary 通过 workflow runtime 默认注入到 `spec-plan`/`spec-work`/`spec-work-beta`/`spec-review`，并补齐 unit/smoke 守卫 (user-visible)
- v1.6.0 2026-04-18 19:33:35 kuang: feat(stage0-runtime-telemetry): 将 `stage0-context` 接成默认 workflow telemetry 写入链，按真实 workflow id 落盘 `spec-plan/spec-work/spec-work-beta/spec-review` 运行记录，补充 `level/verification_summary` 字段并收口 contract/smoke 守卫 (user-visible)
- v1.6.0 2026-04-18 20:05:00 kuang: feat(stage0-runtime-verification-summary): 将 diff-aware verification recommendation 接到 `compileWorkspaceContext` 的 runtime `verification_summary`，收口 `spec-work` / `spec-work-beta` / `spec-review` 的 effective checklist 消费语义，并补齐 workspace docs-only 不回填 repo baseline 的守卫测试 (user-visible)
- v1.6.0 2026-04-18 18:40:00 矿工: feat(spec-review): 为 review workflow 增加 change-discipline contract，打通 `dimension_tag` 在 reviewer compact return、merge/synthesize 与最终报告中的保留链路，并补齐 source/docs mirror 与 contract tests (user-visible)
...
- v1.6.0 2026-04-18 18:55:00 kuang: docs(spec-graph-bootstrap): 同步 `spec-plan`/`spec-work`/`spec-work-beta`/`spec-review` 的 verification summary docs mirror，并补充 Stage-0 verification-profile 的版本更新说明与 mirror 守卫测试 (user-visible)
...
- v1.6.0 2026-04-18 13:40:59 矿工: docs(plans): 新增 `2026-04-18-001-refactor-karpathy-execution-contract-integration-plan.md`——把 Karpathy 四原则作为横切执行 contract 嵌入 `spec-plan` / `spec-brainstorm` / `spec-work` / `spec-review` / `lfg` 五条主干 workflow，不新增用户可见入口；明确 source-of-truth + docs mirror + contract tests + 全量 glob byte-equal mirror 守卫的 5 个实现单元与治理前置 Unit 0；Observational Goals 非 gating，不预判 semver bump
...
- v1.6.0 2026-04-17 18:08:47 矿工: docs(项目介绍): `2026-04-17-CRG-优化工作清单.md` Round 4 事后复盘修订（Phase 1a+1b 代码已 ship 情况下的文档与事实对齐）——新增"消费路径说明"顶栏明确 `/spec:review|plan|work` 主路径走 Stage-0 静态路由（`src/context-routing/evaluator.js`）而非 CRG `retrieveContext` 管道；每项加受益链条标签（🟢 主路径 / 🟡 间接基建 / 🔵 CLI / 💎 Phase 1a 核心价值）；C7 标注为 Phase 1a 里唯一直接打在 spec-review 运行时主路径的改动；§3 Phase 1b 改为"代码已 ship，验证步骤待做"状态，D1/C6 分别加 ✅ 已实施标记；新增"Round 4 事后复盘"段记录 C6 受益面事后核对的教训（retrieveContext 管道不被三个主工作流调用，C6 仅惠及 CLI 用户）——不撤回已做工作，但建立"下次看起来属于 retrieval 栈的改动前先做消费路径调查"规则；未动代码，仅文档 meta 对齐
...
- v1.6.0 2026-04-16 03:15:06 kuang: feat(skill-parity): 追平 document-review、git-commit-push-pr、lfg、resolve-pr-feedback 的核心 contract，补齐 headless、PR 描述上下文与 review feedback 守卫测试 (user-visible)
...
- v1.6.0 2026-04-15 23:58:00 kuang: feat(spec-review): 恢复 ce-review 的 reviewer/mode/merge 核心 contract，新增 cli-readiness 与 previous-comments reviewers，并补齐 spec-review contract 守卫测试 (user-visible)
