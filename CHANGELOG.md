...
- v1.8.1 2026-05-09 23:04:10 leokuang: fix(spec-code-review): 在 reviewer dispatch 前增加 `spec-mcp-setup` runtime readiness preflight，必需 MCP host 配置未就绪时降级 single-agent report-only，并细化 Codex spawn backpressure、MCP startup failure 与参数错误分类，避免 Serena 启动失败被复制到每个 reviewer (user-visible)
